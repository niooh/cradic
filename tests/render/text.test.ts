...
describe('renderText', () => {
  it('renders basic text output', async () => {
    const text = await from('一').to('text').toString();
    expect(text).toBe('一');
  });
  ...
  it('splits characters vertically', async () => {
    const text = await from('字').to('text').with({ mode: 'v' }).toString();
    expect(text).toBe('宀子');
  });

  it('splits characters both ways (h for 汉, v for 字)', async () => {
    const hText = await from('汉').to('text').with({ mode: 'b' }).toString();
    expect(hText).toBe('氵又');
    const vText = await from('字').to('text').with({ mode: 'b' }).toString();
  ...
  it('applies separator - space', async () => {
    const text = await from('一二').to('text').with({ sep: ' ' }).toString();
