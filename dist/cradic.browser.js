//#region \0rolldown/runtime.js
var e = Object.defineProperty, t = (e, t) => () => (e && (t = e(e = 0)), t), n = (t, n) => {
	let r = {};
	for (var i in t) e(r, i, {
		get: t[i],
		enumerable: !0
	});
	return n || e(r, Symbol.toStringTag, { value: "Module" }), r;
}, r = {
	mode: "b",
	boxWidth: 60,
	boxHeight: 60,
	showBoxBorder: !0,
	boxBorderColor: "#eee",
	boxMarginH: 2,
	partScale: .75,
	hLeftScaleX: .75,
	hRightScaleX: .8,
	vTopScaleY: .7,
	vBottomScaleY: .85,
	hLeftOffsetX: .3,
	hRightOffsetX: -.3,
	vTopOffsetY: .5,
	vBottomOffsetY: -.5,
	fontWeight: 380,
	textStroke: "1px",
	textStrokeColor: "#000",
	fontFamily: "'Noto Sans CJK SC', 'Noto Serif CJK SC'"
}, i = {
	mode: "b",
	boxWidth: 60,
	boxHeight: 60,
	showBoxBorder: !0,
	boxBorderColor: "#eee",
	boxGapH: 4,
	boxGapV: 8,
	cols: 6,
	partScale: .8,
	hLeftScaleX: .7,
	hRightScaleX: .7,
	vTopScaleY: .7,
	vBottomScaleY: .7,
	hLeftOffsetX: 2,
	hRightOffsetX: -2,
	vTopOffsetY: 6,
	vBottomOffsetY: -6,
	fontFamily: "'Noto Sans CJK SC', 'Noto Serif CJK SC'"
}, a = {
	mode: "b",
	boxSize: 40,
	cols: 6,
	boxGap: 5,
	fontSize: 30,
	hScale: .65,
	vScale: .65,
	hTightness: -4,
	vTightness: -2,
	stroke: "0.5pt + gray"
}, o = {
	mode: "b",
	sep: ""
}, s = r, c = [
	"html",
	"svg",
	"typ",
	"text",
	"png",
	"jpg",
	"pdf"
], l = {
	txt: "text",
	htm: "html",
	jpeg: "jpg"
}, u = {
	乢: "山乚",
	乣: "幺乚",
	乨: "台乚",
	乩: "占乚",
	乱: "舌乚",
	乳: "孚乚",
	乵: "辛乚",
	亃: "粦乚",
	亄: "壹乚",
	亸: "享单",
	亿: "亻乙",
	什: "亻十",
	仁: "亻二",
	仂: "亻力",
	仃: "亻丁",
	仅: "亻又",
	仆: "亻卜",
	仇: "亻九",
	仈: "亻八",
	仉: "亻几",
	仍: "亻乃",
	从: "人人",
	仏: "亻厶",
	仔: "亻子",
	仕: "亻士",
	他: "亻也",
	仗: "亻丈",
	付: "亻寸",
	仙: "亻山",
	仛: "亻乇",
	仜: "亻工",
	仞: "亻刃",
	仟: "亻千",
	仠: "亻干",
	仡: "亻乞",
	仢: "亻勺",
	代: "亻弋",
	仦: "亻小",
	仨: "亻三",
	仩: "亻上",
	仪: "亻义",
	仫: "亻么",
	们: "亻门",
	仭: "亻刄",
	仮: "亻反",
	仯: "亻少",
	仰: "亻卬",
	仱: "亻今",
	仲: "亻中",
	仳: "亻比",
	仴: "亻月",
	仵: "亻午",
	件: "亻牛",
	价: "亻介",
	仸: "亻夭",
	仹: "亻丰",
	任: "亻壬",
	仼: "亻王",
	份: "亻分",
	仾: "亻互",
	仿: "亻方",
	伀: "亻公",
	伂: "亻巿",
	伃: "亻予",
	伄: "亻弔",
	伅: "亻屯",
	伆: "亻勿",
	伇: "亻殳",
	伈: "亻心",
	伉: "亻亢",
	伊: "亻尹",
	伋: "亻及",
	伌: "亻厄",
	伍: "亻五",
	伎: "亻支",
	伏: "亻犬",
	伐: "亻戈",
	休: "亻木",
	伒: "亻斤",
	伓: "亻不",
	伔: "亻冗",
	伕: "亻夫",
	伖: "亻友",
	优: "亻尤",
	伙: "亻火",
	伛: "亻区",
	伜: "亻卆",
	伝: "亻云",
	伟: "亻韦",
	传: "亻专",
	伡: "亻车",
	伢: "亻牙",
	伣: "亻见",
	伥: "亻长",
	伦: "亻仑",
	伧: "亻仓",
	伨: "亻匀",
	伩: "亻文",
	伪: "亻为",
	伬: "亻尺",
	伭: "亻玄",
	伮: "亻奴",
	伯: "亻白",
	估: "亻古",
	伱: "亻尒",
	伲: "亻尼",
	伳: "亻世",
	伴: "亻半",
	伵: "亻四",
	伶: "亻令",
	伷: "亻由",
	伸: "亻申",
	伹: "亻且",
	伺: "亻司",
	伻: "亻平",
	似: "亻以",
	伽: "亻加",
	伾: "亻丕",
	伿: "亻只",
	佁: "亻台",
	佂: "亻正",
	佃: "亻田",
	佄: "亻甘",
	佅: "亻未",
	但: "亻旦",
	佇: "亻宁",
	佈: "亻布",
	佉: "亻去",
	佊: "亻皮",
	佋: "亻召",
	佌: "亻此",
	位: "亻立",
	低: "亻氐",
	住: "亻主",
	佐: "亻左",
	佑: "亻右",
	佒: "亻央",
	体: "亻本",
	佔: "亻占",
	何: "亻可",
	佖: "亻必",
	佗: "亻它",
	佚: "亻失",
	佛: "亻弗",
	作: "亻乍",
	佝: "亻句",
	佟: "亻冬",
	你: "亻尔",
	佡: "亻仚",
	佢: "亻巨",
	佣: "亻用",
	佤: "亻瓦",
	佦: "亻石",
	佧: "亻卡",
	佨: "亻包",
	佪: "亻回",
	佫: "亻各",
	佬: "亻老",
	佭: "亻夅",
	佮: "亻合",
	佯: "亻羊",
	佰: "亻百",
	佲: "亻名",
	佳: "亻圭",
	佴: "亻耳",
	併: "亻并",
	佶: "亻吉",
	佷: "亻艮",
	佸: "亻舌",
	佹: "亻危",
	佺: "亻全",
	佻: "亻兆",
	佼: "亻交",
	佽: "亻次",
	使: "亻吏",
	侀: "亻刑",
	侁: "亻先",
	侄: "亻至",
	侅: "亻亥",
	侇: "亻夷",
	侈: "亻多",
	侉: "亻夸",
	侊: "亻光",
	例: "亻列",
	侍: "亻寺",
	侎: "亻米",
	侏: "亻朱",
	侐: "亻血",
	侑: "亻有",
	侒: "亻安",
	侓: "亻聿",
	侔: "亻牟",
	侕: "亻而",
	侗: "亻同",
	侘: "亻宅",
	侙: "亻式",
	侚: "亻旬",
	供: "亻共",
	侜: "亻舟",
	依: "亻衣",
	侞: "亻如",
	侟: "亻存",
	侠: "亻夹",
	価: "亻覀",
	侢: "亻再",
	侣: "亻吕",
	侤: "亻考",
	侥: "亻尧",
	侦: "亻贞",
	侧: "亻则",
	侨: "亻乔",
	侩: "亻会",
	侪: "亻齐",
	侫: "亻妄",
	侬: "亻农",
	侭: "亻尽",
	侮: "亻每",
	侰: "亻君",
	侱: "亻呈",
	侲: "亻辰",
	侳: "亻坐",
	侶: "亻呂",
	侷: "亻局",
	侸: "亻豆",
	侹: "亻廷",
	侺: "亻岑",
	侻: "亻兑",
	侼: "亻孛",
	侽: "亻男",
	侾: "亻孝",
	便: "亻更",
	俀: "亻妥",
	俁: "亻吳",
	係: "亻系",
	促: "亻足",
	俄: "亻我",
	俅: "亻求",
	俆: "亻余",
	俇: "亻狂",
	俈: "亻告",
	俉: "亻吾",
	俊: "亻夋",
	俋: "亻邑",
	俌: "亻甫",
	俍: "亻良",
	俎: "仌且",
	俏: "亻肖",
	俐: "亻利",
	俑: "亻甬",
	俒: "亻完",
	俓: "亻巠",
	俔: "亻見",
	俕: "亻宋",
	俖: "亻否",
	俗: "亻谷",
	俘: "亻孚",
	俙: "亻希",
	俚: "亻里",
	俛: "亻免",
	俜: "亻甹",
	保: "亻呆",
	俟: "亻矣",
	俠: "亻夾",
	信: "亻言",
	俣: "亻吴",
	俤: "亻弟",
	俥: "亻車",
	俦: "亻寿",
	俧: "亻志",
	俨: "亻严",
	俩: "亻两",
	俪: "亻丽",
	俫: "亻来",
	俬: "亻私",
	俭: "亻佥",
	俯: "亻府",
	俰: "亻和",
	俱: "亻具",
	俲: "亻効",
	俳: "亻非",
	俴: "亻戔",
	俵: "亻表",
	俶: "亻叔",
	俷: "亻肥",
	俸: "亻奉",
	俹: "亻亞",
	俺: "亻奄",
	俻: "亻备",
	俼: "亻育",
	俽: "亻欣",
	俾: "亻卑",
	俿: "亻虎",
	倀: "亻長",
	倁: "亻知",
	倂: "亻幷",
	倃: "亻咎",
	倄: "亻肴",
	倅: "亻卒",
	倆: "亻兩",
	倇: "亻宛",
	倈: "亻來",
	倊: "亻忩",
	個: "亻固",
	倌: "亻官",
	倍: "亻咅",
	倎: "亻典",
	們: "亻門",
	倒: "亻到",
	倓: "亻炎",
	倔: "亻屈",
	倕: "亻垂",
	倖: "亻幸",
	倗: "亻朋",
	倘: "亻尚",
	倚: "亻奇",
	倛: "亻其",
	倜: "亻周",
	倞: "亻京",
	借: "亻昔",
	倠: "亻隹",
	倡: "亻昌",
	倢: "亻疌",
	倣: "亻放",
	値: "亻直",
	倥: "亻空",
	倦: "亻卷",
	倧: "亻宗",
	倨: "亻居",
	倩: "亻青",
	倪: "亻兒",
	倫: "亻侖",
	倬: "亻卓",
	倭: "亻委",
	倮: "亻果",
	倯: "亻松",
	倰: "亻夌",
	倱: "亻昆",
	倲: "亻東",
	倳: "亻事",
	倴: "亻奔",
	倵: "亻武",
	倶: "亻具",
	倷: "亻奈",
	倸: "亻采",
	债: "亻责",
	倻: "亻耶",
	值: "亻直",
	倽: "亻舍",
	倾: "亻顷",
	倿: "亻妾",
	偀: "亻英",
	偁: "亻爯",
	偂: "亻前",
	偃: "亻匽",
	偄: "亻耎",
	偅: "亻重",
	偆: "亻春",
	假: "亻叚",
	偈: "亻曷",
	偉: "亻韋",
	偊: "亻禹",
	偋: "亻屏",
	偌: "亻若",
	偍: "亻是",
	偎: "亻畏",
	偏: "亻扁",
	偐: "亻彦",
	偑: "亻風",
	偒: "亻昜",
	偓: "亻屋",
	偔: "亻咢",
	偕: "亻皆",
	偖: "亻者",
	偗: "亻省",
	偘: "亻品",
	偙: "亻帝",
	做: "亻故",
	偛: "亻臿",
	停: "亻亭",
	偝: "亻背",
	偞: "亻枼",
	偟: "亻皇",
	偠: "亻要",
	偡: "亻甚",
	偢: "亻秋",
	偣: "亻音",
	偤: "亻酋",
	健: "亻建",
	偦: "亻胥",
	偧: "亻奓",
	偨: "亻柴",
	偩: "亻負",
	偪: "亻畐",
	偫: "亻待",
	偬: "亻怱",
	偭: "亻面",
	偮: "亻咠",
	偯: "亻哀",
	偰: "亻契",
	偱: "亻盾",
	偲: "亻思",
	偳: "亻耑",
	側: "亻則",
	偵: "亻貞",
	偶: "亻禺",
	偷: "亻俞",
	偸: "亻兪",
	偺: "亻昝",
	偻: "亻娄",
	偽: "亻為",
	偾: "亻贲",
	偿: "亻尝",
	傀: "亻鬼",
	傁: "亻叟",
	傂: "亻虒",
	傃: "亻素",
	傄: "亻眘",
	傅: "亻尃",
	傆: "亻原",
	傇: "亻茸",
	傈: "亻栗",
	傉: "亻辱",
	傊: "亻員",
	傋: "亻冓",
	傌: "亻馬",
	傍: "亻旁",
	傎: "亻真",
	傏: "亻唐",
	傐: "亻高",
	傑: "亻桀",
	傒: "亻奚",
	傓: "亻扇",
	傔: "亻兼",
	傕: "亻隺",
	傖: "亻倉",
	傗: "亻畜",
	傚: "亻效",
	傛: "亻容",
	傞: "亻差",
	傟: "亻翁",
	傠: "亻討",
	傡: "亻竝",
	傢: "亻家",
	傣: "亻泰",
	傤: "亻载",
	傥: "亻党",
	傦: "亻骨",
	傧: "亻宾",
	储: "亻诸",
	傩: "亻难",
	傪: "亻參",
	傫: "亻累",
	催: "亻崔",
	傭: "亻庸",
	傮: "亻曹",
	傯: "亻悤",
	傰: "亻崩",
	傱: "亻從",
	傲: "亻敖",
	傳: "亻專",
	傴: "亻區",
	債: "亻責",
	傶: "亻戚",
	傸: "亻爽",
	傹: "亻竟",
	傺: "亻祭",
	傽: "亻章",
	傾: "亻頃",
	傿: "亻焉",
	僀: "亻帶",
	僁: "亻悉",
	僂: "亻婁",
	僄: "亻票",
	僅: "亻堇",
	僆: "亻連",
	僇: "亻翏",
	僈: "亻曼",
	僋: "亻貪",
	働: "亻動",
	僎: "亻巽",
	像: "亻象",
	僐: "亻善",
	僑: "亻喬",
	僒: "亻窘",
	僓: "亻貴",
	僔: "亻尊",
	僕: "亻菐",
	僖: "亻喜",
	僗: "亻勞",
	僘: "亻敞",
	僙: "亻黄",
	僚: "亻尞",
	僛: "亻欺",
	僜: "亻登",
	僝: "亻孱",
	僞: "亻爲",
	僟: "亻幾",
	僠: "亻番",
	僡: "亻惠",
	僢: "亻舜",
	僣: "亻替",
	僤: "亻單",
	僥: "亻堯",
	僦: "亻就",
	僧: "亻曾",
	僨: "亻賁",
	僩: "亻閒",
	僪: "亻矞",
	僫: "亻惡",
	僬: "亻焦",
	僭: "亻朁",
	僮: "亻童",
	僯: "亻粦",
	僱: "亻雇",
	僳: "亻粟",
	僴: "亻間",
	僵: "亻畺",
	僶: "亻黽",
	僷: "亻葉",
	僸: "亻禁",
	價: "亻賈",
	僺: "亻喿",
	僻: "亻辟",
	僼: "亻豊",
	僽: "亻愁",
	僾: "亻愛",
	僿: "亻塞",
	儀: "亻義",
	儁: "亻雋",
	儂: "亻農",
	儃: "亻亶",
	億: "亻意",
	儅: "亻當",
	儆: "亻敬",
	儇: "亻睘",
	儈: "亻會",
	儉: "亻僉",
	儊: "亻楚",
	儋: "亻詹",
	儌: "亻敫",
	儎: "亻載",
	儏: "亻粲",
	儐: "亻賓",
	儒: "亻需",
	儓: "亻臺",
	儔: "亻壽",
	儕: "亻齊",
	儖: "亻監",
	儗: "亻疑",
	儘: "亻盡",
	儙: "亻遣",
	儚: "亻夢",
	儛: "亻舞",
	儜: "亻寧",
	儝: "亻榮",
	儞: "亻爾",
	償: "亻賞",
	儠: "亻巤",
	儡: "亻畾",
	儢: "亻慮",
	儣: "亻廣",
	儤: "亻暴",
	儥: "亻賣",
	儦: "亻麃",
	儧: "亻賛",
	儨: "亻質",
	儩: "亻賜",
	優: "亻憂",
	儫: "亻豪",
	儬: "亻靚",
	儭: "亻親",
	儮: "亻歷",
	儯: "亻駦",
	儰: "亻蔿",
	儱: "亻龍",
	儲: "亻諸",
	儳: "亻毚",
	儴: "亻襄",
	儶: "亻巂",
	儷: "亻麗",
	儸: "亻羅",
	儹: "亻贊",
	儺: "亻難",
	儻: "亻黨",
	儼: "亻嚴",
	儽: "亻纍",
	儾: "亻囊",
	儿: "丿乚",
	兓: "旡旡",
	兟: "先先",
	兢: "克克",
	冁: "单展",
	冮: "冫工",
	冯: "冫马",
	冰: "冫水",
	冱: "冫互",
	冲: "冫中",
	决: "冫夬",
	冴: "冫牙",
	况: "冫兄",
	冶: "冫台",
	冷: "冫令",
	冸: "冫半",
	冹: "冫犮",
	冺: "冫民",
	冻: "冫东",
	冼: "冫先",
	冽: "冫列",
	冾: "冫合",
	冿: "冫聿",
	净: "冫争",
	凁: "冫束",
	凂: "冫免",
	凃: "冫余",
	凄: "冫妻",
	凅: "冫固",
	准: "冫隹",
	凇: "冫松",
	凈: "冫爭",
	凉: "冫京",
	凊: "冫青",
	凋: "冫周",
	凌: "冫夌",
	凍: "冫東",
	凎: "冫金",
	减: "冫咸",
	凐: "冫垔",
	凑: "冫奏",
	凒: "冫豈",
	凓: "冫栗",
	凔: "冫倉",
	凕: "冫冥",
	凗: "冫崔",
	凘: "冫斯",
	凙: "冫睪",
	凚: "冫禁",
	凛: "冫禀",
	凜: "冫稟",
	凝: "冫疑",
	凞: "冫熈",
	凟: "冫賣",
	凯: "岂几",
	凱: "豈几",
	刅: "刃丶",
	切: "七刀",
	刉: "乞刂",
	刊: "干刂",
	刋: "千刂",
	刌: "寸刂",
	刎: "勿刂",
	刏: "气刂",
	刐: "丹刂",
	刑: "开刂",
	划: "戈刂",
	刓: "元刂",
	刔: "夬刂",
	刖: "月刂",
	列: "歹刂",
	刘: "文刂",
	则: "贝刂",
	刚: "冈刂",
	创: "仓刂",
	刜: "弗刂",
	初: "衤刀",
	刞: "且刂",
	刟: "召刂",
	删: "册刂",
	刡: "民刂",
	刢: "令刂",
	刣: "台刂",
	判: "半刂",
	刦: "去刂",
	刧: "去刀",
	刨: "包刂",
	利: "禾刂",
	刪: "冊刂",
	别: "另刂",
	刬: "戋刂",
	刮: "舌刂",
	刯: "亘刂",
	到: "至刂",
	刱: "井刅",
	刲: "圭刂",
	刳: "夸刂",
	刴: "朶刂",
	刵: "耳刂",
	刹: "杀刂",
	刺: "朿刂",
	刻: "亥刂",
	刼: "去刃",
	刽: "会刂",
	刾: "夹刂",
	刿: "岁刂",
	剀: "岂刂",
	剁: "朵刂",
	剂: "齐刂",
	剃: "弟刂",
	剄: "巠刂",
	剅: "豆刂",
	則: "貝刂",
	剈: "肙刂",
	剉: "坐刂",
	削: "肖刂",
	剌: "束刂",
	剏: "并刃",
	剐: "呙刂",
	剑: "佥刂",
	剒: "昔刂",
	剔: "易刂",
	剕: "非刂",
	剖: "咅刂",
	剗: "戔刂",
	剘: "其刂",
	剙: "并刅",
	剚: "事刂",
	剛: "岡刂",
	剜: "宛刂",
	剝: "彔刂",
	剞: "奇刂",
	剟: "叕刂",
	剠: "京刂",
	剡: "炎刂",
	剢: "豖刂",
	剤: "斉刂",
	剥: "录刂",
	剦: "奄刂",
	剧: "居刂",
	剨: "砉刂",
	剩: "乘刂",
	剫: "度刂",
	剬: "耑刂",
	剭: "屋刂",
	剮: "咼刂",
	副: "畐刂",
	剰: "乗刂",
	割: "害刂",
	剳: "荅刂",
	剴: "豈刂",
	創: "倉刂",
	剶: "彖刂",
	剷: "産刂",
	剸: "專刂",
	剹: "翏刂",
	剻: "崩刂",
	剼: "參刂",
	剽: "票刂",
	剾: "區刂",
	剿: "巢刂",
	劀: "矞刂",
	劁: "焦刂",
	劂: "厥刂",
	劃: "畫刂",
	劄: "答刂",
	劅: "蜀刂",
	劆: "廉刂",
	劇: "豦刂",
	劊: "會刂",
	劋: "喿刂",
	劌: "歲刂",
	劍: "僉刂",
	劎: "僉刀",
	劏: "當刂",
	劐: "蒦刂",
	劑: "齊刂",
	劒: "僉刃",
	劓: "鼻刂",
	劔: "僉刄",
	劕: "質刂",
	劖: "毚刂",
	劗: "贊刂",
	劘: "靡刂",
	劙: "蠡刂",
	劚: "屬刂",
	劜: "力乚",
	劝: "又力",
	功: "工力",
	加: "力口",
	劢: "万力",
	劤: "斤力",
	劥: "亢力",
	劧: "手力",
	动: "云力",
	助: "且力",
	劫: "去力",
	劬: "句力",
	劭: "召力",
	劮: "失力",
	劯: "石力",
	劰: "白力",
	励: "厉力",
	劶: "后力",
	劷: "羊力",
	劸: "圭力",
	効: "交力",
	劺: "牟力",
	劻: "匡力",
	劼: "吉力",
	劾: "亥力",
	勀: "克力",
	勁: "巠力",
	勂: "告力",
	勃: "孛力",
	勄: "每力",
	勅: "束力",
	勈: "甬力",
	勋: "员力",
	勌: "卷力",
	勍: "京力",
	勎: "坴力",
	勏: "咅力",
	勐: "孟力",
	勑: "來力",
	勒: "革力",
	勓: "皆力",
	勔: "面力",
	動: "重力",
	勖: "冒力",
	勘: "甚力",
	務: "矛务",
	勚: "贳力",
	勛: "員力",
	勜: "翁力",
	勝: "月劵",
	勠: "翏力",
	勡: "票力",
	勣: "責力",
	勤: "堇力",
	勦: "巢力",
	勨: "象力",
	勩: "貰力",
	勪: "喬力",
	勫: "番力",
	勬: "絭力",
	勭: "童力",
	勮: "豦力",
	勯: "亶力",
	勰: "劦思",
	勱: "萬力",
	勳: "熏力",
	勴: "慮力",
	勵: "厲力",
	勷: "襄力",
	勸: "雚力",
	匏: "夸包",
	协: "十办",
	協: "十劦",
	卙: "甚十",
	博: "十尃",
	卟: "口卜",
	卦: "圭卜",
	卧: "臣卜",
	卭: "工卩",
	卲: "召卩",
	却: "去卩",
	卶: "多卩",
	卹: "血卩",
	卻: "谷卩",
	卽: "皀卩",
	卾: "咢卩",
	厀: "桼卩",
	厸: "厶厶",
	叆: "云爱",
	叇: "云逮",
	双: "又又",
	収: "丩又",
	叔: "尗又",
	取: "耳又",
	叙: "余又",
	叛: "半反",
	叡: "睿又",
	叨: "口刀",
	叩: "口卩",
	叫: "口丩",
	叭: "口八",
	叮: "口丁",
	叶: "口十",
	叹: "口又",
	叺: "口入",
	叻: "口力",
	叼: "口刁",
	叽: "口几",
	叿: "口工",
	吀: "口千",
	吁: "口于",
	吃: "口乞",
	吅: "口口",
	吆: "口幺",
	吇: "口子",
	吋: "口寸",
	吐: "口土",
	吒: "口乇",
	吓: "口下",
	吔: "口也",
	吖: "口丫",
	吗: "口马",
	吘: "口午",
	吙: "口火",
	吚: "口尹",
	吜: "口丑",
	吟: "口今",
	吠: "口犬",
	吡: "口比",
	吣: "口心",
	吤: "口介",
	吥: "口不",
	吧: "口巴",
	吨: "口屯",
	吩: "口分",
	吪: "口化",
	听: "口斤",
	吭: "口亢",
	吮: "口允",
	吰: "口厷",
	吱: "口支",
	吲: "口引",
	吵: "口少",
	吶: "口內",
	吷: "口夬",
	吸: "口及",
	吹: "口欠",
	吺: "口殳",
	吻: "口勿",
	吼: "口孔",
	吽: "口牛",
	呀: "口牙",
	呁: "口匀",
	呃: "口厄",
	呅: "口文",
	呋: "口夫",
	呌: "口斗",
	呍: "口云",
	呎: "口尺",
	呏: "口升",
	呐: "口内",
	呒: "口无",
	呓: "口艺",
	呔: "口太",
	呕: "口区",
	呖: "口历",
	呗: "口贝",
	呚: "口攵",
	呛: "口仓",
	呜: "口乌",
	呝: "口戹",
	呞: "口司",
	呟: "口玄",
	呠: "口本",
	呡: "口民",
	呢: "口尼",
	呣: "口母",
	呤: "口令",
	呥: "口冉",
	呦: "口幼",
	呧: "口氐",
	呩: "口示",
	呪: "口兄",
	呫: "口占",
	呬: "口四",
	呭: "口世",
	呮: "口只",
	呯: "口平",
	呱: "口瓜",
	呲: "口此",
	味: "口未",
	呴: "口句",
	呵: "口可",
	呶: "口奴",
	呷: "口甲",
	呸: "口丕",
	呹: "口失",
	呺: "口号",
	呻: "口申",
	呼: "口乎",
	呾: "口旦",
	呿: "口去",
	咀: "口且",
	咁: "口甘",
	咂: "口匝",
	咃: "口他",
	咄: "口出",
	咆: "口包",
	咇: "口必",
	咈: "口弗",
	咉: "口央",
	咊: "口禾",
	咋: "口乍",
	和: "禾口",
	咍: "口台",
	咏: "口永",
	咐: "口付",
	咑: "口打",
	咓: "口瓦",
	咔: "口卡",
	咕: "口古",
	咖: "口加",
	咗: "口左",
	咘: "口布",
	咙: "口龙",
	咚: "口冬",
	咛: "口宁",
	咜: "口它",
	咝: "口丝",
	咞: "口开",
	咟: "口百",
	咡: "口耳",
	咣: "口光",
	咤: "口宅",
	咥: "口至",
	咦: "口夷",
	咧: "口列",
	咩: "口羊",
	咪: "口米",
	咬: "口交",
	咭: "口吉",
	咮: "口朱",
	咯: "口各",
	咰: "口旬",
	咱: "口自",
	咲: "口关",
	咳: "口亥",
	咴: "口灰",
	咵: "口夸",
	咶: "口舌",
	咷: "口兆",
	咹: "口安",
	咺: "口亘",
	咻: "口休",
	咽: "口因",
	咾: "口老",
	咿: "口伊",
	哂: "口西",
	哃: "口同",
	哄: "口共",
	哅: "口匈",
	哆: "口多",
	哇: "口圭",
	哈: "口合",
	哊: "口有",
	哋: "口地",
	响: "口向",
	哎: "口艾",
	哏: "口艮",
	哐: "口匡",
	哑: "口亚",
	哒: "口达",
	哓: "口尧",
	哔: "口毕",
	哕: "口岁",
	哖: "口年",
	哗: "口华",
	哘: "口行",
	哙: "口会",
	哚: "口朵",
	哜: "口齐",
	哝: "口农",
	哞: "口牟",
	哟: "口约",
	哠: "口告",
	哢: "口弄",
	哣: "口豆",
	哤: "口尨",
	哦: "口我",
	哧: "口赤",
	哨: "口肖",
	哩: "口里",
	哪: "口那",
	哫: "口足",
	哬: "口何",
	哮: "口孝",
	哯: "口見",
	哰: "口牢",
	哱: "口孛",
	哳: "口折",
	哴: "口良",
	哵: "口别",
	哷: "口寽",
	哸: "口妥",
	哹: "口孚",
	哺: "口甫",
	哻: "口旱",
	哼: "口亨",
	哽: "口更",
	哾: "口兑",
	唀: "口秀",
	唁: "口言",
	唂: "口谷",
	唃: "口角",
	唄: "口貝",
	唅: "口含",
	唆: "口夋",
	唈: "口邑",
	唉: "口矣",
	唊: "口夾",
	唋: "口余",
	唌: "口延",
	唍: "口完",
	唎: "口利",
	唏: "口希",
	唑: "口坐",
	唒: "口酉",
	唓: "口車",
	唔: "口吾",
	唕: "口皁",
	唖: "口亜",
	唗: "口走",
	唙: "口狄",
	唛: "口麦",
	唝: "口贡",
	唞: "口抖",
	唠: "口劳",
	唡: "口两",
	唣: "口皂",
	唤: "口奂",
	唥: "口冷",
	唦: "口沙",
	唧: "口即",
	唨: "口阻",
	唩: "口委",
	唪: "口奉",
	唫: "口金",
	唬: "口虎",
	唭: "口其",
	唯: "口隹",
	唰: "口刷",
	唱: "口昌",
	唲: "口兒",
	唳: "口戾",
	唴: "口羌",
	唵: "口奄",
	唶: "口昔",
	唷: "口育",
	唸: "口念",
	唹: "口於",
	唺: "口典",
	唻: "口來",
	唼: "口妾",
	唽: "口析",
	唾: "口垂",
	唿: "口忽",
	啀: "口厓",
	啁: "口周",
	啂: "口乳",
	啃: "口肯",
	啄: "口豖",
	啅: "口卓",
	啈: "口幸",
	啉: "口林",
	啊: "口阿",
	啋: "口采",
	啌: "口空",
	啍: "口享",
	啎: "午吾",
	啐: "口卒",
	啑: "口疌",
	啒: "口屈",
	啕: "口匋",
	啖: "口炎",
	啗: "口臽",
	啘: "口宛",
	啛: "口妻",
	啜: "口叕",
	啝: "口和",
	啞: "口亞",
	啟: "启攵",
	啡: "口非",
	啢: "口兩",
	啣: "口卸",
	啤: "口卑",
	啥: "口舍",
	啦: "口拉",
	啧: "口责",
	啨: "口青",
	啩: "口卦",
	啪: "口拍",
	啫: "口者",
	啭: "口转",
	啮: "口齿",
	啯: "口国",
	啰: "口罗",
	啱: "口岩",
	啲: "口的",
	啳: "口卷",
	啴: "口单",
	啵: "口波",
	啶: "口定",
	啷: "口郎",
	啸: "口肃",
	啹: "口居",
	啺: "口昜",
	啼: "口帝",
	啽: "口弇",
	啾: "口秋",
	啿: "口甚",
	喀: "口客",
	喁: "口禺",
	喂: "口畏",
	喃: "口南",
	喅: "口昱",
	喆: "吉吉",
	喇: "口剌",
	喈: "口皆",
	喉: "口侯",
	喊: "口咸",
	喋: "口枼",
	喍: "口柴",
	喎: "口咼",
	喏: "口若",
	喐: "口郁",
	喑: "口音",
	喒: "口昝",
	喓: "口要",
	喔: "口屋",
	喕: "口面",
	喖: "口枯",
	喗: "口軍",
	喘: "口耑",
	喙: "口彖",
	喚: "口奐",
	喛: "口爰",
	喝: "口曷",
	喞: "口卽",
	喟: "口胃",
	喠: "口重",
	喡: "口韋",
	喢: "口臿",
	喤: "口皇",
	喥: "口度",
	喧: "口宣",
	喨: "口亮",
	喩: "口兪",
	喫: "口契",
	喭: "口彦",
	喯: "口奔",
	喰: "口食",
	喱: "口厘",
	喲: "口約",
	喳: "口查",
	喴: "口威",
	喵: "口苗",
	喷: "口贲",
	喹: "口奎",
	喺: "口係",
	喻: "口俞",
	喼: "口急",
	喽: "口娄",
	嗁: "口虒",
	嗃: "口高",
	嗄: "口夏",
	嗅: "口臭",
	嗆: "口倉",
	嗈: "口邕",
	嗉: "口素",
	嗊: "口貢",
	嗋: "口脅",
	嗌: "口益",
	嗍: "口朔",
	嗎: "口馬",
	嗏: "口茶",
	嗐: "口害",
	嗑: "口盍",
	嗒: "口荅",
	嗓: "口桑",
	嗔: "口真",
	嗕: "口辱",
	嗖: "口叟",
	嗗: "口骨",
	嗘: "口奚",
	嗙: "口旁",
	嗚: "口烏",
	嗛: "口兼",
	嗜: "口耆",
	嗝: "口鬲",
	嗞: "口兹",
	嗟: "口差",
	嗠: "孚各",
	嗡: "口翁",
	嗢: "口昷",
	嗤: "口蚩",
	嗥: "口皋",
	嗦: "口索",
	嗨: "口海",
	嗪: "口秦",
	嗫: "口聂",
	嗬: "口荷",
	嗮: "口晒",
	嗯: "口恩",
	嗰: "口個",
	嗱: "口拿",
	嗲: "口爹",
	嗳: "口爱",
	嗴: "口羗",
	嗵: "口通",
	嗶: "口畢",
	嗷: "口敖",
	嗹: "口連",
	嗺: "口崔",
	嗻: "口庶",
	嗼: "口莫",
	嗽: "口欶",
	嗾: "口族",
	嗿: "口貪",
	嘀: "口啇",
	嘁: "口戚",
	嘃: "口庸",
	嘄: "口梟",
	嘅: "口既",
	嘇: "口參",
	嘈: "口曹",
	嘊: "口崖",
	嘋: "口教",
	嘌: "口票",
	嘍: "口婁",
	嘎: "口戛",
	嘏: "古叚",
	嘐: "口翏",
	嘑: "口虖",
	嘒: "口彗",
	嘓: "口國",
	嘔: "口區",
	嘕: "口焉",
	嘖: "口責",
	嘘: "口虚",
	嘙: "口婆",
	嘚: "口得",
	嘛: "口麻",
	嘜: "口麥",
	嘝: "口斛",
	嘞: "口勒",
	嘟: "口都",
	嘠: "口戞",
	嘡: "口堂",
	嘢: "口野",
	嘣: "口崩",
	嘤: "口婴",
	嘥: "口徙",
	嘧: "口密",
	嘨: "口粛",
	嘩: "口華",
	嘪: "口買",
	嘫: "口然",
	嘬: "口最",
	嘭: "口彭",
	嘮: "口勞",
	嘯: "口肅",
	嘰: "口幾",
	嘱: "口属",
	嘲: "口朝",
	嘳: "口貴",
	嘴: "口觜",
	嘵: "口堯",
	嘶: "口斯",
	嘷: "口臯",
	嘸: "口無",
	嘹: "口尞",
	嘺: "口喬",
	嘻: "口喜",
	嘽: "口單",
	嘾: "口覃",
	嘿: "口黑",
	噀: "口巽",
	噁: "口惡",
	噂: "口尊",
	噃: "口番",
	噄: "口絜",
	噅: "口爲",
	噆: "口朁",
	噇: "口童",
	噈: "口就",
	噉: "口敢",
	噊: "口矞",
	噋: "口敦",
	噌: "口曾",
	噍: "口焦",
	噎: "口壹",
	噏: "口翕",
	噑: "口皐",
	噒: "口粦",
	噓: "口虛",
	噔: "口登",
	噖: "口琴",
	噗: "口菐",
	噘: "口厥",
	噙: "口禽",
	噚: "口尋",
	噛: "口歯",
	噜: "口鲁",
	噝: "口絲",
	噞: "口僉",
	噠: "口達",
	噡: "口詹",
	噢: "口奥",
	噣: "口蜀",
	噤: "口禁",
	噥: "口農",
	噦: "口歲",
	噧: "口萬",
	噪: "口喿",
	噫: "口意",
	噬: "口筮",
	噭: "口敫",
	噮: "口睘",
	噯: "口愛",
	噰: "口雍",
	噱: "口豦",
	噲: "口會",
	噳: "口虞",
	噴: "口賁",
	噶: "口葛",
	噷: "口歆",
	噸: "口頓",
	噹: "口當",
	噺: "口新",
	噻: "口塞",
	噼: "口辟",
	噽: "喜丕",
	噾: "口窨",
	噿: "口翠",
	嚀: "口寧",
	嚁: "口翟",
	嚂: "口監",
	嚃: "口遝",
	嚄: "口蒦",
	嚅: "口需",
	嚆: "口蒿",
	嚇: "口赫",
	嚈: "口厭",
	嚉: "口對",
	嚊: "口鼻",
	嚋: "口壽",
	嚌: "口齊",
	嚍: "口盡",
	嚎: "口豪",
	嚏: "口疐",
	嚐: "口嘗",
	嚑: "口熏",
	嚒: "口麼",
	嚓: "口察",
	嚕: "口魯",
	嚖: "口慧",
	嚗: "口暴",
	嚘: "口憂",
	嚙: "口齒",
	嚛: "口樂",
	嚜: "口墨",
	嚝: "口廣",
	嚟: "口黎",
	嚠: "口劉",
	嚡: "口鞋",
	嚤: "口摩",
	嚥: "口燕",
	嚦: "口歷",
	嚧: "口盧",
	嚨: "口龍",
	嚩: "口縛",
	嚪: "口閻",
	嚫: "口親",
	嚬: "口頻",
	嚭: "喜否",
	嚯: "口霍",
	嚰: "口磨",
	嚱: "口戲",
	嚲: "享單",
	嚵: "口毚",
	嚶: "口嬰",
	嚷: "口襄",
	嚸: "口點",
	嚹: "口罅",
	嚼: "口爵",
	嚽: "口贅",
	嚾: "口雚",
	嚿: "口舊",
	囀: "口轉",
	囁: "口聶",
	囃: "口雜",
	囄: "口離",
	囅: "單展",
	囆: "口蠆",
	囇: "口麗",
	囈: "口藝",
	囉: "口羅",
	囋: "口贊",
	囌: "口蘇",
	囍: "喜喜",
	囎: "口贈",
	囐: "口獻",
	囑: "口屬",
	囒: "口蘭",
	囓: "口齧",
	囔: "口囊",
	囕: "口覽",
	囖: "口籮",
	圠: "土乚",
	圢: "土丁",
	圤: "土卜",
	圦: "土入",
	圩: "土于",
	圪: "土乞",
	圫: "土乇",
	圬: "土亏",
	圮: "土己",
	圯: "土巳",
	地: "土也",
	圱: "千土",
	圲: "土千",
	圳: "土川",
	圴: "土勺",
	圵: "土上",
	圷: "土下",
	圸: "土山",
	圹: "土广",
	圻: "土斤",
	圽: "土勿",
	圾: "土及",
	圿: "土介",
	址: "土止",
	坁: "土氏",
	坂: "土反",
	坃: "土元",
	坄: "土殳",
	坅: "土今",
	坆: "土攵",
	均: "土匀",
	坈: "土冗",
	坉: "土屯",
	坊: "土方",
	坋: "土分",
	坍: "土丹",
	坎: "土欠",
	坏: "土不",
	坑: "土亢",
	块: "土夬",
	坘: "土互",
	坛: "土云",
	坜: "土历",
	坝: "土贝",
	坞: "土乌",
	坟: "土文",
	坡: "土皮",
	坢: "土半",
	坤: "土申",
	坥: "土且",
	坦: "土旦",
	坧: "土石",
	坨: "土它",
	坩: "土甘",
	坪: "土平",
	坫: "土占",
	坬: "土瓜",
	坭: "土尼",
	坮: "土台",
	坯: "土丕",
	坰: "土冋",
	坱: "土央",
	坲: "土弗",
	坳: "土幼",
	坵: "土丘",
	坶: "土母",
	坷: "土可",
	坸: "土句",
	坹: "土穴",
	坺: "土犮",
	坻: "土氐",
	坼: "土斥",
	坽: "土令",
	坾: "土宁",
	坿: "土付",
	垀: "土乎",
	垁: "土矢",
	垃: "土立",
	垅: "土龙",
	垆: "土卢",
	垇: "土凹",
	垉: "土包",
	垊: "土民",
	垌: "土同",
	垍: "土自",
	垎: "土各",
	垏: "土聿",
	垑: "土多",
	垓: "土亥",
	垗: "土兆",
	垘: "土伏",
	垙: "土光",
	垛: "土朵",
	垜: "土朶",
	垝: "土危",
	垞: "土宅",
	垟: "土羊",
	垠: "土艮",
	垢: "土后",
	垣: "土亘",
	垤: "土至",
	垥: "土合",
	垧: "土向",
	垨: "土守",
	垪: "土并",
	垬: "土共",
	垭: "土亚",
	垮: "土夸",
	垯: "土达",
	垱: "土当",
	垲: "土岂",
	垳: "土行",
	垵: "土安",
	垶: "土辛",
	垷: "土見",
	垸: "土完",
	垹: "土邦",
	垺: "土孚",
	垻: "土貝",
	垾: "土旱",
	垿: "土序",
	埁: "土岑",
	埂: "土更",
	埃: "土矣",
	埄: "土夆",
	埆: "土角",
	埇: "土甬",
	埈: "土夋",
	埉: "土夾",
	埋: "土里",
	埌: "土良",
	埍: "土肙",
	城: "土成",
	埏: "土延",
	埒: "土寽",
	埔: "土甫",
	埕: "土呈",
	埖: "土花",
	埗: "土步",
	埘: "土时",
	埙: "土员",
	埚: "土呙",
	埛: "土冏",
	埝: "土念",
	埞: "土定",
	域: "土或",
	埠: "土阜",
	埡: "土亞",
	埢: "土卷",
	埣: "土卒",
	埤: "土卑",
	埥: "土青",
	埦: "土宛",
	埧: "土具",
	埨: "土侖",
	埩: "土争",
	埪: "土空",
	埫: "土尚",
	埬: "土東",
	埭: "土隶",
	埮: "土炎",
	埯: "土奄",
	埰: "土采",
	埱: "土叔",
	埲: "土奉",
	埳: "土臽",
	埴: "土直",
	埵: "土垂",
	埶: "坴丸",
	執: "幸丸",
	埸: "土易",
	培: "土咅",
	埻: "土享",
	埼: "土奇",
	埽: "土帚",
	堀: "土屈",
	堁: "土果",
	堄: "土兒",
	堆: "土隹",
	堈: "土岡",
	堉: "土育",
	堋: "土朋",
	堌: "土固",
	堍: "土兔",
	堎: "土夌",
	堐: "土厓",
	堒: "土昆",
	堓: "土岸",
	堔: "土罙",
	堗: "土突",
	堙: "土垔",
	堚: "土軍",
	堛: "土畐",
	堜: "土柬",
	堝: "土咼",
	堞: "土枼",
	堟: "土彖",
	堠: "土侯",
	堢: "土保",
	堣: "土禺",
	堤: "土是",
	堦: "土皆",
	堧: "土耎",
	堨: "土曷",
	堩: "土恆",
	堪: "土甚",
	堬: "土俞",
	堭: "土皇",
	堮: "土咢",
	堰: "土匽",
	堳: "土眉",
	場: "土昜",
	堵: "土者",
	堷: "土音",
	堸: "土風",
	堹: "土重",
	堺: "土界",
	堽: "土罡",
	堾: "土春",
	堿: "土咸",
	塀: "土屏",
	塂: "土巷",
	塃: "土荒",
	塅: "土段",
	塆: "土弯",
	塇: "土宣",
	塉: "土脊",
	塊: "土鬼",
	塎: "土容",
	塏: "土豈",
	塐: "土素",
	塒: "土時",
	塓: "土冥",
	塔: "土荅",
	塕: "土翁",
	塖: "土乘",
	塘: "土唐",
	塙: "土高",
	塚: "土冢",
	塛: "土栗",
	塜: "土冡",
	塝: "土旁",
	塠: "土追",
	塡: "土眞",
	塢: "土烏",
	塤: "土員",
	塥: "土鬲",
	塧: "土益",
	塨: "土恭",
	塪: "土舀",
	填: "土真",
	塬: "土原",
	塭: "土昷",
	塮: "土射",
	塯: "土留",
	塳: "土逢",
	塴: "土崩",
	塶: "土鹿",
	塷: "土鹵",
	塸: "土區",
	塻: "土莫",
	塼: "土專",
	塽: "土爽",
	塿: "土婁",
	墀: "土犀",
	墁: "土曼",
	墂: "土票",
	境: "土竟",
	墄: "土戚",
	墆: "土帶",
	墇: "土章",
	墈: "土勘",
	墉: "土庸",
	墋: "土參",
	墌: "土庶",
	墎: "土郭",
	墐: "土堇",
	墑: "土啇",
	墒: "土商",
	墔: "土崔",
	墕: "土焉",
	墖: "土畣",
	増: "土曽",
	墘: "土乾",
	墙: "土啬",
	墚: "土梁",
	墛: "土尉",
	墝: "土堯",
	增: "土曾",
	墟: "土虚",
	墠: "土單",
	墡: "土善",
	墢: "土發",
	墣: "土菐",
	墤: "土貴",
	墥: "土童",
	墦: "土番",
	墧: "土喬",
	墩: "土敦",
	墫: "土尊",
	墭: "土盛",
	墰: "土覃",
	墱: "土登",
	墲: "土無",
	墳: "土賁",
	墴: "土黄",
	墵: "土雲",
	墶: "土達",
	墷: "土華",
	墸: "土著",
	墹: "土間",
	墺: "土奥",
	墻: "土嗇",
	墽: "土敫",
	墿: "土睪",
	壀: "土辟",
	壃: "土畺",
	壇: "土亶",
	壈: "土稟",
	壉: "土豦",
	壋: "土當",
	壎: "土熏",
	壏: "土監",
	壒: "土蓋",
	壔: "土壽",
	壕: "土豪",
	壖: "土需",
	壗: "土盡",
	壙: "土廣",
	壚: "土盧",
	壛: "土閻",
	壜: "土曇",
	壝: "土遺",
	壞: "土褱",
	壠: "土龍",
	壡: "睿圣",
	壢: "土歷",
	壣: "土聯",
	壤: "土襄",
	壦: "土雚",
	壧: "土嚴",
	壩: "土霸",
	壪: "土彎",
	壮: "丬士",
	壯: "爿士",
	壻: "士胥",
	壾: "壴巨",
	壿: "士尊",
	外: "夕卜",
	夘: "夕卩",
	夝: "夕生",
	够: "句多",
	夠: "多句",
	夡: "多吉",
	夥: "果多",
	夦: "甚多",
	失: "丿夫",
	夶: "大大",
	奲: "奢單",
	奴: "女又",
	奵: "女丁",
	奶: "女乃",
	奷: "女千",
	奸: "女干",
	她: "女也",
	奺: "女久",
	奻: "女女",
	奼: "女乇",
	好: "女子",
	奾: "女山",
	妀: "己女",
	妁: "女勺",
	如: "女口",
	妃: "女己",
	妅: "女工",
	妆: "丬女",
	妇: "女彐",
	妈: "女马",
	妉: "女冘",
	妊: "女壬",
	妋: "女夫",
	妌: "女井",
	妍: "女开",
	妎: "女介",
	妏: "女文",
	妐: "女公",
	妑: "女巴",
	妒: "女户",
	妓: "女支",
	妔: "女亢",
	妕: "女中",
	妖: "女夭",
	妗: "女今",
	妘: "女云",
	妙: "女少",
	妚: "女不",
	妜: "女夬",
	妝: "爿女",
	妞: "女丑",
	妠: "女内",
	妡: "女斤",
	妢: "女分",
	妣: "女比",
	妤: "女予",
	妦: "女丰",
	妧: "女元",
	妨: "女方",
	妩: "女无",
	妪: "女区",
	妫: "女为",
	妬: "女石",
	妭: "女犮",
	妮: "女尼",
	妯: "女由",
	妰: "女乍",
	妱: "女召",
	妲: "女旦",
	妳: "女尔",
	妵: "女主",
	妶: "女玄",
	妷: "女失",
	妸: "女可",
	妹: "女未",
	妺: "女末",
	妼: "女必",
	妽: "女申",
	姀: "女禾",
	姁: "女句",
	姂: "女乏",
	姃: "女正",
	姄: "女民",
	姅: "女半",
	姆: "女母",
	姈: "女令",
	姉: "女巿",
	始: "女台",
	姌: "女冉",
	姍: "女冊",
	姎: "女央",
	姏: "女甘",
	姐: "女且",
	姑: "女古",
	姒: "女以",
	姓: "女生",
	姖: "女巨",
	姗: "女册",
	姘: "女并",
	姙: "女任",
	姚: "女兆",
	姛: "女同",
	姝: "女朱",
	姞: "女吉",
	姟: "女亥",
	姠: "女向",
	姡: "女舌",
	姣: "女交",
	姤: "女后",
	姥: "女老",
	姨: "女夷",
	姩: "女年",
	姪: "女至",
	姫: "女臣",
	姮: "女亘",
	姯: "女光",
	姰: "女旬",
	姱: "女夸",
	姲: "女安",
	姳: "女名",
	姶: "女合",
	姷: "女有",
	姸: "女幵",
	姹: "女宅",
	姺: "女先",
	姻: "女因",
	姼: "女多",
	姽: "女危",
	姾: "女全",
	娀: "女戎",
	娂: "女共",
	娃: "女圭",
	娅: "女亚",
	娆: "女尧",
	娇: "女乔",
	娉: "女甹",
	娊: "女見",
	娋: "女肖",
	娌: "女里",
	娍: "女成",
	娏: "女尨",
	娐: "女孚",
	娒: "女每",
	娓: "女尾",
	娔: "女克",
	娕: "女束",
	娖: "女足",
	娗: "女廷",
	娘: "女良",
	娙: "女巠",
	娚: "女男",
	娛: "女吳",
	娜: "女那",
	娝: "女否",
	娞: "女妥",
	娟: "女肙",
	娠: "女辰",
	娡: "女志",
	娢: "女含",
	娣: "女弟",
	娥: "女我",
	娦: "女兵",
	娧: "女兑",
	娨: "女旱",
	娩: "女免",
	娪: "女吾",
	娫: "女延",
	娬: "女武",
	娭: "女矣",
	娮: "女言",
	娯: "女呉",
	娰: "女似",
	娱: "女吴",
	娲: "女呙",
	娳: "女利",
	娴: "女闲",
	娵: "女取",
	娷: "女垂",
	娸: "女其",
	娹: "女弦",
	娺: "女叕",
	娻: "女東",
	娼: "女昌",
	娽: "女录",
	娾: "女厓",
	婀: "女阿",
	婂: "女帛",
	婃: "女宗",
	婄: "女咅",
	婅: "女匊",
	婇: "女采",
	婈: "女夌",
	婉: "女宛",
	婊: "女表",
	婋: "女虎",
	婌: "女叔",
	婍: "女奇",
	婎: "女隹",
	婏: "女兔",
	婐: "女果",
	婑: "女委",
	婒: "女炎",
	婔: "女非",
	婕: "女疌",
	婖: "女忝",
	婗: "女兒",
	婘: "女卷",
	婙: "女争",
	婚: "女昏",
	婛: "女京",
	婝: "女定",
	婞: "女幸",
	婟: "女固",
	婠: "女官",
	婡: "女來",
	婢: "女卑",
	婤: "女周",
	婥: "女卓",
	婦: "女帚",
	婧: "女青",
	婨: "女侖",
	婩: "女岸",
	婫: "女昆",
	婭: "女亞",
	婮: "女居",
	婰: "女典",
	婲: "女花",
	婳: "女画",
	婵: "女单",
	婶: "女审",
	婷: "女亭",
	婸: "女昜",
	婹: "女要",
	婻: "女南",
	婼: "女若",
	婽: "女叚",
	婾: "女兪",
	婿: "女胥",
	媀: "女禺",
	媁: "女韋",
	媂: "女帝",
	媃: "女柔",
	媄: "女美",
	媅: "女甚",
	媆: "女耎",
	媇: "女亲",
	媈: "女軍",
	媉: "女屋",
	媊: "女前",
	媋: "女春",
	媌: "女苗",
	媍: "女負",
	媎: "女者",
	媏: "女耑",
	媑: "女重",
	媒: "女某",
	媓: "女皇",
	媔: "女面",
	媕: "女弇",
	媖: "女英",
	媗: "女宣",
	媘: "女皆",
	媙: "女威",
	媚: "女眉",
	媛: "女爰",
	媜: "女貞",
	媞: "女是",
	媟: "女枼",
	媡: "女柬",
	媢: "女冒",
	媣: "女染",
	媤: "女思",
	媥: "女扁",
	媦: "女胃",
	媧: "女咼",
	媨: "女酋",
	媩: "女胡",
	媪: "女昷",
	媬: "女保",
	媮: "女俞",
	媯: "女為",
	媰: "女芻",
	媳: "女息",
	媴: "女袁",
	媶: "女茸",
	媷: "女辱",
	媸: "女蚩",
	媹: "女留",
	媽: "女馬",
	媾: "女冓",
	媿: "女鬼",
	嫀: "女秦",
	嫁: "女家",
	嫂: "女叟",
	嫃: "女真",
	嫄: "女原",
	嫅: "女差",
	嫆: "女容",
	嫇: "女冥",
	嫉: "女疾",
	嫊: "女素",
	嫋: "女弱",
	嫌: "女兼",
	嫍: "女舀",
	嫎: "女旁",
	嫏: "女郎",
	嫒: "女爱",
	嫔: "女宾",
	嫕: "女悘",
	嫖: "女票",
	嫗: "女區",
	嫘: "女累",
	嫙: "女旋",
	嫚: "女曼",
	嫜: "女章",
	嫝: "女康",
	嫞: "女庸",
	嫟: "女匿",
	嫡: "女啇",
	嫣: "女焉",
	嫤: "女堇",
	嫥: "女專",
	嫦: "女常",
	嫧: "女責",
	嫩: "女敕",
	嫪: "女翏",
	嫫: "女莫",
	嫬: "女庶",
	嫭: "女虖",
	嫮: "女雩",
	嫰: "女欶",
	嫱: "女啬",
	嫲: "女麻",
	嫴: "女辜",
	嫵: "女無",
	嫶: "女焦",
	嫷: "女隋",
	嫸: "女善",
	嫹: "女黄",
	嫺: "女閒",
	嫻: "女閑",
	嫼: "女黑",
	嫽: "女尞",
	嫾: "女粦",
	嫿: "女畫",
	嬀: "女爲",
	嬁: "女登",
	嬂: "女戠",
	嬄: "女壹",
	嬅: "女華",
	嬆: "女翕",
	嬇: "女貴",
	嬈: "女堯",
	嬉: "女喜",
	嬋: "女單",
	嬌: "女喬",
	嬏: "女番",
	嬐: "女僉",
	嬑: "女意",
	嬒: "女會",
	嬓: "女敫",
	嬕: "女睪",
	嬗: "女亶",
	嬘: "女遂",
	嬙: "女嗇",
	嬚: "女廉",
	嬛: "女睘",
	嬝: "女裊",
	嬞: "女董",
	嬟: "女義",
	嬠: "女喿",
	嬡: "女愛",
	嬣: "女寧",
	嬤: "女麼",
	嬥: "女翟",
	嬦: "女壽",
	嬧: "女盡",
	嬨: "女慈",
	嬩: "女與",
	嬪: "女賓",
	嬫: "女榮",
	嬬: "女需",
	嬭: "女爾",
	嬯: "女臺",
	嬳: "女蒦",
	嬵: "女綿",
	嬶: "女鼻",
	嬷: "女麽",
	嬸: "女審",
	嬹: "女興",
	嬺: "女慝",
	嬻: "女賣",
	嬼: "女劉",
	嬾: "女頼",
	嬿: "女燕",
	孀: "女霜",
	孂: "女簋",
	孃: "女襄",
	孄: "女闌",
	孅: "女韱",
	孆: "女嬰",
	孇: "女雙",
	孈: "女巂",
	孉: "女雚",
	孊: "女靡",
	孋: "女麗",
	孍: "女嚴",
	孎: "女屬",
	孏: "女蘭",
	孔: "子乚",
	孖: "子子",
	孙: "子小",
	孜: "子攵",
	孡: "子台",
	孢: "子包",
	孤: "子瓜",
	孩: "子亥",
	孫: "子系",
	孭: "子貝",
	孮: "子宗",
	孰: "享丸",
	孲: "子亞",
	孵: "卵孚",
	孺: "子需",
	孻: "子盡",
	孾: "子嬰",
	对: "又寸",
	対: "文寸",
	尀: "叵寸",
	封: "圭寸",
	射: "身寸",
	尌: "壴寸",
	尠: "甚少",
	尡: "光昆",
	就: "京尤",
	屔: "丘尼",
	屷: "山乃",
	屸: "山工",
	屹: "山乞",
	屺: "山己",
	屻: "山刃",
	屼: "山兀",
	屽: "山干",
	屾: "山山",
	屿: "山与",
	岄: "山月",
	岅: "山反",
	岆: "山夭",
	岈: "山牙",
	岉: "山勿",
	岋: "山及",
	岍: "山开",
	岎: "山分",
	岏: "山元",
	岐: "山支",
	岒: "山今",
	岓: "山斤",
	岖: "山区",
	岘: "山见",
	岞: "山乍",
	岟: "山央",
	岠: "山巨",
	岣: "山句",
	岤: "山穴",
	岥: "山皮",
	岨: "山且",
	岫: "山由",
	岬: "山甲",
	岭: "山令",
	岮: "山它",
	岯: "山丕",
	岰: "山幼",
	岲: "山兄",
	岴: "山丘",
	岵: "山古",
	岶: "山白",
	岷: "山民",
	岹: "山召",
	岻: "山氐",
	岼: "山平",
	岾: "山占",
	峂: "山冬",
	峅: "山弁",
	峆: "山合",
	峈: "山各",
	峋: "山旬",
	峌: "山至",
	峍: "山聿",
	峏: "山而",
	峐: "山亥",
	峒: "山同",
	峓: "山夷",
	峔: "山老",
	峖: "山安",
	峗: "山危",
	峘: "山亘",
	峙: "山寺",
	峡: "山夹",
	峢: "山列",
	峣: "山尧",
	峤: "山乔",
	峥: "山争",
	峧: "山交",
	峨: "山我",
	峪: "山谷",
	峫: "山邪",
	峬: "山甫",
	峭: "山肖",
	峮: "山君",
	峰: "山夆",
	峴: "山見",
	峵: "山宏",
	峸: "山成",
	峺: "山更",
	峻: "山夋",
	峼: "山告",
	峽: "山夾",
	峿: "山吾",
	崂: "山劳",
	崃: "山来",
	崄: "山佥",
	崅: "山角",
	崆: "山空",
	崉: "山沓",
	崊: "山林",
	崌: "山居",
	崍: "山來",
	崎: "山奇",
	崏: "山昏",
	崐: "山昆",
	崓: "山固",
	崕: "山厓",
	崘: "山侖",
	崚: "山夌",
	崛: "山屈",
	崜: "山垂",
	崝: "山青",
	崞: "山享",
	崠: "山東",
	崡: "山函",
	崢: "山爭",
	崤: "山肴",
	崥: "山卑",
	崦: "山奄",
	崨: "山疌",
	崪: "山卒",
	崯: "山金",
	崰: "山甾",
	崲: "山皇",
	崳: "山俞",
	崵: "山昜",
	崷: "山酋",
	崸: "山頁",
	崹: "山帝",
	崻: "山待",
	崼: "山是",
	崾: "山要",
	崿: "山咢",
	嵁: "山甚",
	嵃: "山彦",
	嵄: "山美",
	嵈: "山爰",
	嵉: "山亭",
	嵊: "山乘",
	嵋: "山眉",
	嵎: "山禺",
	嵑: "山曷",
	嵖: "山查",
	嵘: "山荣",
	嵝: "山娄",
	嵠: "山奚",
	嵢: "山倉",
	嵣: "山唐",
	嵥: "山桀",
	嵦: "山豈",
	嵧: "山留",
	嵨: "山烏",
	嵪: "山高",
	嵫: "山兹",
	嵭: "山旁",
	嵮: "山真",
	嵯: "山差",
	嵰: "山兼",
	嵱: "山容",
	嵲: "山臬",
	嵴: "山脊",
	嵶: "山弱",
	嵷: "山從",
	嵺: "山翏",
	嵻: "山康",
	嵼: "山産",
	嵽: "山帶",
	嶀: "山雩",
	嶁: "山婁",
	嶂: "山章",
	嶃: "山斬",
	嶆: "山曹",
	嶇: "山區",
	嶋: "山鳥",
	嶍: "山習",
	嶑: "山象",
	嶒: "山曾",
	嶓: "山番",
	嶕: "山焦",
	嶖: "山翕",
	嶗: "山勞",
	嶙: "山粦",
	嶛: "山尞",
	嶝: "山登",
	嶟: "山尊",
	嶠: "山喬",
	嶢: "山堯",
	嶥: "山厥",
	嶦: "山詹",
	嶧: "山睪",
	嶩: "山農",
	嶫: "山業",
	嶬: "山義",
	嶮: "山僉",
	嶰: "山解",
	嶱: "山葛",
	嶵: "山罪",
	嶸: "山榮",
	嶹: "山壽",
	嶼: "山與",
	嶿: "山需",
	巁: "山厲",
	巄: "山龍",
	巆: "山營",
	巇: "山戲",
	巉: "山毚",
	巊: "山嬰",
	巎: "山夒",
	巏: "山雚",
	巑: "山贊",
	巗: "山嚴",
	巘: "山獻",
	巙: "山夔",
	巧: "工丂",
	巩: "工凡",
	巶: "召巳",
	帄: "巾丁",
	帆: "巾凡",
	帉: "巾分",
	帊: "巾巴",
	帎: "巾冘",
	帏: "巾韦",
	帐: "巾长",
	帓: "巾末",
	帔: "巾皮",
	帕: "巾白",
	帖: "巾占",
	帗: "巾犮",
	帙: "巾失",
	帜: "巾只",
	帞: "巾百",
	帡: "巾并",
	帢: "巾合",
	帧: "巾贞",
	帨: "巾兑",
	帩: "巾肖",
	帪: "巾辰",
	帱: "巾寿",
	帲: "巾幷",
	帳: "巾長",
	帴: "巾戔",
	帵: "巾宛",
	帷: "巾隹",
	帹: "巾妾",
	帺: "巾其",
	帻: "巾责",
	帼: "巾国",
	帽: "巾冒",
	帾: "巾者",
	帿: "巾侯",
	幀: "巾貞",
	幁: "巾頁",
	幃: "巾韋",
	幄: "巾屋",
	幅: "巾畐",
	幆: "巾曷",
	幈: "巾屏",
	幉: "巾枼",
	幊: "巾貢",
	幌: "巾晃",
	幍: "巾舀",
	幎: "巾冥",
	幏: "巾家",
	幐: "月帣",
	幒: "巾悤",
	幓: "巾參",
	幔: "巾曼",
	幖: "巾票",
	幗: "巾國",
	幘: "巾責",
	幙: "巾莫",
	幛: "巾章",
	幜: "巾景",
	幝: "巾單",
	幞: "巾菐",
	幟: "巾戠",
	幠: "巾無",
	幡: "巾番",
	幢: "巾童",
	幥: "巾掌",
	幧: "巾喿",
	幨: "巾詹",
	幩: "巾賁",
	幪: "巾蒙",
	幬: "巾壽",
	幭: "巾蔑",
	幮: "巾廚",
	幯: "巾節",
	幰: "巾憲",
	幱: "巾闌",
	幵: "干干",
	幼: "幺力",
	弑: "杀式",
	引: "弓丨",
	弘: "弓厶",
	弙: "弓于",
	弛: "弓也",
	弜: "弓弓",
	弝: "弓巴",
	弞: "弓欠",
	张: "弓长",
	弡: "弓巨",
	弣: "弓付",
	弤: "弓氐",
	弥: "弓尔",
	弦: "弓玄",
	弧: "弓瓜",
	弨: "弓召",
	弫: "弓臣",
	弭: "弓耳",
	弰: "弓肖",
	弲: "弓肙",
	弳: "弓巠",
	弴: "弓享",
	張: "弓長",
	弶: "弓京",
	弸: "弓朋",
	弹: "弓单",
	强: "弓虽",
	弽: "弓枼",
	弾: "弓単",
	彁: "弓哥",
	彃: "弓畢",
	彄: "弓區",
	彅: "弓剪",
	彇: "弓肅",
	彈: "弓單",
	彉: "弓黄",
	彊: "弓畺",
	彋: "弓睘",
	彌: "弓爾",
	彍: "弓廣",
	彏: "弓矍",
	彟: "寻蒦",
	彠: "尋蒦",
	形: "开彡",
	彤: "丹彡",
	彨: "丽彡",
	彩: "采彡",
	彫: "周彡",
	彬: "林彡",
	彭: "壴彡",
	彮: "容彡",
	彯: "票彡",
	彰: "章彡",
	影: "景彡",
	彲: "麗彡",
	彴: "彳勺",
	彵: "彳也",
	彶: "彳及",
	彷: "彳方",
	彸: "彳公",
	役: "彳殳",
	彺: "彳王",
	彻: "彳切",
	彼: "彳皮",
	彽: "彳氐",
	彾: "彳令",
	彿: "彳弗",
	往: "彳主",
	征: "彳正",
	徂: "彳且",
	徃: "彳生",
	待: "彳寺",
	徆: "彳西",
	徇: "彳旬",
	很: "彳艮",
	徉: "彳羊",
	徊: "彳回",
	律: "彳聿",
	徍: "彳圭",
	徎: "彳呈",
	徏: "彳步",
	徐: "彳余",
	徑: "彳巠",
	徒: "彳走",
	徔: "彳芝",
	徕: "彳来",
	徖: "彳宗",
	徘: "彳非",
	徙: "彳歨",
	徚: "彳柬",
	徛: "彳奇",
	徜: "彳尚",
	徝: "彳直",
	徟: "彳周",
	徠: "彳來",
	御: "彳卸",
	徢: "彳疌",
	徣: "彳昔",
	徤: "彳建",
	徥: "彳是",
	徦: "彳叚",
	徧: "彳扁",
	徨: "彳皇",
	復: "彳复",
	循: "彳盾",
	徫: "彳韋",
	徬: "彳旁",
	徯: "彳奚",
	徱: "彳票",
	徲: "彳犀",
	徶: "彳敝",
	徸: "彳童",
	徺: "彳堯",
	徻: "彳會",
	徼: "彳敫",
	徿: "彳龍",
	忀: "彳襄",
	忂: "彳瞿",
	忆: "忄乙",
	忇: "忄力",
	忉: "忄刀",
	忊: "忄丁",
	忋: "忄己",
	忏: "忄千",
	忓: "忄干",
	忔: "忄乞",
	忕: "忄大",
	忖: "忄寸",
	忙: "忄亡",
	忚: "忄也",
	忛: "忄凡",
	忟: "忄文",
	忡: "忄中",
	忣: "忄及",
	忤: "忄午",
	忦: "忄介",
	忧: "忄尤",
	忨: "忄元",
	忪: "忄公",
	快: "忄夬",
	忬: "忄予",
	忭: "忄卞",
	忮: "忄支",
	忯: "忄氏",
	忰: "忄卆",
	忱: "忄冘",
	忲: "忄太",
	忳: "忄屯",
	忴: "忄今",
	忶: "忄云",
	忷: "忄凶",
	忸: "忄丑",
	忹: "忄王",
	忺: "忄欠",
	忻: "忄斤",
	忼: "忄亢",
	忾: "忄气",
	怀: "忄不",
	怃: "忄无",
	怄: "忄区",
	怅: "忄长",
	怆: "忄仓",
	怇: "忄巨",
	怈: "忄世",
	怉: "忄包",
	怊: "忄召",
	怋: "忄民",
	怌: "忄丕",
	怍: "忄乍",
	怏: "忄央",
	怐: "忄句",
	怑: "忄半",
	怓: "忄奴",
	怔: "忄正",
	怕: "忄白",
	怖: "忄布",
	怗: "忄占",
	怙: "忄古",
	怚: "忄且",
	怛: "忄旦",
	怜: "忄令",
	怞: "忄由",
	怟: "忄氐",
	怡: "忄台",
	怢: "忄失",
	怦: "忄平",
	性: "忄生",
	怩: "忄尼",
	怪: "忄圣",
	怫: "忄弗",
	怬: "忄四",
	怭: "忄必",
	怮: "忄幼",
	怯: "忄去",
	怰: "忄玄",
	怲: "忄丙",
	怳: "忄兄",
	怴: "忄戉",
	怵: "忄术",
	怶: "忄皮",
	怺: "忄永",
	怽: "忄未",
	怾: "忄只",
	恀: "忄多",
	恂: "忄旬",
	恃: "忄寺",
	恄: "忄吉",
	恅: "忄老",
	恆: "忄亙",
	恇: "忄匡",
	恈: "忄牟",
	恉: "忄旨",
	恊: "忄劦",
	恌: "忄兆",
	恍: "忄光",
	恎: "忄至",
	恑: "忄危",
	恒: "忄亘",
	恓: "忄西",
	恔: "忄交",
	恗: "忄夸",
	恘: "忄休",
	恛: "忄回",
	恜: "忄式",
	恞: "忄夷",
	恟: "忄匈",
	恠: "忄在",
	恢: "忄灰",
	恤: "忄血",
	恥: "耳心",
	恦: "忄向",
	恨: "忄艮",
	恪: "忄各",
	恫: "忄同",
	恬: "忄舌",
	恮: "忄全",
	恰: "忄合",
	恱: "忄兊",
	恲: "忄并",
	恸: "忄动",
	恹: "忄厌",
	恺: "忄岂",
	恻: "忄则",
	恽: "忄军",
	恾: "忄芒",
	悀: "忄甬",
	悁: "忄肙",
	悂: "忄坒",
	悃: "忄困",
	悄: "忄肖",
	悅: "忄兌",
	悇: "忄余",
	悈: "忄戒",
	悋: "忄吝",
	悌: "忄弟",
	悍: "忄旱",
	悎: "忄告",
	悏: "忄夾",
	悑: "忄甫",
	悒: "忄邑",
	悓: "忄見",
	悔: "忄每",
	悕: "忄希",
	悖: "忄孛",
	悗: "忄免",
	悙: "忄亨",
	悚: "忄束",
	悛: "忄夋",
	悜: "忄呈",
	悝: "忄里",
	悞: "忄吳",
	悟: "忄吾",
	悢: "忄良",
	悦: "忄兑",
	悧: "忄利",
	悭: "忄坚",
	悮: "忄吴",
	悯: "忄闵",
	悰: "忄宗",
	悱: "忄非",
	悴: "忄卒",
	悵: "忄長",
	悷: "忄戾",
	悸: "忄季",
	悺: "忄官",
	悻: "忄幸",
	悼: "忄卓",
	悽: "忄妻",
	悾: "忄空",
	悿: "忄忝",
	惀: "忄侖",
	惂: "忄臽",
	惃: "忄昆",
	情: "忄青",
	惆: "忄周",
	惇: "忄享",
	惈: "忄果",
	惊: "忄京",
	惋: "忄宛",
	惍: "忄金",
	惏: "忄林",
	惐: "忄或",
	惓: "忄卷",
	惔: "忄炎",
	惕: "忄易",
	惗: "忄念",
	惘: "忄罔",
	惙: "忄叕",
	惚: "忄忽",
	惛: "忄昏",
	惜: "忄昔",
	惝: "忄尚",
	惞: "忄欣",
	惟: "忄隹",
	惤: "忄弦",
	惦: "忄店",
	惧: "忄具",
	惨: "忄参",
	惭: "忄斩",
	惮: "忄单",
	惯: "忄贯",
	惲: "忄軍",
	惴: "忄耑",
	惵: "忄枼",
	惶: "忄皇",
	惺: "忄星",
	惻: "忄則",
	惼: "忄扁",
	惽: "忄昬",
	惿: "忄是",
	愀: "忄秋",
	愃: "忄宣",
	愄: "忄畏",
	愅: "忄革",
	愇: "忄韋",
	愉: "忄俞",
	愊: "忄畐",
	愋: "忄爰",
	愌: "忄奐",
	愎: "忄复",
	愐: "忄面",
	愑: "忄勇",
	愒: "忄曷",
	愓: "忄昜",
	愔: "忄音",
	愕: "忄咢",
	愖: "忄甚",
	愘: "忄客",
	愜: "忄匧",
	愝: "忄匽",
	愞: "忄耎",
	愠: "忄昷",
	愡: "忄怱",
	愢: "忄思",
	愤: "忄贲",
	愥: "忄英",
	愦: "忄贵",
	愧: "忄鬼",
	愩: "忄貢",
	愪: "忄員",
	愫: "忄素",
	愭: "忄耆",
	愯: "忄隻",
	愰: "忄晃",
	愱: "忄疾",
	愲: "忄骨",
	愴: "忄倉",
	愵: "忄弱",
	愶: "忄脅",
	愷: "忄豈",
	愹: "忄容",
	愺: "忄草",
	愼: "忄眞",
	愽: "忄尃",
	愾: "忄氣",
	慀: "忄奚",
	慃: "忄翁",
	慄: "忄栗",
	慅: "忄蚤",
	慆: "忄舀",
	慉: "忄畜",
	慊: "忄兼",
	慌: "忄荒",
	慎: "忄真",
	慏: "忄冥",
	慑: "忄聂",
	慒: "忄曹",
	慓: "忄票",
	慔: "忄莫",
	慖: "忄國",
	慘: "忄參",
	慚: "忄斬",
	慛: "忄崔",
	慞: "忄章",
	慟: "忄動",
	慠: "忄敖",
	慡: "忄爽",
	慢: "忄曼",
	慣: "忄貫",
	慥: "忄造",
	慨: "忄既",
	慩: "忄連",
	慪: "忄區",
	慬: "忄堇",
	慱: "忄專",
	慳: "忄堅",
	慴: "忄習",
	慵: "忄庸",
	慷: "忄康",
	慺: "忄婁",
	慻: "忄眷",
	慽: "忄戚",
	憀: "忄翏",
	憁: "忄悤",
	憆: "忄堂",
	憈: "忄虚",
	憉: "忄彭",
	憍: "忄喬",
	憎: "忄曾",
	憏: "忄祭",
	憐: "忄粦",
	憒: "忄貴",
	憓: "忄惠",
	憔: "忄焦",
	憕: "忄登",
	憘: "忄喜",
	憚: "忄單",
	憛: "忄覃",
	憜: "忄隋",
	憞: "忄敦",
	憟: "忄粟",
	憡: "忄策",
	憢: "忄堯",
	憣: "忄番",
	憤: "忄賁",
	憦: "忄勞",
	憧: "忄童",
	憪: "忄閒",
	憫: "忄閔",
	憬: "忄景",
	憭: "忄尞",
	憮: "忄無",
	憯: "忄朁",
	憰: "忄矞",
	憱: "忄就",
	憳: "忄尋",
	憴: "忄黽",
	憶: "忄意",
	憷: "忄楚",
	憸: "忄僉",
	憹: "忄農",
	憺: "忄詹",
	憻: "忄亶",
	憽: "忄葱",
	憾: "忄感",
	憿: "忄敫",
	懀: "忄會",
	懁: "忄睘",
	懂: "忄董",
	懄: "忄勤",
	懅: "忄豦",
	懆: "忄喿",
	懈: "忄解",
	懊: "忄奥",
	懌: "忄睪",
	懍: "忄稟",
	懎: "忄嗇",
	懏: "忄雋",
	懒: "忄赖",
	懓: "忄愛",
	懔: "忄禀",
	懗: "忄赫",
	懙: "忄與",
	懛: "忄臺",
	懜: "忄夢",
	懝: "忄疑",
	懞: "忄蒙",
	懠: "忄齊",
	懡: "忄麽",
	懢: "忄監",
	懤: "忄壽",
	懥: "忄疐",
	懦: "忄需",
	懧: "忄寧",
	懨: "忄厭",
	懩: "忄養",
	懪: "忄暴",
	懫: "忄質",
	懭: "忄廣",
	懮: "忄憂",
	懰: "忄劉",
	懱: "忄蔑",
	懳: "忄慧",
	懴: "忄韯",
	懵: "忄瞢",
	懶: "忄賴",
	懷: "忄褱",
	懹: "忄襄",
	懺: "忄韱",
	懻: "忄冀",
	懼: "忄瞿",
	懽: "忄雚",
	懾: "忄聶",
	懿: "壹恣",
	戂: "忄靡",
	戃: "忄黨",
	戄: "忄矍",
	戏: "又戈",
	戕: "爿戈",
	戗: "仓戈",
	战: "占戈",
	戙: "同戈",
	戝: "貝戈",
	戡: "甚戈",
	戢: "咠戈",
	戣: "癸戈",
	戤: "盈戈",
	戥: "星戈",
	戦: "単戈",
	戧: "倉戈",
	戨: "哥戈",
	戩: "晉戈",
	戫: "有或",
	戬: "晋戈",
	戭: "寅戈",
	戮: "翏戈",
	戯: "虚戈",
	戰: "單戈",
	戱: "虛戈",
	戳: "翟戈",
	戵: "瞿戈",
	戺: "户巳",
	所: "戶斤",
	扎: "扌乚",
	扏: "扌九",
	扐: "扌力",
	扑: "扌卜",
	扒: "扌八",
	打: "扌丁",
	扔: "扌乃",
	払: "扌厶",
	扖: "扌入",
	扗: "扌土",
	托: "扌乇",
	扙: "扌丈",
	扚: "扌勺",
	扛: "扌工",
	扜: "扌于",
	扝: "扌亏",
	扞: "扌干",
	扟: "扌卂",
	扠: "扌叉",
	扡: "扌也",
	扢: "扌乞",
	扣: "扌口",
	扤: "扌兀",
	扦: "扌千",
	执: "扌丸",
	扨: "扌刃",
	扩: "扌广",
	扪: "扌门",
	扫: "扌彐",
	扭: "扌丑",
	扮: "扌分",
	扯: "扌止",
	扰: "扌尤",
	扱: "扌及",
	扲: "扌今",
	扳: "扌反",
	扴: "扌介",
	扵: "扌仒",
	扶: "扌夫",
	扷: "扌夭",
	扸: "扌片",
	批: "扌比",
	扺: "扌氏",
	扻: "扌欠",
	扼: "扌厄",
	扽: "扌屯",
	找: "扌戈",
	技: "扌支",
	抁: "扌允",
	抂: "扌王",
	抃: "扌卞",
	抄: "扌少",
	抅: "扌勾",
	抆: "扌文",
	抇: "扌日",
	抈: "扌月",
	抉: "扌夬",
	把: "扌巴",
	抋: "扌心",
	抌: "扌冘",
	抍: "扌升",
	抎: "扌云",
	抏: "扌元",
	抐: "扌内",
	抑: "扌卬",
	抒: "扌予",
	抓: "扌爪",
	抔: "扌不",
	投: "扌殳",
	抖: "扌斗",
	抗: "扌亢",
	折: "扌斤",
	抙: "扌手",
	抚: "扌无",
	抜: "扌友",
	抝: "扌幻",
	択: "扌尺",
	抟: "扌专",
	抠: "扌区",
	抡: "扌仑",
	抢: "扌仓",
	抣: "扌匀",
	护: "扌户",
	抦: "扌丙",
	抧: "扌只",
	抨: "扌平",
	抩: "扌冉",
	抪: "扌布",
	披: "扌皮",
	抬: "扌台",
	抭: "扌宂",
	抯: "扌且",
	抰: "扌央",
	抱: "扌包",
	抲: "扌可",
	抳: "扌尼",
	抴: "扌世",
	抵: "扌氐",
	抶: "扌失",
	抷: "扌丕",
	抸: "扌乏",
	抹: "扌末",
	抺: "扌未",
	抻: "扌申",
	押: "扌甲",
	抽: "扌由",
	抾: "扌去",
	抿: "扌民",
	拀: "扌兄",
	拁: "扌加",
	拂: "扌弗",
	拃: "扌乍",
	拄: "扌主",
	担: "扌旦",
	拆: "扌斥",
	拇: "扌母",
	拈: "扌占",
	拉: "扌立",
	拊: "扌付",
	拌: "扌半",
	拍: "扌白",
	拎: "扌令",
	拐: "扌另",
	拑: "扌甘",
	拒: "扌巨",
	拓: "扌石",
	拔: "扌犮",
	拕: "扌它",
	拗: "扌幼",
	拘: "扌句",
	拙: "扌出",
	拚: "扌弁",
	招: "扌召",
	拞: "扌丘",
	拟: "扌以",
	拠: "扌処",
	拡: "扌広",
	拢: "扌龙",
	拤: "扌卡",
	拥: "扌用",
	拦: "扌兰",
	拧: "扌宁",
	拨: "扌发",
	拪: "扌西",
	拫: "扌艮",
	括: "扌舌",
	拭: "扌式",
	拮: "扌吉",
	拯: "扌丞",
	拰: "扌任",
	拱: "扌共",
	拴: "扌全",
	拵: "扌存",
	拷: "扌考",
	拸: "扌多",
	拹: "扌劦",
	拺: "扌朿",
	拻: "扌灰",
	拼: "扌并",
	拽: "扌曳",
	拾: "扌合",
	持: "扌寺",
	挂: "扌圭",
	挃: "扌至",
	挄: "扌光",
	挅: "扌朶",
	挆: "扌朵",
	指: "扌旨",
	按: "扌安",
	挋: "扌臣",
	挌: "扌各",
	挍: "扌交",
	挎: "扌夸",
	挏: "扌同",
	挑: "扌兆",
	挒: "扌列",
	挓: "扌宅",
	挔: "扌衣",
	挕: "扌耳",
	挖: "扌穵",
	挗: "扌夷",
	挘: "扌劣",
	挜: "扌亚",
	挝: "扌过",
	挞: "扌达",
	挟: "扌夹",
	挠: "扌尧",
	挡: "扌当",
	挢: "扌乔",
	挣: "扌争",
	挤: "扌齐",
	挥: "扌军",
	挦: "扌寻",
	挧: "扌羽",
	挨: "扌矣",
	挩: "扌兌",
	挪: "扌那",
	挫: "扌坐",
	挬: "扌孛",
	挭: "扌更",
	挮: "扌弟",
	振: "扌辰",
	挰: "扌呈",
	挱: "扌沙",
	挳: "扌巠",
	挴: "扌每",
	挵: "扌弄",
	挶: "扌局",
	挷: "扌邦",
	挸: "扌見",
	挹: "扌邑",
	挺: "扌廷",
	挻: "扌延",
	挼: "扌妥",
	挽: "扌免",
	挾: "扌夾",
	捀: "扌夆",
	捁: "扌告",
	捂: "扌吾",
	捃: "扌君",
	捄: "扌求",
	捅: "扌甬",
	捆: "扌困",
	捇: "扌赤",
	捈: "扌余",
	捉: "扌足",
	捊: "扌孚",
	捋: "扌寽",
	捌: "扌别",
	捍: "扌旱",
	捎: "扌肖",
	捏: "扌圼",
	捐: "扌肙",
	捒: "扌束",
	捓: "扌邪",
	捔: "扌角",
	捕: "扌甫",
	捖: "扌完",
	捗: "扌步",
	捘: "扌夋",
	捙: "扌車",
	捚: "扌里",
	捛: "扌吕",
	捝: "扌兑",
	捞: "扌劳",
	损: "扌员",
	捠: "扌兵",
	捡: "扌佥",
	换: "扌奂",
	捣: "扌岛",
	捤: "扌尾",
	捥: "扌宛",
	捦: "扌金",
	捧: "扌奉",
	捨: "扌舍",
	捩: "扌戾",
	捪: "扌昏",
	捫: "扌門",
	捬: "扌府",
	捭: "扌卑",
	据: "扌居",
	捯: "扌到",
	捰: "扌果",
	捱: "扌厓",
	捲: "扌卷",
	捳: "扌岳",
	捴: "扌忩",
	捵: "扌典",
	捶: "扌垂",
	捷: "扌疌",
	捸: "扌隶",
	捹: "扌奔",
	捺: "扌奈",
	捻: "扌念",
	捼: "扌委",
	捽: "扌卒",
	捾: "扌官",
	捿: "扌妻",
	掀: "扌欣",
	掁: "扌長",
	掂: "扌店",
	掃: "扌帚",
	掄: "扌侖",
	掅: "扌青",
	掆: "扌岡",
	掇: "扌叕",
	授: "扌受",
	掉: "扌卓",
	掊: "扌咅",
	掋: "扌底",
	掍: "扌昆",
	掎: "扌奇",
	掏: "扌匋",
	掐: "扌臽",
	掑: "扌其",
	排: "扌非",
	掓: "扌叔",
	掕: "扌夌",
	掖: "扌夜",
	掗: "扌亞",
	掘: "扌屈",
	掙: "扌爭",
	掚: "扌兩",
	掛: "扌卦",
	掜: "扌兒",
	掝: "扌或",
	掞: "扌炎",
	掟: "扌定",
	掠: "扌京",
	採: "扌采",
	探: "扌罙",
	掤: "扌朋",
	接: "扌妾",
	掦: "扌易",
	控: "扌空",
	推: "扌隹",
	掩: "扌奄",
	措: "扌昔",
	掫: "扌取",
	掬: "扌匊",
	掭: "扌忝",
	掮: "扌肩",
	掯: "扌肯",
	掳: "扌虏",
	掴: "扌国",
	掵: "扌命",
	掶: "扌庚",
	掷: "扌郑",
	掸: "扌单",
	掹: "扌孟",
	掺: "扌参",
	掼: "扌贯",
	掽: "扌並",
	掾: "扌彖",
	掿: "扌若",
	揀: "扌柬",
	揁: "扌貞",
	揂: "扌酋",
	揃: "扌前",
	揄: "扌俞",
	揆: "扌癸",
	揇: "扌南",
	揈: "扌訇",
	揉: "扌柔",
	揊: "扌畐",
	揋: "扌畏",
	揌: "扌思",
	揍: "扌奏",
	揎: "扌宣",
	描: "扌苗",
	提: "扌是",
	插: "扌臿",
	揓: "扌施",
	揔: "扌怱",
	揕: "扌甚",
	揖: "扌咠",
	揗: "扌盾",
	揘: "扌皇",
	揙: "扌扁",
	揚: "扌昜",
	換: "扌奐",
	揜: "扌弇",
	揝: "扌昝",
	揞: "扌音",
	揟: "扌胥",
	揠: "扌匽",
	握: "扌屋",
	揢: "扌客",
	揣: "扌耑",
	揤: "扌即",
	揥: "扌帝",
	揦: "扌剌",
	揨: "扌亭",
	揩: "扌皆",
	揪: "扌秋",
	揬: "扌突",
	揭: "扌曷",
	揮: "扌軍",
	揯: "扌恆",
	揰: "扌重",
	揲: "扌枼",
	揳: "扌契",
	援: "扌爰",
	揵: "扌建",
	揶: "扌耶",
	揸: "扌查",
	揹: "扌背",
	揻: "扌威",
	揼: "扌泵",
	揽: "扌览",
	揾: "扌昷",
	揿: "扌钦",
	搁: "扌阁",
	搂: "扌娄",
	搃: "扌总",
	搄: "扌恒",
	搅: "扌觉",
	搆: "扌冓",
	搇: "扌衾",
	搈: "扌容",
	搉: "扌隺",
	搊: "扌芻",
	搋: "扌虒",
	搌: "扌展",
	損: "扌員",
	搎: "扌孫",
	搏: "扌尃",
	搐: "扌畜",
	搑: "扌茸",
	搒: "扌旁",
	搓: "扌差",
	搔: "扌蚤",
	搕: "扌盍",
	搗: "扌島",
	搘: "扌耆",
	搙: "扌辱",
	搚: "扌脅",
	搛: "扌兼",
	搜: "扌叟",
	搝: "扌臭",
	搞: "扌高",
	搟: "扌軒",
	搠: "扌朔",
	搡: "扌桑",
	搢: "扌晋",
	搣: "扌烕",
	搤: "扌益",
	搥: "扌追",
	搦: "扌弱",
	搧: "扌扇",
	搩: "扌桀",
	搪: "扌唐",
	搬: "扌般",
	搭: "扌荅",
	搮: "扌栗",
	搯: "扌舀",
	搰: "扌骨",
	搱: "扌屖",
	搲: "扌窊",
	搳: "扌害",
	搶: "扌倉",
	搷: "扌真",
	搸: "扌秦",
	搹: "扌鬲",
	携: "扌隽",
	搼: "扌拳",
	搽: "扌茶",
	搾: "扌窄",
	摀: "扌烏",
	摁: "扌恩",
	摃: "扌貢",
	摄: "扌聂",
	摅: "扌虑",
	摆: "扌罢",
	摈: "扌宾",
	摊: "扌难",
	摋: "扌殺",
	摌: "扌産",
	摍: "扌宿",
	摎: "扌翏",
	摏: "扌舂",
	摐: "扌從",
	摑: "扌國",
	摒: "扌屏",
	摓: "扌逢",
	摔: "扌率",
	摕: "扌帶",
	摖: "扌祭",
	摗: "扌欶",
	摘: "扌啇",
	摙: "扌連",
	摚: "扌堂",
	摛: "扌离",
	摜: "扌貫",
	摝: "扌鹿",
	摞: "扌累",
	摟: "扌婁",
	摠: "扌悤",
	摡: "扌既",
	摢: "扌虖",
	摣: "扌虘",
	摤: "扌爽",
	摦: "扌瓠",
	摧: "扌崔",
	摨: "扌犀",
	摪: "扌將",
	摫: "扌規",
	摬: "扌竟",
	摭: "扌庶",
	摱: "扌曼",
	摲: "扌斬",
	摳: "扌區",
	摴: "扌雩",
	摵: "扌戚",
	摶: "扌專",
	摷: "扌巢",
	摸: "扌莫",
	摺: "扌習",
	摻: "扌參",
	摼: "扌堅",
	摽: "扌票",
	摾: "扌强",
	撁: "扌牽",
	撂: "扌畧",
	撄: "扌婴",
	撅: "扌厥",
	撇: "扌敝",
	撈: "扌勞",
	撊: "扌閒",
	撋: "扌閏",
	撌: "扌貴",
	撍: "扌朁",
	撎: "扌壹",
	撏: "扌尋",
	撐: "扌牚",
	撑: "扌掌",
	撒: "扌散",
	撓: "扌堯",
	撔: "扌景",
	撕: "扌斯",
	撖: "扌敢",
	撗: "扌黄",
	撘: "扌答",
	撙: "扌尊",
	撚: "扌然",
	撛: "扌粦",
	撜: "扌登",
	撝: "扌爲",
	撞: "扌童",
	撟: "扌喬",
	撠: "扌戟",
	撡: "扌叅",
	撢: "扌覃",
	撣: "扌單",
	撥: "扌發",
	撦: "扌奢",
	撧: "扌絕",
	撨: "扌焦",
	撩: "扌尞",
	撪: "扌軬",
	撫: "扌無",
	撬: "扌毳",
	播: "扌番",
	撮: "扌最",
	撯: "扌着",
	撰: "扌巽",
	撱: "扌隋",
	撲: "扌菐",
	撳: "扌欽",
	撴: "扌敦",
	撵: "扌辇",
	撶: "扌華",
	撷: "扌颉",
	撸: "扌鲁",
	撹: "扌覚",
	撺: "扌窜",
	撻: "扌達",
	撼: "扌感",
	撽: "扌敫",
	撾: "扌過",
	撿: "扌僉",
	擀: "扌幹",
	擁: "扌雍",
	擂: "扌雷",
	擃: "扌農",
	擄: "扌虜",
	擅: "扌亶",
	擆: "扌著",
	擇: "扌睪",
	擈: "扌業",
	擉: "扌蜀",
	擋: "扌當",
	擌: "扌筴",
	操: "扌喿",
	擏: "扌敬",
	擐: "扌睘",
	擑: "扌戢",
	擒: "扌禽",
	擓: "扌匯",
	擔: "扌詹",
	擕: "扌雋",
	擖: "扌葛",
	擗: "扌辟",
	擙: "扌奧",
	據: "扌豦",
	擛: "扌葉",
	擜: "扌献",
	擝: "扌盟",
	擞: "扌数",
	擟: "扌爾",
	擠: "扌齊",
	擡: "扌臺",
	擢: "扌翟",
	擣: "扌壽",
	擤: "扌鼻",
	擦: "扌察",
	擨: "扌歋",
	擩: "扌需",
	擫: "扌厭",
	擬: "扌疑",
	擭: "扌蒦",
	擮: "扌截",
	擯: "扌賓",
	擰: "扌寧",
	擱: "扌閣",
	擲: "扌鄭",
	擳: "扌節",
	擴: "扌廣",
	擵: "扌摩",
	擶: "扌箭",
	擷: "扌頡",
	擸: "扌巤",
	擹: "扌歎",
	擺: "扌罷",
	擻: "扌數",
	擼: "扌魯",
	擽: "扌樂",
	擾: "扌憂",
	擿: "扌適",
	攁: "扌養",
	攂: "扌畾",
	攃: "扌蔡",
	攄: "扌慮",
	攅: "扌賛",
	攆: "扌輦",
	攇: "扌憲",
	攈: "扌麇",
	攉: "扌霍",
	攊: "扌歷",
	攋: "扌賴",
	攌: "扌圜",
	攍: "扌嬴",
	攎: "扌盧",
	攏: "扌龍",
	攐: "扌褰",
	攑: "扌舉",
	攒: "扌赞",
	攓: "扌蹇",
	攔: "扌闌",
	攕: "扌韱",
	攖: "扌嬰",
	攗: "扌麋",
	攘: "扌襄",
	攙: "扌毚",
	攚: "扌營",
	攛: "扌竄",
	攜: "扌巂",
	攝: "扌聶",
	攞: "扌羅",
	攟: "扌麕",
	攠: "扌靡",
	攡: "扌離",
	攢: "扌贊",
	攤: "扌難",
	攥: "扌纂",
	攦: "扌麗",
	攧: "扌顛",
	攩: "扌黨",
	攪: "扌覺",
	攫: "扌矍",
	攬: "扌覽",
	攭: "扌蠡",
	攮: "扌囊",
	攱: "立支",
	攲: "奇支",
	收: "丩攵",
	攷: "丂攵",
	改: "己攵",
	攺: "巳攵",
	攻: "工攵",
	攼: "干攵",
	攽: "分攵",
	放: "方攵",
	政: "正攵",
	敀: "白攵",
	敁: "占攴",
	敂: "句攵",
	敃: "民攵",
	敄: "矛攵",
	故: "古攵",
	敆: "合攴",
	敇: "朿攵",
	效: "交攵",
	敉: "米攵",
	敊: "尗攴",
	敋: "各攵",
	敌: "舌攵",
	敍: "余攴",
	敏: "每攵",
	敐: "辰攵",
	救: "求攵",
	敒: "伸攵",
	敓: "兌攵",
	敔: "吾攵",
	敕: "束攵",
	敗: "貝攵",
	敘: "余攵",
	教: "孝攵",
	敚: "兑攵",
	敛: "佥攵",
	敜: "念攵",
	敞: "尚攵",
	敟: "典攵",
	敠: "叕攴",
	敡: "易攴",
	敤: "果攴",
	敥: "炎攴",
	敦: "享攵",
	敧: "奇攴",
	敨: "咅攵",
	敩: "学攵",
	敪: "叕攵",
	敬: "苟攵",
	敭: "昜攵",
	敮: "臿攴",
	敯: "昬攴",
	数: "娄攵",
	敱: "豈攴",
	敲: "高攴",
	敳: "豈攵",
	敵: "啇攵",
	敶: "陳攵",
	敷: "旉攵",
	數: "婁攵",
	敺: "區攴",
	敼: "喜攴",
	敽: "喬攴",
	敾: "善攵",
	敿: "喬攵",
	斀: "蜀攴",
	斁: "睪攵",
	斂: "僉攵",
	斅: "學攴",
	斆: "學攵",
	斌: "文武",
	斏: "文良",
	斒: "文扁",
	斓: "文阑",
	斔: "蚉臾",
	斕: "文闌",
	斘: "夕斗",
	料: "米斗",
	斛: "角斗",
	斜: "余斗",
	斞: "臾斗",
	斟: "甚斗",
	斠: "冓斗",
	斢: "黄斗",
	斣: "蜀斗",
	斦: "斤斤",
	斨: "爿斤",
	斩: "车斤",
	斪: "句斤",
	斫: "石斤",
	斬: "車斤",
	斮: "昔斤",
	斯: "其斤",
	新: "亲斤",
	斱: "者斤",
	斳: "堇斤",
	斴: "粦斤",
	斶: "斤蜀",
	斸: "屬斤",
	斺: "方介",
	於: "方仒",
	旀: "方厼",
	旑: "方奇",
	旔: "方建",
	旘: "方戠",
	旙: "方番",
	旣: "皀旡",
	旤: "咼旡",
	旧: "丨日",
	旪: "日十",
	旫: "日刀",
	旰: "日干",
	旳: "日勺",
	旴: "日于",
	时: "日寸",
	旷: "日广",
	旺: "日王",
	旼: "日文",
	旽: "日屯",
	旿: "日午",
	昀: "日匀",
	昁: "日巿",
	昄: "日反",
	昅: "日及",
	昈: "日户",
	昉: "日方",
	昍: "日日",
	明: "日月",
	昐: "日分",
	昑: "日今",
	昒: "日勿",
	昕: "日斤",
	昖: "日公",
	昛: "日巨",
	昞: "日丙",
	映: "日央",
	昡: "日玄",
	昢: "日出",
	昤: "日令",
	昧: "日未",
	昨: "日乍",
	昩: "日末",
	昫: "日句",
	昭: "日召",
	昲: "日弗",
	昳: "日失",
	昵: "日尼",
	昸: "日冬",
	昹: "日永",
	昽: "日龙",
	昿: "日広",
	晀: "日兆",
	時: "日寺",
	晄: "日光",
	晅: "日亘",
	晆: "日圭",
	晇: "日夸",
	晈: "日交",
	晊: "日至",
	晌: "日向",
	晍: "日同",
	晎: "日共",
	晐: "日亥",
	晒: "日西",
	晓: "日尧",
	晔: "日华",
	晖: "日军",
	晗: "日含",
	晘: "日旱",
	晙: "日夋",
	晚: "日免",
	晛: "日見",
	晞: "日希",
	晠: "日成",
	晡: "日甫",
	晣: "日折",
	晤: "日吾",
	晥: "日完",
	晦: "日每",
	晧: "日告",
	晩: "日免",
	晪: "日典",
	晫: "日卓",
	晬: "日卒",
	晭: "日周",
	晰: "日析",
	晱: "日炎",
	晲: "日兒",
	晴: "日青",
	晹: "日易",
	晻: "日奄",
	晼: "日宛",
	晽: "日林",
	晾: "日京",
	晿: "日昌",
	暀: "日往",
	暁: "日尭",
	暄: "日宣",
	暅: "日恒",
	暆: "日施",
	暇: "日叚",
	暉: "日軍",
	暊: "日頁",
	暌: "日癸",
	暍: "日曷",
	暎: "日英",
	暏: "日者",
	暐: "日韋",
	暒: "日星",
	暔: "日南",
	暕: "日柬",
	暖: "日爰",
	暗: "日音",
	暘: "日昜",
	暙: "日春",
	暛: "日差",
	暝: "日冥",
	暞: "日臬",
	暟: "日豈",
	暡: "日翁",
	暢: "申昜",
	暣: "日氣",
	暤: "日皋",
	暥: "日晏",
	暧: "日爱",
	暩: "日祭",
	暭: "日皐",
	暯: "日莫",
	暰: "日從",
	暱: "日匿",
	暲: "日章",
	暳: "日彗",
	暶: "日旋",
	暷: "日專",
	暸: "日尞",
	暺: "日單",
	暻: "日景",
	暽: "日粦",
	暾: "日敦",
	暿: "日喜",
	曀: "日壹",
	曂: "日黄",
	曃: "日逮",
	曄: "日華",
	曈: "日童",
	曉: "日堯",
	曊: "日費",
	曋: "日覃",
	曍: "日臯",
	曎: "日睪",
	曒: "日敫",
	曔: "日敬",
	曕: "日詹",
	曖: "日愛",
	曗: "日業",
	曘: "日需",
	曙: "日署",
	曚: "日蒙",
	曛: "日熏",
	曜: "日翟",
	曝: "日暴",
	曞: "日厲",
	曠: "日廣",
	曢: "日寮",
	曣: "日燕",
	曤: "日霍",
	曥: "日盧",
	曦: "日羲",
	曨: "日龍",
	曪: "日羅",
	曬: "日麗",
	曭: "日黨",
	曮: "日嚴",
	曯: "日屬",
	朄: "申柬",
	朅: "去曷",
	朆: "曾勿",
	朇: "會卑",
	朊: "月元",
	朋: "月月",
	朌: "月分",
	朎: "月令",
	朏: "月出",
	朐: "月句",
	朑: "月世",
	朒: "月肉",
	朓: "月兆",
	朔: "屰月",
	朕: "月关",
	朖: "月良",
	朘: "月夋",
	朙: "囧月",
	朜: "月享",
	期: "其月",
	朠: "月英",
	朣: "月童",
	朥: "月勞",
	朦: "月蒙",
	朧: "月龍",
	札: "木乚",
	朲: "木人",
	朳: "木八",
	朴: "木卜",
	朷: "木刀",
	朸: "木力",
	朹: "木九",
	机: "木几",
	朻: "木丩",
	朼: "木匕",
	朽: "木丂",
	朾: "木丁",
	杁: "木入",
	权: "木又",
	杄: "木千",
	杅: "木于",
	杆: "木干",
	杇: "木亏",
	杈: "木叉",
	杉: "木彡",
	杊: "木川",
	杋: "木凡",
	杌: "木兀",
	杍: "木子",
	材: "木才",
	村: "木寸",
	杒: "木刃",
	杓: "木勺",
	杔: "木乇",
	杕: "木大",
	杖: "木丈",
	杙: "木弋",
	杚: "木乞",
	杛: "木弓",
	杜: "木土",
	杝: "木也",
	杞: "木己",
	杠: "木工",
	杣: "木山",
	杤: "木万",
	杦: "木久",
	杧: "木亡",
	杩: "木马",
	杪: "木少",
	杫: "木止",
	杬: "木元",
	杭: "木亢",
	杮: "木巿",
	杯: "木不",
	杴: "木欠",
	杵: "木午",
	杶: "木屯",
	杷: "木巴",
	杸: "木殳",
	杹: "木化",
	杺: "木心",
	杻: "木丑",
	杼: "木予",
	杽: "木手",
	松: "木公",
	板: "木反",
	极: "木及",
	枂: "木月",
	枃: "木匀",
	构: "木勾",
	枅: "木开",
	枆: "木毛",
	枇: "木比",
	枉: "木王",
	枊: "木卬",
	枋: "木方",
	枌: "木分",
	枍: "木兮",
	枎: "木夫",
	枏: "木冄",
	析: "木斤",
	枑: "木互",
	枒: "木牙",
	枓: "木斗",
	枔: "木今",
	枕: "木冘",
	枖: "木夭",
	林: "木木",
	枘: "木内",
	枙: "木厄",
	枚: "木攵",
	枛: "木爪",
	枝: "木支",
	枞: "木从",
	枟: "木云",
	枠: "木卆",
	枡: "木升",
	枢: "木区",
	枤: "木犬",
	枥: "木历",
	枦: "木户",
	枧: "木见",
	枨: "木长",
	枪: "木仓",
	枫: "木风",
	枬: "木丹",
	枮: "木占",
	枯: "木古",
	枰: "木平",
	枱: "木台",
	枳: "木只",
	枴: "木叧",
	枵: "木号",
	枷: "木加",
	枸: "木句",
	枹: "木包",
	枺: "木末",
	枻: "木世",
	枿: "木卉",
	柀: "木皮",
	柁: "木它",
	柃: "木令",
	柄: "木丙",
	柅: "木尼",
	柆: "木立",
	柇: "木禾",
	柈: "木半",
	柉: "木乏",
	柊: "木冬",
	柌: "木司",
	柍: "木央",
	柎: "木付",
	柏: "木白",
	柑: "木甘",
	柕: "木矛",
	柖: "木召",
	柘: "木石",
	柙: "木甲",
	柚: "木由",
	柛: "木申",
	柜: "木巨",
	柝: "木斥",
	柞: "木乍",
	柟: "木冉",
	柠: "木宁",
	柢: "木氐",
	柣: "木失",
	柤: "木且",
	柦: "木旦",
	柧: "木瓜",
	柨: "木布",
	柩: "木匛",
	柪: "木幼",
	柫: "木弗",
	柭: "木犮",
	柮: "木出",
	柯: "木可",
	柱: "木主",
	柲: "木必",
	柳: "木卯",
	柵: "木冊",
	柶: "木四",
	柷: "木兄",
	柸: "木丕",
	柺: "木另",
	柼: "木穴",
	柽: "木圣",
	柾: "木正",
	柿: "木市",
	栀: "木卮",
	栁: "木夘",
	栂: "木母",
	栅: "木册",
	标: "木示",
	栈: "木戋",
	栉: "木节",
	栊: "木龙",
	栋: "木东",
	栌: "木卢",
	栍: "木生",
	栎: "木乐",
	栏: "木兰",
	栐: "木永",
	树: "木对",
	栒: "木旬",
	栓: "木全",
	栕: "木臣",
	栖: "木西",
	栘: "木多",
	栙: "木夅",
	栚: "木关",
	栛: "木劦",
	栜: "木朿",
	栝: "木舌",
	栟: "木并",
	校: "木交",
	栢: "木百",
	栣: "木任",
	栤: "木冰",
	栦: "木州",
	栧: "木曳",
	栨: "木次",
	栩: "木羽",
	株: "木朱",
	栫: "木存",
	栬: "木色",
	栭: "木而",
	栮: "木耳",
	栯: "木有",
	栰: "木伐",
	栱: "木共",
	栲: "木考",
	栳: "木老",
	栵: "木列",
	栶: "木因",
	样: "木羊",
	核: "木亥",
	根: "木艮",
	栺: "木旨",
	栻: "木式",
	格: "木各",
	栿: "木伏",
	桁: "木行",
	桂: "木圭",
	桃: "木兆",
	桄: "木光",
	桅: "木危",
	框: "木匡",
	桉: "木安",
	桋: "木夷",
	桍: "木夸",
	桎: "木至",
	桏: "木邛",
	桐: "木同",
	桓: "木亘",
	桔: "木吉",
	桕: "木臼",
	桖: "木血",
	桗: "木朶",
	桙: "木牟",
	桝: "木舛",
	桞: "木邜",
	桠: "木亚",
	桡: "木尧",
	桢: "木贞",
	档: "木当",
	桤: "木岂",
	桥: "木乔",
	桦: "木华",
	桧: "木会",
	桩: "木庄",
	桪: "木寻",
	桫: "木沙",
	桭: "木辰",
	桮: "木否",
	桯: "木呈",
	桱: "木巠",
	桲: "木孛",
	桴: "木孚",
	桵: "木妥",
	桶: "木甬",
	桷: "木角",
	桸: "木希",
	桹: "木良",
	桺: "木丣",
	桻: "木夆",
	桾: "木君",
	桿: "木旱",
	梀: "木束",
	梂: "木求",
	梃: "木廷",
	梄: "木酉",
	梅: "木每",
	梆: "木邦",
	梇: "木弄",
	梈: "木亨",
	梉: "木壯",
	梋: "木肙",
	梌: "木余",
	梍: "木皂",
	梎: "木皀",
	梏: "木告",
	梐: "木坒",
	梑: "木狄",
	梒: "木含",
	梓: "木辛",
	梔: "木巵",
	梕: "木忍",
	梖: "木貝",
	梗: "木更",
	梘: "木見",
	梙: "木串",
	梚: "木免",
	梛: "木那",
	梜: "木夾",
	梞: "木忌",
	梠: "木吕",
	梡: "木完",
	梢: "木肖",
	梣: "木岑",
	梧: "木吾",
	梩: "木里",
	梪: "木豆",
	梬: "木甹",
	梭: "木夋",
	梮: "木局",
	梯: "木弟",
	械: "木戒",
	梱: "木困",
	梲: "木兌",
	梴: "木延",
	梶: "木尾",
	梸: "木利",
	梹: "木兵",
	梻: "木佛",
	梼: "木寿",
	梽: "木志",
	梾: "木来",
	梿: "木连",
	检: "木佥",
	棁: "木兑",
	棂: "木灵",
	棅: "木秉",
	棆: "木侖",
	棇: "木忩",
	棈: "木青",
	棉: "木帛",
	棋: "木其",
	棌: "木采",
	棍: "木昆",
	棎: "木罙",
	棑: "木非",
	棒: "木奉",
	棓: "木咅",
	棔: "木昏",
	棕: "木宗",
	棖: "木長",
	棘: "朿朿",
	棙: "木戾",
	棚: "木朋",
	棛: "木育",
	棜: "木於",
	棝: "木固",
	棞: "木囷",
	棟: "木東",
	棡: "木岡",
	棢: "木罔",
	棣: "木隶",
	棤: "木昔",
	棦: "木爭",
	棧: "木戔",
	棪: "木炎",
	棫: "木或",
	棬: "木卷",
	棭: "木夜",
	棯: "木念",
	棰: "木垂",
	棱: "木夌",
	棲: "木妻",
	棳: "木叕",
	棴: "木服",
	棵: "木果",
	棶: "木來",
	棷: "木取",
	棹: "木卓",
	棺: "木官",
	棿: "木兒",
	椀: "木宛",
	椁: "木享",
	椂: "木录",
	椃: "木虎",
	椄: "木妾",
	椅: "木奇",
	椆: "木周",
	椇: "木具",
	椈: "木匊",
	椊: "木卒",
	椋: "木京",
	椌: "木空",
	植: "木直",
	椎: "木隹",
	椏: "木亞",
	椐: "木居",
	椑: "木卑",
	椒: "木叔",
	椓: "木豖",
	椔: "木甾",
	椖: "木房",
	椗: "木定",
	椙: "木昌",
	椚: "木門",
	椛: "木花",
	椟: "木卖",
	椡: "木到",
	椢: "木国",
	椣: "木典",
	椤: "木罗",
	椥: "木知",
	椦: "木劵",
	椧: "木命",
	椨: "木府",
	椩: "木庚",
	椪: "木並",
	椫: "木单",
	椬: "木宜",
	椭: "木陏",
	椮: "木参",
	椯: "木耑",
	椰: "木耶",
	椱: "木复",
	椲: "木韋",
	椳: "木畏",
	椴: "木段",
	椵: "木叚",
	椷: "木咸",
	椸: "木施",
	椹: "木甚",
	椺: "木保",
	椻: "木匽",
	椼: "木衍",
	椽: "木彖",
	椾: "木前",
	椿: "木春",
	楀: "木禹",
	楁: "木客",
	楂: "木查",
	楃: "木屋",
	楄: "木扁",
	楅: "木畐",
	楆: "木要",
	楇: "木咼",
	楈: "木胥",
	楉: "木若",
	楊: "木昜",
	楋: "木剌",
	楌: "木彦",
	楎: "木軍",
	楏: "木奎",
	楐: "木界",
	楑: "木癸",
	楒: "木思",
	楓: "木風",
	楔: "木契",
	楖: "木即",
	楗: "木建",
	楛: "木苦",
	楜: "木胡",
	楝: "木柬",
	楟: "木亭",
	楠: "木南",
	楡: "木兪",
	楢: "木酋",
	楣: "木眉",
	楤: "木怱",
	楥: "木爰",
	楦: "木宣",
	楧: "木英",
	楨: "木貞",
	楩: "木便",
	楪: "木枼",
	楫: "木咠",
	楬: "木曷",
	楮: "木者",
	楯: "木盾",
	楰: "木臾",
	楱: "木奏",
	楲: "木威",
	楳: "木某",
	楴: "木帝",
	極: "木亟",
	楷: "木皆",
	楸: "木秋",
	楹: "木盈",
	楺: "木柔",
	楻: "木皇",
	楼: "木娄",
	楾: "木泉",
	楿: "木香",
	榀: "木品",
	榁: "木室",
	概: "木既",
	榄: "木览",
	榅: "木昷",
	榆: "木俞",
	榇: "木亲",
	榈: "木闾",
	榉: "木举",
	榊: "木神",
	榌: "木飛",
	榍: "木屑",
	榎: "木夏",
	榏: "木益",
	榐: "木展",
	榑: "木尃",
	榒: "木弱",
	榔: "木郎",
	榕: "木容",
	榗: "木晋",
	榙: "木荅",
	榚: "木羔",
	榛: "木秦",
	榜: "木旁",
	榝: "木殺",
	榞: "木原",
	榟: "木宰",
	榠: "木冥",
	榡: "木素",
	榢: "木家",
	榤: "木桀",
	榥: "木晃",
	榧: "木匪",
	榨: "木窄",
	榩: "木虔",
	榪: "木馬",
	榫: "木隼",
	榬: "木袁",
	榭: "木射",
	榯: "木時",
	榰: "木耆",
	榱: "木衰",
	榳: "木庭",
	榴: "木留",
	榵: "木茸",
	榶: "木唐",
	榷: "木隺",
	榸: "木埋",
	榹: "木虒",
	榺: "月桊",
	榼: "木盍",
	榽: "木奚",
	榾: "木骨",
	榿: "木豈",
	槁: "木高",
	槂: "木孫",
	槄: "木舀",
	槅: "木鬲",
	槆: "木荀",
	槇: "木眞",
	槈: "木辱",
	槉: "木疾",
	構: "木冓",
	槌: "木追",
	槍: "木倉",
	槎: "木差",
	槏: "木兼",
	槐: "木鬼",
	槑: "呆呆",
	槒: "木畜",
	槓: "木貢",
	槔: "木皋",
	槕: "木桌",
	槙: "木真",
	槚: "木贾",
	槛: "木监",
	槜: "木隽",
	槝: "木島",
	槞: "木竜",
	槟: "木宾",
	槠: "木诸",
	槡: "木桑",
	槢: "木習",
	槣: "木寄",
	槤: "木連",
	槥: "木彗",
	槦: "木庸",
	槨: "木郭",
	槪: "木旣",
	槫: "木專",
	槬: "木瓠",
	槭: "木戚",
	槮: "木參",
	槯: "木崔",
	槰: "木逢",
	槲: "木斛",
	槴: "木扈",
	槵: "木患",
	槶: "木國",
	槸: "木埶",
	槹: "木皐",
	槺: "木康",
	槻: "木規",
	槽: "木曹",
	槾: "木曼",
	槿: "木堇",
	樀: "木啇",
	樁: "木舂",
	樃: "木朗",
	樄: "木陳",
	樅: "木從",
	樆: "木离",
	樇: "木脩",
	樈: "木竟",
	樉: "木爽",
	樋: "木通",
	樌: "木貫",
	樍: "木責",
	樎: "木宿",
	樏: "木累",
	樐: "木鹵",
	樑: "木梁",
	樒: "木密",
	樓: "木婁",
	樔: "木巢",
	樕: "木欶",
	樗: "木雩",
	樘: "木堂",
	標: "木票",
	樚: "木鹿",
	樛: "木翏",
	樜: "木庶",
	樝: "木虘",
	樞: "木區",
	樟: "木章",
	模: "木莫",
	樢: "木鳥",
	樣: "木羕",
	樤: "木條",
	樥: "木莑",
	樧: "木殺",
	樨: "木犀",
	横: "木黄",
	樫: "木堅",
	樬: "木悤",
	樭: "木基",
	樯: "木啬",
	樰: "木雪",
	樱: "木婴",
	樲: "木貳",
	樳: "木尋",
	樴: "木戠",
	樵: "木焦",
	樶: "木最",
	樸: "木菐",
	樹: "木尌",
	樺: "木華",
	樻: "木貴",
	樼: "木孱",
	樽: "木尊",
	樾: "木越",
	樿: "木單",
	橁: "木筍",
	橂: "木奠",
	橃: "木發",
	橄: "木敢",
	橅: "木無",
	橇: "木毳",
	橈: "木堯",
	橉: "木粦",
	橊: "木畱",
	橋: "木喬",
	橌: "木閒",
	橍: "木閏",
	橎: "木番",
	橏: "木善",
	橑: "木尞",
	橒: "木雲",
	橓: "木舜",
	橔: "木敦",
	橕: "木牚",
	橖: "木棠",
	橗: "木萌",
	橘: "木矞",
	橙: "木登",
	橚: "木肅",
	橛: "木厥",
	橝: "木覃",
	橞: "木惠",
	機: "木幾",
	橠: "木袲",
	橡: "木象",
	橢: "木隋",
	橣: "木甯",
	橦: "木童",
	橧: "木曾",
	橨: "木賁",
	橩: "木焭",
	橪: "木然",
	橫: "木黃",
	橬: "木朁",
	橭: "木辜",
	橮: "木貿",
	橯: "木勞",
	橰: "木臯",
	橱: "木厨",
	橲: "木喜",
	橳: "木勝",
	橴: "木紫",
	橵: "木散",
	橶: "木戟",
	橷: "木兠",
	橸: "木晶",
	橹: "木鲁",
	橺: "木間",
	橼: "木缘",
	橽: "木達",
	橾: "木喿",
	橿: "木畺",
	檀: "木亶",
	檁: "木稟",
	檂: "木農",
	檄: "木敫",
	檅: "木歲",
	檆: "木煔",
	檇: "木雋",
	檈: "木睘",
	檉: "木聖",
	檊: "木幹",
	檋: "木輂",
	檌: "木罪",
	檍: "木意",
	檎: "木禽",
	檏: "木業",
	檐: "木詹",
	檑: "木雷",
	檓: "木毁",
	檔: "木當",
	檖: "木遂",
	檘: "木辟",
	檙: "木鼎",
	檚: "木楚",
	檛: "木過",
	檜: "木會",
	檝: "木戢",
	檞: "木解",
	檟: "木賈",
	檡: "木睪",
	檢: "木僉",
	檣: "木嗇",
	檤: "木道",
	檥: "木義",
	檦: "木剽",
	檧: "木葱",
	檨: "木羡",
	檩: "木禀",
	檪: "木楽",
	檫: "木察",
	檬: "木蒙",
	檭: "木銀",
	檮: "木壽",
	檯: "木臺",
	檰: "木綿",
	檱: "木箕",
	檲: "木團",
	檳: "木賓",
	檴: "木蒦",
	檶: "木奩",
	檷: "木爾",
	檸: "木寧",
	檹: "木旖",
	檺: "木豪",
	檻: "木監",
	檽: "木需",
	櫀: "木綦",
	櫁: "木蜜",
	櫂: "木翟",
	櫃: "木匱",
	櫄: "木熏",
	櫅: "木齊",
	櫆: "木魁",
	櫇: "木頗",
	櫈: "木凳",
	櫊: "木閣",
	櫋: "木臱",
	櫌: "木憂",
	櫍: "木質",
	櫎: "木廣",
	櫏: "木遷",
	櫑: "木畾",
	櫒: "木蔡",
	櫓: "木魯",
	櫔: "木厲",
	櫕: "木賛",
	櫖: "木慮",
	櫗: "木蔑",
	櫘: "木慧",
	櫙: "木蓲",
	櫚: "木閭",
	櫛: "木節",
	櫝: "木賣",
	櫞: "木緣",
	櫟: "木樂",
	櫠: "木廢",
	櫡: "木箸",
	櫢: "木數",
	櫤: "木箭",
	櫥: "木廚",
	櫦: "木慶",
	櫧: "木諸",
	櫨: "木盧",
	櫩: "木閻",
	櫪: "木歷",
	櫬: "木親",
	櫭: "木暨",
	櫮: "木噩",
	櫯: "木穌",
	櫰: "木褱",
	櫲: "木豫",
	櫳: "木龍",
	櫴: "木賴",
	櫵: "木蕉",
	櫶: "木憲",
	櫷: "木龜",
	櫸: "木舉",
	櫹: "木蕭",
	櫺: "木霝",
	櫻: "木嬰",
	櫼: "木韱",
	櫾: "木繇",
	櫿: "木營",
	欀: "木襄",
	欂: "木薄",
	欃: "木毚",
	欄: "木闌",
	欅: "木擧",
	欆: "木雙",
	欇: "木聶",
	欈: "木巂",
	欉: "木叢",
	權: "木雚",
	欋: "木瞿",
	欌: "木藏",
	欍: "木舊",
	欏: "木羅",
	欐: "木麗",
	欑: "木贊",
	欓: "木黨",
	欔: "木矍",
	欕: "木嚴",
	欖: "木覽",
	欗: "木蘭",
	欘: "木屬",
	欙: "木纍",
	欚: "木蠡",
	欛: "木霸",
	欜: "木囊",
	欞: "木靈",
	欟: "木觀",
	次: "冫欠",
	欢: "又欠",
	欣: "斤欠",
	欤: "与欠",
	欥: "日欠",
	欦: "今欠",
	欧: "区欠",
	欨: "句欠",
	欩: "召欠",
	欪: "出欠",
	欬: "亥欠",
	欭: "因欠",
	欮: "屰欠",
	欯: "吉欠",
	欰: "血欠",
	欱: "合欠",
	欲: "谷欠",
	欶: "束欠",
	欷: "希欠",
	欸: "矣欠",
	欹: "奇欠",
	欺: "其欠",
	欻: "炎欠",
	欼: "叕欠",
	欽: "金欠",
	欿: "臽欠",
	歀: "柰欠",
	歁: "甚欠",
	歂: "耑欠",
	歃: "臿欠",
	歄: "咼欠",
	歅: "垔欠",
	歆: "音欠",
	歇: "曷欠",
	歈: "俞欠",
	歉: "兼欠",
	歊: "高欠",
	歋: "虒欠",
	歌: "哥欠",
	歍: "烏欠",
	歏: "堇欠",
	歐: "區欠",
	歑: "虖欠",
	歒: "啇欠",
	歔: "虚欠",
	歕: "賁欠",
	歖: "喜欠",
	歗: "肅欠",
	歘: "焱欠",
	歙: "翕欠",
	歚: "善欠",
	歛: "僉欠",
	歜: "蜀欠",
	歝: "睪欠",
	歟: "與欠",
	歡: "雚欠",
	此: "止匕",
	歧: "止支",
	歫: "止巨",
	歭: "止寺",
	歱: "止重",
	歵: "止責",
	歼: "歹千",
	歽: "歹斤",
	歾: "歹勿",
	殀: "歹夭",
	殁: "歹殳",
	殂: "歹且",
	殃: "歹央",
	殅: "歹生",
	殆: "歹台",
	殈: "歹血",
	殉: "歹旬",
	殊: "歹朱",
	残: "歹戋",
	殌: "歹巠",
	殍: "歹孚",
	殎: "歹夾",
	殏: "歹求",
	殐: "歹束",
	殑: "歹克",
	殒: "歹员",
	殓: "歹佥",
	殔: "歹隶",
	殕: "歹咅",
	殖: "歹直",
	殗: "歹奄",
	殘: "歹戔",
	殙: "歹昏",
	殚: "歹单",
	殛: "歹亟",
	殜: "歹枼",
	殝: "歹秦",
	殞: "歹員",
	殠: "歹臭",
	殡: "歹宾",
	殢: "歹帶",
	殣: "歹堇",
	殥: "歹寅",
	殦: "歹鳥",
	殧: "歹就",
	殨: "歹貴",
	殪: "歹壹",
	殫: "歹單",
	殬: "歹睪",
	殭: "歹畺",
	殮: "歹僉",
	殯: "歹賓",
	殰: "歹賣",
	殱: "歹韯",
	殲: "歹韱",
	殴: "区殳",
	殶: "主殳",
	殸: "声殳",
	殹: "医殳",
	殺: "杀殳",
	殻: "壳殳",
	殼: "壳殳",
	殽: "肴殳",
	殾: "者殳",
	毃: "高殳",
	毄: "軎殳",
	毅: "豙殳",
	毆: "區殳",
	毈: "卵段",
	毑: "母也",
	毗: "田比",
	毦: "耳毛",
	毭: "豆毛",
	毮: "扌毟",
	毲: "叕毛",
	毵: "参毛",
	毷: "冒毛",
	毹: "俞毛",
	毼: "曷毛",
	毿: "參毛",
	氀: "婁毛",
	氃: "童毛",
	氄: "矞毛",
	氈: "亶毛",
	氉: "喿毛",
	氋: "蒙毛",
	氍: "瞿毛",
	氎: "疉毛",
	氓: "亡民",
	氻: "氵力",
	氿: "氵九",
	汀: "氵丁",
	汁: "氵十",
	汃: "氵八",
	汄: "氵人",
	汅: "氵丂",
	汇: "氵匚",
	汈: "氵刁",
	汉: "氵又",
	汊: "氵叉",
	汋: "氵勺",
	汌: "氵川",
	汍: "氵丸",
	汎: "氵凡",
	汏: "氵大",
	汐: "氵夕",
	汑: "氵乇",
	汒: "氵亡",
	汓: "氵子",
	汔: "氵乞",
	汕: "氵山",
	汗: "氵干",
	汘: "氵千",
	汙: "氵于",
	汚: "氵亐",
	汛: "氵卂",
	汜: "氵巳",
	汝: "氵女",
	江: "氵工",
	池: "氵也",
	污: "氵亏",
	汢: "氵土",
	汣: "氵久",
	汥: "氵支",
	汦: "氵氏",
	汧: "氵开",
	汨: "氵日",
	汩: "氵曰",
	汪: "氵王",
	汫: "氵井",
	汭: "氵内",
	汮: "氵匀",
	汯: "氵厷",
	汰: "氵太",
	汱: "氵犬",
	汲: "氵及",
	汳: "氵反",
	汴: "氵卞",
	汵: "氵今",
	汶: "氵文",
	汷: "氵夂",
	汸: "氵方",
	汹: "氵凶",
	決: "氵夬",
	汻: "氵午",
	汼: "氵牛",
	汽: "氵气",
	汾: "氵分",
	汿: "氵予",
	沀: "水予",
	沁: "氵心",
	沂: "氵斤",
	沃: "氵夭",
	沄: "氵云",
	沅: "氵元",
	沆: "氵亢",
	沇: "氵允",
	沈: "氵冘",
	沉: "氵冗",
	沋: "氵尤",
	沌: "氵屯",
	沍: "氵互",
	沎: "氵化",
	沏: "氵切",
	沐: "氵木",
	沑: "氵丑",
	沔: "氵丏",
	沕: "氵勿",
	沖: "氵中",
	沘: "氵比",
	沙: "氵少",
	沚: "氵止",
	沛: "氵巿",
	沜: "氵片",
	沝: "水水",
	沞: "氵帀",
	沟: "氵勾",
	沠: "氵爪",
	没: "氵殳",
	沢: "氵尺",
	沣: "氵丰",
	沤: "氵区",
	沥: "氵历",
	沦: "氵仑",
	沧: "氵仓",
	沨: "氵风",
	沩: "氵为",
	沪: "氵户",
	沫: "氵末",
	沬: "氵未",
	沭: "氵术",
	沮: "氵且",
	沰: "氵石",
	沱: "氵它",
	河: "氵可",
	沵: "氵尔",
	沶: "氵示",
	沷: "氵犮",
	沸: "氵弗",
	油: "氵由",
	沺: "氵田",
	治: "氵台",
	沼: "氵召",
	沽: "氵古",
	沾: "氵占",
	泀: "氵司",
	況: "氵兄",
	泂: "氵冋",
	泃: "氵句",
	泄: "氵世",
	泅: "氵囚",
	泆: "氵失",
	泇: "氵加",
	泈: "氵冬",
	泊: "氵白",
	泋: "氵卉",
	泌: "氵必",
	泍: "氵本",
	泎: "氵乍",
	泏: "氵出",
	泐: "氵阞",
	泑: "氵幼",
	泒: "氵瓜",
	泓: "氵弘",
	泔: "氵甘",
	法: "氵去",
	泖: "氵卯",
	泗: "氵四",
	泘: "氵乎",
	泙: "氵平",
	泚: "氵此",
	泛: "氵乏",
	泜: "氵氐",
	泝: "氵斥",
	泞: "氵宁",
	泟: "氵正",
	泠: "氵令",
	泡: "氵包",
	波: "氵皮",
	泣: "氵立",
	泤: "氵以",
	泥: "氵尼",
	泦: "氵尻",
	泧: "氵戉",
	注: "氵主",
	泩: "氵生",
	泪: "氵目",
	泫: "氵玄",
	泬: "氵穴",
	泭: "氵付",
	泮: "氵半",
	泯: "氵民",
	泱: "氵央",
	泳: "氵永",
	泷: "氵龙",
	泸: "氵卢",
	泹: "氵旦",
	泺: "氵乐",
	泻: "氵写",
	泼: "氵发",
	泿: "氵艮",
	洀: "氵舟",
	洁: "氵吉",
	洂: "氵亦",
	洃: "氵灰",
	洄: "氵回",
	洅: "氵再",
	洆: "氵丞",
	洇: "氵因",
	洈: "氵危",
	洉: "氵后",
	洊: "氵存",
	洋: "氵羊",
	洌: "氵列",
	洎: "氵自",
	洏: "氵而",
	洐: "氵行",
	洑: "氵伏",
	洒: "氵西",
	洓: "氵朿",
	洔: "氵寺",
	洖: "氵吴",
	洗: "氵先",
	洘: "氵考",
	洙: "氵朱",
	洚: "氵夅",
	洛: "氵各",
	洝: "氵安",
	洞: "氵同",
	洟: "氵夷",
	洠: "氵牟",
	洡: "氵耒",
	洢: "氵伊",
	洣: "氵米",
	洤: "氵全",
	津: "氵聿",
	洦: "氵百",
	洧: "氵有",
	洨: "氵交",
	洩: "氵曳",
	洪: "氵共",
	洫: "氵血",
	洬: "氵夙",
	洭: "氵匡",
	洮: "氵兆",
	洰: "氵巨",
	洱: "氵耳",
	洲: "氵州",
	洳: "氵如",
	洴: "氵并",
	洵: "氵旬",
	洶: "氵匈",
	洷: "氵至",
	洸: "氵光",
	洹: "氵亘",
	洺: "氵名",
	活: "氵舌",
	洼: "氵圭",
	洽: "氵合",
	洿: "氵夸",
	浀: "氵曲",
	浂: "氵关",
	浃: "氵夹",
	浄: "氵争",
	浅: "氵戋",
	浇: "氵尧",
	浈: "氵贞",
	浉: "氵师",
	浊: "氵虫",
	测: "氵则",
	浌: "氵伐",
	浍: "氵会",
	济: "氵齐",
	浏: "汶刂",
	浐: "氵产",
	浑: "氵军",
	浒: "氵许",
	浓: "氵农",
	浔: "氵寻",
	浕: "氵尽",
	浖: "氵寽",
	浗: "氵求",
	浘: "氵尾",
	浙: "氵折",
	浚: "氵夋",
	浛: "氵含",
	浜: "氵兵",
	浝: "氵尨",
	浞: "氵足",
	浟: "氵攸",
	浠: "氵希",
	浡: "氵孛",
	浢: "氵豆",
	浣: "氵完",
	浤: "氵宏",
	浥: "氵邑",
	浦: "氵甫",
	浧: "氵呈",
	浨: "氵宋",
	浩: "氵告",
	浪: "氵良",
	浫: "氵罕",
	浬: "氵里",
	浭: "氵更",
	浮: "氵孚",
	浯: "氵吾",
	浰: "氵利",
	浱: "氵辰",
	浲: "氵夆",
	浴: "氵谷",
	浵: "氵彤",
	浶: "氵牢",
	海: "氵每",
	浹: "氵夾",
	浺: "氵忡",
	浻: "氵冏",
	浼: "氵免",
	浽: "氵妥",
	浾: "氵赤",
	浿: "氵貝",
	涀: "氵見",
	涁: "氵杉",
	涂: "氵余",
	涃: "氵困",
	涄: "氵甹",
	涅: "氵圼",
	涆: "氵旰",
	涇: "氵巠",
	消: "氵肖",
	涉: "氵步",
	涊: "氵忍",
	涋: "氵宊",
	涌: "氵甬",
	涍: "氵孝",
	涎: "氵延",
	涏: "氵廷",
	涐: "氵我",
	涑: "氵束",
	涒: "氵君",
	涓: "氵肙",
	涔: "氵岑",
	涕: "氵弟",
	涖: "氵位",
	涗: "氵兌",
	涘: "氵矣",
	涙: "氵戻",
	涚: "氵兑",
	涛: "氵寿",
	涜: "氵売",
	涝: "氵劳",
	涞: "氵来",
	涟: "氵连",
	涠: "氵围",
	涡: "氵呙",
	涢: "氵员",
	涣: "氵奂",
	涤: "氵条",
	涥: "氵亨",
	润: "氵闰",
	涧: "氵间",
	涨: "氵张",
	涪: "氵咅",
	涫: "氵官",
	涬: "氵幸",
	涭: "氵受",
	涮: "氵刷",
	涯: "氵厓",
	涰: "氵叕",
	涱: "氵長",
	液: "氵夜",
	涳: "氵空",
	涴: "氵宛",
	涵: "氵函",
	涶: "氵垂",
	涷: "氵東",
	涸: "氵固",
	涹: "氵委",
	涺: "氵居",
	涻: "氵舍",
	涼: "氵京",
	涽: "氵昏",
	涾: "氵沓",
	涿: "氵豖",
	淀: "氵定",
	淁: "氵妾",
	淃: "氵卷",
	淄: "氵甾",
	淅: "氵析",
	淆: "氵肴",
	淇: "氵其",
	淈: "氵屈",
	淉: "氵果",
	淊: "氵臽",
	淋: "氵林",
	淌: "氵尚",
	淍: "氵周",
	淎: "氵奉",
	淏: "氵昊",
	淐: "氵昌",
	淑: "氵叔",
	淒: "氵妻",
	淓: "氵芳",
	淔: "氵直",
	淕: "氵坴",
	淖: "氵卓",
	淗: "氵匊",
	淘: "氵匋",
	淙: "氵宗",
	淚: "氵戾",
	淛: "氵制",
	淜: "氵朋",
	淝: "氵肥",
	淞: "氵松",
	淟: "氵典",
	淠: "氵畀",
	淡: "氵炎",
	淢: "氵或",
	淣: "氵兒",
	淤: "氵於",
	淥: "氵彔",
	淦: "氵金",
	淧: "氵宓",
	淨: "氵爭",
	淩: "氵夌",
	淪: "氵侖",
	淬: "氵卒",
	淮: "氵隹",
	淯: "氵育",
	淰: "氵念",
	深: "氵罙",
	淲: "氵虎",
	淳: "氵享",
	淴: "氵忽",
	淶: "氵來",
	混: "氵昆",
	淸: "氵靑",
	淹: "氵奄",
	淺: "氵戔",
	添: "氵忝",
	淽: "氵芷",
	淿: "氵帛",
	渀: "氵奔",
	渂: "氵旻",
	渃: "氵若",
	渄: "氵非",
	清: "氵青",
	済: "氵斉",
	渉: "氵歩",
	渌: "氵录",
	渍: "氵责",
	渎: "氵卖",
	渏: "氵奇",
	渐: "氵斩",
	渑: "氵黾",
	渒: "氵卑",
	渔: "氵鱼",
	渖: "氵审",
	渗: "氵参",
	渘: "氵柔",
	渙: "氵奐",
	渚: "氵者",
	減: "氵咸",
	渜: "氵耎",
	渝: "氵俞",
	渞: "氵首",
	渟: "氵亭",
	渡: "氵度",
	渢: "氵風",
	渣: "氵查",
	渤: "氵勃",
	渥: "氵屋",
	渦: "氵咼",
	渧: "氵帝",
	渨: "氵畏",
	温: "氵昷",
	渪: "氵禹",
	渫: "氵枼",
	測: "氵則",
	渭: "氵胃",
	渮: "氵苛",
	港: "氵巷",
	渰: "氵弇",
	渱: "氵虹",
	渲: "氵宣",
	渳: "氵弭",
	渴: "氵曷",
	渵: "氵苗",
	渶: "氵英",
	渷: "氵兗",
	游: "氵斿",
	渹: "氵訇",
	渺: "氵眇",
	渻: "氵省",
	渼: "氵美",
	渽: "氵哉",
	渾: "氵軍",
	渿: "氵柰",
	湀: "氵癸",
	湁: "氵拾",
	湂: "氵咢",
	湃: "氵拜",
	湄: "氵眉",
	湅: "氵柬",
	湆: "氵音",
	湈: "氵某",
	湉: "氵恬",
	湊: "氵奏",
	湋: "氵韋",
	湌: "氵食",
	湍: "氵耑",
	湎: "氵面",
	湏: "氵頁",
	湐: "氵柏",
	湑: "氵胥",
	湒: "氵咠",
	湓: "氵盆",
	湔: "氵前",
	湕: "氵建",
	湖: "氵胡",
	湗: "氵封",
	湘: "氵相",
	湙: "氵奕",
	湚: "氵胤",
	湛: "氵甚",
	湜: "氵是",
	湝: "氵皆",
	湞: "氵貞",
	湟: "氵皇",
	湠: "氵炭",
	湡: "氵禺",
	湢: "氵畐",
	湣: "氵昬",
	湤: "氵施",
	湥: "氵突",
	湦: "氵星",
	湧: "氵勇",
	湨: "氵狊",
	湩: "氵重",
	湪: "氵彖",
	湫: "氵秋",
	湭: "氵酋",
	湮: "氵垔",
	湯: "氵昜",
	湱: "氵砉",
	湲: "氵爰",
	湳: "氵南",
	湴: "氵並",
	湵: "氵羑",
	湶: "氵泉",
	湷: "氵春",
	湸: "氵亮",
	湹: "氵厘",
	湺: "氵保",
	湻: "氵亯",
	湾: "氵弯",
	湿: "氵显",
	溁: "氵荣",
	溂: "氵剌",
	溃: "氵贵",
	溅: "氵贱",
	溆: "氵叙",
	溇: "氵娄",
	溈: "氵為",
	溉: "氵既",
	溊: "氵叚",
	溋: "氵盈",
	溌: "氵発",
	溍: "氵晋",
	溎: "氵桂",
	溏: "氵唐",
	源: "氵原",
	溒: "氵袁",
	溓: "氵兼",
	溔: "氵羔",
	溕: "氵冡",
	溗: "氵乘",
	溘: "氵盍",
	溙: "氵泰",
	溚: "氵荅",
	溛: "氵窊",
	溜: "氵留",
	溝: "氵冓",
	溞: "氵蚤",
	溟: "氵冥",
	溠: "氵差",
	溡: "氵時",
	溢: "氵益",
	溣: "氵倫",
	溤: "氵馬",
	溥: "氵尃",
	溧: "氵栗",
	溨: "氵栽",
	溩: "氵烏",
	溪: "氵奚",
	溬: "氵羗",
	溭: "氵畟",
	溮: "氵師",
	溯: "氵朔",
	溰: "氵豈",
	溱: "氵秦",
	溲: "氵叟",
	溳: "氵員",
	溴: "氵臭",
	溵: "氵殷",
	溶: "氵容",
	溷: "氵圂",
	溸: "氵素",
	溹: "氵索",
	溺: "氵弱",
	溽: "氵辱",
	溾: "氵鬼",
	溿: "氵畔",
	滀: "氵畜",
	滁: "氵除",
	滂: "氵旁",
	滃: "氵翁",
	滄: "氵倉",
	滅: "氵烕",
	滆: "氵鬲",
	滇: "氵真",
	滈: "氵高",
	滉: "氵晃",
	滊: "氵氣",
	滋: "氵兹",
	滌: "氵條",
	滍: "氵蚩",
	滏: "氵釜",
	滐: "氵桀",
	滑: "氵骨",
	滒: "氵哥",
	滓: "氵宰",
	滔: "氵舀",
	滖: "氵衰",
	滗: "氵笔",
	滙: "氵龨",
	滚: "氵衮",
	滜: "氵皋",
	滝: "氵竜",
	滞: "氵带",
	滟: "氵艳",
	滠: "氵聂",
	滢: "氵莹",
	滣: "氵唇",
	滤: "氵虑",
	滥: "氵监",
	滦: "氵栾",
	滧: "氵效",
	滨: "氵宾",
	滩: "氵难",
	滪: "氵预",
	滫: "氵脩",
	滬: "氵扈",
	滭: "氵畢",
	滮: "氵彪",
	滯: "氵帶",
	滰: "氵竟",
	滱: "氵寇",
	滲: "氵參",
	滳: "氵商",
	滴: "氵啇",
	滵: "氵密",
	滶: "氵敖",
	滷: "氵鹵",
	滸: "氵許",
	滹: "氵虖",
	滺: "氵悠",
	滻: "氵産",
	滼: "氵梵",
	滽: "氵庸",
	滾: "氵袞",
	漁: "氵魚",
	漂: "氵票",
	漃: "氵寂",
	漄: "氵崖",
	漅: "氵巢",
	漆: "氵桼",
	漇: "氵徙",
	漈: "氵祭",
	漉: "氵鹿",
	漊: "氵婁",
	漋: "氵隆",
	漌: "氵堇",
	漍: "氵國",
	漎: "氵從",
	漏: "氵屚",
	漑: "氵旣",
	漒: "氵强",
	漓: "氵离",
	演: "氵寅",
	漕: "氵曹",
	漖: "氵教",
	漗: "氵悤",
	漘: "氵脣",
	漙: "氵專",
	漚: "氵區",
	漜: "氵埜",
	漝: "氵習",
	漞: "氵覓",
	漟: "氵堂",
	漠: "氵莫",
	漣: "氵連",
	漤: "氵婪",
	漥: "氵窐",
	漧: "氵乾",
	漨: "氵逢",
	漩: "氵旋",
	漪: "氵猗",
	漫: "氵曼",
	漬: "氵責",
	漭: "氵莽",
	漮: "氵康",
	漯: "氵累",
	漰: "氵崩",
	漱: "氵欶",
	漲: "氵張",
	漳: "氵章",
	漴: "氵崇",
	漵: "氵敘",
	漶: "氵患",
	漷: "氵郭",
	漸: "氵斬",
	漹: "氵焉",
	漺: "氵爽",
	漻: "氵翏",
	漼: "氵崔",
	漽: "氵犀",
	漾: "氵羕",
	潀: "氵眾",
	潄: "氵敕",
	潆: "氵萦",
	潇: "氵萧",
	潉: "氵崑",
	潊: "氵敍",
	潋: "氵敛",
	潌: "氵窒",
	潍: "氵维",
	潎: "氵敝",
	潏: "氵矞",
	潐: "氵焦",
	潑: "氵發",
	潒: "氵象",
	潓: "氵惠",
	潔: "氵絜",
	潕: "氵無",
	潖: "氵琶",
	潗: "氵集",
	潘: "氵番",
	潙: "氵爲",
	潚: "氵肅",
	潛: "氵朁",
	潜: "氵替",
	潝: "氵翕",
	潞: "氵路",
	潟: "氵舄",
	潠: "氵巽",
	潡: "氵敦",
	潢: "氵黄",
	潣: "氵閔",
	潤: "氵閏",
	潥: "氵粟",
	潦: "氵尞",
	潧: "氵曾",
	潨: "氵衆",
	潩: "氵異",
	潪: "氵智",
	潫: "氵絭",
	潬: "氵單",
	潭: "氵覃",
	潮: "氵朝",
	潯: "氵尋",
	潰: "氵貴",
	潱: "氵壹",
	潲: "氵稍",
	潳: "氵屠",
	潴: "氵猪",
	潵: "氵散",
	潶: "氵黑",
	潷: "氵筆",
	潹: "氵森",
	潺: "氵孱",
	潻: "氵黍",
	潼: "氵童",
	潽: "氵普",
	潾: "氵粦",
	潿: "氵圍",
	澀: "氵歰",
	澁: "氵歮",
	澄: "氵登",
	澅: "氵畫",
	澆: "氵堯",
	澇: "氵勞",
	澉: "氵敢",
	澊: "氵尊",
	澋: "氵景",
	澌: "氵斯",
	澍: "氵尌",
	澎: "氵彭",
	澏: "氵嵒",
	澐: "氵雲",
	澑: "氵畱",
	澒: "氵項",
	澓: "氵復",
	澔: "氵皓",
	澕: "氵華",
	澖: "氵閑",
	澗: "氵間",
	澙: "氵舃",
	澛: "氵鲁",
	澜: "氵阑",
	澝: "氵甯",
	澞: "氵虞",
	澟: "氵稟",
	澠: "氵黽",
	澡: "氵喿",
	澢: "氵當",
	澣: "氵幹",
	澤: "氵睪",
	澥: "氵解",
	澦: "氵預",
	澧: "氵豊",
	澨: "氵筮",
	澪: "氵零",
	澫: "氵萬",
	澬: "氵資",
	澭: "氵雍",
	澮: "氵會",
	澯: "氵粲",
	澰: "氵僉",
	澱: "氵殿",
	澲: "氵業",
	澳: "氵奥",
	澴: "氵睘",
	澵: "氵新",
	澶: "氵亶",
	澸: "氵感",
	澹: "氵詹",
	澺: "氵意",
	澻: "氵遂",
	澼: "氵辟",
	澽: "氵豦",
	澾: "氵達",
	澿: "氵禁",
	激: "氵敫",
	濁: "氵蜀",
	濂: "氵廉",
	濃: "氵農",
	濄: "氵過",
	濆: "氵賁",
	濇: "氵嗇",
	濈: "氵戢",
	濉: "氵睢",
	濊: "氵歲",
	濋: "氵楚",
	濌: "重沓",
	濍: "氵葱",
	濎: "氵鼎",
	濏: "氵瑟",
	濐: "氵暑",
	濑: "氵赖",
	濒: "氵频",
	濓: "氵亷",
	濔: "氵爾",
	濖: "氵署",
	濗: "氵幕",
	濘: "氵寧",
	濙: "氵熒",
	濚: "氵榮",
	濛: "氵蒙",
	濜: "氵盡",
	濝: "氵綦",
	濞: "氵鼻",
	濟: "氵齊",
	濠: "氵豪",
	濡: "氵需",
	濢: "氵翠",
	濣: "氵斡",
	濤: "氵壽",
	濥: "氵夤",
	濧: "氵對",
	濨: "氵慈",
	濩: "氵蒦",
	濪: "氵靘",
	濫: "氵監",
	濬: "氵睿",
	濭: "氵蓋",
	濮: "氵僕",
	濯: "氵翟",
	濰: "氵維",
	濱: "氵賓",
	濲: "氵穀",
	濴: "氵滎",
	濵: "氵賔",
	濸: "氵蒼",
	濹: "氵墨",
	濺: "氵賤",
	濻: "氵隤",
	濼: "氵樂",
	濽: "氵賛",
	濾: "氵慮",
	濿: "氵厲",
	瀀: "氵憂",
	瀁: "氵養",
	瀂: "氵魯",
	瀃: "氵賜",
	瀄: "氵節",
	瀅: "氵瑩",
	瀆: "氵賣",
	瀇: "氵廣",
	瀈: "氵翬",
	瀉: "氵寫",
	瀊: "氵盤",
	瀋: "氵審",
	瀌: "氵麃",
	瀍: "氵廛",
	瀎: "氵蔑",
	瀏: "氵劉",
	瀐: "氵韯",
	瀑: "氵暴",
	瀓: "氵徵",
	瀔: "氵穀",
	瀕: "氵頻",
	瀖: "氵霍",
	瀗: "氵憲",
	瀘: "氵盧",
	瀙: "氵親",
	瀚: "氵翰",
	瀛: "氵嬴",
	瀜: "氵融",
	瀝: "氵歷",
	瀞: "氵静",
	瀟: "氵蕭",
	瀠: "氵縈",
	瀡: "氵隨",
	瀢: "氵遺",
	瀣: "氵韰",
	瀤: "氵褱",
	瀥: "氵翯",
	瀦: "氵豬",
	瀧: "氵龍",
	瀨: "氵賴",
	瀩: "氵頽",
	瀫: "氵縠",
	瀬: "氵頼",
	瀭: "氵輸",
	瀮: "氵霖",
	瀯: "氵營",
	瀰: "氵彌",
	瀱: "氵罽",
	瀲: "氵斂",
	瀳: "氵薦",
	瀴: "氵嬰",
	瀵: "氵糞",
	瀶: "氵臨",
	瀷: "氵翼",
	瀸: "氵韱",
	瀹: "氵龠",
	瀺: "氵毚",
	瀻: "氵戴",
	瀼: "氵襄",
	瀽: "氵蹇",
	瀾: "氵闌",
	瀿: "氵繁",
	灀: "氵霜",
	灁: "氵闎",
	灂: "氵爵",
	灃: "氵豐",
	灄: "氵聶",
	灅: "氵壘",
	灆: "氵藍",
	灇: "氵叢",
	灈: "氵瞿",
	灉: "氵雝",
	灊: "氵鬵",
	灌: "氵雚",
	灍: "氵闕",
	灎: "澧盍",
	灏: "澋页",
	灐: "氵鎣",
	灑: "氵麗",
	灒: "氵贊",
	灔: "氵艶",
	灕: "氵離",
	灖: "氵靡",
	灗: "氵蟺",
	灘: "氵難",
	灙: "氵黨",
	灚: "氵覺",
	灛: "氵闡",
	灜: "氵贏",
	灝: "氵顥",
	灞: "氵霸",
	灟: "氵屬",
	灠: "氵覽",
	灡: "氵蘭",
	灢: "氵囊",
	灣: "氵彎",
	灤: "氵欒",
	灦: "氵顯",
	灧: "氵艷",
	灨: "氵贛",
	灩: "氵豔",
	灪: "氵鬱",
	灯: "火丁",
	灱: "火刀",
	灲: "火刂",
	灴: "火工",
	灶: "火土",
	灹: "火乇",
	灺: "火也",
	灼: "火勺",
	灿: "火山",
	炂: "火公",
	炄: "火丑",
	炆: "火文",
	炇: "火攵",
	炈: "火殳",
	炉: "火户",
	炊: "火欠",
	炋: "火不",
	炌: "火介",
	炍: "火反",
	炏: "火火",
	炐: "火丰",
	炑: "火木",
	炒: "火少",
	炓: "火斗",
	炔: "火夬",
	炕: "火亢",
	炖: "火屯",
	炘: "火斤",
	炚: "日火",
	炜: "火韦",
	炝: "火仓",
	炞: "火卞",
	炟: "火旦",
	炠: "火甲",
	炡: "火正",
	炢: "火术",
	炣: "火可",
	炤: "火召",
	炥: "火弗",
	炦: "火犮",
	炨: "火它",
	炩: "火令",
	炪: "火出",
	炫: "火玄",
	炬: "火巨",
	炮: "火包",
	炯: "火冋",
	炲: "火台",
	炳: "火丙",
	炴: "火央",
	炵: "火冬",
	炶: "火占",
	炷: "火主",
	炸: "火乍",
	炻: "火石",
	炽: "火只",
	炾: "火兄",
	烀: "火乎",
	烁: "火乐",
	烂: "火兰",
	烄: "火交",
	烅: "火血",
	烆: "火行",
	烇: "火全",
	烊: "火羊",
	烌: "火休",
	烍: "火先",
	烐: "火舟",
	烑: "火兆",
	烒: "火式",
	烓: "火圭",
	烔: "火同",
	烗: "火亥",
	烘: "火共",
	烙: "火各",
	烚: "火合",
	烛: "火虫",
	烜: "火亘",
	烞: "火朴",
	烟: "火因",
	烠: "火有",
	烢: "火宅",
	烣: "火灰",
	烤: "火考",
	烥: "火臣",
	烦: "火页",
	烧: "火尧",
	烨: "火华",
	烩: "火会",
	烪: "火关",
	烬: "火尽",
	烯: "火希",
	烰: "火孚",
	烱: "火冏",
	烳: "火甫",
	烴: "火巠",
	烵: "火芍",
	烶: "火廷",
	烷: "火完",
	烸: "火每",
	烺: "火良",
	烻: "火延",
	烼: "火豕",
	烽: "火夆",
	烿: "火彤",
	焀: "火谷",
	焃: "火赤",
	焅: "火告",
	焆: "火肙",
	焇: "火肖",
	焊: "火旱",
	焌: "火夋",
	焍: "火弟",
	焐: "火吾",
	焑: "火困",
	焒: "火吕",
	焓: "火含",
	焕: "火奂",
	焖: "火闷",
	焗: "火局",
	焙: "火咅",
	焜: "火昆",
	焝: "火昏",
	焞: "火享",
	焟: "火昔",
	焠: "火卒",
	焢: "火空",
	焥: "火宛",
	焧: "火忩",
	焨: "火冐",
	焩: "火朋",
	焪: "火穹",
	焫: "火芮",
	焬: "火易",
	焮: "火欣",
	焯: "火卓",
	焰: "火臽",
	焲: "火夜",
	焳: "火隹",
	焴: "火育",
	焵: "火岡",
	焷: "火卑",
	焹: "火罔",
	焺: "火昇",
	焻: "火昌",
	焼: "火尭",
	焾: "火念",
	焿: "火庚",
	煀: "火屈",
	煁: "火甚",
	煂: "火革",
	煃: "火奎",
	煄: "火重",
	煅: "火段",
	煆: "火叚",
	煇: "火軍",
	煈: "火風",
	煉: "火柬",
	煊: "火宣",
	煋: "火星",
	煌: "火皇",
	煍: "火秋",
	煏: "火畐",
	煐: "火英",
	煒: "火韋",
	煓: "火耑",
	煔: "炎占",
	煖: "火爰",
	煗: "火耎",
	煘: "火咸",
	煙: "火垔",
	煜: "火昱",
	煝: "火眉",
	煟: "火胃",
	煠: "火枼",
	煡: "火建",
	煣: "火柔",
	煤: "火某",
	煥: "火奐",
	煨: "火畏",
	煩: "火頁",
	煪: "火酋",
	煬: "火昜",
	煯: "火皆",
	煰: "火品",
	煱: "火咼",
	煳: "火胡",
	煴: "火昷",
	煵: "火南",
	煶: "火是",
	煷: "火亮",
	煸: "火扁",
	煹: "火冓",
	煺: "火退",
	煻: "火唐",
	煼: "火芻",
	煽: "火扇",
	煾: "火恩",
	煿: "火尃",
	熀: "火晃",
	熁: "火脅",
	熂: "火氣",
	熄: "火息",
	熆: "火盍",
	熇: "火高",
	熉: "火員",
	熌: "火閃",
	熍: "火宮",
	熐: "火冥",
	熑: "火兼",
	熓: "火烏",
	熔: "火容",
	熕: "火貢",
	熖: "火舀",
	熗: "火倉",
	熘: "火留",
	熚: "火畢",
	熛: "火票",
	熜: "火悤",
	熝: "火鹿",
	熞: "火堅",
	熠: "火習",
	熡: "火婁",
	熢: "火逢",
	熣: "火崔",
	熤: "火翌",
	熥: "火通",
	熦: "火雀",
	熩: "火扈",
	熪: "火移",
	熫: "火庶",
	熮: "火翏",
	熰: "火區",
	熳: "火曼",
	熴: "火崑",
	熵: "火商",
	熶: "火最",
	熷: "火曾",
	熸: "火朁",
	熺: "火喜",
	熻: "火翕",
	熼: "火異",
	熽: "火肅",
	熾: "火戠",
	熿: "火黄",
	燀: "火單",
	燁: "火華",
	燂: "火覃",
	燃: "火然",
	燄: "臽炎",
	燅: "坴炎",
	燆: "火喬",
	燇: "火尊",
	燈: "火登",
	燉: "火敦",
	燋: "火焦",
	燌: "火賁",
	燍: "火斯",
	燎: "火尞",
	燏: "火矞",
	燐: "火粦",
	燑: "火童",
	燒: "火堯",
	燔: "火番",
	燖: "火尋",
	燗: "火閒",
	燘: "火閔",
	燜: "火悶",
	燝: "火景",
	燠: "火奥",
	燡: "火睪",
	燣: "火稟",
	燤: "火萬",
	燥: "火喿",
	燦: "火粲",
	燧: "火遂",
	燨: "火義",
	燩: "火敫",
	燪: "火葱",
	燫: "火廉",
	燬: "火毁",
	燭: "火蜀",
	燯: "火零",
	燰: "火愛",
	燱: "火意",
	燳: "火照",
	燴: "火會",
	燵: "火達",
	燶: "火農",
	燷: "火禀",
	燸: "火需",
	燺: "火槀",
	燻: "火熏",
	燼: "火盡",
	燽: "火壽",
	燿: "火翟",
	爀: "火赫",
	爁: "火監",
	爃: "火榮",
	爄: "火厲",
	爅: "火墨",
	爆: "火暴",
	爈: "火慮",
	爉: "火巤",
	爊: "火麃",
	爋: "火勳",
	爌: "火廣",
	爍: "火樂",
	爎: "火寮",
	爏: "火歷",
	爐: "火盧",
	爑: "火蕉",
	爒: "炙尞",
	爓: "火閻",
	爔: "火羲",
	爖: "火龍",
	爗: "火曅",
	爘: "火餐",
	爙: "火襄",
	爚: "火龠",
	爛: "火闌",
	爜: "火叢",
	爝: "火爵",
	爞: "火蟲",
	爟: "火雚",
	爠: "火瞿",
	爡: "火蠆",
	爣: "火黨",
	爤: "火蘭",
	爥: "火屬",
	爦: "火覽",
	爧: "火靈",
	爩: "火鬱",
	爴: "國爪",
	爼: "爻且",
	牀: "爿木",
	牁: "爿可",
	牂: "爿羊",
	牃: "爿枼",
	牄: "爿倉",
	牅: "爿庸",
	牆: "爿嗇",
	版: "片反",
	牉: "片半",
	牊: "片召",
	牋: "片戔",
	牌: "片卑",
	牍: "片卖",
	牎: "片怱",
	牏: "片俞",
	牐: "片臿",
	牑: "片扁",
	牒: "片枼",
	牓: "片旁",
	牔: "片尃",
	牕: "片悤",
	牗: "片庸",
	牘: "片賣",
	牝: "牛匕",
	牞: "牛力",
	牠: "牛也",
	牡: "牛土",
	牣: "牛刃",
	牤: "牛亡",
	牥: "牛方",
	牦: "牛毛",
	牧: "牛攵",
	牨: "牛亢",
	物: "牛勿",
	牪: "牛牛",
	牫: "牛戈",
	牭: "牛四",
	牯: "牛古",
	牰: "牛由",
	牱: "牛可",
	牲: "牛生",
	牳: "牛母",
	牴: "牛氐",
	牷: "牛全",
	牸: "牛字",
	特: "牛寺",
	牺: "牛西",
	牻: "牛尨",
	牼: "牛巠",
	牾: "牛吾",
	牿: "牛告",
	犃: "牛咅",
	犄: "牛奇",
	犅: "牛岡",
	犆: "牛直",
	犈: "牛卷",
	犉: "牛享",
	犊: "牛卖",
	犋: "牛具",
	犌: "牛叚",
	犍: "牛建",
	犏: "牛扁",
	犐: "牛科",
	犑: "牛狊",
	犒: "牛高",
	犓: "牛芻",
	犔: "牛氣",
	犗: "牛害",
	犙: "牛參",
	犜: "牛敦",
	犝: "牛童",
	犞: "牛喬",
	犠: "牛義",
	犡: "牛厲",
	犢: "牛賣",
	犣: "牛巤",
	犤: "牛罷",
	犥: "牛麃",
	犦: "牛暴",
	犧: "牛羲",
	犪: "牛夔",
	犰: "犭九",
	犱: "犭丸",
	犲: "犭才",
	犳: "犭勺",
	犴: "犭干",
	犵: "犭乞",
	状: "丬犬",
	犷: "犭广",
	犸: "犭马",
	犹: "犭尤",
	犺: "犭亢",
	犻: "犭巿",
	犼: "犭孔",
	犽: "犭牙",
	犾: "犭犬",
	犿: "犭卞",
	狀: "爿犬",
	狁: "犭允",
	狂: "犭王",
	狃: "犭丑",
	狄: "犭火",
	狅: "犭壬",
	狆: "犭中",
	狇: "犭木",
	狈: "犭贝",
	狉: "犭丕",
	狋: "犭示",
	狌: "犭生",
	狍: "犭包",
	狎: "犭甲",
	狐: "犭瓜",
	狑: "犭令",
	狒: "犭弗",
	狓: "犭皮",
	狔: "犭尼",
	狕: "犭幼",
	狖: "犭穴",
	狗: "犭句",
	狘: "犭戉",
	狙: "犭且",
	狚: "犭旦",
	狛: "犭白",
	狜: "犭古",
	狝: "犭尔",
	狞: "犭宁",
	狟: "犭亘",
	狠: "犭艮",
	狡: "犭交",
	狢: "犭各",
	狣: "犭兆",
	狤: "犭吉",
	狥: "犭旬",
	狧: "犭舌",
	狨: "犭戎",
	狩: "犭守",
	狪: "犭同",
	狫: "犭老",
	独: "犭虫",
	狭: "犭夹",
	狮: "犭师",
	狯: "犭会",
	狰: "犭争",
	狲: "犭孙",
	狳: "犭余",
	狴: "犭坒",
	狵: "犭尨",
	狶: "犭希",
	狷: "犭肙",
	狸: "犭里",
	狹: "犭夾",
	狺: "犭言",
	狻: "犭夋",
	狼: "犭良",
	狽: "犭貝",
	狾: "犭折",
	狿: "犭延",
	猀: "犭沙",
	猁: "犭利",
	猂: "犭旱",
	猃: "犭佥",
	猄: "犭京",
	猅: "犭非",
	猇: "犭虎",
	猈: "犭卑",
	猉: "犭其",
	猊: "犭兒",
	猌: "來犬",
	猍: "犭來",
	猎: "犭昔",
	猏: "犭肩",
	猐: "犭羌",
	猑: "犭昆",
	猒: "冐犬",
	猓: "犭果",
	猔: "犭宗",
	猕: "犭弥",
	猖: "犭昌",
	猗: "犭奇",
	猘: "犭制",
	猙: "犭爭",
	猚: "犭隹",
	猛: "犭孟",
	猜: "犭青",
	猝: "犭卒",
	猞: "犭舍",
	猟: "犭鼡",
	猠: "犭典",
	猡: "犭罗",
	猢: "犭胡",
	猤: "犭癸",
	猥: "犭畏",
	猦: "犭風",
	猧: "犭咼",
	猨: "犭爰",
	猩: "犭星",
	猪: "犭者",
	猫: "犭苗",
	猬: "犭胃",
	猭: "犭彖",
	献: "南犬",
	猯: "犭耑",
	猰: "犭契",
	猱: "犭柔",
	猲: "犭曷",
	猳: "犭叚",
	猴: "犭侯",
	猵: "犭扁",
	猶: "犭酋",
	猷: "酋犬",
	猸: "犭眉",
	猹: "犭查",
	猻: "犭孫",
	猼: "犭尃",
	猽: "犭冥",
	猾: "犭骨",
	猿: "犭袁",
	獀: "犭叟",
	獁: "犭馬",
	獂: "犭原",
	獃: "豈犬",
	獅: "犭師",
	獆: "犭皋",
	獇: "犭羗",
	獈: "犭益",
	獉: "犭秦",
	獊: "犭倉",
	獋: "犭臯",
	獌: "犭曼",
	獍: "犭竟",
	獏: "犭莫",
	獐: "犭章",
	獑: "犭斬",
	獓: "犭敖",
	獔: "犭皐",
	獕: "犭崔",
	獖: "犭賁",
	獗: "犭厥",
	獙: "犭敝",
	獚: "犭黄",
	獛: "犭菐",
	獜: "犭粦",
	獝: "犭矞",
	獞: "犭童",
	獟: "犭堯",
	獠: "犭尞",
	獡: "犭舄",
	獢: "犭喬",
	獤: "犭敦",
	獥: "犭敫",
	獦: "犭葛",
	獧: "犭睘",
	獨: "犭蜀",
	獩: "犭歲",
	獪: "犭會",
	獫: "犭僉",
	獬: "犭解",
	獭: "犭赖",
	獮: "犭爾",
	獯: "犭熏",
	獰: "犭寧",
	獱: "犭賓",
	獲: "犭蒦",
	獳: "犭需",
	獴: "犭蒙",
	獵: "犭巤",
	獶: "犭憂",
	獷: "犭廣",
	獸: "嘼犬",
	獹: "犭盧",
	獺: "犭賴",
	獻: "鬳犬",
	獼: "犭彌",
	獽: "犭襄",
	獾: "犭雚",
	獿: "犭夒",
	玀: "犭羅",
	玁: "犭嚴",
	玂: "犭蘄",
	玃: "犭矍",
	玅: "玄少",
	玆: "玄玄",
	玌: "王乚",
	玎: "王丁",
	玏: "王力",
	玐: "王八",
	玑: "王几",
	玒: "王工",
	玓: "王勺",
	玔: "王川",
	玕: "王干",
	玖: "王久",
	玗: "王于",
	玘: "王己",
	玙: "王与",
	玛: "王马",
	玜: "王公",
	玝: "王午",
	玞: "王夫",
	玟: "王文",
	玠: "王介",
	玡: "王牙",
	玢: "王分",
	玣: "王卞",
	玤: "王丰",
	玥: "王月",
	玦: "王夬",
	玧: "王允",
	玨: "王王",
	玩: "王元",
	玪: "王今",
	玫: "王攵",
	玬: "王丹",
	玭: "王比",
	玮: "王韦",
	环: "王不",
	现: "王见",
	玱: "王仓",
	玲: "王令",
	玳: "王代",
	玴: "王世",
	玵: "王甘",
	玶: "王平",
	玷: "王占",
	玸: "王包",
	玹: "王玄",
	玻: "王皮",
	玼: "王此",
	玽: "王句",
	玾: "王甲",
	玿: "王召",
	珀: "王白",
	珁: "王瓦",
	珂: "王可",
	珃: "王冉",
	珄: "王生",
	珅: "王申",
	珆: "王台",
	珇: "王且",
	珈: "王加",
	珉: "王民",
	珊: "王册",
	珋: "王卯",
	珌: "王必",
	珎: "王尔",
	珏: "王玉",
	珐: "王去",
	珑: "王龙",
	珒: "王聿",
	珓: "王交",
	珔: "王存",
	珕: "王劦",
	珖: "王光",
	珗: "王先",
	珘: "王舟",
	珙: "王共",
	珚: "王因",
	珛: "王有",
	珜: "王羊",
	珝: "王羽",
	珞: "王各",
	珟: "王夙",
	珠: "王朱",
	珢: "王艮",
	珣: "王旬",
	珤: "王缶",
	珥: "王耳",
	珦: "王向",
	珧: "王兆",
	珨: "王合",
	珩: "王行",
	珪: "王圭",
	珫: "王充",
	珬: "王戌",
	珯: "王老",
	珰: "王当",
	珲: "王军",
	珳: "王彣",
	珴: "王我",
	珵: "王呈",
	珶: "王弟",
	珷: "王武",
	珸: "王吾",
	珹: "王成",
	珺: "王君",
	珻: "王每",
	珼: "王貝",
	珽: "王廷",
	現: "王見",
	珿: "王足",
	琀: "王含",
	琂: "王言",
	球: "王求",
	琄: "王肙",
	琅: "王良",
	理: "王里",
	琇: "王秀",
	琈: "王孚",
	琊: "王邪",
	琋: "王希",
	琌: "王岑",
	琍: "王利",
	琎: "王进",
	琏: "王连",
	琑: "王肖",
	琒: "王夆",
	琓: "王完",
	琔: "王定",
	琕: "王卑",
	琖: "王戔",
	琗: "王卒",
	琘: "王昏",
	琙: "王或",
	琚: "王居",
	琛: "王罙",
	琜: "王來",
	琝: "王旻",
	琟: "王隹",
	琠: "王典",
	琡: "王叔",
	琢: "王豖",
	琣: "王咅",
	琤: "王争",
	琥: "王虎",
	琦: "王奇",
	琨: "王昆",
	琩: "王昌",
	琪: "王其",
	琫: "王奉",
	琬: "王宛",
	琭: "王录",
	琮: "王宗",
	琯: "王官",
	琰: "王炎",
	琱: "王周",
	琲: "王非",
	琳: "王林",
	琷: "王羌",
	琸: "王卓",
	琺: "王法",
	琻: "王金",
	琼: "王京",
	琽: "王者",
	琾: "王界",
	琿: "王軍",
	瑀: "王禹",
	瑁: "王冒",
	瑂: "王眉",
	瑃: "王春",
	瑄: "王宣",
	瑅: "王是",
	瑆: "王星",
	瑇: "王毒",
	瑈: "王柔",
	瑉: "王昬",
	瑊: "王咸",
	瑋: "王韋",
	瑌: "王耎",
	瑍: "王奐",
	瑎: "王皆",
	瑏: "王穿",
	瑐: "王前",
	瑑: "王彖",
	瑒: "王昜",
	瑓: "王柬",
	瑔: "王泉",
	瑕: "王叚",
	瑖: "王段",
	瑗: "王爰",
	瑘: "王耶",
	瑚: "王胡",
	瑛: "王英",
	瑜: "王俞",
	瑝: "王皇",
	瑞: "王耑",
	瑠: "王留",
	瑡: "王師",
	瑢: "王容",
	瑥: "王昷",
	瑦: "王烏",
	瑧: "王秦",
	瑨: "王晋",
	瑪: "王馬",
	瑫: "王舀",
	瑭: "王唐",
	瑮: "王栗",
	瑯: "王郎",
	瑰: "王鬼",
	瑱: "王真",
	瑲: "王倉",
	瑳: "王差",
	瑵: "王蚤",
	瑷: "王爱",
	瑸: "王宾",
	瑹: "王荼",
	瑺: "王常",
	瑻: "王貫",
	瑼: "王專",
	瑽: "王從",
	瑾: "王堇",
	璀: "王崔",
	璁: "王悤",
	璂: "王基",
	璃: "王离",
	璄: "王竟",
	璅: "王巢",
	璆: "王翏",
	璇: "王旋",
	璈: "王敖",
	璉: "王連",
	璋: "王章",
	璌: "王寅",
	璍: "王華",
	璎: "王婴",
	璏: "王彘",
	璐: "王路",
	璑: "王無",
	璒: "王登",
	璓: "王莠",
	璔: "王曾",
	璕: "王尋",
	璖: "王渠",
	璘: "王粦",
	璙: "王尞",
	璚: "王矞",
	璛: "王肅",
	璜: "王黄",
	璝: "王貴",
	璞: "王菐",
	璟: "王景",
	璠: "王番",
	璡: "王進",
	璢: "王畱",
	璣: "王幾",
	璤: "王惠",
	璥: "王敬",
	璦: "王愛",
	璨: "王粲",
	璩: "王豦",
	璪: "王喿",
	璫: "王當",
	璬: "王敫",
	璭: "王運",
	璮: "王亶",
	璯: "王會",
	環: "王睘",
	璱: "王瑟",
	璲: "王遂",
	璴: "王楚",
	璵: "王與",
	璶: "王盡",
	璷: "王敷",
	璸: "王賓",
	璹: "王壽",
	璻: "王翠",
	璼: "王監",
	璾: "王齊",
	璿: "王睿",
	瓀: "王需",
	瓁: "王蒦",
	瓂: "王蓋",
	瓃: "王畾",
	瓄: "王賣",
	瓅: "王樂",
	瓆: "王質",
	瓇: "王憂",
	瓈: "王黎",
	瓉: "王賛",
	瓊: "王夐",
	瓋: "王適",
	瓌: "王褱",
	瓍: "王隨",
	瓎: "王賴",
	瓏: "王龍",
	瓐: "王盧",
	瓑: "王歷",
	瓒: "王赞",
	瓓: "王闌",
	瓔: "王嬰",
	瓖: "王襄",
	瓗: "王巂",
	瓘: "王雚",
	瓙: "王燾",
	瓚: "王贊",
	瓛: "王獻",
	瓠: "夸瓜",
	瓡: "幸瓜",
	瓢: "票瓜",
	瓤: "襄瓜",
	瓨: "工瓦",
	瓬: "方瓦",
	瓭: "冘瓦",
	瓯: "区瓦",
	瓳: "古瓦",
	瓴: "令瓦",
	瓵: "台瓦",
	瓶: "并瓦",
	瓹: "肙瓦",
	瓺: "镸瓦",
	瓻: "希瓦",
	瓾: "委瓦",
	瓿: "咅瓦",
	甀: "垂瓦",
	甁: "幷瓦",
	甂: "扁瓦",
	甄: "垔瓦",
	甈: "臬瓦",
	甉: "兼瓦",
	甊: "婁瓦",
	甋: "啇瓦",
	甌: "區瓦",
	甎: "專瓦",
	甐: "粦瓦",
	甑: "曾瓦",
	甒: "無瓦",
	甔: "詹瓦",
	甗: "鬳瓦",
	甛: "甘舌",
	甜: "舌甘",
	甠: "日生",
	甡: "生生",
	甥: "生男",
	町: "田丁",
	甽: "田川",
	甿: "田亡",
	畂: "田久",
	畃: "申勺",
	畆: "亩厶",
	畇: "田勻",
	畈: "田反",
	畉: "田夫",
	畊: "田井",
	畋: "田攵",
	畍: "田介",
	畎: "田犬",
	畑: "火田",
	畒: "亩人",
	畔: "田半",
	畖: "田瓜",
	畘: "田冉",
	畝: "亩久",
	畡: "田亥",
	畤: "田寺",
	略: "田各",
	畦: "田圭",
	畩: "田衣",
	畭: "田余",
	畮: "田每",
	畯: "田夋",
	畴: "田寿",
	畷: "田叕",
	畸: "田奇",
	畹: "田宛",
	畼: "田昜",
	畽: "田重",
	疀: "甾疌",
	疁: "田翏",
	疃: "田童",
	疄: "田粦",
	疅: "田畺",
	疇: "田壽",
	癿: "白乚",
	的: "白勺",
	皅: "白巴",
	皈: "白反",
	皉: "白此",
	皊: "白令",
	皌: "白末",
	皎: "白交",
	皏: "白并",
	皑: "白岂",
	皒: "白我",
	皓: "白告",
	皔: "白旱",
	皕: "百百",
	皖: "白完",
	皗: "白周",
	皘: "白青",
	皚: "白豈",
	皜: "白高",
	皝: "皇光",
	皞: "白皋",
	皟: "白責",
	皠: "白崔",
	皡: "白皐",
	皢: "白堯",
	皣: "白華",
	皤: "白番",
	皥: "白臯",
	皦: "白敫",
	皧: "白愛",
	皩: "皇晃",
	皪: "白樂",
	皫: "白麃",
	皬: "白霍",
	皭: "白爵",
	皯: "皮干",
	皰: "皮包",
	皱: "刍皮",
	皲: "军皮",
	皳: "求皮",
	皴: "夋皮",
	皵: "昔皮",
	皶: "查皮",
	皷: "壴皮",
	皸: "軍皮",
	皹: "皮軍",
	皺: "芻皮",
	皻: "虘皮",
	皼: "壹皮",
	皽: "亶皮",
	皾: "賣皮",
	盯: "目丁",
	盰: "目干",
	盱: "目于",
	盳: "目亡",
	盵: "目乞",
	盶: "目元",
	盷: "目匀",
	相: "木目",
	盹: "目屯",
	盺: "目斤",
	盻: "目兮",
	盼: "目分",
	盽: "目丰",
	盿: "目文",
	眀: "目月",
	眂: "目氏",
	眃: "目云",
	眄: "目丏",
	眅: "目反",
	眆: "目方",
	眇: "目少",
	眈: "目冘",
	眊: "目毛",
	眍: "目区",
	眎: "目示",
	眏: "目央",
	眐: "目正",
	眑: "目幼",
	眒: "目申",
	眓: "目戉",
	眖: "目兄",
	眗: "目句",
	眙: "目台",
	眛: "目未",
	眜: "目末",
	眝: "目宁",
	眠: "目民",
	眡: "目氐",
	眣: "目失",
	眤: "目尼",
	眦: "目此",
	眧: "目召",
	眨: "目乏",
	眩: "目玄",
	眪: "目丙",
	眫: "目半",
	眬: "目龙",
	眭: "目圭",
	眮: "目同",
	眯: "目米",
	眰: "目至",
	眱: "目夷",
	眲: "目耳",
	眳: "目名",
	眴: "目旬",
	眵: "目多",
	眶: "目匡",
	眸: "目牟",
	眹: "目关",
	眺: "目兆",
	眻: "目羊",
	眼: "目艮",
	眿: "目永",
	睁: "目争",
	睃: "目夋",
	睄: "目肖",
	睅: "目旱",
	睆: "目完",
	睇: "目弟",
	睈: "目呈",
	睉: "目坐",
	睊: "目肙",
	睋: "目我",
	睌: "目免",
	睍: "目見",
	睎: "目希",
	睏: "目困",
	睐: "目来",
	睑: "目佥",
	睒: "目炎",
	睓: "目典",
	睔: "目侖",
	睕: "目宛",
	睖: "目夌",
	睗: "目易",
	睙: "目戾",
	睚: "目厓",
	睛: "目青",
	睜: "目爭",
	睞: "目來",
	睟: "目卒",
	睠: "目卷",
	睡: "目垂",
	睢: "目隹",
	睤: "目畀",
	睥: "目卑",
	睦: "目坴",
	睧: "目昏",
	睨: "目兒",
	睩: "目录",
	睫: "目疌",
	睬: "目采",
	睭: "目周",
	睮: "目俞",
	睰: "目若",
	睱: "目叚",
	睲: "目星",
	睳: "目奎",
	睴: "目軍",
	睵: "目哉",
	睶: "目春",
	睷: "目建",
	睸: "目眉",
	睹: "目者",
	睺: "目侯",
	睻: "目宣",
	睼: "目是",
	睽: "目癸",
	瞁: "目狊",
	瞂: "盾犮",
	瞃: "目盾",
	瞄: "目苗",
	瞅: "目秋",
	瞆: "目贵",
	瞇: "目迷",
	瞈: "目翁",
	瞊: "目唐",
	瞋: "目真",
	瞌: "目盍",
	瞍: "目叟",
	瞎: "目害",
	瞑: "目冥",
	瞓: "目訓",
	瞔: "目責",
	瞕: "目章",
	瞗: "目鳥",
	瞘: "目區",
	瞙: "目莫",
	瞚: "目寅",
	瞛: "目從",
	瞜: "目婁",
	瞝: "目离",
	瞟: "目票",
	瞠: "目堂",
	瞡: "目規",
	瞣: "目患",
	瞤: "目閏",
	瞦: "目喜",
	瞧: "目焦",
	瞨: "目菐",
	瞩: "目属",
	瞪: "目登",
	瞫: "目覃",
	瞬: "目舜",
	瞭: "目尞",
	瞯: "目閒",
	瞰: "目敢",
	瞱: "目華",
	瞲: "目矞",
	瞳: "目童",
	瞴: "目無",
	瞵: "目粦",
	瞶: "目貴",
	瞷: "目間",
	瞸: "目葉",
	瞹: "目愛",
	瞺: "目會",
	瞻: "目詹",
	瞼: "目僉",
	矀: "目微",
	矁: "目愁",
	矂: "目喿",
	矃: "目寧",
	矄: "目熏",
	矅: "目翟",
	矆: "目蒦",
	矇: "目蒙",
	矉: "目賓",
	矊: "目綿",
	矋: "目厲",
	矌: "目廣",
	矎: "目夐",
	矏: "目臱",
	矐: "目霍",
	矑: "目盧",
	矒: "目瞢",
	矓: "目龍",
	矔: "目雚",
	矖: "目麗",
	矘: "目黨",
	矙: "目闞",
	矚: "目屬",
	矜: "矛今",
	矝: "矛令",
	矟: "矛肖",
	矠: "矛昔",
	矡: "矛矍",
	矤: "弓矢",
	知: "矢口",
	矧: "矢引",
	矨: "矢夭",
	矩: "矢巨",
	矪: "矢舟",
	矫: "矢乔",
	矬: "矢坐",
	短: "矢豆",
	矮: "矢委",
	矯: "矢喬",
	矰: "矢曾",
	矱: "矢蒦",
	矲: "矢罷",
	矴: "石丁",
	矵: "石刂",
	矶: "石几",
	矷: "石子",
	矸: "石干",
	矹: "石兀",
	矺: "石乇",
	矻: "石乞",
	矼: "石工",
	矽: "石夕",
	矾: "石凡",
	矿: "石广",
	码: "石马",
	砂: "石少",
	砃: "石丹",
	砄: "石夬",
	砅: "石水",
	砆: "石夫",
	砇: "石文",
	砈: "石厄",
	砊: "石亢",
	砋: "石止",
	砌: "石切",
	砍: "石欠",
	砎: "石介",
	砏: "石分",
	砐: "石及",
	砑: "石牙",
	砒: "石比",
	砓: "石殳",
	研: "石开",
	砕: "石卆",
	砖: "石专",
	砗: "石车",
	砘: "石屯",
	砙: "石瓦",
	砚: "石见",
	砛: "石今",
	砜: "石风",
	砝: "石去",
	砞: "石末",
	砟: "石乍",
	砠: "石且",
	砡: "石玉",
	砢: "石可",
	砣: "石它",
	砥: "石氐",
	砧: "石占",
	砨: "石戹",
	砩: "石弗",
	砪: "石母",
	砫: "石主",
	砬: "石立",
	砭: "石乏",
	砯: "石氷",
	砰: "石平",
	砱: "石令",
	砲: "石包",
	砳: "石石",
	破: "石皮",
	砵: "石本",
	砶: "石白",
	砷: "石申",
	砸: "石匝",
	砹: "石艾",
	砺: "石厉",
	砼: "石仝",
	砽: "石用",
	砾: "石乐",
	砿: "石広",
	础: "石出",
	硂: "石全",
	硃: "石朱",
	硄: "石光",
	硅: "石圭",
	硆: "石合",
	硇: "石囟",
	硈: "石吉",
	硉: "石聿",
	硊: "石危",
	硋: "石亥",
	硌: "石各",
	硍: "石艮",
	硎: "石刑",
	硏: "石幵",
	硐: "石同",
	硑: "石并",
	硒: "石西",
	硓: "石老",
	硔: "石共",
	硕: "石页",
	硖: "石夹",
	硗: "石尧",
	硘: "石回",
	硙: "石岂",
	硚: "石乔",
	硛: "石亦",
	硜: "石巠",
	硝: "石肖",
	硞: "石告",
	硟: "石延",
	硠: "石良",
	硡: "石宏",
	硢: "石余",
	硣: "石孝",
	硤: "石夾",
	硥: "石尨",
	硦: "石弄",
	硧: "石甬",
	硨: "石車",
	硪: "石我",
	硬: "石更",
	硭: "石芒",
	确: "石角",
	硯: "石見",
	硱: "石困",
	硲: "石谷",
	硳: "石赤",
	硴: "石花",
	硵: "石卤",
	硶: "石岑",
	硷: "石佥",
	硸: "石岸",
	硹: "石松",
	硺: "石豖",
	硼: "石朋",
	硽: "石奄",
	硾: "石垂",
	硿: "石空",
	碀: "石争",
	碂: "石宗",
	碃: "石青",
	碄: "石林",
	碅: "石囷",
	碇: "石定",
	碈: "石昏",
	碉: "石周",
	碊: "石戔",
	碋: "石岢",
	碌: "石录",
	碎: "石卒",
	碏: "石昔",
	碐: "石夌",
	碑: "石卑",
	碒: "石金",
	碓: "石隹",
	碔: "石武",
	碕: "石奇",
	碖: "石侖",
	碗: "石宛",
	碘: "石典",
	碙: "石岡",
	碚: "石咅",
	碛: "石责",
	碜: "石参",
	碝: "石耎",
	碟: "石枼",
	碠: "石亭",
	碡: "石毒",
	碢: "石咼",
	碣: "石曷",
	碤: "石英",
	碥: "石扁",
	碦: "石客",
	碨: "石畏",
	碩: "石頁",
	碪: "石甚",
	碫: "石段",
	碬: "石叚",
	碭: "石昜",
	碮: "石是",
	碰: "石並",
	碱: "石咸",
	碲: "石帝",
	碳: "石炭",
	碴: "石查",
	碵: "石貞",
	碶: "石契",
	碷: "石盾",
	碸: "石風",
	碹: "石宣",
	確: "石隺",
	碻: "石高",
	碼: "石馬",
	碽: "石貢",
	碾: "石展",
	碿: "石屑",
	磀: "石逆",
	磁: "石兹",
	磂: "石留",
	磃: "石虒",
	磄: "石唐",
	磅: "石旁",
	磆: "石骨",
	磈: "石鬼",
	磉: "石桑",
	磋: "石差",
	磌: "石真",
	磍: "石害",
	磎: "石奚",
	磏: "石兼",
	磑: "石豈",
	磒: "石員",
	磓: "石追",
	磔: "石桀",
	磕: "石盍",
	磖: "石習",
	磗: "石尃",
	磙: "石衮",
	磚: "石專",
	磜: "石祭",
	磝: "石敖",
	磞: "石崩",
	磟: "石翏",
	磠: "石鹵",
	磡: "石勘",
	磢: "石爽",
	磣: "石參",
	磤: "石殷",
	磥: "石累",
	磦: "石票",
	磧: "石責",
	磩: "石戚",
	磪: "石崔",
	磫: "石從",
	磭: "石脣",
	磮: "石崙",
	磯: "石幾",
	磰: "石善",
	磱: "石勞",
	磲: "石渠",
	磳: "石曾",
	磴: "石登",
	磵: "石間",
	磶: "石舄",
	磷: "石粦",
	磸: "石奠",
	磹: "石覃",
	磺: "石黄",
	磻: "石番",
	磼: "石集",
	磽: "石堯",
	磾: "石單",
	礀: "石閒",
	礁: "石焦",
	礂: "石喜",
	礃: "石掌",
	礄: "石喬",
	礅: "石敦",
	礆: "石僉",
	礇: "石奥",
	礈: "石遂",
	礉: "石敫",
	礋: "石睪",
	礌: "石雷",
	礍: "石葛",
	礎: "石楚",
	礏: "石業",
	礑: "石當",
	礒: "石義",
	礓: "石畺",
	礔: "石辟",
	礖: "石與",
	礗: "石賓",
	礙: "石疑",
	礚: "石蓋",
	礛: "石監",
	礝: "石需",
	礞: "石蒙",
	礟: "石駁",
	礠: "石慈",
	礡: "石蒪",
	礢: "石養",
	礣: "石蔑",
	礤: "石蔡",
	礥: "石賢",
	礦: "石廣",
	礧: "石畾",
	礩: "石質",
	礪: "石厲",
	礫: "石樂",
	礭: "石霍",
	礮: "石駮",
	礯: "石縈",
	礰: "石歷",
	礲: "石龍",
	礳: "石磨",
	礴: "石薄",
	礵: "石霜",
	礶: "石雚",
	礷: "石藍",
	礸: "石贊",
	礹: "石嚴",
	礼: "礻乚",
	礽: "礻乃",
	社: "礻土",
	礿: "礻勺",
	祀: "礻巳",
	祁: "礻阝",
	祂: "礻也",
	祃: "礻马",
	祄: "礻介",
	祅: "礻夭",
	祆: "礻天",
	祇: "礻氏",
	祈: "礻斤",
	祉: "礻止",
	祊: "礻方",
	祋: "礻殳",
	祌: "礻中",
	祍: "礻壬",
	祎: "礻韦",
	祏: "礻石",
	祐: "礻右",
	祑: "礻失",
	祒: "礻召",
	祓: "礻犮",
	祔: "礻付",
	祕: "礻必",
	祖: "礻且",
	祗: "礻氐",
	祘: "示示",
	祙: "礻未",
	祚: "礻乍",
	祛: "礻去",
	祜: "礻古",
	祝: "礻兄",
	神: "礻申",
	祠: "礻司",
	祢: "礻尔",
	祤: "礻羽",
	祥: "礻羊",
	祦: "礻吴",
	祧: "礻兆",
	祩: "礻朱",
	祪: "礻危",
	祫: "礻合",
	祬: "礻至",
	祮: "礻吉",
	祯: "礻贞",
	祰: "礻告",
	祱: "礻兑",
	祳: "礻辰",
	祴: "礻戒",
	祵: "礻困",
	祶: "礻弟",
	祷: "礻寿",
	祸: "礻呙",
	祹: "礻匋",
	祺: "礻其",
	祻: "礻固",
	祼: "礻果",
	祽: "礻卒",
	祾: "礻夌",
	祿: "礻彔",
	禂: "礻周",
	禃: "礻直",
	禄: "礻录",
	禅: "礻单",
	禆: "礻卑",
	禇: "礻者",
	禈: "礻軍",
	禉: "礻酋",
	禊: "礻契",
	禋: "礻垔",
	禌: "礻兹",
	禍: "礻咼",
	禎: "礻貞",
	福: "礻畐",
	禐: "礻爰",
	禑: "礻禺",
	禒: "礻彖",
	禓: "礻昜",
	禔: "礻是",
	禕: "礻韋",
	禖: "礻某",
	禗: "礻思",
	禘: "礻帝",
	禙: "礻背",
	禚: "礻羔",
	禛: "礻真",
	禝: "礻畟",
	禞: "礻高",
	禟: "礻唐",
	禠: "礻虒",
	禡: "礻馬",
	禣: "礻尃",
	禥: "礻基",
	禧: "礻喜",
	禨: "礻幾",
	禩: "礻異",
	禪: "礻單",
	禫: "礻覃",
	禬: "礻會",
	禭: "礻遂",
	禮: "礻豊",
	禯: "礻農",
	禰: "礻爾",
	禱: "礻壽",
	禲: "礻厲",
	禳: "礻襄",
	禴: "礻龠",
	禵: "礻題",
	禶: "礻贊",
	禷: "礻類",
	私: "禾厶",
	秄: "禾子",
	秅: "禾乇",
	秆: "禾干",
	秇: "禾丸",
	秈: "禾山",
	秋: "禾火",
	秌: "火禾",
	种: "禾中",
	秎: "禾分",
	秏: "禾毛",
	秐: "禾云",
	科: "禾斗",
	秒: "禾少",
	秓: "禾支",
	秔: "禾亢",
	秕: "禾比",
	秖: "禾氏",
	秗: "禾夭",
	秘: "禾必",
	秙: "禾古",
	秚: "禾半",
	秛: "禾皮",
	秜: "禾尼",
	秝: "禾禾",
	秞: "禾由",
	租: "禾且",
	秠: "禾丕",
	秡: "禾犮",
	秢: "禾令",
	秣: "禾末",
	秤: "禾平",
	秥: "禾占",
	秧: "禾央",
	秨: "禾乍",
	秩: "禾失",
	秪: "禾氐",
	秫: "禾术",
	秬: "禾巨",
	秮: "禾台",
	积: "禾只",
	称: "禾尔",
	秱: "禾同",
	秲: "禾寺",
	秳: "禾舌",
	秴: "禾合",
	秵: "禾因",
	秷: "禾至",
	秸: "禾吉",
	秹: "禾任",
	秺: "禾宅",
	移: "禾多",
	秼: "禾朱",
	秽: "禾岁",
	秾: "禾农",
	秿: "禾甫",
	稀: "禾希",
	稂: "禾良",
	稃: "禾孚",
	稄: "禾夋",
	稅: "禾兌",
	稆: "禾吕",
	稇: "禾困",
	稈: "禾旱",
	稉: "禾更",
	稊: "禾弟",
	程: "禾呈",
	稌: "禾余",
	稍: "禾肖",
	税: "禾兑",
	稏: "禾亞",
	稐: "禾侖",
	稑: "禾坴",
	稒: "禾固",
	稓: "禾昔",
	稔: "禾念",
	稕: "禾享",
	稖: "禾咅",
	稗: "禾卑",
	稘: "禾其",
	稙: "禾直",
	稚: "禾隹",
	稛: "禾囷",
	稜: "禾夌",
	稝: "禾朋",
	稞: "禾果",
	稠: "禾周",
	稡: "禾卒",
	稢: "禾或",
	稣: "鱼禾",
	稤: "禾京",
	稦: "禾韋",
	稧: "禾契",
	稨: "禾扁",
	稩: "禾胃",
	稪: "禾复",
	稫: "禾畐",
	稬: "禾耎",
	稭: "禾皆",
	種: "禾重",
	稰: "禾胥",
	稱: "禾爯",
	稳: "禾急",
	稴: "禾兼",
	稵: "禾兹",
	稶: "禾彧",
	稷: "禾畟",
	稸: "禾畜",
	稹: "禾真",
	稺: "禾屖",
	稻: "禾舀",
	稼: "禾家",
	稿: "禾高",
	穁: "禾茸",
	穂: "禾恵",
	穃: "禾容",
	穄: "禾祭",
	穅: "禾康",
	穇: "禾參",
	穉: "禾犀",
	穊: "禾既",
	穋: "禾翏",
	穌: "魚禾",
	積: "禾責",
	穐: "禾亀",
	穑: "禾啬",
	穓: "禾異",
	穔: "禾黄",
	穕: "禾集",
	穖: "禾幾",
	穗: "禾惠",
	穘: "禾堯",
	穙: "禾菐",
	穚: "禾喬",
	穛: "禾焦",
	穜: "禾童",
	穝: "禾最",
	穞: "禾鲁",
	穟: "禾遂",
	穠: "禾農",
	穡: "禾嗇",
	穢: "禾歲",
	穤: "禾需",
	穥: "禾與",
	穦: "禾賓",
	穧: "禾齊",
	穨: "秃貴",
	穪: "禾爾",
	穫: "禾蒦",
	穬: "禾廣",
	穭: "禾魯",
	穮: "禾麃",
	穰: "禾襄",
	穱: "禾爵",
	穲: "禾麗",
	穳: "禾贊",
	竌: "立几",
	竍: "立十",
	竏: "立千",
	竐: "立殳",
	竑: "立厷",
	竓: "立毛",
	竔: "立升",
	竕: "立分",
	竗: "立少",
	竘: "立句",
	站: "立占",
	竚: "立宁",
	竛: "立令",
	竝: "立立",
	竡: "立百",
	竢: "立矣",
	竣: "立夋",
	竤: "立宏",
	竦: "立束",
	竧: "立身",
	竨: "立卓",
	竩: "立宜",
	竫: "立争",
	竬: "立禹",
	竭: "立曷",
	竮: "立屏",
	端: "立耑",
	竰: "立厘",
	竱: "立專",
	竲: "立曾",
	竳: "立登",
	竴: "立尊",
	競: "竞竞",
	竸: "竟竟",
	竹: "亇亇",
	籵: "米十",
	籶: "米几",
	籷: "米乇",
	籸: "米卂",
	籹: "米女",
	籺: "米乞",
	籼: "米山",
	籽: "米子",
	籾: "米刃",
	籿: "米寸",
	粀: "米丈",
	粁: "米千",
	粃: "米比",
	粄: "米反",
	粅: "米勿",
	粆: "米少",
	粇: "米亢",
	粈: "米丑",
	粉: "米分",
	粋: "米卆",
	粌: "米引",
	粍: "米毛",
	粎: "米尺",
	粏: "米太",
	粐: "米户",
	粑: "米巴",
	粒: "米立",
	粓: "米甘",
	粔: "米巨",
	粕: "米白",
	粖: "米末",
	粗: "米且",
	粘: "米占",
	粙: "米由",
	粝: "米厉",
	粞: "米西",
	粠: "米共",
	粡: "米同",
	粣: "米册",
	粧: "米庄",
	粨: "米百",
	粩: "米老",
	粫: "米而",
	粬: "米曲",
	粭: "米合",
	粮: "米良",
	粯: "米見",
	粰: "米孚",
	粳: "米更",
	粴: "米里",
	粶: "米录",
	粷: "米匊",
	粸: "米其",
	粹: "米卒",
	粺: "米卑",
	粻: "米長",
	粼: "粦巜",
	粽: "米宗",
	精: "米青",
	粿: "米果",
	糀: "米花",
	糁: "米参",
	糂: "米甚",
	糃: "米昜",
	糄: "米扁",
	糅: "米柔",
	糆: "米面",
	糇: "米侯",
	糈: "米胥",
	糊: "米胡",
	糋: "米前",
	糌: "米昝",
	糍: "米兹",
	糎: "米厘",
	糏: "米屑",
	糐: "米尃",
	糑: "米弱",
	糔: "米蚤",
	糕: "米羔",
	糖: "米唐",
	糗: "米臭",
	糘: "米家",
	糙: "米造",
	糚: "米莊",
	糛: "米堂",
	糝: "米參",
	糟: "米曹",
	糠: "米康",
	糡: "米竟",
	糢: "米莫",
	糣: "米朁",
	糤: "米散",
	糦: "米喜",
	糧: "米量",
	糨: "米强",
	糩: "米會",
	糫: "米睘",
	糬: "米署",
	糮: "米監",
	糯: "米需",
	糰: "米團",
	糲: "米厲",
	糴: "籴翟",
	糶: "粜翟",
	糷: "米蘭",
	糺: "糹乚",
	糼: "糹力",
	糽: "糹丁",
	糾: "糹丩",
	糿: "糹刀",
	紀: "糹己",
	紁: "糹叉",
	紂: "糹寸",
	紃: "糹川",
	約: "糹勺",
	紅: "糹工",
	紆: "糹于",
	紇: "糹乞",
	紈: "糹丸",
	紉: "糹刃",
	紋: "糹文",
	紌: "糹尤",
	納: "糹内",
	紎: "糹犬",
	紏: "糹斗",
	紐: "糹丑",
	紑: "糹不",
	紒: "糹介",
	紓: "糹予",
	純: "糹屯",
	紕: "糹比",
	紖: "糹引",
	紗: "糹少",
	紘: "糹厷",
	紙: "糹氏",
	級: "糹及",
	紛: "糹分",
	紜: "糹云",
	紝: "糹壬",
	紞: "糹冘",
	紟: "糹今",
	紡: "糹方",
	紣: "糹卆",
	紤: "糹斤",
	紦: "糹巴",
	紨: "糹付",
	紩: "糹失",
	紪: "糹此",
	紬: "糹由",
	紭: "糹弘",
	細: "糹田",
	紱: "糹犮",
	紲: "糹世",
	紳: "糹申",
	紴: "糹皮",
	紵: "糹宁",
	紶: "糹去",
	紷: "糹令",
	紸: "糹主",
	紹: "糹召",
	紺: "糹甘",
	紻: "糹央",
	紼: "糹弗",
	紽: "糹它",
	紿: "糹台",
	絀: "糹出",
	終: "糹冬",
	絃: "糹玄",
	組: "糹且",
	絅: "糹冋",
	絆: "糹半",
	絇: "糹句",
	絈: "糹白",
	絉: "糹术",
	絊: "糹本",
	絋: "糹広",
	経: "糹圣",
	絍: "糹任",
	絎: "糹行",
	絏: "糹曳",
	結: "糹吉",
	絑: "糹朱",
	絒: "糹州",
	絓: "糹圭",
	絔: "糹百",
	絖: "糹光",
	絗: "糹回",
	絘: "糹次",
	絙: "糹亘",
	絚: "糹亙",
	絝: "糹夸",
	絞: "糹交",
	絟: "糹全",
	絠: "糹有",
	絡: "糹各",
	絢: "糹旬",
	絣: "糹并",
	絤: "糹西",
	絥: "糹伏",
	給: "糹合",
	絧: "糹同",
	絨: "糹戎",
	絩: "糹兆",
	絪: "糹因",
	絬: "糹舌",
	絯: "糹亥",
	絰: "糹至",
	統: "糹充",
	絲: "糹糸",
	絳: "糹夅",
	絴: "糹羊",
	絵: "糹会",
	絶: "糹色",
	絸: "糹見",
	絹: "糹肙",
	絺: "糹希",
	絻: "糹免",
	絼: "糹豸",
	絽: "糹吕",
	絾: "糹成",
	絿: "糹求",
	綀: "糹束",
	綁: "糹邦",
	綃: "糹肖",
	綄: "糹完",
	綆: "糹更",
	綇: "糹酉",
	綈: "糹弟",
	綉: "糹秀",
	綊: "糹夾",
	綋: "糹宏",
	綌: "糹谷",
	綍: "糹孛",
	綎: "糹廷",
	綏: "糹妥",
	綐: "糹兑",
	綑: "糹困",
	綒: "糹孚",
	經: "糹巠",
	綔: "夸系",
	綕: "糹志",
	綖: "糹延",
	綗: "糹冏",
	綘: "糹夆",
	続: "糹売",
	綛: "糹忍",
	綜: "糹宗",
	綝: "糹林",
	綞: "糹垂",
	綟: "糹戾",
	綠: "糹彔",
	綡: "糹京",
	綢: "糹周",
	綣: "糹卷",
	綥: "糹畀",
	綧: "糹享",
	綨: "糹其",
	綩: "糹宛",
	綪: "糹青",
	綫: "糹戔",
	綬: "糹受",
	維: "糹隹",
	綯: "糹匋",
	綰: "糹官",
	綱: "糹岡",
	網: "糹罔",
	綳: "糹朋",
	綴: "糹叕",
	綵: "糹采",
	綶: "糹果",
	綷: "糹卒",
	綸: "糹侖",
	綹: "糹咎",
	綺: "糹奇",
	綻: "糹定",
	綼: "糹卑",
	綽: "糹卓",
	綾: "糹夌",
	綿: "糹帛",
	緀: "糹妻",
	緁: "糹疌",
	緂: "糹炎",
	緄: "糹昆",
	緅: "糹取",
	緆: "糹易",
	緇: "糹甾",
	緈: "糹幸",
	緉: "糹兩",
	緋: "糹非",
	緌: "糹委",
	緍: "糹昏",
	緎: "糹或",
	総: "糹忩",
	緐: "每系",
	緑: "糹录",
	緒: "糹者",
	緓: "糹英",
	緔: "糹尚",
	緕: "糹斉",
	緖: "糹者",
	緗: "糹相",
	緘: "糹咸",
	緙: "糹革",
	線: "糹泉",
	緛: "糹耎",
	緜: "帛系",
	緝: "糹咠",
	緞: "糹段",
	緟: "糹重",
	締: "糹帝",
	緡: "糹昬",
	緢: "糹苗",
	緣: "糹彖",
	緤: "糹枼",
	緥: "糹保",
	緦: "糹思",
	緧: "糹酋",
	編: "糹扁",
	緩: "糹爰",
	緪: "糹恆",
	緫: "糹怱",
	緬: "糹面",
	緭: "糹胃",
	緮: "糹复",
	緯: "糹韋",
	緰: "糹俞",
	緱: "糹侯",
	緲: "糹眇",
	練: "糹柬",
	緶: "糹便",
	緷: "糹軍",
	緸: "糹垔",
	緹: "糹是",
	緺: "糹咼",
	緻: "糹致",
	緼: "糹昷",
	緽: "糹貞",
	緾: "糹厘",
	緿: "糹怠",
	縀: "糹叚",
	縂: "糹总",
	縃: "糹胥",
	縅: "糹威",
	縆: "糹恒",
	縇: "糹宣",
	縉: "糹晋",
	縊: "糹益",
	縋: "糹追",
	縌: "糹逆",
	縍: "糹旁",
	縎: "糹骨",
	縐: "糹芻",
	縑: "糹兼",
	縒: "糹差",
	縓: "糹原",
	縔: "糹桑",
	縖: "糹害",
	縗: "糹衰",
	縘: "糹奚",
	縙: "糹茸",
	縚: "糹舀",
	縛: "糹尃",
	縜: "糹員",
	縝: "糹真",
	縞: "糹高",
	縟: "糹辱",
	縡: "糹宰",
	縢: "月絭",
	縤: "糹素",
	縥: "糹秦",
	縦: "糹従",
	縧: "糹條",
	縨: "糹晃",
	縩: "糹祭",
	縪: "糹畢",
	縫: "糹逢",
	縬: "糹戚",
	縭: "糹离",
	縮: "糹宿",
	縯: "糹寅",
	縰: "糹徙",
	縱: "糹從",
	縲: "糹累",
	縳: "糹專",
	縴: "糹牽",
	縵: "糹曼",
	縷: "糹婁",
	縸: "糹莫",
	縹: "糹票",
	縺: "糹連",
	縼: "糹旋",
	總: "糹悤",
	績: "糹責",
	縿: "糹參",
	繀: "糹崔",
	繂: "糹率",
	繃: "糹崩",
	繅: "糹巢",
	繆: "糹翏",
	繈: "糹強",
	繍: "糹粛",
	繎: "糹然",
	繏: "糹巽",
	繐: "糹惠",
	繑: "糹喬",
	繒: "糹曾",
	繓: "糹最",
	織: "糹戠",
	繕: "糹善",
	繖: "糹散",
	繗: "糹粦",
	繘: "糹矞",
	繙: "糹番",
	繚: "糹尞",
	繛: "素卓",
	繜: "糹尊",
	繝: "糹閒",
	繞: "糹堯",
	繟: "糹單",
	繡: "糹肅",
	繢: "糹貴",
	繣: "糹畫",
	繥: "糹喜",
	繦: "糹强",
	繧: "糹雲",
	繨: "糹達",
	繩: "糹黽",
	繪: "糹會",
	繬: "糹嗇",
	繮: "糹畺",
	繯: "糹睘",
	繰: "糹喿",
	繱: "糹葱",
	繲: "糹解",
	繳: "糹敫",
	繵: "糹亶",
	繶: "糹意",
	繷: "糹農",
	繸: "糹遂",
	繹: "糹睪",
	繺: "糹煞",
	繻: "糹需",
	繽: "糹賓",
	繾: "糹遣",
	繿: "糹監",
	纀: "糹僕",
	纁: "糹熏",
	纃: "糹齊",
	纄: "糹蓬",
	纅: "糹樂",
	纆: "糹墨",
	纈: "糹頡",
	纉: "糹賛",
	纊: "糹廣",
	纋: "糹憂",
	續: "糹賣",
	纎: "糹韯",
	纏: "糹廛",
	纐: "糹頝",
	纑: "糹盧",
	纓: "糹嬰",
	纔: "糹毚",
	纕: "糹襄",
	纖: "糹韱",
	纗: "糹巂",
	纘: "糹贊",
	纙: "糹羅",
	纚: "糹麗",
	纜: "糹覽",
	纝: "糹纍",
	纞: "糹戀",
	纠: "纟丩",
	纡: "纟于",
	红: "纟工",
	纣: "纟寸",
	纤: "纟千",
	纥: "纟乞",
	约: "纟勺",
	级: "纟及",
	纨: "纟丸",
	纩: "纟广",
	纪: "纟己",
	纫: "纟刃",
	纬: "纟韦",
	纭: "纟云",
	纮: "纟厷",
	纯: "纟屯",
	纰: "纟比",
	纱: "纟少",
	纲: "纟冈",
	纳: "纟内",
	纴: "纟壬",
	纵: "纟从",
	纶: "纟仑",
	纷: "纟分",
	纸: "纟氏",
	纹: "纟文",
	纺: "纟方",
	纼: "纟引",
	纽: "纟丑",
	纾: "纟予",
	线: "纟戋",
	绀: "纟甘",
	绁: "纟世",
	绂: "纟犮",
	组: "纟且",
	绅: "纟申",
	细: "纟田",
	织: "纟只",
	终: "纟冬",
	绉: "纟刍",
	绊: "纟半",
	绋: "纟弗",
	绌: "纟出",
	绍: "纟召",
	绐: "纟台",
	绑: "纟邦",
	绒: "纟戎",
	结: "纟吉",
	绔: "纟夸",
	绕: "纟尧",
	绖: "纟至",
	绗: "纟行",
	绘: "纟会",
	给: "纟合",
	绚: "纟旬",
	绛: "纟夅",
	络: "纟各",
	绝: "纟色",
	绞: "纟交",
	统: "纟充",
	绠: "纟更",
	绡: "纟肖",
	绢: "纟肙",
	绣: "纟秀",
	绤: "纟谷",
	绥: "纟妥",
	绦: "纟条",
	绨: "纟弟",
	绩: "纟责",
	绪: "纟者",
	绫: "纟夌",
	绬: "纟英",
	续: "纟卖",
	绮: "纟奇",
	绯: "纟非",
	绰: "纟卓",
	绱: "纟尚",
	绲: "纟昆",
	绳: "纟黾",
	维: "纟隹",
	绵: "纟帛",
	绶: "纟受",
	绷: "纟朋",
	绸: "纟周",
	绹: "纟匋",
	绺: "纟咎",
	绻: "纟卷",
	综: "纟宗",
	绽: "纟定",
	绾: "纟官",
	绿: "纟录",
	缀: "纟叕",
	缁: "纟甾",
	缂: "纟革",
	缃: "纟相",
	缄: "纟咸",
	缅: "纟面",
	缆: "纟览",
	缇: "纟是",
	缈: "纟眇",
	缉: "纟咠",
	缊: "纟昷",
	缋: "纟贵",
	缌: "纟思",
	缍: "纟垂",
	缎: "纟段",
	缏: "纟便",
	缐: "纟泉",
	缑: "纟侯",
	缒: "纟追",
	缓: "纟爰",
	缔: "纟帝",
	缕: "纟娄",
	编: "纟扁",
	缗: "纟昬",
	缘: "纟彖",
	缙: "纟晋",
	缚: "纟尃",
	缛: "纟辱",
	缜: "纟真",
	缝: "纟逢",
	缞: "纟衰",
	缟: "纟高",
	缡: "纟离",
	缢: "纟益",
	缣: "纟兼",
	缤: "纟宾",
	缥: "纟票",
	缦: "纟曼",
	缧: "纟累",
	缨: "纟婴",
	缩: "纟宿",
	缪: "纟翏",
	缫: "纟巢",
	缬: "纟颉",
	缭: "纟尞",
	缮: "纟善",
	缯: "纟曾",
	缰: "纟畺",
	缱: "纟遣",
	缲: "纟喿",
	缳: "纟睘",
	缴: "纟敫",
	缵: "纟赞",
	缷: "缶卩",
	缸: "缶工",
	缺: "缶夬",
	缻: "缶瓦",
	缼: "缶欠",
	缽: "缶本",
	缾: "缶并",
	缿: "缶后",
	罀: "缶兆",
	罁: "缶岡",
	罅: "缶虖",
	罆: "缶貫",
	罇: "缶尊",
	罈: "缶覃",
	罉: "缶掌",
	罎: "缶曇",
	罏: "缶盧",
	罐: "缶雚",
	羏: "羊彡",
	羒: "羊分",
	羓: "羊巴",
	羖: "羊殳",
	羘: "月羊",
	羚: "羊令",
	羜: "羊宁",
	羝: "羊氐",
	羠: "羊夷",
	羢: "羊戎",
	群: "君羊",
	羥: "羊巠",
	羦: "羊完",
	羧: "羊夋",
	羪: "羊良",
	羫: "羊空",
	羬: "羊咸",
	羭: "羊俞",
	羯: "羊曷",
	羰: "羊炭",
	羱: "羊原",
	羳: "羊番",
	羵: "羊賁",
	羶: "羊亶",
	羷: "羊僉",
	羺: "羊需",
	羻: "羊慶",
	羽: "习习",
	羾: "羽工",
	翀: "羽中",
	翂: "羽分",
	翃: "厷羽",
	翄: "羽支",
	翈: "甲羽",
	翉: "本羽",
	翊: "立羽",
	翋: "羽立",
	翍: "羽皮",
	翎: "令羽",
	翐: "羽失",
	翑: "羽句",
	翓: "吉羽",
	翔: "羊羽",
	翖: "合羽",
	翗: "多羽",
	翙: "岁羽",
	翝: "宏羽",
	翞: "羽京",
	翢: "周羽",
	翧: "宣羽",
	翩: "扁羽",
	翫: "習元",
	翭: "羽侯",
	翮: "鬲羽",
	翱: "皋羽",
	翲: "票羽",
	翴: "羽連",
	翵: "羽鳥",
	翶: "皐羽",
	翷: "粦羽",
	翸: "賁羽",
	翺: "臯羽",
	翻: "番羽",
	翽: "歲羽",
	翾: "睘羽",
	翿: "壽羽",
	耀: "光翟",
	耏: "而彡",
	耐: "而寸",
	耓: "耒丁",
	耔: "耒子",
	耕: "耒井",
	耖: "耒少",
	耗: "耒毛",
	耘: "耒云",
	耙: "耒巴",
	耚: "耒皮",
	耛: "耒台",
	耝: "耒且",
	耞: "耒加",
	耟: "耒巨",
	耠: "耒合",
	耡: "耒助",
	耢: "耒劳",
	耣: "耒侖",
	耤: "耒昔",
	耥: "耒尚",
	耦: "耒禺",
	耧: "耒娄",
	耨: "耒辱",
	耩: "耒冓",
	耪: "耒旁",
	耫: "耒責",
	耬: "耒婁",
	耭: "耒幾",
	耮: "耒勞",
	耯: "耒蒦",
	耰: "耒憂",
	耱: "耒磨",
	耲: "耒褱",
	耴: "耳乚",
	耵: "耳丁",
	耶: "耳阝",
	耹: "耳今",
	耺: "耳云",
	耻: "耳止",
	耼: "耳冄",
	耽: "耳冘",
	耾: "耳厷",
	耿: "耳火",
	聀: "耳戈",
	聁: "耳分",
	聃: "耳冉",
	聅: "耳矢",
	聆: "耳令",
	聇: "耳正",
	聈: "耳幼",
	聉: "耳出",
	聊: "耳卯",
	职: "耳只",
	聍: "耳宁",
	聎: "耳兆",
	聏: "耳而",
	聐: "耳吉",
	聑: "耳耳",
	聒: "耳舌",
	联: "耳关",
	聕: "耳告",
	聗: "耳夾",
	聘: "耳甹",
	聙: "耳青",
	聛: "耳卑",
	聜: "耳空",
	聝: "耳或",
	聠: "耳并",
	聡: "耳忩",
	聢: "耳定",
	聣: "耳兒",
	聤: "耳亭",
	聥: "耳禹",
	聦: "耳怱",
	聧: "耳癸",
	聩: "耳贵",
	聪: "耳总",
	聬: "耳翁",
	聭: "耳鬼",
	聰: "耳悤",
	聵: "耳貴",
	職: "耳戠",
	聸: "耳詹",
	聹: "耳寧",
	聺: "耳察",
	肂: "歹聿",
	肆: "镸聿",
	肊: "月乙",
	肋: "月力",
	肌: "月几",
	肍: "月九",
	肐: "月乞",
	肑: "月勺",
	肒: "月丸",
	肔: "月也",
	肕: "月刃",
	肗: "月女",
	肘: "月寸",
	肚: "月土",
	肛: "月工",
	肜: "月彡",
	肝: "月干",
	肞: "月义",
	肟: "月亏",
	股: "月殳",
	肢: "月支",
	肣: "月今",
	肤: "月夫",
	肥: "月巴",
	肧: "月不",
	肨: "月丰",
	肪: "月方",
	肫: "月屯",
	肬: "月尤",
	肭: "月内",
	肮: "月亢",
	肰: "月犬",
	肱: "月厷",
	肳: "月勿",
	肵: "月斤",
	肶: "月比",
	肷: "月欠",
	肹: "月兮",
	肺: "月巿",
	肼: "月井",
	肽: "月太",
	肿: "月中",
	胀: "月长",
	胁: "月办",
	胂: "月申",
	胅: "月失",
	胆: "月旦",
	胇: "月弗",
	胈: "月犮",
	胉: "月白",
	胊: "月句",
	胋: "月占",
	胍: "月瓜",
	胎: "月台",
	胐: "月出",
	胑: "月只",
	胒: "月尼",
	胓: "月平",
	胕: "月付",
	胖: "月半",
	胘: "月玄",
	胙: "月乍",
	胚: "月丕",
	胛: "月甲",
	胜: "月生",
	胝: "月氐",
	胞: "月包",
	胟: "月母",
	胠: "月去",
	胡: "古月",
	胢: "月可",
	胦: "月央",
	胧: "月龙",
	胨: "月东",
	胩: "月卡",
	胪: "月卢",
	胭: "月因",
	胮: "月夅",
	胯: "月夸",
	胰: "月夷",
	胱: "月光",
	胲: "月亥",
	胳: "月各",
	胴: "月同",
	胵: "月至",
	胶: "月交",
	胸: "月匈",
	胹: "月而",
	胺: "月安",
	胻: "月行",
	胼: "月并",
	胿: "月圭",
	脁: "月兆",
	脂: "月旨",
	脃: "月色",
	脄: "月灰",
	脆: "月危",
	脇: "月劦",
	脉: "月永",
	脌: "月年",
	脍: "月会",
	脎: "月杀",
	脏: "月庄",
	脐: "月齐",
	脒: "月米",
	脓: "月农",
	脕: "月免",
	脖: "月孛",
	脘: "月完",
	脙: "月求",
	脚: "月却",
	脛: "月巠",
	脝: "月亨",
	脞: "月坐",
	脟: "月寽",
	脠: "月延",
	脡: "月廷",
	脢: "月每",
	脤: "月辰",
	脥: "月夾",
	脦: "月忒",
	脧: "月夋",
	脨: "月束",
	脪: "月希",
	脫: "月兌",
	脬: "月孚",
	脭: "月呈",
	脮: "月妥",
	脯: "月甫",
	脰: "月豆",
	脱: "月兑",
	脲: "月尿",
	脴: "月否",
	脵: "月吴",
	脶: "月呙",
	脷: "月利",
	脸: "月佥",
	脹: "月長",
	脺: "月卒",
	脻: "月疌",
	脼: "月兩",
	脽: "月隹",
	脾: "月卑",
	脿: "月表",
	腀: "月侖",
	腁: "月幷",
	腂: "月果",
	腃: "月卷",
	腄: "月垂",
	腅: "月炎",
	腆: "月典",
	腇: "月委",
	腈: "月青",
	腉: "月兒",
	腊: "月昔",
	腋: "月夜",
	腌: "月奄",
	腍: "月念",
	腏: "月叕",
	腑: "月府",
	腒: "月居",
	腓: "月非",
	腔: "月空",
	腕: "月宛",
	腖: "月東",
	腗: "月畁",
	腘: "月国",
	腙: "月宗",
	腚: "月定",
	腛: "月屋",
	腜: "月某",
	腝: "月耎",
	腞: "月彖",
	腟: "月室",
	腠: "月奏",
	腡: "月咼",
	腢: "月禺",
	腣: "月帝",
	腤: "月音",
	腥: "月星",
	腧: "月俞",
	腨: "月耑",
	腩: "月南",
	腪: "月軍",
	腫: "月重",
	腬: "月柔",
	腭: "月咢",
	腮: "月思",
	腯: "月盾",
	腰: "月要",
	腱: "月建",
	腲: "月畏",
	腳: "月卻",
	腴: "月臾",
	腵: "月叚",
	腶: "月段",
	腷: "月畐",
	腸: "月昜",
	腹: "月复",
	腺: "月泉",
	腻: "月贰",
	腼: "月面",
	腽: "月昷",
	腿: "月退",
	膀: "月旁",
	膁: "月兼",
	膄: "月叟",
	膅: "月唐",
	膆: "月素",
	膇: "月追",
	膈: "月鬲",
	膉: "月益",
	膊: "月尃",
	膌: "月脊",
	膎: "月奚",
	膑: "月宾",
	膒: "月區",
	膔: "月鹿",
	膕: "月國",
	膖: "月逢",
	膗: "月崔",
	膘: "月票",
	膙: "月强",
	膛: "月堂",
	膜: "月莫",
	膝: "月桼",
	膞: "月專",
	膟: "月率",
	膠: "月翏",
	膡: "月眷",
	膢: "月婁",
	膣: "月窒",
	膤: "月雪",
	膦: "月粦",
	膧: "月童",
	膨: "月彭",
	膩: "月貳",
	膪: "月啻",
	膫: "月尞",
	膬: "月毳",
	膭: "月貴",
	膮: "月堯",
	膯: "月登",
	膰: "月番",
	膱: "月戠",
	膲: "月焦",
	膳: "月善",
	膴: "月無",
	膵: "月萃",
	膶: "月閏",
	膷: "月鄉",
	膸: "月遀",
	膹: "月賁",
	膻: "月亶",
	膼: "月過",
	膽: "月詹",
	膾: "月會",
	膿: "月農",
	臁: "月廉",
	臃: "月雍",
	臄: "月豦",
	臅: "月蜀",
	臆: "月意",
	臇: "月雋",
	臈: "月葛",
	臉: "月僉",
	臊: "月喿",
	臌: "月鼓",
	臍: "月齊",
	臎: "月翠",
	臏: "月賓",
	臐: "月熏",
	臑: "月需",
	臒: "月蒦",
	臓: "月蔵",
	臔: "月賢",
	臕: "月麃",
	臗: "月寬",
	臘: "月巤",
	臙: "月燕",
	臚: "月盧",
	臛: "月霍",
	臜: "月赞",
	臞: "月瞿",
	臟: "月藏",
	臢: "月贊",
	臤: "臣又",
	臥: "臣人",
	臫: "自乚",
	臲: "臬危",
	致: "至攵",
	臵: "至各",
	臶: "至存",
	臸: "至至",
	臹: "至成",
	臻: "至秦",
	舏: "舌丩",
	舐: "舌氏",
	舑: "舌冉",
	舒: "舍予",
	舓: "舌易",
	舔: "舌忝",
	舕: "舌炎",
	舖: "舍甫",
	舗: "舎甫",
	舘: "舍官",
	舚: "舌詹",
	舠: "舟刀",
	舡: "舟工",
	舢: "舟山",
	舣: "舟义",
	舤: "舟凡",
	舥: "舟巴",
	舦: "舟太",
	舨: "舟反",
	舩: "舟公",
	航: "舟亢",
	舫: "舟方",
	般: "舟殳",
	舭: "舟比",
	舮: "舟户",
	舯: "舟中",
	舰: "舟见",
	舱: "舟仓",
	舲: "舟令",
	舳: "舟由",
	舴: "舟乍",
	舵: "舟它",
	舶: "舟白",
	舷: "舟玄",
	舸: "舟可",
	舺: "舟甲",
	舻: "舟卢",
	舼: "舟共",
	舽: "舟夅",
	舾: "舟西",
	舿: "舟夸",
	艀: "舟孚",
	艁: "舟告",
	艂: "舟夆",
	艃: "舟里",
	艄: "舟肖",
	艅: "舟余",
	艆: "舟良",
	艇: "舟廷",
	艉: "舟尾",
	艊: "舟帛",
	艋: "舟孟",
	艌: "舟念",
	艍: "舟居",
	艎: "舟皇",
	艏: "舟首",
	艑: "舟扁",
	艒: "舟冒",
	艓: "舟枼",
	艔: "舟度",
	艕: "舟旁",
	艖: "舟差",
	艗: "舟益",
	艘: "舟叟",
	艙: "舟倉",
	艚: "舟曹",
	艛: "舟婁",
	艜: "舟帶",
	艝: "舟雪",
	艞: "舟筄",
	艟: "舟童",
	艠: "舟登",
	艡: "舟當",
	艢: "舟嗇",
	艣: "舟虜",
	艤: "舟義",
	艥: "舟戢",
	艦: "舟監",
	艧: "舟蒦",
	艨: "舟蒙",
	艩: "舟齊",
	艪: "舟魯",
	艫: "舟盧",
	艬: "舟毚",
	艭: "舟雙",
	艰: "又艮",
	艳: "丰色",
	艴: "弗色",
	艵: "并色",
	艶: "豊色",
	艷: "豐色",
	艸: "屮屮",
	荆: "茾刂",
	蒯: "萠刂",
	虓: "九虎",
	號: "号虎",
	虠: "交虎",
	虢: "寽虎",
	虣: "武虎",
	虤: "虎虎",
	虥: "虎戔",
	虦: "戔虎",
	虧: "雐亏",
	虪: "虎儵",
	虬: "虫乚",
	虭: "虫刀",
	虮: "虫几",
	虯: "虫丩",
	虰: "虫丁",
	虲: "虫卜",
	虳: "虫勺",
	虴: "虫乇",
	虵: "虫也",
	虶: "虫于",
	虷: "虫干",
	虸: "虫子",
	虹: "虫工",
	虻: "虫亡",
	虼: "虫乞",
	虾: "虫下",
	蚀: "饣虫",
	蚁: "虫义",
	蚂: "虫马",
	蚄: "虫方",
	蚅: "虫厄",
	蚆: "虫巴",
	蚇: "虫尺",
	蚈: "虫开",
	蚊: "虫文",
	蚋: "虫内",
	蚌: "虫丰",
	蚍: "虫比",
	蚎: "虫日",
	蚏: "虫月",
	蚐: "虫匀",
	蚑: "虫支",
	蚒: "虫丹",
	蚓: "虫引",
	蚔: "虫氏",
	蚖: "虫元",
	蚗: "虫夬",
	蚘: "虫尤",
	蚙: "虫今",
	蚚: "虫斤",
	蚛: "虫中",
	蚜: "虫牙",
	蚝: "虫毛",
	蚞: "虫木",
	蚟: "虫王",
	蚡: "虫分",
	蚢: "虫亢",
	蚣: "虫公",
	蚥: "虫父",
	蚦: "虫冄",
	蚧: "虫介",
	蚨: "虫夫",
	蚪: "虫斗",
	蚫: "虫包",
	蚬: "虫见",
	蚭: "虫尼",
	蚮: "虫代",
	蚯: "虫丘",
	蚰: "虫由",
	蚱: "虫乍",
	蚲: "虫平",
	蚳: "虫氐",
	蚴: "虫幼",
	蚵: "虫可",
	蚶: "虫甘",
	蚷: "虫巨",
	蚸: "虫斥",
	蚹: "虫付",
	蚺: "虫冉",
	蚼: "虫句",
	蚽: "虫丕",
	蚾: "虫皮",
	蚿: "虫玄",
	蛀: "虫主",
	蛁: "虫召",
	蛂: "虫犮",
	蛃: "虫丙",
	蛄: "虫古",
	蛅: "虫占",
	蛆: "虫且",
	蛇: "虫它",
	蛈: "虫失",
	蛉: "虫令",
	蛌: "虫瓜",
	蛎: "虫厉",
	蛏: "虫圣",
	蛐: "虫曲",
	蛑: "虫牟",
	蛒: "虫各",
	蛔: "虫回",
	蛕: "虫有",
	蛖: "虫尨",
	蛘: "虫羊",
	蛙: "虫圭",
	蛚: "虫列",
	蛛: "虫朱",
	蛜: "虫伊",
	蛝: "虫艮",
	蛞: "虫舌",
	蛟: "虫交",
	蛠: "虫劦",
	蛡: "虫羽",
	蛢: "虫并",
	蛣: "虫吉",
	蛤: "虫合",
	蛥: "虫多",
	蛦: "虫夷",
	蛧: "虫网",
	蛨: "虫百",
	蛫: "虫危",
	蛭: "虫至",
	蛯: "虫老",
	蛱: "虫夹",
	蛲: "虫尧",
	蛳: "虫师",
	蛴: "虫齐",
	蛵: "虫巠",
	蛶: "虫寽",
	蛷: "虫求",
	蛸: "虫肖",
	蛹: "虫甬",
	蛺: "虫夾",
	蛻: "虫兌",
	蛼: "虫車",
	蛽: "虫貝",
	蛾: "虫我",
	蛿: "虫含",
	蜂: "虫夆",
	蜄: "虫辰",
	蜅: "虫甫",
	蜆: "虫見",
	蜈: "虫吴",
	蜉: "虫孚",
	蜊: "虫利",
	蜋: "虫良",
	蜌: "虫坒",
	蜍: "虫余",
	蜎: "虫肙",
	蜏: "虫秀",
	蜐: "虫劫",
	蜒: "虫延",
	蜓: "虫廷",
	蜔: "虫甸",
	蜕: "虫兑",
	蜖: "虫囬",
	蜗: "虫呙",
	蜘: "虫知",
	蜙: "虫松",
	蜛: "虫居",
	蜞: "虫其",
	蜟: "虫育",
	蜠: "虫囷",
	蜡: "虫昔",
	蜢: "虫孟",
	蜣: "虫羌",
	蜥: "虫析",
	蜦: "虫侖",
	蜧: "虫戾",
	蜨: "虫疌",
	蜩: "虫周",
	蜪: "虫匋",
	蜫: "虫昆",
	蜬: "虫函",
	蜭: "虫臽",
	蜮: "虫或",
	蜯: "虫奉",
	蜱: "虫卑",
	蜲: "虫委",
	蜳: "虫享",
	蜴: "虫易",
	蜶: "虫卒",
	蜷: "虫卷",
	蜹: "虫芮",
	蜺: "虫兒",
	蜻: "虫青",
	蜼: "虫隹",
	蜽: "虫兩",
	蜾: "虫果",
	蜿: "虫宛",
	蝀: "虫東",
	蝂: "虫版",
	蝃: "虫叕",
	蝄: "虫罔",
	蝆: "虫芈",
	蝇: "虫黾",
	蝈: "虫国",
	蝉: "虫单",
	蝊: "虫定",
	蝋: "虫鼡",
	蝌: "虫科",
	蝍: "虫即",
	蝎: "虫曷",
	蝏: "虫亭",
	蝐: "虫冒",
	蝑: "虫胥",
	蝒: "虫面",
	蝓: "虫俞",
	蝔: "虫皆",
	蝕: "飠虫",
	蝖: "虫宣",
	蝗: "虫皇",
	蝘: "虫匽",
	蝙: "虫扁",
	蝚: "虫柔",
	蝛: "虫威",
	蝜: "虫負",
	蝝: "虫彖",
	蝞: "虫眉",
	蝟: "虫胃",
	蝠: "虫畐",
	蝡: "虫耎",
	蝢: "虫頁",
	蝣: "虫斿",
	蝤: "虫酋",
	蝦: "虫叚",
	蝧: "虫英",
	蝩: "虫重",
	蝪: "虫昜",
	蝫: "虫者",
	蝭: "虫是",
	蝮: "虫复",
	蝯: "虫爰",
	蝰: "虫奎",
	蝲: "虫剌",
	蝳: "虫毒",
	蝴: "虫胡",
	蝶: "虫枼",
	蝸: "虫咼",
	蝹: "虫昷",
	蝺: "虫禹",
	蝻: "虫南",
	蝼: "虫娄",
	蝽: "虫春",
	蝾: "虫荣",
	螁: "虫退",
	螂: "虫郎",
	螃: "虫旁",
	螄: "虫師",
	螅: "虫息",
	螆: "虫兹",
	螇: "虫奚",
	螈: "虫原",
	螉: "虫翁",
	螊: "虫兼",
	螋: "虫叟",
	融: "鬲虫",
	螎: "虫鬲",
	螏: "虫疾",
	螐: "虫烏",
	螑: "虫臭",
	螓: "虫秦",
	螔: "虫虒",
	螖: "虫骨",
	螗: "虫唐",
	螘: "虫豈",
	螛: "虫害",
	螝: "虫鬼",
	螞: "虫馬",
	螟: "虫冥",
	螠: "虫益",
	螥: "虫倉",
	螦: "虫素",
	螧: "虫耆",
	螩: "虫條",
	螪: "虫商",
	螬: "虫曹",
	螭: "虫离",
	螮: "虫帶",
	螰: "虫鹿",
	螲: "虫窒",
	螳: "虫堂",
	螵: "虫票",
	螹: "虫斬",
	螺: "虫累",
	螻: "虫婁",
	螼: "虫堇",
	螾: "虫寅",
	蟀: "虫率",
	蟂: "虫梟",
	蟃: "虫曼",
	蟅: "虫庶",
	蟆: "虫莫",
	蟈: "虫國",
	蟉: "虫翏",
	蟋: "虫悉",
	蟌: "虫悤",
	蟍: "虫梨",
	蟏: "虫萧",
	蟐: "虫常",
	蟑: "虫章",
	蟒: "虫莽",
	蟓: "虫象",
	蟔: "虫黑",
	蟕: "虫觜",
	蟖: "虫斯",
	蟘: "虫貸",
	蟙: "虫戠",
	蟛: "虫彭",
	蟜: "虫喬",
	蟝: "虫渠",
	蟟: "虫尞",
	蟠: "虫番",
	蟡: "虫爲",
	蟢: "虫喜",
	蟣: "虫幾",
	蟤: "虫巽",
	蟥: "虫黄",
	蟦: "虫賁",
	蟧: "虫勞",
	蟩: "虫厥",
	蟪: "虫惠",
	蟫: "虫覃",
	蟬: "虫單",
	蟭: "虫焦",
	蟮: "虫善",
	蟯: "虫堯",
	蟰: "虫肅",
	蟱: "虫無",
	蟳: "虫尋",
	蟵: "虫厨",
	蟶: "虫聖",
	蟷: "虫當",
	蟺: "虫亶",
	蟻: "虫義",
	蟽: "虫達",
	蟾: "虫詹",
	蠀: "虫資",
	蠂: "虫葉",
	蠄: "虫禽",
	蠅: "虫黽",
	蠇: "虫萬",
	蠉: "虫睘",
	蠊: "虫廉",
	蠋: "虫蜀",
	蠌: "虫睪",
	蠍: "虫歇",
	蠎: "虫莾",
	蠏: "虫解",
	蠐: "虫齊",
	蠑: "虫榮",
	蠓: "虫蒙",
	蠔: "虫豪",
	蠕: "虫需",
	蠖: "虫蒦",
	蠗: "虫翟",
	蠘: "虫截",
	蠙: "虫賓",
	蠛: "虫蔑",
	蠝: "虫畾",
	蠟: "虫巤",
	蠣: "虫厲",
	蠦: "虫盧",
	蠨: "虫蕭",
	蠩: "虫諸",
	蠬: "虫龍",
	蠮: "虫翳",
	蠰: "虫襄",
	蠲: "益蜀",
	蠳: "虫嬰",
	蠴: "虫薯",
	蠵: "虫巂",
	蠷: "虫瞿",
	蠸: "虫雚",
	蠼: "虫矍",
	蠾: "虫屬",
	衂: "血刃",
	衃: "血不",
	衄: "血丑",
	衅: "血半",
	衈: "血耳",
	衉: "血各",
	衊: "血蔑",
	行: "彳亍",
	补: "衤卜",
	衦: "衤干",
	衧: "衤于",
	衩: "衤叉",
	衪: "衤也",
	衫: "衤彡",
	衬: "衤寸",
	衭: "衤夫",
	衯: "衤分",
	衱: "衤及",
	衲: "衤内",
	衳: "衤公",
	衴: "衤冘",
	衵: "衤日",
	衶: "衤中",
	衸: "衤介",
	衹: "衤氏",
	衻: "衤冄",
	衼: "衤支",
	衽: "衤壬",
	衿: "衤今",
	袀: "衤匀",
	袂: "衤夬",
	袄: "衤夭",
	袆: "衤韦",
	袇: "衤丹",
	袉: "衤它",
	袊: "衤令",
	袍: "衤包",
	袎: "衤幼",
	袏: "衤左",
	袐: "衤必",
	袑: "衤召",
	袒: "衤旦",
	袓: "衤且",
	袔: "衤可",
	袕: "衤穴",
	袖: "衤由",
	袙: "衤白",
	袚: "衤犮",
	袛: "衤氐",
	袜: "衤末",
	袝: "衤付",
	袟: "衤失",
	袡: "衤冉",
	袢: "衤半",
	袣: "衤世",
	袥: "衤石",
	袦: "衤出",
	袧: "衤句",
	袨: "衤玄",
	袩: "衤占",
	袪: "衤去",
	被: "衤皮",
	袮: "衤尓",
	袯: "衤发",
	袱: "衤伏",
	袳: "衤多",
	袴: "衤夸",
	袵: "衤任",
	袶: "衤夅",
	袷: "衤合",
	袸: "衤存",
	袹: "衤百",
	袺: "衤吉",
	袻: "衤而",
	袼: "衤各",
	袽: "衤如",
	袾: "衤朱",
	袿: "衤圭",
	裀: "衤因",
	裄: "衤行",
	裆: "衤当",
	裇: "衤血",
	裈: "衤军",
	裉: "衤艮",
	裋: "衤豆",
	裌: "衤夾",
	裍: "衤困",
	裎: "衤呈",
	裐: "衤肙",
	裑: "衤身",
	裓: "衤戒",
	裕: "衤谷",
	裖: "衤辰",
	裙: "衤君",
	補: "衤甫",
	裞: "衤兑",
	裡: "衤里",
	裢: "衤连",
	裣: "衤佥",
	裤: "衤库",
	裥: "衤间",
	裧: "衤炎",
	裨: "衤卑",
	裩: "衤昆",
	裪: "衤匋",
	裬: "衤夌",
	裭: "衤虎",
	裮: "衤昌",
	裯: "衤周",
	裰: "衤叕",
	裱: "衤表",
	裲: "衤兩",
	裶: "衤非",
	裷: "衤卷",
	裸: "衤果",
	裺: "衤奄",
	裼: "衤易",
	裾: "衤居",
	裿: "衤奇",
	褀: "衤其",
	褂: "衤卦",
	褃: "衤肯",
	褄: "衤妻",
	褅: "衤帝",
	褆: "衤是",
	複: "衤复",
	褈: "衤重",
	褉: "衤契",
	褊: "衤扁",
	褋: "衤枼",
	褌: "衤軍",
	褍: "衤耑",
	褐: "衤曷",
	褑: "衤爰",
	褓: "衤保",
	褔: "衤畐",
	褕: "衤俞",
	褖: "衤彖",
	褗: "衤匽",
	褘: "衤韋",
	褙: "衤背",
	褚: "衤者",
	褛: "衤娄",
	褝: "衤单",
	褞: "衤昷",
	褠: "衤冓",
	褡: "衤荅",
	褣: "衤容",
	褤: "衤袁",
	褥: "衤辱",
	褦: "衤能",
	褨: "衤差",
	褪: "衤退",
	褫: "衤虒",
	褬: "衤桑",
	褯: "衤席",
	褲: "衤庫",
	褳: "衤連",
	褴: "衤监",
	褵: "衤离",
	褶: "衤習",
	褷: "衤徙",
	褸: "衤婁",
	褹: "衤埶",
	褾: "衤票",
	褿: "衤曹",
	襀: "衤責",
	襁: "衤强",
	襂: "衤參",
	襅: "衤畢",
	襆: "衤菐",
	襇: "衤間",
	襈: "衤巽",
	襉: "衤閒",
	襊: "衤最",
	襋: "衤棘",
	襌: "衤單",
	襍: "衤集",
	襎: "衤番",
	襏: "衤發",
	襐: "衤象",
	襑: "衤尋",
	襒: "衤敝",
	襓: "衤堯",
	襕: "衤阑",
	襖: "衤奥",
	襗: "衤睪",
	襘: "衤會",
	襙: "衤喿",
	襚: "衤遂",
	襛: "衤農",
	襜: "衤詹",
	襝: "衤僉",
	襟: "衤禁",
	襠: "衤當",
	襡: "衤蜀",
	襢: "衤亶",
	襣: "衤鼻",
	襤: "衤監",
	襥: "衤僕",
	襦: "衤需",
	襧: "衤爾",
	襨: "衤對",
	襩: "衤賣",
	襪: "衤蔑",
	襫: "衤奭",
	襬: "衤罷",
	襭: "衤頡",
	襮: "衤暴",
	襯: "衤親",
	襰: "衤賴",
	襱: "衤龍",
	襳: "衤韱",
	襴: "衤闌",
	襵: "衤聶",
	襶: "衤戴",
	襷: "衤擧",
	襸: "衤贊",
	襹: "衤麗",
	襺: "衤繭",
	襻: "衤攀",
	襼: "衤藝",
	襽: "衤蘭",
	覅: "要勿",
	覌: "又見",
	規: "夫見",
	覑: "片見",
	覒: "毛見",
	覕: "必見",
	視: "礻見",
	覗: "司見",
	覘: "占見",
	覙: "尓見",
	覜: "兆見",
	覞: "見見",
	覟: "志見",
	覠: "君見",
	覡: "巫見",
	覢: "炎見",
	覣: "委見",
	覤: "虎見",
	覥: "典見",
	覦: "俞見",
	覨: "咢見",
	覩: "者見",
	親: "亲見",
	覫: "旁見",
	覬: "豈見",
	覭: "冥見",
	覯: "冓見",
	覰: "虘見",
	覲: "堇見",
	覴: "登見",
	覵: "閒見",
	覷: "虚見",
	覸: "間見",
	覹: "見微",
	覼: "爾見",
	覾: "審見",
	覿: "賣見",
	觀: "雚見",
	观: "又见",
	规: "夫见",
	视: "礻见",
	觇: "占见",
	觊: "岂见",
	觋: "巫见",
	觌: "卖见",
	觍: "典见",
	觎: "俞见",
	觏: "冓见",
	觐: "堇见",
	觑: "虚见",
	觓: "角丩",
	觔: "角力",
	觕: "牛角",
	觖: "角夬",
	觗: "角氏",
	觘: "角少",
	觙: "角及",
	觚: "角瓜",
	觛: "角旦",
	觝: "角氐",
	觟: "角圭",
	觡: "角各",
	觤: "角危",
	觥: "角光",
	触: "角虫",
	觧: "角羊",
	觨: "角汞",
	觩: "角求",
	觪: "角辛",
	觫: "角束",
	觬: "角兒",
	觭: "角奇",
	觮: "角录",
	觯: "角单",
	觰: "角者",
	觵: "角黄",
	觶: "角單",
	觸: "角蜀",
	觹: "角雋",
	觻: "角樂",
	觼: "角夐",
	觽: "角嶲",
	觾: "角燕",
	觿: "角巂",
	訂: "言丁",
	訃: "言卜",
	訅: "言九",
	訆: "言丩",
	計: "言十",
	訉: "言凡",
	訊: "言卂",
	訋: "言勺",
	訌: "言工",
	訍: "言叉",
	討: "言寸",
	訏: "言于",
	訐: "言干",
	訑: "言也",
	訒: "言刃",
	訓: "言川",
	訕: "言山",
	訖: "言乞",
	託: "言乇",
	記: "言己",
	訙: "言丸",
	訛: "言化",
	訜: "言分",
	訝: "言牙",
	訞: "言夭",
	訟: "言公",
	訠: "言引",
	訡: "言今",
	訢: "言斤",
	訣: "言夬",
	訤: "言爻",
	訥: "言内",
	訦: "言冘",
	訧: "言尤",
	訨: "言止",
	訩: "言凶",
	訪: "言方",
	訫: "言心",
	訬: "言少",
	設: "言殳",
	訮: "言开",
	訯: "言及",
	訰: "言屯",
	許: "言午",
	訲: "言中",
	訳: "言尺",
	訴: "言斥",
	訵: "言四",
	訶: "言可",
	訷: "言申",
	訸: "言禾",
	訹: "言术",
	註: "言主",
	証: "言正",
	訽: "言句",
	訿: "言此",
	詀: "言占",
	詁: "言古",
	詂: "言付",
	詃: "言玄",
	詄: "言失",
	詅: "言令",
	詆: "言氐",
	詇: "言央",
	詉: "言奴",
	詊: "言半",
	詋: "言兄",
	詌: "言甘",
	詍: "言世",
	詎: "言巨",
	詏: "言幼",
	詐: "言乍",
	詑: "言它",
	詒: "言台",
	詓: "言去",
	詔: "言召",
	評: "言平",
	詖: "言皮",
	詗: "言冋",
	詘: "言出",
	詙: "言犮",
	詚: "言旦",
	詛: "言且",
	詝: "言宁",
	詞: "言司",
	詠: "言永",
	詡: "言羽",
	詢: "言旬",
	詣: "言旨",
	詥: "言合",
	試: "言式",
	詨: "言交",
	詩: "言寺",
	詪: "言艮",
	詫: "言宅",
	詬: "言后",
	詭: "言危",
	詮: "言全",
	詯: "言自",
	詰: "言吉",
	話: "言舌",
	該: "言亥",
	詳: "言羊",
	詴: "言有",
	詵: "言先",
	詶: "言州",
	詷: "言同",
	詸: "言米",
	詺: "言名",
	詻: "言各",
	詼: "言灰",
	詽: "言幵",
	詾: "言匈",
	詿: "言圭",
	誀: "言耳",
	誁: "言并",
	誂: "言兆",
	誃: "言多",
	誄: "言耒",
	誅: "言朱",
	誆: "言匡",
	誇: "言夸",
	誈: "言至",
	誋: "言忌",
	誌: "言志",
	認: "言忍",
	誎: "言束",
	誏: "言良",
	誐: "言我",
	誑: "言狂",
	誒: "言矣",
	誔: "言廷",
	誕: "言延",
	誖: "言孛",
	誗: "言利",
	誘: "言秀",
	誙: "言巠",
	誚: "言肖",
	誜: "言夋",
	誝: "言含",
	語: "言吾",
	誟: "言孝",
	誠: "言成",
	誡: "言戒",
	誢: "言見",
	誣: "言巫",
	誤: "言吴",
	誥: "言告",
	誦: "言甬",
	誧: "言甫",
	誨: "言每",
	誩: "言言",
	說: "言兌",
	誫: "言辰",
	説: "言兑",
	読: "言売",
	誮: "言花",
	誯: "言昌",
	誰: "言隹",
	誱: "言疌",
	課: "言果",
	誳: "言屈",
	誴: "言宗",
	誵: "言肴",
	誶: "言卒",
	誷: "言罔",
	誸: "言弦",
	誹: "言非",
	誺: "言來",
	誻: "言沓",
	誼: "言宜",
	誽: "言兒",
	調: "言周",
	諀: "言卑",
	諁: "言叕",
	諂: "言臽",
	諃: "言林",
	諄: "言享",
	諆: "言其",
	談: "言炎",
	諈: "言垂",
	諉: "言委",
	諊: "言匊",
	請: "言青",
	諌: "言東",
	諍: "言争",
	諎: "言昔",
	諏: "言取",
	諑: "言豖",
	諒: "言京",
	諓: "言戔",
	諔: "言叔",
	諕: "言虎",
	論: "言侖",
	諗: "言念",
	諘: "言表",
	諙: "言昏",
	諚: "言定",
	諛: "言臾",
	諜: "言枼",
	諝: "言胥",
	諞: "言扁",
	諟: "言是",
	諠: "言宣",
	諢: "言軍",
	諣: "言咼",
	諤: "言咢",
	諥: "言重",
	諦: "言帝",
	諧: "言皆",
	諨: "言畐",
	諩: "言並",
	諪: "言亭",
	諫: "言柬",
	諭: "言俞",
	諮: "言咨",
	諯: "言耑",
	諰: "言思",
	諱: "言韋",
	諲: "言垔",
	諳: "言音",
	諴: "言咸",
	諵: "言南",
	諶: "言甚",
	諷: "言風",
	諸: "言者",
	諹: "言昜",
	諺: "言彦",
	諻: "言皇",
	諼: "言爰",
	諽: "言革",
	諾: "言若",
	諿: "言咠",
	謀: "言某",
	謁: "言曷",
	謂: "言胃",
	謃: "言星",
	謄: "月誊",
	謅: "言芻",
	謆: "言扇",
	謉: "言鬼",
	謊: "言荒",
	謋: "言桀",
	謌: "言哥",
	謎: "言迷",
	謏: "言叟",
	謑: "言奚",
	謒: "言倉",
	謓: "言真",
	謔: "言虐",
	謕: "言虒",
	謖: "言畟",
	謗: "言旁",
	謘: "言屖",
	謙: "言兼",
	謚: "言益",
	講: "言冓",
	謜: "言原",
	謝: "言射",
	謞: "言高",
	謟: "言舀",
	謢: "言隻",
	謣: "言雩",
	謤: "言票",
	謥: "言悤",
	謧: "言离",
	謨: "言莫",
	謪: "言商",
	謫: "言啇",
	謬: "言翏",
	謭: "言剪",
	謮: "言責",
	謯: "言虘",
	謰: "言連",
	謱: "言婁",
	謲: "言參",
	謳: "言區",
	謴: "言貫",
	謵: "言習",
	謶: "言庶",
	謸: "言敖",
	謹: "言堇",
	謻: "言移",
	謼: "言虖",
	謾: "言曼",
	謿: "言朝",
	譀: "言敢",
	譁: "言華",
	譂: "言單",
	譃: "言虚",
	譄: "言曾",
	譅: "言歰",
	譆: "言喜",
	譇: "言奢",
	譈: "言敦",
	證: "言登",
	譊: "言堯",
	譋: "言閒",
	譌: "言爲",
	譎: "言矞",
	譏: "言幾",
	譐: "言尊",
	譑: "言喬",
	譒: "言番",
	譓: "言惠",
	譔: "言巽",
	譕: "言無",
	譖: "言朁",
	譗: "言答",
	識: "言戠",
	譙: "言焦",
	譚: "言覃",
	譛: "言替",
	譜: "言普",
	譝: "言黽",
	譞: "言睘",
	譟: "言喿",
	譠: "言亶",
	譡: "言當",
	譢: "言遂",
	譣: "言僉",
	譤: "言敫",
	譧: "言廉",
	譨: "言農",
	譩: "言意",
	譪: "言葛",
	譫: "言詹",
	譭: "言毁",
	譮: "言會",
	譯: "言睪",
	議: "言義",
	譳: "言需",
	譴: "言遣",
	譵: "言對",
	護: "言蒦",
	譸: "言壽",
	譹: "言豪",
	譺: "言疑",
	譾: "言翦",
	譿: "言慧",
	讀: "言賣",
	讁: "言適",
	讂: "言夐",
	讃: "言賛",
	讄: "言畾",
	讅: "言審",
	讇: "言閻",
	讈: "言歷",
	讉: "言遺",
	讌: "言燕",
	讍: "言噩",
	讑: "言龠",
	讒: "言毚",
	讓: "言襄",
	讔: "言隱",
	讕: "言闌",
	讖: "言韱",
	讗: "言巂",
	讘: "言聶",
	讙: "言雚",
	讚: "言贊",
	讛: "言藝",
	讜: "言黨",
	讝: "言嚴",
	讞: "言獻",
	讟: "讀言",
	计: "讠十",
	订: "讠丁",
	讣: "讠卜",
	认: "讠人",
	讥: "讠几",
	讦: "讠干",
	讧: "讠工",
	讨: "讠寸",
	让: "讠上",
	讪: "讠山",
	讫: "讠乞",
	讬: "讠乇",
	训: "讠川",
	议: "讠义",
	讯: "讠卂",
	记: "讠己",
	讱: "讠刃",
	讲: "讠井",
	讳: "讠韦",
	讴: "讠区",
	讵: "讠巨",
	讶: "讠牙",
	讷: "讠内",
	许: "讠午",
	讹: "讠化",
	论: "讠仑",
	讻: "讠凶",
	讼: "讠公",
	讽: "讠风",
	设: "讠殳",
	访: "讠方",
	诀: "讠夬",
	证: "讠正",
	诂: "讠古",
	诃: "讠可",
	评: "讠平",
	诅: "讠且",
	识: "讠只",
	诇: "讠冋",
	诈: "讠乍",
	诉: "讠斥",
	诋: "讠氐",
	诌: "讠刍",
	词: "讠司",
	诎: "讠出",
	诏: "讠召",
	诐: "讠皮",
	诒: "讠台",
	诓: "讠匡",
	诔: "讠耒",
	试: "讠式",
	诖: "讠圭",
	诗: "讠寺",
	诘: "讠吉",
	诙: "讠灰",
	诚: "讠成",
	诛: "讠朱",
	诜: "讠先",
	话: "讠舌",
	诞: "讠延",
	诟: "讠后",
	诠: "讠全",
	诡: "讠危",
	询: "讠旬",
	诣: "讠旨",
	诤: "讠争",
	该: "讠亥",
	详: "讠羊",
	诧: "讠宅",
	诨: "讠军",
	诩: "讠羽",
	诪: "讠寿",
	诫: "讠戒",
	诬: "讠巫",
	语: "讠吾",
	诮: "讠肖",
	误: "讠吴",
	诰: "讠告",
	诱: "讠秀",
	诲: "讠每",
	诳: "讠狂",
	说: "讠兑",
	诵: "讠甬",
	诶: "讠矣",
	请: "讠青",
	诸: "讠者",
	诹: "讠取",
	诺: "讠若",
	读: "讠卖",
	诼: "讠豖",
	诽: "讠非",
	课: "讠果",
	诿: "讠委",
	谀: "讠臾",
	谁: "讠隹",
	谂: "讠念",
	调: "讠周",
	谄: "讠臽",
	谅: "讠京",
	谆: "讠享",
	谇: "讠卒",
	谈: "讠炎",
	谉: "讠审",
	谊: "讠宜",
	谋: "讠某",
	谌: "讠甚",
	谍: "讠枼",
	谎: "讠荒",
	谏: "讠柬",
	谐: "讠皆",
	谑: "讠虐",
	谒: "讠曷",
	谓: "讠胃",
	谔: "讠咢",
	谕: "讠俞",
	谖: "讠爰",
	谘: "讠咨",
	谙: "讠音",
	谚: "讠彦",
	谛: "讠帝",
	谜: "讠迷",
	谝: "讠扁",
	谞: "讠胥",
	谟: "讠莫",
	谠: "讠党",
	谡: "讠畟",
	谢: "讠射",
	谤: "讠旁",
	谥: "讠益",
	谦: "讠兼",
	谨: "讠堇",
	谩: "讠曼",
	谪: "讠啇",
	谫: "讠剪",
	谬: "讠翏",
	谭: "讠覃",
	谮: "讠朁",
	谯: "讠焦",
	谰: "讠阑",
	谱: "讠普",
	谲: "讠矞",
	谳: "讠献",
	谴: "讠遣",
	谵: "讠詹",
	谶: "讠韱",
	谸: "千谷",
	谹: "谷厷",
	谺: "谷牙",
	谻: "谷丮",
	谼: "谷共",
	谽: "谷含",
	谾: "谷空",
	谿: "奚谷",
	豀: "谷奚",
	豁: "害谷",
	豂: "谷翏",
	豃: "谷敢",
	豄: "谷賣",
	豅: "谷龍",
	豇: "豆工",
	豉: "豆支",
	豌: "豆宛",
	豍: "豆卑",
	豏: "豆兼",
	豑: "豊弟",
	豒: "豐弟",
	豓: "豐盇",
	豔: "豐盍",
	豘: "豕屯",
	豚: "月豕",
	豛: "豕殳",
	豜: "豕开",
	豝: "豕巴",
	豞: "豕句",
	豟: "豕戹",
	豠: "豕且",
	豣: "豕幵",
	豤: "豕艮",
	豥: "豕亥",
	豧: "豕甫",
	豨: "豕希",
	豩: "豕豕",
	豫: "予象",
	豬: "豕者",
	豭: "豕叚",
	豮: "豕贲",
	豯: "豕奚",
	豱: "豕昷",
	豲: "豕原",
	豴: "豕啇",
	豵: "豕從",
	豶: "豕賁",
	豷: "豕壹",
	豹: "豸勺",
	豺: "豸才",
	豻: "豸干",
	豼: "豸比",
	豽: "豸内",
	豾: "豸丕",
	豿: "豸句",
	貀: "豸出",
	貁: "豸穴",
	貂: "豸召",
	貃: "豸白",
	貄: "豸聿",
	貅: "豸休",
	貆: "豸亘",
	貇: "豸艮",
	貈: "豸舟",
	貉: "豸各",
	貊: "豸百",
	貋: "豸旱",
	貌: "豸皃",
	貍: "豸里",
	貎: "豸兒",
	貏: "豸卑",
	貐: "豸俞",
	貑: "豸叚",
	貒: "豸耑",
	貓: "豸苗",
	貕: "豸奚",
	貖: "豸益",
	貗: "豸婁",
	貘: "豸莫",
	貙: "豸區",
	貚: "豸單",
	貛: "豸雚",
	貜: "豸矍",
	財: "貝才",
	貤: "貝也",
	貥: "貝亢",
	貦: "貝元",
	販: "貝反",
	貯: "貝宁",
	貱: "貝皮",
	貶: "貝乏",
	貹: "貝生",
	貺: "貝兄",
	貼: "貝占",
	貽: "貝台",
	貾: "貝氐",
	賂: "貝各",
	賄: "貝有",
	賅: "貝亥",
	賆: "貝并",
	賉: "貝血",
	賊: "貝戎",
	賋: "貝交",
	賍: "貝庄",
	賏: "貝貝",
	賐: "貝夋",
	賑: "貝辰",
	賒: "貝佘",
	賕: "貝求",
	賖: "貝余",
	賗: "貝串",
	賘: "貝庒",
	賙: "貝周",
	賜: "貝易",
	賝: "貝罙",
	賟: "貝典",
	賠: "貝咅",
	賤: "貝戔",
	賥: "貝卒",
	賦: "貝武",
	賧: "貝炎",
	賩: "貝宗",
	賬: "貝長",
	賭: "貝者",
	賰: "貝春",
	賱: "貝軍",
	賳: "貝哉",
	賴: "束負",
	賵: "貝冒",
	賶: "貝倉",
	賹: "貝益",
	賺: "貝兼",
	賻: "貝尃",
	購: "貝冓",
	賿: "貝翏",
	贂: "貝參",
	贃: "貝患",
	贆: "貝猋",
	贈: "貝曾",
	贉: "貝覃",
	贌: "貝菐",
	贍: "貝詹",
	贎: "貝萬",
	贐: "貝盡",
	贑: "章貢",
	贓: "貝臧",
	贕: "卵賣",
	贖: "貝賣",
	贘: "貝賞",
	贚: "貝龍",
	贜: "貝藏",
	财: "贝才",
	败: "贝攵",
	账: "贝长",
	贩: "贝反",
	贬: "贝乏",
	购: "贝勾",
	贱: "贝戋",
	贴: "贝占",
	贶: "贝兄",
	贻: "贝台",
	贼: "贝戎",
	贿: "贝有",
	赂: "贝各",
	赃: "贝庄",
	赅: "贝亥",
	赆: "贝尽",
	赇: "贝求",
	赈: "贝辰",
	赊: "贝佘",
	赋: "贝武",
	赌: "贝者",
	赎: "贝卖",
	赐: "贝易",
	赒: "贝周",
	赔: "贝咅",
	赕: "贝炎",
	赖: "束负",
	赗: "贝冒",
	赙: "贝尃",
	赚: "贝兼",
	赠: "贝曾",
	赡: "贝詹",
	赥: "赤欠",
	赦: "赤攵",
	赨: "赤虫",
	赩: "赤色",
	赪: "赤贞",
	赫: "赤赤",
	赬: "赤貞",
	赭: "赤者",
	赮: "赤叚",
	赯: "赤唐",
	躬: "身弓",
	躭: "身冘",
	躮: "身分",
	躯: "身区",
	躰: "身本",
	躱: "身朶",
	躲: "身朵",
	躳: "身吕",
	躴: "身良",
	躵: "身忍",
	躶: "身果",
	躷: "身委",
	躸: "身奇",
	躹: "身匊",
	躺: "身尚",
	躻: "身空",
	躼: "身長",
	躽: "身匽",
	躾: "身美",
	躿: "身康",
	軀: "身區",
	軁: "身婁",
	軂: "身勞",
	軃: "身單",
	軄: "身戠",
	軅: "身雁",
	軆: "身豊",
	軇: "身壽",
	軈: "身應",
	軉: "身寶",
	軋: "車乚",
	軌: "車九",
	軏: "車兀",
	軐: "車卂",
	軑: "車大",
	軒: "車干",
	軓: "車凡",
	軔: "車刃",
	軕: "車山",
	軖: "車王",
	軗: "車殳",
	軘: "車屯",
	軙: "車攴",
	軚: "車太",
	軛: "車厄",
	軜: "車内",
	軝: "車氏",
	軞: "車毛",
	軟: "車欠",
	軠: "車壬",
	軡: "車今",
	転: "車云",
	軤: "車乎",
	軥: "車句",
	軦: "車兄",
	軧: "車氐",
	軨: "車令",
	軩: "車台",
	軪: "車幼",
	軭: "車匡",
	軮: "車央",
	軯: "車平",
	軱: "車瓜",
	軲: "車古",
	軳: "車包",
	軴: "車主",
	軵: "車付",
	軶: "車戹",
	軷: "車犮",
	軸: "車由",
	軹: "車只",
	軺: "車召",
	軻: "車可",
	軼: "車失",
	軽: "車圣",
	軾: "車式",
	軿: "車并",
	輀: "車而",
	輁: "車共",
	較: "車交",
	輄: "車光",
	輅: "車各",
	輆: "車亥",
	輇: "車全",
	輈: "車舟",
	輊: "車至",
	輌: "車両",
	輍: "車谷",
	輎: "車肖",
	輏: "車酉",
	輐: "車完",
	輑: "車君",
	輓: "車免",
	輔: "車甫",
	輕: "車巠",
	輖: "車周",
	輗: "車兒",
	輘: "車夌",
	輙: "車取",
	輚: "車戔",
	輛: "車兩",
	輜: "車甾",
	輝: "光軍",
	輞: "車罔",
	輟: "車叕",
	輠: "車果",
	輡: "車臽",
	輢: "車奇",
	輣: "車朋",
	輤: "車青",
	輥: "車昆",
	輧: "車幷",
	輨: "車官",
	輪: "車侖",
	輫: "車非",
	輬: "車京",
	輭: "車耎",
	輮: "車柔",
	輯: "車咠",
	輰: "車昜",
	輱: "車咸",
	輲: "車耑",
	輳: "車奏",
	輴: "車盾",
	輵: "車曷",
	輶: "車酋",
	輷: "車訇",
	輸: "車俞",
	輹: "車复",
	輻: "車畐",
	輼: "車昷",
	輽: "車畚",
	輾: "車展",
	轁: "車舀",
	轃: "車秦",
	轄: "車害",
	轅: "車袁",
	轆: "車鹿",
	轇: "車翏",
	轈: "車巢",
	轉: "車專",
	轊: "車彗",
	轋: "車連",
	轌: "車雪",
	轎: "車喬",
	轏: "車孱",
	轐: "車菐",
	轑: "車尞",
	轒: "車賁",
	轓: "車番",
	轔: "車粦",
	轕: "車葛",
	轖: "車嗇",
	轗: "車感",
	轘: "車睘",
	轙: "車義",
	轛: "車對",
	轜: "車需",
	轞: "車監",
	轠: "車畾",
	轢: "車樂",
	轣: "車歷",
	轤: "車盧",
	轥: "車藺",
	轧: "车乚",
	轨: "车九",
	轩: "车干",
	轪: "车大",
	轫: "车刃",
	转: "车专",
	轭: "车厄",
	轮: "车仑",
	软: "车欠",
	轱: "车古",
	轲: "车可",
	轳: "车卢",
	轴: "车由",
	轵: "车只",
	轶: "车失",
	轷: "车乎",
	轹: "车乐",
	轺: "车召",
	轼: "车式",
	轾: "车至",
	轿: "车乔",
	辀: "车舟",
	辁: "车全",
	辂: "车各",
	较: "车交",
	辅: "车甫",
	辆: "车两",
	辉: "光军",
	辊: "车昆",
	辋: "车罔",
	辌: "车京",
	辍: "车叕",
	辎: "车甾",
	辏: "车奏",
	辐: "车畐",
	辑: "车咠",
	辒: "车昷",
	输: "车俞",
	辕: "车袁",
	辖: "车害",
	辗: "车展",
	辘: "车鹿",
	辚: "车粦",
	辝: "台辛",
	辞: "舌辛",
	辡: "辛辛",
	辢: "束辛",
	辣: "辛束",
	辤: "受辛",
	辴: "單辰",
	邒: "丁阝",
	邓: "又阝",
	邔: "己阝",
	邖: "山阝",
	邗: "干阝",
	邘: "于阝",
	邙: "亡阝",
	邚: "女阝",
	邛: "工阝",
	邜: "夕阝",
	邝: "广阝",
	邞: "夫阝",
	邟: "亢阝",
	邠: "分阝",
	邡: "方阝",
	邢: "开阝",
	邤: "斤阝",
	邥: "冘阝",
	邦: "丰阝",
	邧: "元阝",
	邨: "屯阝",
	邩: "火阝",
	邪: "牙阝",
	邫: "丰邑",
	邬: "乌阝",
	邭: "句阝",
	邮: "由阝",
	邯: "甘阝",
	邰: "台阝",
	邱: "丘阝",
	邲: "必阝",
	邳: "丕阝",
	邴: "丙阝",
	邵: "召阝",
	邶: "北阝",
	邷: "瓦阝",
	邸: "氐阝",
	邹: "刍阝",
	邺: "业阝",
	邻: "令阝",
	邼: "匡阝",
	邽: "圭阝",
	邾: "朱阝",
	邿: "寺阝",
	郀: "夸阝",
	郁: "有阝",
	郂: "亥阝",
	郃: "合阝",
	郅: "至阝",
	郆: "吉阝",
	郇: "旬阝",
	郈: "后阝",
	郉: "幵阝",
	郊: "交阝",
	郋: "自阝",
	郌: "圭邑",
	郍: "舟阝",
	郏: "夹阝",
	郐: "会阝",
	郑: "关阝",
	郓: "军阝",
	郔: "延阝",
	郕: "成阝",
	郖: "豆阝",
	郗: "希阝",
	郘: "吕阝",
	郙: "甫阝",
	郚: "吾阝",
	郛: "孚阝",
	郜: "告阝",
	郝: "赤阝",
	郞: "良阝",
	郟: "夾阝",
	郠: "更阝",
	郡: "君阝",
	郢: "呈阝",
	郣: "孛阝",
	郤: "谷阝",
	郥: "貝阝",
	郦: "丽阝",
	郧: "员阝",
	部: "咅阝",
	郩: "肴阝",
	郪: "妻阝",
	郫: "卑阝",
	郬: "青阝",
	郭: "享阝",
	郮: "周阝",
	郯: "炎阝",
	郰: "取阝",
	郱: "并阝",
	郲: "來阝",
	郳: "兒阝",
	郴: "林阝",
	郵: "垂阝",
	郶: "咅邑",
	郸: "单阝",
	郹: "狊阝",
	郺: "多邕",
	郼: "韋阝",
	都: "者阝",
	郾: "匽阝",
	郿: "眉阝",
	鄀: "若阝",
	鄁: "背阝",
	鄂: "咢阝",
	鄃: "俞阝",
	鄄: "垔阝",
	鄅: "禹阝",
	鄆: "軍阝",
	鄇: "侯阝",
	鄈: "癸阝",
	鄉: "乡郎",
	鄊: "乡耶",
	鄋: "叟阝",
	鄌: "唐阝",
	鄍: "冥阝",
	鄎: "息阝",
	鄏: "辱阝",
	鄐: "畜阝",
	鄑: "晋阝",
	鄒: "芻阝",
	鄓: "臭阝",
	鄔: "烏阝",
	鄖: "員阝",
	鄗: "高阝",
	鄘: "庸阝",
	鄙: "啚阝",
	鄚: "莫阝",
	鄛: "巢阝",
	鄜: "鹿阝",
	鄝: "翏阝",
	鄞: "堇阝",
	鄟: "專阝",
	鄠: "雩阝",
	鄡: "梟阝",
	鄢: "焉阝",
	鄣: "章阝",
	鄤: "曼阝",
	鄥: "鳥阝",
	鄦: "無阝",
	鄧: "登阝",
	鄩: "尋阝",
	鄪: "費阝",
	鄫: "曾阝",
	鄬: "爲阝",
	鄭: "奠阝",
	鄮: "貿阝",
	鄯: "善阝",
	鄰: "粦阝",
	鄱: "番阝",
	鄲: "單阝",
	鄳: "黽阝",
	鄴: "業阝",
	鄵: "喿阝",
	鄶: "會阝",
	鄷: "豊阝",
	鄸: "夢阝",
	鄹: "聚阝",
	鄺: "廣阝",
	鄻: "輦阝",
	鄼: "賛阝",
	鄽: "廛阝",
	鄾: "憂阝",
	鄿: "蕇阝",
	酀: "燕阝",
	酁: "毚阝",
	酂: "赞阝",
	酃: "霝阝",
	酄: "雚阝",
	酅: "巂阝",
	酆: "豐阝",
	酇: "贊阝",
	酈: "麗阝",
	酊: "酉丁",
	酌: "酉勺",
	配: "酉己",
	酎: "酉寸",
	酏: "酉也",
	酐: "酉干",
	酑: "酉于",
	酒: "氵酉",
	酔: "酉卆",
	酕: "酉毛",
	酖: "酉冘",
	酗: "酉凶",
	酘: "酉殳",
	酙: "酉斗",
	酚: "酉分",
	酛: "酉元",
	酜: "酉夫",
	酝: "酉云",
	酞: "酉太",
	酟: "酉占",
	酠: "酉可",
	酡: "酉它",
	酢: "酉乍",
	酣: "酉甘",
	酤: "酉古",
	酥: "酉禾",
	酦: "酉发",
	酧: "酉守",
	酩: "酉名",
	酪: "酉各",
	酫: "酉全",
	酬: "酉州",
	酭: "酉有",
	酮: "酉同",
	酯: "酉旨",
	酰: "酉先",
	酲: "酉呈",
	酴: "酉余",
	酵: "酉孝",
	酶: "酉每",
	酷: "酉告",
	酸: "酉夋",
	酹: "酉寽",
	酺: "酉甫",
	酻: "酉孚",
	酽: "酉严",
	酾: "酉丽",
	酿: "酉良",
	醀: "酉隹",
	醁: "酉录",
	醂: "酉林",
	醃: "酉奄",
	醄: "酉匋",
	醅: "酉咅",
	醆: "酉戔",
	醇: "酉享",
	醈: "酉炎",
	醉: "酉卒",
	醊: "酉叕",
	醋: "酉昔",
	醌: "酉昆",
	醍: "酉是",
	醎: "酉咸",
	醏: "酉者",
	醐: "酉胡",
	醑: "酉胥",
	醒: "酉星",
	醕: "酉亯",
	醖: "酉昷",
	醗: "酉発",
	醘: "酉盍",
	醙: "酉叟",
	醚: "酉迷",
	醛: "酉荃",
	醜: "酉鬼",
	醝: "酉差",
	醠: "酉盎",
	醡: "酉窄",
	醣: "酉唐",
	醥: "酉票",
	醦: "酉參",
	醧: "酉區",
	醨: "酉离",
	醩: "酉曹",
	醪: "酉翏",
	醭: "酉菐",
	醮: "酉焦",
	醰: "酉覃",
	醱: "酉發",
	醲: "酉農",
	醳: "酉睪",
	醴: "酉豊",
	醵: "酉豦",
	醶: "酉僉",
	醷: "酉意",
	醹: "酉需",
	醺: "酉熏",
	醻: "酉壽",
	醼: "酉燕",
	醽: "酉霝",
	醾: "酉糜",
	醿: "酉縻",
	釀: "酉襄",
	釂: "酉爵",
	釃: "酉麗",
	釄: "酉靡",
	釅: "酉嚴",
	釈: "釆尺",
	釉: "釆由",
	釋: "釆睪",
	野: "里予",
	釓: "金乚",
	釔: "金乙",
	釕: "金了",
	釖: "金刀",
	釗: "金刂",
	釘: "金丁",
	釙: "金卜",
	釚: "金九",
	釛: "金力",
	針: "金十",
	釞: "金入",
	釟: "金八",
	釠: "金几",
	釢: "金乃",
	釣: "金勺",
	釤: "金彡",
	釥: "金小",
	釦: "金口",
	釧: "金川",
	釨: "金子",
	釩: "金凡",
	釪: "金于",
	釫: "金亏",
	釬: "金干",
	釭: "金工",
	釮: "金才",
	釯: "金亡",
	釰: "金刃",
	釱: "金大",
	釲: "金巳",
	釳: "金乞",
	釴: "金弋",
	釵: "金叉",
	釶: "金也",
	釷: "金土",
	釸: "金夕",
	釹: "金女",
	釺: "金千",
	釻: "金丸",
	釼: "金刄",
	釽: "金爪",
	釾: "金牙",
	釿: "金斤",
	鈀: "金巴",
	鈁: "金方",
	鈂: "金冘",
	鈃: "金开",
	鈄: "金斗",
	鈅: "金月",
	鈆: "金公",
	鈇: "金夫",
	鈈: "金不",
	鈉: "金内",
	鈊: "金心",
	鈋: "金化",
	鈌: "金夬",
	鈍: "金屯",
	鈎: "金勾",
	鈏: "金引",
	鈐: "金今",
	鈑: "金反",
	鈒: "金及",
	鈓: "金壬",
	鈔: "金少",
	鈕: "金丑",
	鈖: "金分",
	鈗: "金允",
	鈘: "金支",
	鈙: "金攴",
	鈚: "金比",
	鈛: "金戈",
	鈜: "金厷",
	鈝: "金牛",
	鈞: "金匀",
	鈟: "金弔",
	鈠: "金殳",
	鈡: "金中",
	鈢: "金木",
	鈣: "金丐",
	鈤: "金日",
	鈥: "金火",
	鈦: "金太",
	鈧: "金亢",
	鈨: "金元",
	鈩: "金户",
	鈪: "金厄",
	鈫: "金文",
	鈬: "金尺",
	鈮: "金尼",
	鈯: "金出",
	鈰: "金市",
	鈱: "金民",
	鈲: "金瓜",
	鈳: "金可",
	鈴: "金令",
	鈵: "金丙",
	鈶: "金台",
	鈷: "金古",
	鈸: "金犮",
	鈹: "金皮",
	鈺: "金玉",
	鈼: "金乍",
	鈽: "金布",
	鈾: "金由",
	鈿: "金田",
	鉀: "金甲",
	鉂: "金史",
	鉃: "金矢",
	鉄: "金失",
	鉅: "金巨",
	鉆: "金占",
	鉈: "金它",
	鉉: "金玄",
	鉊: "金召",
	鉋: "金包",
	鉌: "金禾",
	鉍: "金必",
	鉎: "金生",
	鉏: "金且",
	鉐: "金石",
	鉑: "金白",
	鉒: "金主",
	鉔: "金匝",
	鉕: "金叵",
	鉖: "金冬",
	鉗: "金甘",
	鉘: "金弗",
	鉙: "金只",
	鉚: "金卯",
	鉜: "金付",
	鉝: "金立",
	鉞: "金戉",
	鉟: "金丕",
	鉠: "金央",
	鉡: "金半",
	鉢: "金本",
	鉣: "金去",
	鉤: "金句",
	鉥: "金术",
	鉦: "金正",
	鉧: "金母",
	鉨: "金尔",
	鉩: "金尒",
	鉪: "金世",
	鉫: "金加",
	鉬: "金目",
	鉭: "金旦",
	鉮: "金申",
	鉯: "金以",
	鉰: "金司",
	鉱: "金広",
	鉲: "金卡",
	鉳: "金北",
	鉵: "金虫",
	鉶: "金刑",
	鉷: "金共",
	鉸: "金交",
	鉹: "金多",
	鉺: "金耳",
	鉻: "金各",
	鉼: "金并",
	鉽: "金式",
	鉾: "金牟",
	鉿: "金合",
	銀: "金艮",
	銁: "金旬",
	銂: "金州",
	銃: "金充",
	銄: "金向",
	銅: "金同",
	銆: "金百",
	銇: "金耒",
	銈: "金圭",
	銉: "金聿",
	銊: "金戌",
	銋: "金任",
	銌: "金存",
	銍: "金至",
	銏: "金册",
	銑: "金先",
	銒: "金幵",
	銓: "金全",
	銕: "金夷",
	銖: "金朱",
	銗: "金后",
	銘: "金名",
	銙: "金夸",
	銚: "金兆",
	銛: "金舌",
	銝: "金休",
	銠: "金老",
	銡: "金吉",
	銣: "金如",
	銤: "金米",
	銥: "金衣",
	銦: "金因",
	銧: "金光",
	銨: "金安",
	銩: "金丢",
	銪: "金有",
	銫: "金色",
	銬: "金考",
	銯: "金糸",
	銰: "金艾",
	銱: "金吊",
	銲: "金旱",
	銳: "金兌",
	銵: "金身",
	銶: "金求",
	銷: "金肖",
	銹: "金秀",
	銻: "金弟",
	銼: "金坐",
	銾: "金汞",
	銿: "金甬",
	鋀: "金豆",
	鋁: "金吕",
	鋂: "金每",
	鋃: "金良",
	鋅: "金辛",
	鋇: "金貝",
	鋉: "金束",
	鋊: "金谷",
	鋋: "金延",
	鋌: "金廷",
	鋍: "金孛",
	鋎: "金完",
	鋏: "金夾",
	鋐: "金宏",
	鋑: "金夋",
	鋒: "金夆",
	鋓: "金利",
	鋔: "金免",
	鋕: "金志",
	鋖: "金妥",
	鋗: "金肙",
	鋘: "金吴",
	鋙: "金吾",
	鋛: "金串",
	鋜: "金足",
	鋝: "金寽",
	鋞: "金巠",
	鋠: "金辰",
	鋡: "金含",
	鋢: "金孚",
	鋣: "金邪",
	鋤: "金助",
	鋥: "金呈",
	鋦: "金局",
	鋧: "金見",
	鋨: "金我",
	鋩: "金芒",
	鋪: "金甫",
	鋭: "金兑",
	鋮: "金成",
	鋯: "金告",
	鋰: "金里",
	鋱: "金忒",
	鋲: "金兵",
	鋳: "金寿",
	鋴: "金克",
	鋵: "金秃",
	鋷: "金取",
	鋸: "金居",
	鋹: "金長",
	鋺: "金宛",
	鋼: "金岡",
	鋽: "金卓",
	鋾: "金匋",
	鋿: "金尚",
	錀: "金侖",
	錁: "金果",
	錂: "金夌",
	錄: "金彔",
	錆: "金青",
	錇: "金咅",
	錈: "金卷",
	錉: "金昏",
	錊: "金卒",
	錋: "金朋",
	錌: "金岸",
	錍: "金卑",
	錎: "金臽",
	錏: "金亞",
	錐: "金隹",
	錑: "金戾",
	錒: "金阿",
	錓: "金羌",
	錔: "金沓",
	錕: "金昆",
	錗: "金委",
	錘: "金垂",
	錙: "金甾",
	錚: "金争",
	錛: "金奔",
	錜: "金念",
	錝: "金宗",
	錞: "金享",
	錟: "金炎",
	錠: "金定",
	錡: "金奇",
	錢: "金戔",
	錣: "金叕",
	錤: "金其",
	錥: "金育",
	錦: "金帛",
	錧: "金官",
	錨: "金苗",
	錩: "金昌",
	錪: "金典",
	錫: "金易",
	錬: "金東",
	錭: "金周",
	錮: "金固",
	錯: "金昔",
	錰: "金杰",
	録: "金录",
	錳: "金孟",
	錴: "金坴",
	錵: "金花",
	錶: "金表",
	錷: "金軋",
	錸: "金來",
	錹: "金肯",
	錺: "金芳",
	錻: "金武",
	錼: "金奈",
	錿: "金虎",
	鍁: "金欣",
	鍂: "金金",
	鍃: "金忽",
	鍄: "金京",
	鍅: "金法",
	鍆: "金門",
	鍇: "金皆",
	鍈: "金英",
	鍉: "金是",
	鍊: "金柬",
	鍋: "金咼",
	鍍: "金度",
	鍎: "金盾",
	鍏: "金韋",
	鍑: "金复",
	鍒: "金柔",
	鍓: "金咠",
	鍔: "金咢",
	鍕: "金軍",
	鍖: "金甚",
	鍗: "金帝",
	鍘: "金則",
	鍚: "金昜",
	鍛: "金段",
	鍜: "金叚",
	鍝: "金禺",
	鍞: "金貞",
	鍟: "金星",
	鍠: "金皇",
	鍡: "金畏",
	鍢: "金畐",
	鍣: "金苕",
	鍤: "金臿",
	鍥: "金契",
	鍦: "金施",
	鍧: "金訇",
	鍨: "金癸",
	鍩: "金若",
	鍬: "金秋",
	鍭: "金侯",
	鍮: "金俞",
	鍯: "金怱",
	鍰: "金爰",
	鍱: "金枼",
	鍲: "金昬",
	鍴: "金耑",
	鍵: "金建",
	鍶: "金思",
	鍷: "金奎",
	鍸: "金胡",
	鍹: "金宣",
	鍺: "金者",
	鍻: "金曷",
	鍼: "金咸",
	鍽: "金扁",
	鍾: "金重",
	鎀: "金修",
	鎁: "金耶",
	鎂: "金美",
	鎃: "金派",
	鎄: "金哀",
	鎅: "金界",
	鎆: "金前",
	鎇: "金眉",
	鎈: "金差",
	鎊: "金旁",
	鎋: "金害",
	鎌: "金兼",
	鎍: "金索",
	鎎: "金氣",
	鎑: "金盍",
	鎒: "金辱",
	鎓: "金翁",
	鎔: "金容",
	鎕: "金唐",
	鎗: "金倉",
	鎘: "金鬲",
	鎙: "金朔",
	鎚: "金追",
	鎛: "金尃",
	鎝: "金荅",
	鎟: "金桑",
	鎠: "金罡",
	鎡: "金兹",
	鎢: "金烏",
	鎤: "金晃",
	鎦: "金留",
	鎧: "金豈",
	鎨: "金隼",
	鎩: "金殺",
	鎪: "金叟",
	鎬: "金高",
	鎭: "金眞",
	鎮: "金真",
	鎯: "金郎",
	鎰: "金益",
	鎱: "金袁",
	鎲: "金党",
	鎳: "金臬",
	鎴: "金息",
	鎵: "金家",
	鎶: "金哥",
	鎷: "金馬",
	鎸: "金隽",
	鎹: "金送",
	鎺: "金祖",
	鎽: "金峯",
	鎾: "金昷",
	鎿: "金拿",
	鏀: "金鹵",
	鏁: "金巢",
	鏂: "金區",
	鏃: "金族",
	鏄: "金專",
	鏅: "金脩",
	鏆: "金貫",
	鏇: "金旋",
	鏈: "金連",
	鏉: "金欶",
	鏌: "金莫",
	鏍: "金累",
	鏎: "金畢",
	鏏: "金彗",
	鏐: "金翏",
	鏑: "金啇",
	鏒: "金參",
	鏓: "金悤",
	鏔: "金寅",
	鏕: "金鹿",
	鏗: "金堅",
	鏘: "金將",
	鏙: "金崔",
	鏚: "金戚",
	鏛: "金常",
	鏜: "金堂",
	鏝: "金曼",
	鏞: "金庸",
	鏟: "金産",
	鏠: "金逢",
	鏡: "金竟",
	鏢: "金票",
	鏣: "金庶",
	鏤: "金婁",
	鏥: "金宿",
	鏦: "金從",
	鏩: "金斬",
	鏪: "金曹",
	鏫: "金梨",
	鏬: "金虖",
	鏭: "金悉",
	鏮: "金康",
	鏯: "金爽",
	鏰: "金崩",
	鏱: "金章",
	鏲: "金牽",
	鏳: "金曾",
	鏴: "金路",
	鏵: "金華",
	鏶: "金集",
	鏷: "金菐",
	鏸: "金惠",
	鏹: "金强",
	鏺: "金發",
	鏻: "金粦",
	鏼: "金策",
	鏽: "金肅",
	鏾: "金散",
	鏿: "金棠",
	鐀: "金貴",
	鐁: "金斯",
	鐂: "金畱",
	鐃: "金堯",
	鐄: "金黄",
	鐇: "金番",
	鐈: "金喬",
	鐉: "金巽",
	鐊: "金陽",
	鐋: "金湯",
	鐌: "金象",
	鐍: "金矞",
	鐎: "金焦",
	鐏: "金尊",
	鐐: "金尞",
	鐑: "金結",
	鐒: "金勞",
	鐓: "金敦",
	鐔: "金覃",
	鐕: "金朁",
	鐖: "金幾",
	鐗: "金閒",
	鐘: "金童",
	鐙: "金登",
	鐚: "金惡",
	鐛: "金景",
	鐝: "金厥",
	鐞: "金尋",
	鐟: "金替",
	鐠: "金普",
	鐣: "金掌",
	鐤: "金鼎",
	鐥: "金善",
	鐦: "金開",
	鐧: "金間",
	鐨: "金費",
	鐩: "金遂",
	鐪: "金虜",
	鐫: "金雋",
	鐬: "金歲",
	鐭: "金奥",
	鐮: "金廉",
	鐯: "金著",
	鐰: "金喿",
	鐱: "金僉",
	鐲: "金蜀",
	鐳: "金雷",
	鐴: "金辟",
	鐶: "金睘",
	鐷: "金葉",
	鐸: "金睪",
	鐹: "金過",
	鐺: "金當",
	鐻: "金豦",
	鐼: "金賁",
	鐽: "金達",
	鐿: "金意",
	鑀: "金愛",
	鑂: "金熏",
	鑃: "金翟",
	鑄: "金壽",
	鑅: "金榮",
	鑆: "金對",
	鑇: "金齊",
	鑈: "金爾",
	鑉: "金蓋",
	鑊: "金蒦",
	鑌: "金賓",
	鑎: "金匱",
	鑏: "金寧",
	鑐: "金需",
	鑑: "金監",
	鑓: "金遣",
	鑔: "金察",
	鑕: "金質",
	鑖: "金蔑",
	鑗: "金黎",
	鑘: "金畾",
	鑚: "金賛",
	鑛: "金廣",
	鑜: "金賞",
	鑞: "金巤",
	鑟: "金賣",
	鑠: "金樂",
	鑡: "金齒",
	鑢: "金慮",
	鑣: "金麃",
	鑤: "金暴",
	鑥: "金魯",
	鑦: "金賢",
	鑧: "金寬",
	鑨: "金龍",
	鑩: "金噩",
	鑪: "金盧",
	鑬: "金覧",
	鑭: "金闌",
	鑮: "金薄",
	鑯: "金韱",
	鑰: "金龠",
	鑱: "金毚",
	鑲: "金襄",
	鑳: "金蹇",
	鑴: "金巂",
	鑵: "金雚",
	鑶: "金藏",
	鑷: "金聶",
	鑸: "金壘",
	鑹: "金竄",
	鑺: "金瞿",
	鑻: "金攀",
	鑼: "金羅",
	鑽: "金贊",
	钀: "金獻",
	钁: "金矍",
	钂: "金黨",
	钃: "金屬",
	钄: "金蘭",
	钆: "钅乚",
	钇: "钅乙",
	针: "钅十",
	钉: "钅丁",
	钊: "钅刂",
	钋: "钅卜",
	钌: "钅了",
	钍: "钅土",
	钎: "钅千",
	钏: "钅川",
	钐: "钅彡",
	钑: "钅及",
	钒: "钅凡",
	钓: "钅勺",
	钔: "钅门",
	钕: "钅女",
	钗: "钅叉",
	钘: "钅开",
	钙: "钅丐",
	钚: "钅不",
	钛: "钅太",
	钜: "钅巨",
	钝: "钅屯",
	钞: "钅少",
	钟: "钅中",
	钠: "钅内",
	钡: "钅贝",
	钢: "钅冈",
	钣: "钅反",
	钤: "钅今",
	钥: "钅月",
	钦: "钅欠",
	钧: "钅匀",
	钨: "钅乌",
	钩: "钅勾",
	钪: "钅亢",
	钫: "钅方",
	钬: "钅火",
	钭: "钅斗",
	钮: "钅丑",
	钯: "钅巴",
	钰: "钅玉",
	钱: "钅戋",
	钲: "钅正",
	钳: "钅甘",
	钴: "钅古",
	钵: "钅本",
	钶: "钅可",
	钷: "钅叵",
	钸: "钅布",
	钹: "钅犮",
	钺: "钅戉",
	钻: "钅占",
	钼: "钅目",
	钽: "钅旦",
	钾: "钅甲",
	钿: "钅田",
	铀: "钅由",
	铁: "钅失",
	铂: "钅白",
	铃: "钅令",
	铄: "钅乐",
	铆: "钅卯",
	铇: "钅包",
	铈: "钅市",
	铉: "钅玄",
	铊: "钅它",
	铋: "钅必",
	铌: "钅尼",
	铍: "钅皮",
	铏: "钅刑",
	铐: "钅考",
	铑: "钅老",
	铒: "钅耳",
	铓: "钅芒",
	铔: "钅亚",
	铕: "钅有",
	铖: "钅成",
	铗: "钅夹",
	铘: "钅邪",
	铙: "钅尧",
	铚: "钅至",
	铛: "钅当",
	铜: "钅同",
	铝: "钅吕",
	铞: "钅吊",
	铟: "钅因",
	铠: "钅岂",
	铡: "钅则",
	铢: "钅朱",
	铣: "钅先",
	铤: "钅廷",
	铥: "钅丢",
	铦: "钅舌",
	铧: "钅华",
	铨: "钅全",
	铩: "钅杀",
	铪: "钅合",
	铫: "钅兆",
	铬: "钅各",
	铭: "钅名",
	铮: "钅争",
	铯: "钅色",
	铰: "钅交",
	铱: "钅衣",
	铲: "钅产",
	铳: "钅充",
	铴: "钅汤",
	铵: "钅安",
	银: "钅艮",
	铷: "钅如",
	铸: "钅寿",
	铹: "钅劳",
	铺: "钅甫",
	铻: "钅吾",
	铼: "钅来",
	铽: "钅忒",
	链: "钅连",
	铿: "钅坚",
	销: "钅肖",
	锂: "钅里",
	锃: "钅呈",
	锄: "钅助",
	锅: "钅呙",
	锆: "钅告",
	锇: "钅我",
	锈: "钅秀",
	锉: "钅坐",
	锊: "钅寽",
	锋: "钅夆",
	锌: "钅辛",
	锏: "钅间",
	锐: "钅兑",
	锑: "钅弟",
	锒: "钅良",
	锔: "钅局",
	锕: "钅阿",
	锖: "钅青",
	锗: "钅者",
	锘: "钅若",
	错: "钅昔",
	锚: "钅苗",
	锛: "钅奔",
	锜: "钅奇",
	锞: "钅果",
	锟: "钅昆",
	锠: "钅昌",
	锡: "钅易",
	锢: "钅固",
	锣: "钅罗",
	锤: "钅垂",
	锥: "钅隹",
	锦: "钅帛",
	锧: "钅质",
	锨: "钅欣",
	锩: "钅卷",
	锪: "钅忽",
	锫: "钅咅",
	锬: "钅炎",
	锭: "钅定",
	键: "钅建",
	锯: "钅居",
	锰: "钅孟",
	锱: "钅甾",
	锲: "钅契",
	锳: "钅英",
	锴: "钅皆",
	锵: "钅将",
	锶: "钅思",
	锷: "钅咢",
	锸: "钅臿",
	锹: "钅秋",
	锺: "钅重",
	锻: "钅段",
	锼: "钅叟",
	锽: "钅皇",
	锾: "钅爰",
	锿: "钅哀",
	镀: "钅度",
	镁: "钅美",
	镂: "钅娄",
	镃: "钅兹",
	镄: "钅费",
	镅: "钅眉",
	镆: "钅莫",
	镇: "钅真",
	镈: "钅尃",
	镉: "钅鬲",
	镊: "钅聂",
	镋: "钅党",
	镌: "钅隽",
	镍: "钅臬",
	镎: "钅拿",
	镏: "钅留",
	镐: "钅高",
	镑: "钅旁",
	镒: "钅益",
	镓: "钅家",
	镔: "钅宾",
	镕: "钅容",
	镖: "钅票",
	镗: "钅堂",
	镘: "钅曼",
	镙: "钅累",
	镚: "钅崩",
	镛: "钅庸",
	镜: "钅竟",
	镝: "钅啇",
	镞: "钅族",
	镟: "钅旋",
	镠: "钅翏",
	镡: "钅覃",
	镢: "钅厥",
	镣: "钅尞",
	镤: "钅菐",
	镥: "钅鲁",
	镦: "钅敦",
	镧: "钅阑",
	镨: "钅普",
	镩: "钅窜",
	镪: "钅强",
	镫: "钅登",
	镬: "钅蒦",
	镭: "钅雷",
	镮: "钅睘",
	镯: "钅蜀",
	镰: "钅廉",
	镱: "钅意",
	镲: "钅察",
	镳: "钅麃",
	镴: "钅巤",
	镵: "钅毚",
	镶: "钅襄",
	镹: "镸久",
	镺: "镸夭",
	镻: "镸失",
	镼: "镸屈",
	镽: "镸尞",
	镾: "镸爾",
	閄: "門人",
	閷: "杀閃",
	阞: "阝力",
	队: "阝人",
	阠: "阝卂",
	阡: "阝千",
	阢: "阝兀",
	阣: "阝乞",
	阤: "阝也",
	阥: "阝水",
	阦: "阝火",
	阧: "阝斗",
	阨: "阝厄",
	阩: "阝升",
	阪: "阝反",
	阫: "阝不",
	阬: "阝亢",
	阭: "阝允",
	阮: "阝元",
	阯: "阝止",
	阰: "阝比",
	阱: "阝井",
	防: "阝方",
	阳: "阝日",
	阴: "阝月",
	阵: "阝车",
	阶: "阝介",
	阷: "阝正",
	阸: "阝戹",
	阹: "阝去",
	阺: "阝氐",
	阻: "阝且",
	阼: "阝乍",
	阽: "阝占",
	阾: "阝令",
	阿: "阝可",
	陀: "阝它",
	陂: "阝皮",
	陃: "阝丙",
	附: "阝付",
	际: "阝示",
	陆: "阝击",
	陇: "阝龙",
	陈: "阝东",
	陊: "阝多",
	陌: "阝百",
	降: "阝夅",
	陎: "阝朱",
	陏: "阝有",
	限: "阝艮",
	陑: "阝而",
	陒: "阝危",
	陓: "阝夸",
	陔: "阝亥",
	陕: "阝夹",
	陖: "阝夋",
	陗: "阝肖",
	陘: "阝巠",
	陙: "阝辰",
	陚: "阝武",
	陛: "阝坒",
	陜: "阝夾",
	陟: "阝步",
	陠: "阝甫",
	陡: "阝走",
	院: "阝完",
	陣: "阝車",
	除: "阝余",
	陦: "阝寿",
	陧: "阝圼",
	陨: "阝员",
	险: "阝佥",
	陪: "阝咅",
	陫: "阝非",
	陬: "阝取",
	陭: "阝奇",
	陮: "阝隹",
	陯: "阝侖",
	陰: "阝侌",
	陱: "阝匊",
	陲: "阝垂",
	陳: "阝東",
	陴: "阝卑",
	陵: "阝夌",
	陶: "阝匋",
	陷: "阝臽",
	陸: "阝坴",
	陻: "阝垔",
	陼: "阝者",
	陽: "阝昜",
	陾: "阝耎",
	陿: "阝匧",
	隀: "阝重",
	隁: "阝匽",
	隃: "阝俞",
	隄: "阝是",
	隅: "阝禺",
	隇: "阝威",
	隈: "阝畏",
	隌: "阝音",
	隍: "阝皇",
	階: "阝皆",
	随: "阝迶",
	隐: "阝急",
	隑: "阝豈",
	隒: "阝兼",
	隔: "阝鬲",
	隕: "阝員",
	隖: "阝烏",
	隗: "阝鬼",
	隘: "阝益",
	隚: "阝堂",
	際: "阝祭",
	障: "阝章",
	隝: "阝鳥",
	隞: "阝敖",
	隟: "阝巢",
	隡: "阝産",
	隢: "阝堯",
	隣: "阝粦",
	隤: "阝貴",
	隥: "阝登",
	隦: "阝辟",
	隧: "阝遂",
	隨: "阝遀",
	隩: "阝奥",
	險: "阝僉",
	隫: "阝賁",
	隬: "阝爾",
	隭: "阝需",
	隮: "阝齊",
	隯: "阝壽",
	隴: "阝龍",
	隵: "阝戲",
	隸: "柰隶",
	难: "又隹",
	隿: "弋隹",
	雂: "今隹",
	雃: "开隹",
	雄: "厷隹",
	雅: "牙隹",
	雉: "矢隹",
	雊: "句隹",
	雌: "此隹",
	雎: "且隹",
	雏: "刍隹",
	雑: "杂隹",
	雒: "各隹",
	雓: "余隹",
	雔: "隹隹",
	雕: "周隹",
	雖: "虽隹",
	雘: "丹蒦",
	雛: "芻隹",
	雝: "邕隹",
	雞: "奚隹",
	雡: "翏隹",
	離: "离隹",
	霒: "雲今",
	霕: "雲屯",
	霴: "雲隶",
	霼: "雲氣",
	靅: "雲費",
	靆: "雲逮",
	靉: "雲愛",
	靓: "青见",
	靔: "青气",
	靕: "正青",
	靖: "立青",
	靗: "青光",
	靘: "青色",
	静: "青争",
	靚: "青見",
	靛: "青定",
	靜: "青爭",
	靝: "青氣",
	靤: "面包",
	靦: "面見",
	靧: "面貴",
	靪: "革丁",
	靫: "革叉",
	靬: "革干",
	靭: "革刃",
	靮: "革勺",
	靯: "革土",
	靰: "革兀",
	靱: "革刄",
	靲: "革今",
	靳: "革斤",
	靴: "革化",
	靵: "革丑",
	靶: "革巴",
	靷: "革引",
	靸: "革及",
	靹: "革内",
	靺: "革末",
	靻: "革且",
	靼: "革旦",
	靽: "革半",
	靾: "革世",
	靿: "革幼",
	鞀: "革召",
	鞁: "革皮",
	鞂: "革禾",
	鞃: "革弘",
	鞄: "革包",
	鞅: "革央",
	鞆: "革丙",
	鞇: "革因",
	鞈: "革合",
	鞉: "革兆",
	鞊: "革吉",
	鞋: "革圭",
	鞍: "革安",
	鞎: "革艮",
	鞑: "革达",
	鞒: "革乔",
	鞓: "革呈",
	鞔: "革免",
	鞕: "革更",
	鞖: "革妥",
	鞘: "革肖",
	鞙: "革肙",
	鞚: "革空",
	鞛: "革咅",
	鞜: "革沓",
	鞝: "革尚",
	鞞: "革卑",
	鞟: "革享",
	鞠: "革匊",
	鞡: "革拉",
	鞢: "革枼",
	鞣: "革柔",
	鞥: "革弇",
	鞦: "革秋",
	鞧: "革酋",
	鞨: "革曷",
	鞩: "革削",
	鞫: "革訇",
	鞬: "革建",
	鞭: "革便",
	鞮: "革是",
	鞯: "革荐",
	鞰: "革昷",
	鞱: "革舀",
	鞲: "革冓",
	鞳: "革荅",
	鞵: "革奚",
	鞸: "革畢",
	鞹: "革郭",
	鞺: "革堂",
	鞻: "革婁",
	鞼: "革貴",
	鞽: "革喬",
	鞾: "革華",
	鞿: "革幾",
	韀: "革廌",
	韁: "革畺",
	韂: "革詹",
	韃: "革達",
	韄: "革蒦",
	韆: "革遷",
	韇: "革賣",
	韈: "革蔑",
	韉: "革薦",
	韊: "革蘭",
	韌: "韋刃",
	韍: "韋犮",
	韎: "韋末",
	韐: "韋合",
	韒: "韋肖",
	韔: "韋長",
	韕: "韋享",
	韖: "韋柔",
	韗: "韋軍",
	韘: "韋枼",
	韚: "韋革",
	韜: "韋舀",
	韝: "韋冓",
	韞: "韋昷",
	韟: "韋皋",
	韠: "韋畢",
	韡: "韋華",
	韢: "韋惠",
	韣: "韋蜀",
	韤: "韋蔑",
	韥: "韋賣",
	韧: "韦刃",
	韨: "韦犮",
	韫: "韦昷",
	韬: "韦舀",
	韴: "音帀",
	韵: "音匀",
	韶: "音召",
	韷: "音出",
	韸: "音夆",
	韹: "音皇",
	韺: "音英",
	韻: "音員",
	韼: "音逢",
	韽: "酓音",
	頀: "音蒦",
	頂: "丁頁",
	頃: "匕頁",
	頄: "九頁",
	項: "工頁",
	順: "川頁",
	頇: "干頁",
	須: "彡頁",
	頉: "止頁",
	頊: "王頁",
	頋: "厄頁",
	頌: "公頁",
	頍: "支頁",
	頎: "斤頁",
	頏: "亢頁",
	預: "予頁",
	頑: "元頁",
	頒: "分頁",
	頓: "屯頁",
	頔: "由頁",
	頕: "占頁",
	頖: "半頁",
	頗: "皮頁",
	領: "令頁",
	頙: "正頁",
	頚: "圣頁",
	頛: "耒頁",
	頜: "合頁",
	頝: "交頁",
	頞: "安頁",
	頟: "各頁",
	頠: "危頁",
	頡: "吉頁",
	頢: "舌頁",
	頣: "臣頁",
	頦: "亥頁",
	頨: "羽頁",
	頩: "并頁",
	頪: "米頁",
	頫: "兆頁",
	頬: "夹頁",
	頭: "豆頁",
	頰: "夾頁",
	頱: "寽頁",
	頲: "廷頁",
	頳: "赤頁",
	頵: "君頁",
	頶: "告頁",
	頷: "含頁",
	頸: "巠頁",
	頹: "禿頁",
	頺: "秀頁",
	頻: "步頁",
	頼: "束頁",
	頽: "秃頁",
	顀: "隹頁",
	顁: "定頁",
	顂: "來頁",
	顃: "炎頁",
	顄: "函頁",
	顅: "肩頁",
	顆: "果頁",
	顇: "卒頁",
	顉: "金頁",
	顊: "阜頁",
	顋: "思頁",
	額: "客頁",
	顎: "咢頁",
	顏: "彥頁",
	顐: "軍頁",
	顑: "咸頁",
	顒: "禺頁",
	顓: "耑頁",
	顔: "彦頁",
	顕: "显頁",
	顖: "恖頁",
	顗: "豈頁",
	願: "原頁",
	顙: "桑頁",
	顚: "眞頁",
	顛: "真頁",
	顜: "冓頁",
	顝: "骨頁",
	顟: "翏頁",
	顠: "票頁",
	顡: "豙頁",
	顣: "戚頁",
	顤: "堯頁",
	顥: "景頁",
	顦: "焦頁",
	顧: "雇頁",
	顩: "僉頁",
	顪: "歲頁",
	顫: "亶頁",
	顬: "需頁",
	顭: "夢頁",
	顮: "賓頁",
	顱: "盧頁",
	顳: "聶頁",
	顴: "雚頁",
	顶: "丁页",
	顷: "匕页",
	顸: "干页",
	项: "工页",
	顺: "川页",
	须: "彡页",
	顼: "王页",
	顽: "元页",
	顾: "厄页",
	顿: "屯页",
	颀: "斤页",
	颁: "分页",
	颂: "公页",
	颃: "亢页",
	预: "予页",
	颅: "卢页",
	领: "令页",
	颇: "皮页",
	颉: "吉页",
	颊: "夹页",
	颋: "廷页",
	颌: "合页",
	颏: "亥页",
	频: "步页",
	颓: "秃页",
	颔: "含页",
	颗: "果页",
	颙: "禺页",
	颚: "咢页",
	颛: "耑页",
	颜: "彦页",
	额: "客页",
	颞: "聂页",
	颠: "真页",
	颡: "桑页",
	颢: "景页",
	颤: "亶页",
	颥: "需页",
	颧: "雚页",
	颯: "立風",
	颽: "豈風",
	颿: "馬風",
	飁: "習風",
	飄: "票風",
	飆: "猋風",
	飊: "焱風",
	飌: "雚風",
	飒: "立风",
	飘: "票风",
	飙: "猋风",
	飜: "番飛",
	飡: "冫食",
	飢: "飠几",
	飣: "飠丁",
	飤: "飠人",
	飥: "飠乇",
	飦: "飠干",
	飧: "夕食",
	飨: "乡食",
	飩: "飠屯",
	飪: "飠壬",
	飫: "飠夭",
	飯: "飠反",
	飰: "飠卞",
	飱: "歹食",
	飲: "飠欠",
	飳: "飠主",
	飴: "飠台",
	飵: "飠乍",
	飶: "飠必",
	飷: "飠且",
	飸: "号食",
	飹: "飠卯",
	飼: "飠司",
	飽: "飠包",
	飿: "飠出",
	餀: "飠艾",
	餁: "飠任",
	餂: "飠舌",
	餃: "飠交",
	餄: "飠合",
	餅: "飠并",
	餆: "飠兆",
	餇: "飠同",
	餉: "飠向",
	餌: "飠耳",
	餎: "飠各",
	餏: "飠衣",
	餑: "飠孛",
	餒: "飠妥",
	餓: "飠我",
	餔: "飠甫",
	餕: "飠夋",
	餖: "飠豆",
	餗: "飠束",
	餘: "飠余",
	餙: "飠希",
	餚: "飠肴",
	餛: "飠昆",
	餜: "飠果",
	餝: "飠芳",
	餞: "飠戔",
	餟: "飠叕",
	餠: "飠幷",
	餡: "飠臽",
	餢: "飠咅",
	餣: "飠奄",
	餤: "飠炎",
	餦: "飠長",
	餧: "飠委",
	館: "飠官",
	餪: "飠耎",
	餫: "飠軍",
	餬: "飠胡",
	餭: "飠皇",
	餯: "飠彖",
	餰: "飠衍",
	餱: "飠侯",
	餲: "飠曷",
	餳: "飠昜",
	餴: "飠奔",
	餵: "飠畏",
	餶: "飠骨",
	餷: "飠查",
	餸: "飠送",
	餹: "飠唐",
	餺: "飠尃",
	餻: "飠羔",
	餼: "飠氣",
	餽: "飠鬼",
	餾: "飠留",
	餿: "飠叟",
	饀: "飠舀",
	饁: "飠盍",
	饂: "飠昷",
	饃: "飠莫",
	饄: "飠堂",
	饅: "飠曼",
	饆: "飠畢",
	饇: "飠區",
	饈: "飠羞",
	饉: "飠堇",
	饊: "飠散",
	饋: "飠貴",
	饌: "飠巽",
	饍: "飠善",
	饎: "飠喜",
	饐: "飠壹",
	饑: "飠幾",
	饒: "飠堯",
	饓: "飠棠",
	饖: "飠歲",
	饘: "飠亶",
	饙: "飠賁",
	饚: "飠蓋",
	饛: "飠蒙",
	饝: "飠磨",
	饞: "飠毚",
	饟: "飠襄",
	饠: "飠羅",
	饡: "飠贊",
	饢: "飠囊",
	饤: "饣丁",
	饥: "饣几",
	饦: "饣乇",
	饨: "饣屯",
	饩: "饣气",
	饪: "饣壬",
	饫: "饣夭",
	饭: "饣反",
	饮: "饣欠",
	饯: "饣戋",
	饱: "饣包",
	饲: "饣司",
	饳: "饣出",
	饴: "饣台",
	饵: "饣耳",
	饶: "饣尧",
	饷: "饣向",
	饸: "饣合",
	饹: "饣各",
	饺: "饣交",
	饻: "饣衣",
	饼: "饣并",
	饽: "饣孛",
	饾: "饣豆",
	饿: "饣我",
	馀: "饣余",
	馁: "饣妥",
	馂: "饣夋",
	馃: "饣果",
	馄: "饣昆",
	馅: "饣臽",
	馆: "饣官",
	馇: "饣查",
	馈: "饣贵",
	馉: "饣骨",
	馊: "饣叟",
	馌: "饣盍",
	馍: "饣莫",
	馎: "饣尃",
	馏: "饣留",
	馐: "饣羞",
	馑: "饣堇",
	馒: "饣曼",
	馓: "饣散",
	馔: "饣巽",
	馕: "饣囊",
	馘: "首或",
	馚: "香分",
	馛: "香犮",
	馜: "香尼",
	馝: "香必",
	馞: "香孛",
	馟: "香孚",
	馠: "香含",
	馡: "香非",
	馢: "香戔",
	馣: "香奄",
	馤: "香曷",
	馥: "香复",
	馦: "香兼",
	馩: "香賁",
	馪: "香賓",
	馭: "馬又",
	馮: "冫馬",
	馯: "馬干",
	馰: "馬勺",
	馱: "馬大",
	馲: "馬乇",
	馳: "馬也",
	馴: "馬川",
	馶: "馬支",
	馷: "馬巿",
	馸: "馬斤",
	馹: "馬日",
	馺: "馬及",
	馻: "馬允",
	馼: "馬文",
	馾: "馬冘",
	馿: "馬户",
	駀: "馬尤",
	駁: "馬爻",
	駃: "馬夬",
	駄: "馬太",
	駅: "馬尺",
	駆: "馬区",
	駇: "馬攵",
	駈: "馬丘",
	駉: "馬冋",
	駊: "馬皮",
	駋: "馬召",
	駍: "馬平",
	駎: "馬由",
	駏: "馬巨",
	駐: "馬主",
	駒: "馬句",
	駓: "馬丕",
	駔: "馬且",
	駖: "馬令",
	駘: "馬台",
	駙: "馬付",
	駚: "馬央",
	駛: "馬史",
	駜: "馬必",
	駝: "馬它",
	駟: "馬四",
	駠: "馬卯",
	駢: "馬并",
	駣: "馬兆",
	駤: "馬至",
	駥: "馬戎",
	駧: "馬同",
	駨: "馬旬",
	駩: "馬全",
	駪: "馬先",
	駫: "馬光",
	駬: "馬耳",
	駭: "馬亥",
	駮: "馬交",
	駯: "馬朱",
	駰: "馬因",
	駱: "馬各",
	駲: "馬州",
	駳: "馬延",
	駴: "馬戒",
	駵: "馬丣",
	駶: "馬局",
	駷: "馬束",
	駹: "馬尨",
	駺: "馬良",
	駻: "馬旱",
	駼: "馬余",
	駽: "馬肙",
	駾: "馬兑",
	駿: "馬夋",
	騀: "馬我",
	騁: "馬甹",
	騂: "馬辛",
	騃: "馬矣",
	騄: "馬录",
	騅: "馬隹",
	騆: "馬周",
	騇: "馬舍",
	騈: "馬幷",
	騉: "馬昆",
	騊: "馬匋",
	騋: "馬來",
	騌: "馬宗",
	騍: "馬果",
	騎: "馬奇",
	騏: "馬其",
	騐: "馬念",
	騑: "馬非",
	騔: "馬曷",
	騕: "馬要",
	騗: "扁馬",
	騘: "馬怱",
	騙: "馬扁",
	騚: "馬前",
	騛: "馬飛",
	騜: "馬皇",
	騝: "馬建",
	騞: "馬砉",
	騟: "馬俞",
	騠: "馬是",
	騡: "馬泉",
	騢: "馬叚",
	騤: "馬癸",
	騥: "馬柔",
	騦: "馬思",
	騧: "馬咼",
	騨: "馬単",
	騩: "馬鬼",
	騪: "馬叟",
	騬: "馬乘",
	騮: "馬留",
	騯: "馬旁",
	騰: "月駦",
	騱: "馬奚",
	騲: "馬草",
	騳: "馬馬",
	騴: "馬晏",
	騵: "馬原",
	騶: "馬芻",
	騷: "馬蚤",
	騸: "馬扇",
	騹: "馬堇",
	騻: "馬爽",
	騼: "馬鹿",
	騽: "馬習",
	騾: "馬累",
	騿: "馬章",
	驂: "馬參",
	驃: "馬票",
	驄: "馬悤",
	驅: "馬區",
	驆: "馬畢",
	驈: "馬矞",
	驉: "馬虚",
	驊: "馬華",
	驋: "馬發",
	驌: "馬肅",
	驍: "馬堯",
	驎: "馬粦",
	驏: "馬孱",
	驐: "馬敦",
	驑: "馬畱",
	驒: "馬單",
	驓: "馬曾",
	驔: "馬覃",
	驕: "馬喬",
	驗: "馬僉",
	驙: "馬亶",
	驛: "馬睪",
	驜: "馬業",
	驝: "馬槖",
	驞: "馬賓",
	驟: "馬聚",
	驠: "馬燕",
	驢: "馬盧",
	驣: "馬駦",
	驤: "馬襄",
	驥: "馬冀",
	驦: "馬霜",
	驧: "馬鞠",
	驨: "馬巂",
	驩: "馬雚",
	驪: "馬麗",
	驭: "马又",
	驮: "马大",
	驯: "马川",
	驰: "马也",
	驱: "马区",
	驲: "马日",
	驳: "马爻",
	驴: "马户",
	驵: "马且",
	驶: "马史",
	驷: "马四",
	驸: "马付",
	驹: "马句",
	驺: "马刍",
	驻: "马主",
	驼: "马它",
	骀: "马台",
	骁: "马尧",
	骃: "马因",
	骄: "马乔",
	骅: "马华",
	骆: "马各",
	骇: "马亥",
	骈: "马并",
	骊: "马丽",
	骋: "马甹",
	验: "马佥",
	骍: "马辛",
	骏: "马夋",
	骐: "马其",
	骑: "马奇",
	骒: "马果",
	骓: "马隹",
	骔: "马宗",
	骕: "马肃",
	骖: "马参",
	骗: "马扁",
	骙: "马癸",
	骚: "马蚤",
	骝: "马留",
	骟: "马扇",
	骠: "马票",
	骡: "马累",
	骢: "马悤",
	骣: "马孱",
	骤: "马聚",
	骥: "马冀",
	骦: "马霜",
	骧: "马襄",
	骩: "骨九",
	骪: "骨凡",
	骫: "骨丸",
	骬: "骨于",
	骭: "骨干",
	骮: "骨弋",
	骯: "骨亢",
	骰: "骨殳",
	骱: "骨介",
	骲: "骨包",
	骳: "骨皮",
	骴: "骨此",
	骵: "骨本",
	骶: "骨氐",
	骷: "骨古",
	骸: "骨亥",
	骹: "骨交",
	骺: "骨后",
	骻: "骨夸",
	骼: "骨各",
	骽: "骨妥",
	骾: "骨更",
	骿: "骨并",
	髀: "骨卑",
	髁: "骨果",
	髂: "骨客",
	髃: "骨禺",
	髄: "骨迶",
	髅: "骨娄",
	髆: "骨尃",
	髇: "骨高",
	髈: "骨旁",
	髉: "骨隺",
	髊: "骨差",
	髋: "骨宽",
	髌: "骨宾",
	髍: "骨麻",
	髎: "骨翏",
	髏: "骨婁",
	髐: "骨堯",
	髑: "骨蜀",
	髒: "骨葬",
	髓: "骨遀",
	體: "骨豊",
	髕: "骨賓",
	髖: "骨寬",
	髗: "骨盧",
	髚: "高亢",
	髛: "高尻",
	髜: "高昇",
	髝: "高勞",
	髞: "高喿",
	髟: "镸彡",
	鬴: "鬲甫",
	鬸: "鬲留",
	魂: "云鬼",
	魄: "白鬼",
	魏: "委鬼",
	魗: "壽鬼",
	魛: "魚刀",
	魜: "魚人",
	魝: "魚刂",
	魞: "魚入",
	魟: "魚工",
	魠: "魚乇",
	魡: "魚勺",
	魢: "魚己",
	魣: "魚予",
	魤: "魚化",
	魥: "魚及",
	魦: "魚少",
	魧: "魚亢",
	魨: "魚屯",
	魩: "魚勿",
	魪: "魚介",
	魫: "魚冘",
	魬: "魚反",
	魭: "魚元",
	魮: "魚比",
	魰: "魚文",
	魱: "魚互",
	魲: "魚户",
	魳: "魚帀",
	魴: "魚方",
	魵: "魚分",
	魶: "魚内",
	魷: "魚尤",
	魸: "魚片",
	魹: "魚毛",
	魺: "魚可",
	魻: "魚甲",
	魼: "魚去",
	魽: "魚甘",
	魾: "魚丕",
	魿: "魚令",
	鮀: "魚它",
	鮁: "魚犮",
	鮂: "魚囚",
	鮃: "魚平",
	鮄: "魚弗",
	鮅: "魚必",
	鮇: "魚未",
	鮈: "魚句",
	鮉: "魚召",
	鮊: "魚白",
	鮋: "魚由",
	鮌: "魚玄",
	鮍: "魚皮",
	鮎: "魚占",
	鮏: "魚生",
	鮐: "魚台",
	鮑: "魚包",
	鮒: "魚付",
	鮓: "魚乍",
	鮔: "魚巨",
	鮕: "魚古",
	鮖: "魚石",
	鮗: "魚冬",
	鮘: "魚代",
	鮙: "魚羽",
	鮚: "魚吉",
	鮛: "魚尗",
	鮜: "魚后",
	鮞: "魚而",
	鮟: "魚安",
	鮠: "魚危",
	鮡: "魚兆",
	鮢: "魚朱",
	鮣: "魚印",
	鮥: "魚各",
	鮦: "魚同",
	鮧: "魚夷",
	鮨: "魚旨",
	鮩: "魚并",
	鮪: "魚有",
	鮫: "魚交",
	鮬: "魚夸",
	鮭: "魚圭",
	鮮: "魚羊",
	鮯: "魚合",
	鮰: "魚回",
	鮱: "魚老",
	鮲: "魚伏",
	鮳: "魚考",
	鮴: "魚休",
	鮵: "魚兑",
	鮶: "魚君",
	鮷: "魚弟",
	鮸: "魚免",
	鮹: "魚肖",
	鮻: "魚夋",
	鮽: "魚余",
	鮾: "魚妥",
	鯀: "魚系",
	鯁: "魚更",
	鯂: "魚酉",
	鯃: "魚吾",
	鯄: "魚求",
	鯅: "魚延",
	鯆: "魚甫",
	鯇: "魚完",
	鯉: "魚里",
	鯋: "魚沙",
	鯌: "魚告",
	鯎: "魚成",
	鯏: "魚利",
	鯐: "魚走",
	鯑: "魚希",
	鯒: "魚甬",
	鯓: "魚身",
	鯔: "魚甾",
	鯕: "魚其",
	鯖: "魚青",
	鯘: "魚委",
	鯙: "魚享",
	鯚: "魚季",
	鯛: "魚周",
	鯜: "魚妾",
	鯝: "魚固",
	鯞: "魚帚",
	鯟: "魚東",
	鯠: "魚來",
	鯡: "魚非",
	鯢: "魚兒",
	鯣: "魚易",
	鯤: "魚昆",
	鯥: "魚坴",
	鯦: "魚咎",
	鯧: "魚昌",
	鯨: "魚京",
	鯩: "魚侖",
	鯪: "魚夌",
	鯫: "魚取",
	鯭: "魚孟",
	鯮: "魚宗",
	鯯: "魚制",
	鯰: "魚念",
	鯱: "魚虎",
	鯲: "魚於",
	鯳: "魚底",
	鯴: "魚虱",
	鯵: "魚参",
	鯶: "魚軍",
	鯷: "魚是",
	鯸: "魚侯",
	鯹: "魚星",
	鯺: "魚者",
	鯻: "魚剌",
	鯽: "魚即",
	鯾: "魚便",
	鯿: "魚扁",
	鰀: "魚爰",
	鰁: "魚泉",
	鰂: "魚則",
	鰃: "魚畏",
	鰄: "魚威",
	鰅: "魚禺",
	鰆: "魚春",
	鰇: "魚柔",
	鰈: "魚枼",
	鰉: "魚皇",
	鰊: "魚柬",
	鰋: "魚匽",
	鰌: "魚酋",
	鰍: "魚秋",
	鰎: "魚建",
	鰏: "魚畐",
	鰐: "魚咢",
	鰑: "魚昜",
	鰒: "魚复",
	鰓: "魚思",
	鰔: "魚咸",
	鰕: "魚叚",
	鰗: "魚胡",
	鰘: "魚室",
	鰙: "魚若",
	鰚: "魚宣",
	鰛: "魚昷",
	鰜: "魚兼",
	鰝: "魚高",
	鰞: "魚烏",
	鰟: "魚旁",
	鰠: "魚蚤",
	鰡: "魚留",
	鰢: "魚馬",
	鰣: "魚時",
	鰤: "魚師",
	鰥: "魚眔",
	鰦: "魚兹",
	鰧: "月鮝",
	鰪: "魚盍",
	鰫: "魚容",
	鰬: "魚虔",
	鰭: "魚耆",
	鰯: "魚弱",
	鰰: "魚神",
	鰱: "魚連",
	鰳: "魚勒",
	鰶: "魚祭",
	鰷: "魚條",
	鰸: "魚區",
	鰹: "魚堅",
	鰺: "魚參",
	鰻: "魚曼",
	鰼: "魚習",
	鰽: "魚曹",
	鰾: "魚票",
	鰿: "魚責",
	鱁: "魚逐",
	鱂: "魚將",
	鱃: "魚羞",
	鱄: "魚專",
	鱅: "魚庸",
	鱆: "魚章",
	鱇: "魚康",
	鱈: "魚雪",
	鱊: "魚矞",
	鱋: "魚虚",
	鱌: "魚象",
	鱍: "魚發",
	鱎: "魚喬",
	鱏: "魚覃",
	鱐: "魚肅",
	鱑: "魚黄",
	鱒: "魚尊",
	鱓: "魚單",
	鱔: "魚善",
	鱕: "魚番",
	鱖: "魚厥",
	鱗: "魚粦",
	鱘: "魚尋",
	鱙: "魚堯",
	鱚: "魚喜",
	鱛: "魚曾",
	鱜: "魚鄉",
	鱝: "魚賁",
	鱞: "魚睘",
	鱠: "魚會",
	鱡: "魚賊",
	鱢: "魚喿",
	鱣: "魚亶",
	鱤: "魚感",
	鱥: "魚歲",
	鱦: "魚黽",
	鱧: "魚豊",
	鱨: "魚嘗",
	鱩: "魚雷",
	鱪: "魚暑",
	鱫: "魚愛",
	鱬: "魚需",
	鱭: "魚齊",
	鱮: "魚與",
	鱯: "魚蒦",
	鱰: "魚署",
	鱱: "魚厲",
	鱲: "魚巤",
	鱳: "魚樂",
	鱴: "魚蔑",
	鱵: "魚箴",
	鱶: "魚養",
	鱷: "魚噩",
	鱸: "魚盧",
	鱹: "魚雚",
	鱺: "魚麗",
	鱽: "鱼刀",
	鱾: "鱼己",
	鱿: "鱼尤",
	鲀: "鱼屯",
	鲂: "鱼方",
	鲃: "鱼巴",
	鲄: "鱼可",
	鲅: "鱼犮",
	鲆: "鱼平",
	鲇: "鱼占",
	鲈: "鱼卢",
	鲉: "鱼由",
	鲊: "鱼乍",
	鲋: "鱼付",
	鲌: "鱼白",
	鲍: "鱼包",
	鲏: "鱼皮",
	鲐: "鱼台",
	鲑: "鱼圭",
	鲒: "鱼吉",
	鲓: "鱼考",
	鲔: "鱼有",
	鲕: "鱼而",
	鲖: "鱼同",
	鲗: "鱼则",
	鲘: "鱼后",
	鲙: "鱼会",
	鲚: "鱼齐",
	鲛: "鱼交",
	鲜: "鱼羊",
	鲟: "鱼寻",
	鲠: "鱼更",
	鲡: "鱼丽",
	鲢: "鱼连",
	鲣: "鱼坚",
	鲤: "鱼里",
	鲥: "鱼时",
	鲦: "鱼条",
	鲧: "鱼系",
	鲩: "鱼完",
	鲪: "鱼君",
	鲫: "鱼即",
	鲬: "鱼甬",
	鲭: "鱼青",
	鲮: "鱼夌",
	鲯: "鱼其",
	鲰: "鱼取",
	鲱: "鱼非",
	鲲: "鱼昆",
	鲳: "鱼昌",
	鲴: "鱼固",
	鲵: "鱼兒",
	鲶: "鱼念",
	鲷: "鱼周",
	鲸: "鱼京",
	鲹: "鱼参",
	鲺: "鱼虱",
	鲻: "鱼甾",
	鲼: "鱼贲",
	鲽: "鱼枼",
	鲾: "鱼畐",
	鲿: "鱼尝",
	鳀: "鱼是",
	鳁: "鱼昷",
	鳂: "鱼畏",
	鳃: "鱼思",
	鳄: "鱼咢",
	鳅: "鱼秋",
	鳆: "鱼复",
	鳇: "鱼皇",
	鳈: "鱼泉",
	鳉: "鱼将",
	鳊: "鱼扁",
	鳋: "鱼蚤",
	鳍: "鱼耆",
	鳏: "鱼眔",
	鳑: "鱼旁",
	鳒: "鱼兼",
	鳓: "鱼勒",
	鳔: "鱼票",
	鳕: "鱼雪",
	鳗: "鱼曼",
	鳙: "鱼庸",
	鳚: "鱼尉",
	鳛: "鱼習",
	鳜: "鱼厥",
	鳝: "鱼善",
	鳞: "鱼粦",
	鳟: "鱼尊",
	鳠: "鱼蒦",
	鳡: "鱼感",
	鳢: "鱼豊",
	鳣: "鱼亶",
	鳤: "鱼管",
	鳦: "鳥乙",
	鳨: "鳥力",
	鳩: "九鳥",
	鳪: "卜鳥",
	鳭: "刀鳥",
	鳮: "又鳥",
	鳰: "入鳥",
	鳱: "干鳥",
	鳲: "尸鳥",
	鳴: "口鳥",
	鳵: "卂鳥",
	鳷: "支鳥",
	鳹: "今鳥",
	鳺: "夫鳥",
	鳻: "分鳥",
	鳼: "文鳥",
	鳽: "开鳥",
	鳾: "帀鳥",
	鳿: "王鳥",
	鴀: "不鳥",
	鴁: "夭鳥",
	鴂: "夬鳥",
	鴃: "鳥夬",
	鴄: "匹鳥",
	鴅: "丹鳥",
	鴆: "冘鳥",
	鴉: "牙鳥",
	鴊: "正鳥",
	鴋: "方鳥",
	鴎: "区鳥",
	鴏: "鳥代",
	鴒: "令鳥",
	鴓: "必鳥",
	鴔: "乏鳥",
	鴕: "鳥它",
	鴖: "民鳥",
	鴗: "立鳥",
	鴘: "弁鳥",
	鴙: "矢鳥",
	鴚: "鳥可",
	鴝: "句鳥",
	鴞: "号鳥",
	鴟: "氐鳥",
	鴠: "旦鳥",
	鴡: "且鳥",
	鴢: "幼鳥",
	鴣: "古鳥",
	鴤: "鳥冬",
	鴥: "鳥穴",
	鴧: "宂鳥",
	鴨: "甲鳥",
	鴩: "失鳥",
	鴪: "穴鳥",
	鴫: "田鳥",
	鴮: "夸鳥",
	鴯: "而鳥",
	鴰: "舌鳥",
	鴱: "艾鳥",
	鴲: "旨鳥",
	鴳: "安鳥",
	鴴: "行鳥",
	鴵: "兆鳥",
	鴶: "吉鳥",
	鴸: "朱鳥",
	鴹: "羊鳥",
	鴺: "夷鳥",
	鴼: "鳥各",
	鴾: "牟鳥",
	鴿: "合鳥",
	鵁: "交鳥",
	鵂: "休鳥",
	鵃: "舟鳥",
	鵄: "至鳥",
	鵅: "各鳥",
	鵇: "年鳥",
	鵈: "耳鳥",
	鵊: "夾鳥",
	鵋: "忌鳥",
	鵌: "鳥余",
	鵍: "完鳥",
	鵎: "妥鳥",
	鵏: "甫鳥",
	鵐: "巫鳥",
	鵑: "肙鳥",
	鵒: "谷鳥",
	鵓: "孛鳥",
	鵔: "鳥夋",
	鵕: "夋鳥",
	鵗: "希鳥",
	鵘: "君鳥",
	鵙: "貝鳥",
	鵚: "秃鳥",
	鵛: "巠鳥",
	鵜: "弟鳥",
	鵝: "我鳥",
	鵠: "告鳥",
	鵡: "武鳥",
	鵢: "身鳥",
	鵣: "束鳥",
	鵤: "角鳥",
	鵦: "鳥录",
	鵧: "并鳥",
	鵨: "舍鳥",
	鵩: "服鳥",
	鵪: "奄鳥",
	鵫: "卓鳥",
	鵬: "朋鳥",
	鵭: "金鳥",
	鵮: "臽鳥",
	鵯: "卑鳥",
	鵰: "周鳥",
	鵱: "坴鳥",
	鵲: "昔鳥",
	鵳: "肩鳥",
	鵴: "匊鳥",
	鵵: "兔鳥",
	鵶: "亞鳥",
	鵷: "宛鳥",
	鵸: "奇鳥",
	鵺: "夜鳥",
	鵻: "鳥隹",
	鵼: "空鳥",
	鵽: "叕鳥",
	鵾: "昆鳥",
	鵿: "鳥昇",
	鶀: "鳥其",
	鶁: "京鳥",
	鶂: "兒鳥",
	鶃: "鳥兒",
	鶄: "青鳥",
	鶅: "甾鳥",
	鶆: "來鳥",
	鶇: "東鳥",
	鶈: "妻鳥",
	鶉: "享鳥",
	鶊: "庚鳥",
	鶋: "居鳥",
	鶌: "屈鳥",
	鶍: "易鳥",
	鶎: "宗鳥",
	鶐: "述鳥",
	鶒: "勅鳥",
	鶓: "苗鳥",
	鶔: "柔鳥",
	鶕: "音鳥",
	鶘: "胡鳥",
	鶙: "帝鳥",
	鶚: "咢鳥",
	鶛: "皆鳥",
	鶜: "茅鳥",
	鶝: "畐鳥",
	鶞: "盾鳥",
	鶟: "突鳥",
	鶠: "匽鳥",
	鶡: "曷鳥",
	鶢: "爰鳥",
	鶣: "扁鳥",
	鶤: "軍鳥",
	鶥: "眉鳥",
	鶧: "英鳥",
	鶨: "彖鳥",
	鶪: "狊鳥",
	鶫: "柬鳥",
	鶬: "倉鳥",
	鶮: "高鳥",
	鶰: "員鳥",
	鶲: "翁鳥",
	鶳: "鳥師",
	鶴: "隺鳥",
	鶵: "芻鳥",
	鶶: "唐鳥",
	鶷: "害鳥",
	鶸: "弱鳥",
	鶹: "留鳥",
	鶺: "脊鳥",
	鶻: "骨鳥",
	鶼: "兼鳥",
	鶽: "隼鳥",
	鷀: "兹鳥",
	鷁: "益鳥",
	鷃: "晏鳥",
	鷄: "奚鳥",
	鷅: "栗鳥",
	鷆: "眞鳥",
	鷈: "鳥虒",
	鷉: "虒鳥",
	鷊: "鬲鳥",
	鷋: "荼鳥",
	鷌: "鳥馬",
	鷍: "臬鳥",
	鷎: "皋鳥",
	鷏: "真鳥",
	鷐: "晨鳥",
	鷑: "笠鳥",
	鷒: "專鳥",
	鷓: "庶鳥",
	鷗: "區鳥",
	鷚: "翏鳥",
	鷛: "庸鳥",
	鷜: "婁鳥",
	鷝: "畢鳥",
	鷞: "爽鳥",
	鷠: "魚鳥",
	鷡: "無鳥",
	鷣: "覃鳥",
	鷤: "單鳥",
	鷦: "焦鳥",
	鷧: "壹鳥",
	鷨: "華鳥",
	鷫: "肅鳥",
	鷬: "黄鳥",
	鷭: "番鳥",
	鷮: "喬鳥",
	鷯: "尞鳥",
	鷱: "臯鳥",
	鷳: "閒鳥",
	鷴: "閑鳥",
	鷵: "屠鳥",
	鷶: "買鳥",
	鷷: "尊鳥",
	鷸: "矞鳥",
	鷻: "鳥敦",
	鷼: "間鳥",
	鷾: "意鳥",
	鸀: "蜀鳥",
	鸂: "溪鳥",
	鸃: "鳥義",
	鸅: "睪鳥",
	鸆: "虞鳥",
	鸇: "亶鳥",
	鸈: "業鳥",
	鸊: "辟鳥",
	鸋: "寧鳥",
	鸌: "鳥蒦",
	鸍: "爾鳥",
	鸏: "蒙鳥",
	鸐: "翟鳥",
	鸓: "畾鳥",
	鸔: "鳥暴",
	鸕: "盧鳥",
	鸖: "霍鳥",
	鸘: "霜鳥",
	鸙: "龠鳥",
	鸚: "嬰鳥",
	鸛: "雚鳥",
	鸜: "瞿鳥",
	鸝: "麗鳥",
	鸠: "九鸟",
	鸡: "又鸟",
	鸣: "口鸟",
	鸤: "尸鸟",
	鸥: "区鸟",
	鸦: "牙鸟",
	鸧: "仓鸟",
	鸩: "冘鸟",
	鸪: "古鸟",
	鸫: "东鸟",
	鸬: "卢鸟",
	鸭: "甲鸟",
	鸮: "号鸟",
	鸰: "令鸟",
	鸱: "氐鸟",
	鸲: "句鸟",
	鸵: "鸟它",
	鸸: "而鸟",
	鸹: "舌鸟",
	鸺: "休鸟",
	鸻: "行鸟",
	鸼: "舟鸟",
	鸽: "合鸟",
	鸿: "江鸟",
	鹀: "巫鸟",
	鹁: "孛鸟",
	鹂: "丽鸟",
	鹃: "肙鸟",
	鹄: "告鸟",
	鹅: "我鸟",
	鹆: "谷鸟",
	鹇: "闲鸟",
	鹈: "弟鸟",
	鹉: "武鸟",
	鹊: "昔鸟",
	鹋: "苗鸟",
	鹌: "奄鸟",
	鹍: "昆鸟",
	鹎: "卑鸟",
	鹏: "朋鸟",
	鹐: "臽鸟",
	鹑: "享鸟",
	鹒: "庚鸟",
	鹓: "宛鸟",
	鹔: "肃鸟",
	鹕: "胡鸟",
	鹖: "曷鸟",
	鹗: "咢鸟",
	鹘: "骨鸟",
	鹚: "兹鸟",
	鹛: "眉鸟",
	鹝: "鬲鸟",
	鹟: "翁鸟",
	鹠: "留鸟",
	鹡: "脊鸟",
	鹢: "益鸟",
	鹣: "兼鸟",
	鹤: "隺鸟",
	鹦: "婴鸟",
	鹧: "庶鸟",
	鹨: "翏鸟",
	鹩: "尞鸟",
	鹪: "焦鸟",
	鹬: "矞鸟",
	鹮: "睘鸟",
	鹯: "亶鸟",
	鹱: "鸟蒦",
	鹲: "蒙鸟",
	鹳: "雚鸟",
	鹴: "霜鸟",
	鹶: "鹵今",
	鹷: "鹵令",
	鹹: "鹵咸",
	鹺: "鹵差",
	鹻: "鹵兼",
	鹼: "鹵僉",
	鹾: "卤差",
	麒: "鹿其",
	麟: "鹿粦",
	麣: "鹿嚴",
	麧: "麥乞",
	麨: "麥少",
	麩: "麥夫",
	麪: "麥丏",
	麫: "麥丐",
	麬: "麥皮",
	麭: "麥包",
	麮: "麥去",
	麯: "麥曲",
	麰: "麥牟",
	麱: "麥甫",
	麲: "麥見",
	麳: "麥來",
	麴: "麥匊",
	麵: "麥面",
	麶: "麥离",
	麷: "麥豐",
	麸: "麦夫",
	麹: "麦匊",
	麺: "麦面",
	黅: "黄今",
	黆: "黄冘",
	黇: "黄占",
	黈: "黄主",
	黊: "黄圭",
	黏: "黍占",
	黐: "黍离",
	黓: "黑弋",
	黔: "黑今",
	黕: "黑冘",
	黖: "黑旡",
	黗: "黑屯",
	默: "黑犬",
	黚: "黑甘",
	黜: "黑出",
	黝: "黑幼",
	點: "黑占",
	黟: "黑多",
	黠: "黑吉",
	黢: "黑夋",
	黣: "黑每",
	黤: "黑奄",
	黥: "黑京",
	黦: "黑宛",
	黩: "黑卖",
	黪: "黑参",
	黫: "黑垔",
	黬: "黑咸",
	黭: "黑弇",
	黮: "黑甚",
	黯: "黑音",
	黰: "黑真",
	黲: "黑參",
	黵: "黑詹",
	黷: "黑賣",
	黸: "黑盧",
	黺: "黹分",
	黻: "黹犮",
	黼: "黹甫",
	鼆: "冥黽",
	鼓: "壴支",
	鼔: "壴攴",
	鼢: "鼠分",
	鼣: "鼠犬",
	鼤: "鼠文",
	鼥: "鼠犮",
	鼦: "鼠召",
	鼧: "鼠它",
	鼨: "鼠冬",
	鼩: "鼠句",
	鼪: "鼠生",
	鼫: "鼠石",
	鼬: "鼠由",
	鼭: "鼠寺",
	鼮: "鼠廷",
	鼯: "鼠吾",
	鼰: "鼠貝",
	鼱: "鼠青",
	鼲: "鼠軍",
	鼳: "鼠狊",
	鼴: "鼠匽",
	鼵: "鼠突",
	鼶: "鼠虒",
	鼷: "鼠奚",
	鼸: "鼠兼",
	鼹: "鼠晏",
	鼺: "鼠畾",
	鼼: "鼻丩",
	鼽: "鼻九",
	鼾: "鼻干",
	鼿: "鼻兀",
	齀: "鼻瓦",
	齁: "鼻句",
	齂: "鼻隶",
	齃: "鼻曷",
	齄: "鼻查",
	齅: "鼻臭",
	齆: "鼻邕",
	齇: "鼻虘",
	齈: "鼻農",
	齉: "鼻囊",
	齓: "齒乚",
	齔: "齒匕",
	齕: "齒乞",
	齖: "齒牙",
	齗: "齒斤",
	齘: "齒介",
	齙: "齒包",
	齚: "齒乍",
	齛: "齒世",
	齜: "齒此",
	齝: "齒台",
	齞: "齒只",
	齟: "齒且",
	齠: "齒召",
	齡: "齒令",
	齢: "歯令",
	齣: "齒句",
	齥: "齒曳",
	齦: "齒艮",
	齨: "齒臼",
	齩: "齒交",
	齪: "齒足",
	齫: "齒困",
	齬: "齒吾",
	齭: "齒所",
	齮: "齒奇",
	齯: "齒兒",
	齰: "齒昔",
	齱: "齒取",
	齲: "齒禹",
	齳: "齒軍",
	齴: "齒彦",
	齵: "齒禺",
	齶: "齒咢",
	齷: "齒屋",
	齸: "齒益",
	齺: "齒芻",
	齻: "齒真",
	齼: "齒楚",
	齽: "齒禁",
	龀: "齿匕",
	龁: "齿乞",
	龂: "齿斤",
	龃: "齿且",
	龄: "齿令",
	龅: "齿包",
	龆: "齿召",
	龇: "齿此",
	龈: "齿艮",
	龉: "齿吾",
	龊: "齿足",
	龋: "齿禹",
	龌: "齿屋",
	龓: "有龍",
	龖: "龍龍",
	龝: "禾龜",
	龡: "龠欠",
	龢: "龠禾",
	龣: "龠录",
	龤: "龠皆",
	龥: "龠頁",
	龦: "火常",
	龧: "日暑",
	龬: "金罔",
	龭: "馬異",
	龮: "馬翼",
	龯: "金戊",
	龲: "金庫",
	龳: "坴坴",
	龼: "土睿",
	龽: "火魚",
	龾: "皮匊",
	鿁: "言共",
	鿂: "車鳥",
	鿅: "礻零",
	鿆: "礻友",
	鿇: "亻争",
	鿈: "彳尺",
	鿉: "彳五",
	鿋: "衤喜",
	鿌: "氵亰",
	鿍: "土夅",
	鿎: "石达",
	鿏: "钅麦",
	鿐: "魚酒",
	鿑: "冫玉",
	鿔: "钅哥",
	鿕: "鱼丹",
	鿘: "伊合",
	鿙: "伊克",
	鿚: "伊格",
	鿛: "凌爾",
	鿜: "拉爾",
	鿝: "楞爾",
	鿞: "列爾",
	鿟: "利尔",
	鿠: "利爾",
	鿡: "羅爾",
	鿢: "耶合",
	鿣: "耶克",
	鿤: "耶格",
	鿥: "郎爾",
	鿦: "英微",
	鿧: "隆爾",
	鿨: "雷爾",
	鿩: "魯爾",
	鿬: "石田",
	鿭: "钅尔"
}, d = {
	丁: "一亅",
	丄: "丨一",
	丅: "一丨",
	丆: "一丿",
	三: "一二",
	下: "一卜",
	丒: "刃一",
	丕: "不一",
	丙: "一内",
	丛: "从一",
	丞: "氶一",
	丟: "王厶",
	丠: "北一",
	丢: "丿去",
	个: "人丨",
	丫: "丷丨",
	主: "亠土",
	乇: "丿七",
	么: "丿厶",
	乊: "丿丷",
	乏: "丿之",
	乒: "丘丿",
	乓: "丘丶",
	乤: "下乙",
	乥: "乊乙",
	乧: "斗乙",
	乫: "加乙",
	乬: "巨乙",
	乭: "石乙",
	乮: "卯乙",
	乯: "乎乙",
	买: "乛头",
	乲: "次乙",
	乴: "折乙",
	乶: "甫乙",
	乷: "沙乙",
	乺: "所乙",
	乻: "於乙",
	乼: "注乙",
	乽: "者乙",
	了: "乛亅",
	二: "一一",
	亍: "一丁",
	亏: "一丂",
	云: "二厶",
	互: "一彑",
	亓: "一丌",
	亖: "二二",
	亗: "山二",
	亘: "一旦",
	亚: "一业",
	些: "此二",
	亝: "厽二",
	亠: "丶一",
	亢: "亠几",
	亣: "丶丌",
	交: "亠父",
	亩: "亠田",
	亪: "亦乁",
	亱: "亠但",
	亲: "立朩",
	亹: "亠舋",
	亼: "人一",
	亽: "人丶",
	仌: "人人",
	仐: "人十",
	仑: "人匕",
	仚: "人山",
	仝: "人工",
	令: "亽龴",
	仧: "上人",
	企: "人止",
	众: "人从",
	会: "人云",
	佘: "人示",
	余: "亼朩",
	佱: "亼正",
	侌: "今云",
	俞: "亼刖",
	僰: "棘人",
	兀: "一儿",
	允: "厶儿",
	元: "一兀",
	兄: "口儿",
	兇: "凶儿",
	兊: "公儿",
	克: "古儿",
	兌: "八兄",
	児: "旧儿",
	兑: "丷兄",
	兒: "臼儿",
	兕: "凹儿",
	兖: "亠兊",
	兗: "亠兌",
	党: "龸兄",
	全: "人王",
	公: "八厶",
	六: "亠八",
	兮: "八丂",
	兰: "丷三",
	共: "龷八",
	兲: "王八",
	关: "丷天",
	兵: "丘八",
	兺: "分叱",
	兾: "丷異",
	冀: "北異",
	再: "一冉",
	冐: "冃月",
	冑: "由冃",
	冒: "冃目",
	冔: "冃吁",
	冕: "冃免",
	冗: "冖几",
	写: "冖与",
	冚: "冖山",
	军: "冖车",
	冝: "冖且",
	冞: "冖米",
	冟: "冖皀",
	冢: "冖豖",
	冣: "冖取",
	冤: "冖兔",
	冥: "冖昗",
	冧: "冖林",
	冨: "冖畐",
	冩: "冖舄",
	冪: "冖幕",
	凖: "准十",
	凢: "丿几",
	凣: "丨几",
	凭: "任几",
	凳: "登几",
	凴: "馮几",
	出: "屮凵",
	分: "八刀",
	券: "龹刀",
	剪: "前刀",
	劈: "辟刀",
	务: "夂力",
	劣: "少力",
	努: "奴力",
	劵: "龹力",
	劽: "列力",
	势: "执力",
	勇: "甬力",
	勗: "日助",
	募: "莫力",
	勢: "埶力",
	勥: "強力",
	勲: "動灬",
	勶: "徹力",
	千: "丿十",
	卆: "九十",
	升: "丿廾",
	卉: "十廾",
	卋: "十廿",
	华: "化十",
	卖: "十买",
	卞: "丶下",
	卡: "上卜",
	厁: "斜卩",
	厹: "九厶",
	厺: "大厶",
	去: "土厶",
	厼: "厶小",
	厽: "厶厸",
	县: "且厶",
	叐: "丿友",
	叒: "又双",
	叕: "双双",
	变: "亦又",
	叠: "叒冝",
	叢: "丵取",
	古: "十口",
	另: "口力",
	叧: "口刀",
	只: "口八",
	召: "刀口",
	台: "厶口",
	叴: "九口",
	号: "口丂",
	叾: "了口",
	吂: "亡口",
	各: "夂口",
	合: "亼口",
	吉: "士口",
	吊: "口巾",
	名: "夕口",
	吕: "口口",
	吝: "文口",
	吞: "天口",
	吢: "心口",
	否: "不口",
	含: "今口",
	吴: "口天",
	吾: "五口",
	吿: "牛口",
	呄: "乃古",
	呆: "口木",
	呇: "水口",
	呈: "口王",
	呑: "夭口",
	员: "口贝",
	呙: "口内",
	呰: "此口",
	命: "亼叩",
	咅: "立口",
	咎: "处口",
	咒: "吅几",
	咠: "口耳",
	咢: "吅亏",
	咨: "次口",
	品: "口吅",
	哛: "叱分",
	員: "口貝",
	哥: "可可",
	哭: "吅犬",
	哲: "折口",
	哿: "加可",
	唜: "末叱",
	唟: "去叱",
	售: "隹口",
	啙: "此吅",
	啠: "斦口",
	啻: "帝口",
	喌: "吅州",
	喜: "壴口",
	喣: "呴灬",
	喦: "品山",
	喬: "呑冋",
	喸: "甫叱",
	喿: "品木",
	嗧: "加侖",
	嗭: "直叱",
	嗸: "敖口",
	嘉: "壴加",
	嘦: "只要",
	噕: "爲口",
	噵: "道口",
	嚞: "吉喆",
	嚮: "鄉向",
	土: "十一",
	圣: "又土",
	圥: "土儿",
	圭: "土土",
	圶: "大土",
	圼: "日土",
	坌: "分土",
	坒: "比土",
	坓: "井土",
	坔: "水土",
	坕: "爪土",
	坖: "元土",
	坠: "队土",
	坣: "龸土",
	坴: "圥土",
	垄: "龙土",
	垈: "代土",
	型: "刑土",
	垐: "次土",
	垒: "厽土",
	垔: "覀土",
	垕: "后土",
	垡: "伐土",
	垦: "艮土",
	垩: "亚土",
	垫: "执土",
	垼: "役土",
	垽: "沂土",
	埅: "防土",
	埊: "汖土",
	埑: "折土",
	埜: "林土",
	基: "其土",
	埾: "取土",
	埿: "泥土",
	堅: "臤土",
	堊: "亞土",
	堏: "枋土",
	堑: "斩土",
	堕: "陏土",
	堡: "保土",
	堥: "敄土",
	堯: "垚兀",
	堲: "即土",
	堻: "津土",
	堼: "封土",
	塈: "既土",
	塑: "朔土",
	塗: "涂土",
	塣: "浧土",
	塦: "陣土",
	塰: "海土",
	塱: "朗土",
	塹: "斬土",
	塾: "孰土",
	墅: "野土",
	墊: "執土",
	墍: "旣土",
	墏: "將土",
	墓: "莫土",
	墜: "隊土",
	墨: "黑土",
	墪: "敦土",
	墮: "隋土",
	墯: "惰土",
	墾: "貇土",
	壁: "辟土",
	壂: "殿土",
	壄: "楙土",
	壅: "雍土",
	壍: "漸土",
	壐: "爾土",
	壓: "厭土",
	壘: "畾土",
	壟: "龍土",
	士: "十一",
	壬: "丿士",
	壳: "士冗",
	壴: "十豆",
	夆: "夂丰",
	备: "夂田",
	夈: "夂米",
	変: "亦夂",
	夎: "坐夂",
	夔: "丷夒",
	多: "夕夕",
	夛: "彐夕",
	夞: "外叱",
	夤: "夕寅",
	天: "一大",
	夭: "丿大",
	夯: "大力",
	夲: "大十",
	夳: "大二",
	夵: "大小",
	夸: "大亏",
	夺: "大寸",
	夻: "大口",
	夼: "大川",
	夽: "大云",
	夿: "大巴",
	奀: "不大",
	奁: "大区",
	奃: "大氐",
	奄: "大电",
	奅: "大卯",
	奆: "大巨",
	奇: "大可",
	奈: "大示",
	奋: "大田",
	奌: "占大",
	奎: "大圭",
	奒: "大亥",
	奓: "大多",
	奔: "大卉",
	奕: "亦大",
	套: "大镸",
	奘: "壯大",
	奙: "厶奄",
	奛: "大明",
	奜: "非大",
	奝: "大周",
	奞: "大隹",
	奟: "大朋",
	奠: "酋大",
	奢: "大者",
	奣: "天明",
	奤: "大面",
	奦: "務大",
	奨: "将大",
	奩: "大區",
	奪: "奞寸",
	奫: "大淵",
	奬: "將大",
	奮: "奞田",
	奯: "大歲",
	妄: "亡女",
	妟: "日女",
	妥: "爫女",
	妴: "夗女",
	妾: "立女",
	妿: "加女",
	姇: "付女",
	委: "禾女",
	姕: "此女",
	姦: "女奻",
	姭: "劦女",
	姴: "列女",
	姿: "次女",
	娄: "米女",
	娈: "亦女",
	娎: "折女",
	娑: "沙女",
	娤: "壯女",
	娶: "取女",
	娿: "阿女",
	婆: "波女",
	婓: "非女",
	婜: "臤女",
	婪: "林女",
	婱: "弦女",
	婺: "敄女",
	媐: "巸女",
	媝: "秋女",
	媭: "须女",
	媻: "般女",
	嫑: "不要",
	嫛: "殹女",
	嫢: "規女",
	嫯: "敖女",
	嫳: "敝女",
	嬃: "須女",
	嬖: "辟女",
	嬜: "歆女",
	嬰: "賏女",
	嬱: "漸女",
	孁: "霝女",
	孕: "乃子",
	字: "宀子",
	孚: "爫子",
	孞: "子心",
	孟: "子皿",
	孠: "司子",
	季: "禾子",
	孥: "奴子",
	孧: "幼子",
	孨: "子孖",
	孪: "亦子",
	孬: "不好",
	孯: "臤子",
	孳: "兹子",
	孴: "孨日",
	孶: "茲子",
	孹: "辟子",
	孼: "辥子",
	孽: "薛子",
	宁: "宀丁",
	宂: "宀几",
	它: "宀匕",
	宄: "宀九",
	宅: "宀乇",
	宆: "宀弓",
	宇: "宀于",
	守: "宀寸",
	安: "宀女",
	宊: "宀犬",
	宋: "宀木",
	完: "宀元",
	宍: "宀六",
	宎: "宀夭",
	宏: "宀厷",
	宐: "宀彑",
	宑: "宀井",
	宒: "宀毛",
	宓: "宀必",
	宔: "宀主",
	宕: "宀石",
	宖: "宀弘",
	宗: "宀示",
	宙: "宀由",
	宛: "宀夗",
	宜: "宀且",
	宝: "宀玉",
	实: "宀头",
	宠: "宀龙",
	审: "宀申",
	客: "宀各",
	宣: "宀亘",
	室: "宀至",
	宥: "宀有",
	宦: "宀臣",
	宨: "宀兆",
	宩: "宀米",
	宪: "宀先",
	宫: "宀吕",
	宬: "宀成",
	宭: "宀君",
	宮: "宀呂",
	宯: "宀孝",
	宰: "宀辛",
	宱: "宀作",
	宲: "宀呆",
	宴: "宀妟",
	宵: "宀肖",
	家: "宀豕",
	宷: "宀釆",
	宸: "宀辰",
	容: "宀谷",
	宽: "宀苋",
	宾: "宀兵",
	宿: "宀佰",
	寀: "宀采",
	寁: "宀疌",
	寂: "宀叔",
	寃: "宀兔",
	寄: "宀奇",
	密: "宓山",
	寈: "宀青",
	寉: "宀隹",
	寊: "宀貞",
	富: "宀畐",
	寏: "宀奐",
	寑: "宀侵",
	寓: "宀禺",
	寔: "宀是",
	寖: "宀浸",
	寗: "宓冉",
	寘: "宀真",
	寛: "宀莧",
	寜: "寍亅",
	寞: "宀莫",
	察: "宀祭",
	寠: "宀婁",
	寥: "宀翏",
	實: "宀貫",
	寧: "寍丁",
	審: "宀番",
	寪: "宀爲",
	寫: "宀舄",
	寬: "宀萈",
	寭: "宀惠",
	寮: "宀尞",
	寯: "宀雋",
	寰: "宀睘",
	寲: "宀疑",
	寴: "宀親",
	寵: "宀龍",
	寷: "宀豐",
	寺: "土寸",
	寻: "彐寸",
	导: "巳寸",
	寽: "爫寸",
	尃: "甫寸",
	尊: "酋寸",
	導: "道寸",
	尐: "小乀",
	少: "小丿",
	尒: "人小",
	尕: "乃小",
	尖: "小大",
	尗: "上小",
	尘: "小土",
	尙: "小冋",
	尜: "小夵",
	尝: "龸云",
	尞: "昚小",
	尣: "八儿",
	尭: "卉兀",
	屲: "丿山",
	屳: "入山",
	屴: "山力",
	屵: "山厂",
	屶: "山刀",
	岀: "山山",
	岁: "山夕",
	岂: "山己",
	岃: "山刃",
	岇: "山卬",
	岊: "巴山",
	岌: "山及",
	岑: "山今",
	岔: "分山",
	岕: "山介",
	岗: "山冈",
	岙: "夭山",
	岚: "山风",
	岜: "山巴",
	岝: "山乍",
	岢: "山可",
	岦: "山立",
	岧: "山召",
	岩: "山石",
	岪: "山弗",
	岱: "代山",
	岳: "丘山",
	岸: "山厈",
	岺: "山令",
	岽: "山东",
	岿: "山归",
	峀: "山由",
	峁: "山卯",
	峇: "山合",
	峉: "山各",
	峎: "山艮",
	峑: "山全",
	峚: "山圶",
	峛: "山列",
	峜: "山企",
	峝: "山同",
	峞: "山危",
	峟: "山有",
	峦: "亦山",
	峩: "山我",
	峯: "山夆",
	峲: "山利",
	峳: "山攸",
	峷: "山辛",
	峹: "余山",
	峾: "沂山",
	崀: "山良",
	崁: "山坎",
	崇: "山宗",
	崈: "宗山",
	崑: "山昆",
	崒: "山卒",
	崔: "山隹",
	崖: "山厓",
	崗: "山岡",
	崙: "山侖",
	崟: "山金",
	崣: "山委",
	崧: "山松",
	崩: "山朋",
	崫: "山屈",
	崬: "山東",
	崭: "山斩",
	崮: "山固",
	崱: "山則",
	崴: "山威",
	崶: "山封",
	崺: "山施",
	崽: "山思",
	嵀: "山柱",
	嵂: "山律",
	嵅: "山咸",
	嵍: "敄山",
	嵐: "山風",
	嵒: "品山",
	嵓: "山品",
	嵔: "山畏",
	嵙: "山科",
	嵚: "山钦",
	嵛: "山俞",
	嵜: "山竒",
	嵞: "余屾",
	嵡: "山翁",
	嵩: "山高",
	嵬: "山鬼",
	嵳: "山差",
	嵵: "山時",
	嵸: "山從",
	嵹: "山强",
	嵾: "山參",
	嵿: "山頂",
	嶄: "山斬",
	嶅: "敖山",
	嶈: "山將",
	嶉: "山唯",
	嶊: "山推",
	嶌: "山鳥",
	嶎: "山尉",
	嶐: "山隆",
	嶔: "山欽",
	嶘: "山棧",
	嶚: "山尞",
	嶜: "山朁",
	嶞: "隋山",
	嶡: "山厥",
	嶣: "山焦",
	嶤: "山堯",
	嶪: "山業",
	嶯: "山戢",
	嶲: "山雋",
	嶴: "奥山",
	嶶: "山微",
	嶷: "山疑",
	嶺: "山領",
	嶻: "山截",
	嶽: "山獄",
	巂: "崔冏",
	巃: "山龍",
	巅: "山颠",
	巈: "山鞠",
	巋: "山歸",
	巌: "山厳",
	巍: "山魏",
	巐: "山髜",
	巓: "山顚",
	巔: "山顛",
	巖: "山嚴",
	巚: "山獻",
	巢: "巛果",
	巭: "功夫",
	巷: "共巳",
	巹: "丞己",
	巻: "龹己",
	巼: "巴叱",
	帀: "一巾",
	币: "丿巾",
	市: "亠巾",
	帋: "氏巾",
	帑: "奴巾",
	帒: "代巾",
	帘: "穴巾",
	帛: "白巾",
	帟: "亦巾",
	帠: "臼巾",
	帣: "龹巾",
	帤: "如巾",
	帬: "君巾",
	帮: "邦巾",
	幇: "封巾",
	幋: "般巾",
	幕: "莫巾",
	幚: "邦帛",
	幣: "敝巾",
	幤: "敞巾",
	幦: "辟巾",
	幫: "封帛",
	并: "丷开",
	开: "一廾",
	弁: "厶廾",
	异: "巳廾",
	弄: "王廾",
	弅: "分廾",
	弆: "去廾",
	弇: "合廾",
	弈: "亦廾",
	弉: "壯廾",
	弊: "敝廾",
	弖: "弓一",
	弚: "丷弔",
	弩: "奴弓",
	弮: "龹弓",
	弯: "亦弓",
	彂: "业矤",
	彆: "敝弓",
	彔: "彑氺",
	录: "彐氺",
	忈: "二心",
	忌: "己心",
	忍: "刃心",
	忎: "千心",
	忐: "上心",
	忑: "下心",
	志: "士心",
	忘: "亡心",
	忞: "文心",
	忠: "中心",
	忢: "五心",
	忩: "公心",
	念: "今心",
	忽: "勿心",
	忿: "分心",
	态: "太心",
	怂: "从心",
	怎: "乍心",
	怒: "奴心",
	怘: "古心",
	思: "田心",
	怠: "台心",
	怣: "失心",
	怤: "付心",
	急: "刍心",
	怨: "夗心",
	怱: "匆心",
	怷: "朮心",
	怸: "术心",
	怹: "他心",
	怼: "对心",
	恁: "任心",
	恋: "亦心",
	恏: "好心",
	恐: "巩心",
	恕: "如心",
	恖: "囟心",
	恚: "圭心",
	恣: "次心",
	恧: "而心",
	恩: "因心",
	息: "自心",
	恳: "艮心",
	恶: "亚心",
	恷: "休心",
	恿: "甬心",
	悆: "余心",
	悉: "釆心",
	悊: "折心",
	悐: "狄心",
	悘: "医心",
	悠: "攸心",
	悡: "利心",
	患: "串心",
	悤: "囱心",
	悥: "言心",
	您: "你心",
	悪: "亜心",
	悫: "壳心",
	悬: "县心",
	悲: "非心",
	悳: "直心",
	悹: "官心",
	惁: "析心",
	惄: "叔心",
	惉: "沾心",
	惌: "宛心",
	惎: "其心",
	惑: "或心",
	惒: "和心",
	惖: "易心",
	惡: "亞心",
	惣: "物心",
	惥: "臾心",
	惩: "征心",
	惪: "直心",
	惫: "夂思",
	想: "相心",
	惷: "春心",
	惹: "若心",
	愁: "秋心",
	愂: "勃心",
	愆: "衍心",
	愈: "俞心",
	愍: "敃心",
	意: "音心",
	愗: "敄心",
	愙: "客心",
	愚: "禺心",
	感: "咸心",
	愬: "朔心",
	愸: "勑心",
	愻: "孫心",
	慁: "圂心",
	慂: "涌心",
	慇: "殷心",
	慈: "兹心",
	態: "能心",
	慐: "貢心",
	慗: "敕心",
	慙: "斬心",
	慜: "敏心",
	慝: "匿心",
	慤: "殻心",
	慦: "救心",
	慧: "彗心",
	慫: "從心",
	慰: "尉心",
	慸: "帶心",
	慹: "執心",
	慼: "戚心",
	慾: "欲心",
	憃: "舂心",
	憄: "徝心",
	憅: "動心",
	憇: "甜心",
	憊: "備心",
	憋: "敝心",
	憌: "鈞心",
	憑: "馮心",
	憖: "猌心",
	憙: "喜心",
	憝: "敦心",
	憠: "厥心",
	憥: "勞心",
	憨: "敢心",
	憵: "辟心",
	憼: "敬心",
	懃: "勤心",
	懇: "貇心",
	懋: "楙心",
	懑: "满心",
	懖: "銛心",
	懘: "滯心",
	懟: "對心",
	懣: "滿心",
	懯: "敷心",
	懲: "徵心",
	懸: "縣心",
	戁: "難心",
	戅: "贑心",
	戆: "赣心",
	戇: "贛心",
	戔: "戈戈",
	戶: "丿尸",
	户: "丶尸",
	戸: "一尸",
	拏: "奴手",
	拲: "共手",
	拳: "龹手",
	拿: "合手",
	挐: "如手",
	挙: "兴手",
	挚: "执手",
	挛: "亦手",
	挲: "沙手",
	掔: "臤手",
	掣: "制手",
	揅: "研手",
	揧: "剌手",
	揫: "秋手",
	揱: "削手",
	搫: "般手",
	摮: "敖手",
	摯: "執手",
	摰: "埶手",
	摹: "莫手",
	撀: "殸手",
	撃: "軗手",
	撆: "敝手",
	撉: "敦手",
	擎: "敬手",
	擘: "辟手",
	擧: "與手",
	攀: "樊手",
	支: "十又",
	整: "敕正",
	斃: "敝死",
	文: "亠乂",
	斈: "文子",
	斊: "文耳",
	斋: "文而",
	斍: "文見",
	斐: "非文",
	斖: "文舋",
	斧: "父斤",
	旉: "甫方",
	旕: "於叱",
	无: "一尢",
	旦: "日一",
	旨: "匕日",
	早: "日十",
	旮: "九日",
	旯: "日九",
	旱: "日干",
	旲: "日大",
	旵: "日山",
	旻: "日文",
	旾: "屯日",
	昂: "日卬",
	昃: "日仄",
	昆: "日比",
	昇: "日升",
	昊: "日天",
	昋: "天日",
	昌: "日日",
	昏: "氏日",
	易: "日勿",
	昔: "龷日",
	昗: "日六",
	昘: "日方",
	昙: "日云",
	昜: "旦勿",
	昝: "处日",
	星: "日生",
	昦: "日夰",
	昪: "日弁",
	昬: "民日",
	昮: "日功",
	昰: "日正",
	昱: "日立",
	昴: "日卯",
	昷: "日皿",
	昺: "日丙",
	昻: "日卭",
	昼: "尺旦",
	显: "日业",
	晁: "日兆",
	晃: "日光",
	晋: "亚日",
	晏: "日安",
	晑: "日向",
	晕: "日军",
	晜: "日弟",
	晟: "日成",
	晢: "折日",
	晨: "日辰",
	普: "並日",
	景: "日京",
	晳: "析日",
	晶: "日昍",
	晷: "日咎",
	晸: "日政",
	智: "知日",
	暂: "斩日",
	暃: "日非",
	暈: "日軍",
	暋: "敃日",
	暑: "日者",
	暓: "敄日",
	暜: "竝日",
	暠: "日高",
	暨: "既旦",
	暫: "斬日",
	暬: "埶日",
	暮: "莫日",
	暼: "敝日",
	曁: "旣旦",
	曅: "日華",
	曇: "日雲",
	曌: "明空",
	曏: "日鄉",
	曐: "晶生",
	曟: "晶辰",
	曡: "晶冝",
	曧: "日融",
	曩: "日襄",
	曶: "勿曰",
	曷: "日匃",
	曻: "曰舛",
	最: "日取",
	朁: "兓日",
	朚: "亡明",
	朞: "其月",
	朤: "朋朋",
	朰: "木乙",
	朵: "几木",
	朶: "乃木",
	杂: "九朩",
	李: "木子",
	杏: "木口",
	杗: "亡木",
	条: "夂朩",
	杢: "木工",
	杰: "木灬",
	杲: "日木",
	杳: "木日",
	枀: "公木",
	枈: "比木",
	枩: "木公",
	枲: "台木",
	架: "加木",
	枼: "世木",
	枽: "卋木",
	柋: "代木",
	某: "甘木",
	染: "氿木",
	柔: "矛木",
	查: "木旦",
	柰: "木示",
	柴: "此木",
	査: "木且",
	栗: "覀木",
	栞: "幵木",
	栠: "任木",
	栥: "次木",
	栾: "亦木",
	桀: "舛木",
	桇: "如木",
	案: "安木",
	桑: "叒木",
	桒: "卉木",
	桬: "沙木",
	桽: "木坐",
	梊: "折木",
	梥: "宀枀",
	梦: "林夕",
	梨: "利木",
	梵: "林凡",
	梺: "林下",
	棊: "其木",
	棐: "非木",
	棗: "朿朿",
	森: "木林",
	棸: "取木",
	棻: "芬木",
	棼: "林分",
	棽: "林今",
	棾: "林冗",
	椉: "亠桀",
	椘: "林之",
	椝: "规木",
	椞: "析木",
	椠: "斩木",
	楘: "敄木",
	楚: "林疋",
	楶: "次呆",
	榃: "林田",
	榘: "矩木",
	槀: "高木",
	槃: "般木",
	槊: "朔木",
	槧: "斬木",
	槩: "既木",
	槳: "將木",
	槷: "埶木",
	槼: "規木",
	樊: "棥大",
	樖: "合柯",
	樷: "林取",
	橜: "厥木",
	橤: "惢木",
	橥: "猪木",
	檒: "林風",
	檗: "辟木",
	檠: "敬木",
	櫐: "畾木",
	櫫: "豬木",
	櫱: "辥木",
	櫽: "隱木",
	正: "一止",
	歨: "止龰",
	歩: "止少",
	歪: "不正",
	歬: "止舟",
	歶: "此禺",
	歹: "一夕",
	毉: "殹巫",
	毊: "殸喬",
	毐: "士毋",
	毒: "龶母",
	毕: "比十",
	毖: "比必",
	毘: "田比",
	毙: "比死",
	毞: "比毛",
	毟: "少毛",
	毠: "加毛",
	毣: "羽毛",
	氅: "敞毛",
	氐: "氏丶",
	氒: "氏十",
	氶: "乛水",
	氼: "水人",
	氽: "人水",
	汆: "入水",
	汖: "山水",
	汞: "工水",
	汬: "井水",
	沓: "水日",
	沗: "天氺",
	沯: "水石",
	泉: "白水",
	泴: "水皿",
	泵: "石水",
	洜: "各水",
	淼: "水沝",
	淾: "金水",
	渠: "洰木",
	湬: "秋水",
	準: "淮十",
	漀: "殸水",
	漐: "執水",
	漿: "將水",
	澃: "須水",
	濷: "水滿",
	瀪: "敏泉",
	火: "八人",
	灭: "一火",
	灮: "火儿",
	灵: "彐火",
	灷: "火廾",
	灸: "久火",
	灻: "土火",
	災: "巛火",
	灾: "宀火",
	炁: "旡灬",
	炃: "分火",
	炅: "日火",
	炎: "火火",
	炗: "廿火",
	炛: "火化",
	炭: "山灰",
	炰: "包灬",
	炱: "台火",
	点: "占灬",
	烈: "列灬",
	烋: "休灬",
	烎: "开火",
	烝: "丞灬",
	烡: "火共",
	烫: "汤火",
	热: "执灬",
	烮: "列火",
	烲: "折火",
	烹: "亨灬",
	烾: "炎土",
	焁: "吹火",
	焂: "攸火",
	焄: "君灬",
	焈: "戺火",
	焋: "壯火",
	焎: "折灬",
	焘: "寿灬",
	焚: "林火",
	焣: "取灬",
	焤: "府火",
	焦: "隹灬",
	焱: "火炏",
	焸: "昍火",
	焽: "明火",
	煎: "前灬",
	煑: "者火",
	煕: "巸灬",
	煚: "昛火",
	煦: "昫灬",
	照: "昭灬",
	煮: "者灬",
	煲: "保火",
	熃: "務火",
	熊: "能灬",
	熋: "能火",
	熙: "巸灬",
	熟: "孰灬",
	熧: "從火",
	熨: "尉火",
	熬: "敖灬",
	熭: "彗火",
	熱: "埶灬",
	熹: "喜灬",
	燊: "焱木",
	燓: "棥火",
	燙: "湯火",
	燚: "炏炏",
	燛: "臦火",
	燹: "豩火",
	燾: "壽灬",
	爇: "艹熱",
	爯: "爫冉",
	爳: "爲了",
	父: "八乂",
	爷: "父卩",
	爸: "父巴",
	爹: "父多",
	爺: "父耶",
	牟: "厶牛",
	牢: "宀牛",
	牮: "代牛",
	牶: "龹牛",
	犁: "利牛",
	犇: "牛牪",
	犎: "封牛",
	犚: "尉牛",
	犟: "强牛",
	犨: "雔牛",
	犩: "魏牛",
	犫: "讎牛",
	狊: "目犬",
	猆: "非犬",
	獎: "將犬",
	獒: "敖犬",
	獘: "敝犬",
	玄: "亠幺",
	王: "一土",
	玺: "尔玉",
	珡: "玨人",
	琞: "明玉",
	琧: "亞玉",
	琴: "玨今",
	琵: "玨比",
	琶: "玨巴",
	琹: "玨木",
	瑟: "玨必",
	瑬: "流玉",
	瑿: "殹玉",
	璗: "湯玉",
	璧: "辟玉",
	璽: "爾玉",
	瓕: "彌玉",
	瓫: "分瓦",
	瓮: "公瓦",
	瓷: "次瓦",
	甃: "秋瓦",
	甆: "兹瓦",
	甏: "彭瓦",
	甓: "辟瓦",
	甕: "雍瓦",
	甖: "賏瓦",
	甧: "甡月",
	甪: "丿用",
	甬: "龴用",
	甭: "不用",
	甮: "勿用",
	甶: "丿田",
	男: "田力",
	甹: "由丂",
	甼: "田丁",
	甾: "巛田",
	畀: "田丌",
	畁: "由丌",
	界: "田介",
	畓: "水田",
	畕: "田田",
	畜: "玄田",
	畠: "白田",
	畣: "合田",
	畧: "田各",
	畨: "米田",
	番: "釆田",
	畬: "余田",
	異: "田共",
	畱: "丣田",
	畲: "佘田",
	畳: "田冝",
	疊: "畾冝",
	疍: "疋旦",
	癷: "癶干",
	癸: "癶天",
	癹: "癶殳",
	登: "癶豆",
	百: "一白",
	皀: "白匕",
	皁: "白十",
	皂: "白七",
	皃: "白儿",
	皆: "比白",
	皇: "白王",
	皋: "白夲",
	皙: "析白",
	皨: "皛土",
	盀: "卜皿",
	盁: "乃皿",
	盂: "于皿",
	盃: "不皿",
	盄: "弔皿",
	盅: "中皿",
	盆: "分皿",
	盇: "太皿",
	盈: "夃皿",
	盉: "禾皿",
	盋: "犮皿",
	盌: "夗皿",
	盍: "去皿",
	盎: "央皿",
	盏: "戋皿",
	盐: "圤皿",
	盒: "合皿",
	盓: "汙皿",
	盔: "灰皿",
	盕: "汎皿",
	盗: "次皿",
	盘: "舟皿",
	盙: "甫皿",
	盚: "求皿",
	盛: "成皿",
	盝: "录皿",
	盞: "戔皿",
	盟: "明皿",
	盠: "彖皿",
	盢: "畎皿",
	盤: "般皿",
	盦: "酓皿",
	盨: "須皿",
	盪: "湯皿",
	盲: "亡目",
	省: "少目",
	看: "龵目",
	眚: "生目",
	真: "十具",
	眢: "夗目",
	眥: "此目",
	眷: "龹目",
	眾: "罒乑",
	督: "叔目",
	睪: "罒幸",
	睯: "敃目",
	睾: "丿睪",
	瞀: "敄目",
	瞏: "罒袁",
	瞖: "殹目",
	瞥: "敝目",
	瞽: "鼓目",
	矍: "瞿又",
	矞: "矛冏",
	矣: "厶矢",
	砉: "丰石",
	砦: "此石",
	砮: "奴石",
	砻: "龙石",
	硩: "折石",
	硰: "沙石",
	硻: "臤石",
	碁: "其石",
	碆: "波石",
	碞: "品石",
	碧: "珀石",
	磊: "石砳",
	磐: "般石",
	磛: "斬石",
	磬: "殸石",
	礕: "辟石",
	礜: "與石",
	礨: "畾石",
	礬: "樊石",
	礱: "龍石",
	祟: "出示",
	祡: "此示",
	票: "覀示",
	禁: "林示",
	禦: "御示",
	禼: "卤禸",
	禽: "人离",
	禾: "丿木",
	禿: "禾儿",
	秀: "禾乃",
	秂: "禾人",
	秃: "禾几",
	秊: "禾千",
	秶: "次禾",
	稾: "高禾",
	穒: "禾鳥",
	穴: "宀八",
	穵: "穴乙",
	究: "穴九",
	穷: "穴力",
	穸: "穴夕",
	穹: "穴弓",
	空: "穴工",
	穻: "穴于",
	穼: "穴木",
	穽: "穴井",
	穾: "穴夭",
	穿: "穴牙",
	窀: "穴屯",
	突: "穴犬",
	窂: "穴牛",
	窃: "穴切",
	窄: "穴乍",
	窅: "穴目",
	窆: "穴乏",
	窇: "穴包",
	窈: "穴幼",
	窉: "穴丙",
	窊: "穴瓜",
	窋: "穴出",
	窌: "穴卯",
	窍: "穴巧",
	窎: "穴鸟",
	窏: "穴汙",
	窐: "穴圭",
	窑: "穴缶",
	窒: "穴至",
	窔: "穴交",
	窕: "穴兆",
	窖: "穴告",
	窗: "穴囱",
	窘: "穴君",
	窙: "穴孝",
	窚: "穴成",
	窜: "穴串",
	窝: "穴呙",
	窞: "穴臽",
	窟: "穴屈",
	窠: "穴果",
	窡: "穴叕",
	窢: "穴或",
	窣: "穴卒",
	窤: "穴昆",
	窥: "穴规",
	窦: "穴卖",
	窧: "穴卓",
	窨: "穴音",
	窩: "穴咼",
	窪: "穴洼",
	窫: "穴契",
	窬: "穴俞",
	窭: "穴娄",
	窮: "穴躬",
	窯: "穴羔",
	窱: "穴條",
	窲: "穴料",
	窴: "穴真",
	窵: "穴鳥",
	窶: "穴婁",
	窷: "穴聊",
	窸: "穴悉",
	窺: "穴規",
	窻: "穴悤",
	窼: "穴巢",
	窽: "穴欵",
	窾: "穴款",
	窿: "穴隆",
	竁: "穴毳",
	竂: "穴尞",
	竄: "穴鼠",
	竅: "穴敫",
	竆: "穴躳",
	竇: "穴賣",
	竉: "穴龍",
	竎: "立廾",
	竞: "立兄",
	竟: "音儿",
	章: "立早",
	童: "立里",
	竪: "臤立",
	竺: "⺮二",
	竻: "⺮力",
	竼: "⺮凡",
	竽: "⺮于",
	竾: "⺮也",
	竿: "⺮干",
	笀: "⺮亡",
	笁: "⺮工",
	笂: "⺮丸",
	笃: "⺮马",
	笄: "⺮开",
	笅: "⺮爻",
	笆: "⺮巴",
	笇: "⺮卞",
	笈: "⺮及",
	笉: "⺮匀",
	笊: "⺮爪",
	笋: "⺮尹",
	笌: "⺮牙",
	笍: "⺮内",
	笎: "⺮元",
	笏: "⺮勿",
	笐: "⺮亢",
	笑: "⺮夭",
	笒: "⺮今",
	笓: "⺮比",
	笔: "⺮毛",
	笕: "⺮见",
	笖: "⺮以",
	笗: "⺮冬",
	笘: "⺮占",
	笙: "⺮生",
	笚: "⺮甲",
	笛: "⺮由",
	笜: "⺮出",
	笞: "⺮台",
	笟: "⺮瓜",
	笠: "⺮立",
	笡: "⺮且",
	笢: "⺮民",
	笣: "⺮包",
	笤: "⺮召",
	笥: "⺮司",
	符: "⺮付",
	笧: "⺮册",
	笨: "⺮本",
	笩: "⺮代",
	笪: "⺮旦",
	笭: "⺮令",
	笮: "⺮乍",
	笯: "⺮奴",
	笰: "⺮弗",
	笱: "⺮句",
	笲: "⺮弁",
	笳: "⺮加",
	笴: "⺮可",
	笵: "⺮氾",
	笶: "⺮矢",
	笷: "⺮卯",
	笸: "⺮叵",
	笹: "⺮世",
	笺: "⺮戋",
	笻: "⺮卭",
	笼: "⺮龙",
	笽: "⺮皿",
	笾: "⺮边",
	笿: "⺮各",
	筀: "⺮圭",
	筁: "⺮曲",
	筂: "⺮池",
	筃: "⺮因",
	筄: "⺮兆",
	筅: "⺮先",
	筆: "⺮聿",
	筇: "⺮邛",
	筈: "⺮舌",
	等: "⺮寺",
	筊: "⺮交",
	筋: "⺮肋",
	筌: "⺮全",
	筍: "⺮旬",
	筎: "⺮如",
	筏: "⺮伐",
	筐: "⺮匡",
	筑: "⺮巩",
	筒: "⺮同",
	筓: "⺮幵",
	答: "⺮合",
	筕: "⺮行",
	策: "⺮朿",
	筗: "⺮仲",
	筘: "⺮扣",
	筙: "⺮耒",
	筚: "笓十",
	筛: "⺮师",
	筜: "⺮当",
	筝: "⺮争",
	筞: "⺮宋",
	筟: "⺮孚",
	筠: "⺮均",
	筡: "⺮余",
	筢: "⺮把",
	筣: "⺮利",
	筤: "⺮良",
	筥: "⺮吕",
	筦: "⺮完",
	筧: "⺮見",
	筨: "⺮含",
	筩: "⺮甬",
	筪: "⺮匣",
	筫: "⺮貝",
	筬: "⺮成",
	筭: "⺮弄",
	筮: "⺮巫",
	筯: "⺮助",
	筰: "⺮作",
	筱: "⺮攸",
	筲: "⺮肖",
	筳: "⺮廷",
	筴: "⺮夾",
	筵: "⺮延",
	筶: "⺮告",
	筷: "⺮快",
	筸: "⺮旱",
	筹: "⺮寿",
	筺: "⺮匤",
	筻: "⺮更",
	筼: "⺮员",
	筽: "⺮吴",
	签: "⺮佥",
	筿: "⺮条",
	简: "⺮间",
	箁: "⺮咅",
	箂: "⺮來",
	箃: "⺮取",
	箄: "⺮卑",
	箅: "⺮畀",
	箇: "⺮固",
	箈: "⺮治",
	箉: "⺮拐",
	箊: "⺮於",
	箋: "⺮戔",
	箌: "⺮到",
	箎: "⺮虎",
	箏: "⺮爭",
	箐: "⺮青",
	箑: "⺮疌",
	箒: "⺮帚",
	箓: "⺮录",
	箔: "⺮泊",
	箕: "⺮其",
	箖: "⺮林",
	箘: "⺮囷",
	箙: "⺮服",
	箛: "⺮孤",
	箜: "⺮空",
	箝: "⺮拑",
	箞: "⺮卷",
	箟: "⺮昆",
	箠: "⺮垂",
	管: "⺮官",
	箢: "⺮宛",
	箣: "⺮刺",
	箤: "⺮卒",
	箥: "⺮波",
	箦: "⺮责",
	箨: "⺮择",
	箩: "⺮罗",
	箪: "⺮单",
	箫: "⺮肃",
	箬: "⺮若",
	箭: "⺮前",
	箮: "⺮宣",
	箯: "⺮便",
	箰: "筍子",
	箱: "⺮相",
	箲: "⺮洗",
	箳: "⺮屏",
	箴: "⺮咸",
	箵: "⺮省",
	箶: "⺮胡",
	箷: "⺮施",
	箸: "⺮者",
	箹: "⺮約",
	箺: "⺮春",
	箻: "⺮律",
	箼: "⺮屋",
	箽: "⺮重",
	箾: "⺮削",
	箿: "⺮咠",
	節: "⺮即",
	篁: "⺮皇",
	篂: "⺮星",
	篃: "⺮眉",
	篅: "⺮耑",
	篆: "⺮彖",
	篇: "⺮扁",
	篈: "⺮封",
	築: "筑木",
	篊: "⺮洪",
	篋: "⺮匧",
	篌: "⺮侯",
	篍: "⺮秋",
	篎: "⺮眇",
	篑: "⺮贵",
	篒: "⺮食",
	篓: "⺮娄",
	篔: "⺮員",
	篕: "⺮盍",
	篖: "⺮唐",
	篗: "⺮隻",
	篘: "⺮芻",
	篙: "⺮高",
	篚: "⺮匪",
	篛: "⺮弱",
	篜: "⺮烝",
	篝: "⺮冓",
	篞: "⺮涅",
	篟: "⺮倩",
	篠: "⺮條",
	篢: "⺮貢",
	篣: "⺮旁",
	篤: "⺮馬",
	篥: "⺮栗",
	篧: "⺮隺",
	篨: "⺮除",
	篩: "⺮師",
	篪: "⺮虒",
	篫: "筑手",
	篬: "⺮倉",
	篭: "⺮竜",
	篮: "⺮监",
	篯: "⺮钱",
	篰: "⺮部",
	篱: "⺮离",
	篲: "⺮彗",
	篳: "⺮畢",
	篴: "⺮逐",
	篵: "⺮從",
	篶: "⺮焉",
	篷: "⺮逢",
	篸: "⺮參",
	篺: "⺮捭",
	篻: "⺮票",
	篼: "⺮兜",
	篽: "⺮御",
	篿: "⺮專",
	簀: "⺮責",
	簁: "⺮徙",
	簂: "⺮國",
	簃: "⺮移",
	簄: "⺮扈",
	簅: "⺮産",
	簇: "⺮族",
	簈: "⺮屛",
	簉: "⺮造",
	簊: "⺮基",
	簌: "⺮欶",
	簍: "⺮婁",
	簎: "⺮措",
	簏: "⺮鹿",
	簐: "⺮軟",
	簑: "⺮衰",
	簓: "⺮彫",
	簔: "⺮衰",
	簕: "⺮勒",
	簖: "⺮断",
	簗: "⺮梁",
	簘: "⺮粛",
	簙: "⺮博",
	簛: "⺮斯",
	簜: "⺮湯",
	簝: "⺮尞",
	簞: "⺮單",
	簟: "⺮覃",
	簠: "⺮盙",
	簡: "⺮間",
	簢: "⺮閔",
	簣: "⺮貴",
	簤: "⺮買",
	簥: "⺮喬",
	簦: "⺮登",
	簧: "⺮黄",
	簨: "⺮巽",
	簩: "⺮勞",
	簪: "⺮朁",
	簫: "⺮肅",
	簬: "⺮路",
	簮: "⺮替",
	簯: "⺮棋",
	簰: "⺮牌",
	簲: "⺮脾",
	簳: "⺮幹",
	簴: "⺮虡",
	簵: "⺮輅",
	簶: "⺮禄",
	簷: "⺮詹",
	簹: "⺮當",
	簺: "⺮塞",
	簻: "⺮過",
	簼: "⺮搆",
	簽: "⺮僉",
	簾: "⺮廉",
	簿: "⺮溥",
	籁: "⺮赖",
	籂: "⺮飾",
	籃: "⺮監",
	籄: "⺮匱",
	籅: "⺮與",
	籆: "⺮蒦",
	籇: "⺮豪",
	籈: "⺮甄",
	籉: "⺮臺",
	籊: "⺮翟",
	籋: "⺮爾",
	籌: "⺮壽",
	籍: "⺮耤",
	籎: "⺮疑",
	籏: "⺮旗",
	籐: "⺮滕",
	籓: "⺮潘",
	籔: "⺮數",
	籕: "⺮榴",
	籖: "⺮韯",
	籗: "⺮霍",
	籘: "⺮縢",
	籙: "⺮録",
	籚: "⺮盧",
	籛: "⺮錢",
	籜: "⺮擇",
	籝: "⺮嬴",
	籞: "⺮禦",
	籟: "⺮賴",
	籠: "⺮龍",
	籡: "⺮撿",
	籢: "⺮斂",
	籣: "⺮闌",
	籤: "⺮韱",
	籥: "⺮龠",
	籦: "⺮鍾",
	籧: "⺮遽",
	籨: "⺮歛",
	籩: "⺮邊",
	籪: "⺮斷",
	籫: "⺮贊",
	籬: "⺮離",
	籭: "⺮麗",
	籮: "⺮羅",
	籯: "⺮贏",
	籰: "⺮矍",
	籱: "⺮靃",
	籲: "⺮龥",
	籴: "入米",
	类: "米大",
	粂: "久米",
	粊: "比米",
	粜: "出米",
	粟: "覀米",
	粢: "次米",
	粦: "米舛",
	粪: "米共",
	糞: "米異",
	糪: "辟米",
	糱: "辥米",
	糵: "薛米",
	糸: "幺小",
	系: "丿糸",
	紊: "文糸",
	素: "龶糸",
	紥: "扎糸",
	紫: "此糸",
	紮: "札糸",
	累: "田糸",
	絫: "厽糸",
	絭: "龹糸",
	絮: "如糸",
	絷: "执糸",
	綤: "邵糸",
	綦: "其糸",
	緊: "臤糸",
	縏: "般糸",
	縶: "執糸",
	繁: "敏糸",
	繄: "殹糸",
	繋: "軗糸",
	繠: "惢糸",
	繴: "辟糸",
	纍: "畾糸",
	纛: "毒縣",
	缹: "缶灬",
	罄: "殸缶",
	罋: "雍缶",
	罌: "賏缶",
	罍: "畾缶",
	罖: "罒亽",
	罗: "罒夕",
	罘: "罒不",
	罛: "罒瓜",
	罜: "罒主",
	罝: "罒且",
	罞: "罒矛",
	罟: "罒古",
	罠: "罒民",
	罡: "罒正",
	罢: "罒去",
	罣: "罒圭",
	罤: "罒弟",
	罥: "罒肙",
	罦: "罒孚",
	罧: "罒林",
	罨: "罒奄",
	罩: "罒卓",
	罪: "罒非",
	罫: "罒卦",
	罬: "罒叕",
	罭: "罒或",
	置: "罒直",
	罯: "罒音",
	罱: "罒南",
	署: "罒者",
	罳: "罒思",
	罴: "罢灬",
	罵: "罒馬",
	罶: "罒留",
	罷: "罒能",
	罸: "罒討",
	罹: "罒惟",
	罺: "罒巢",
	罻: "罒尉",
	罼: "罒畢",
	罾: "罒曾",
	罿: "罒童",
	羀: "网畱",
	羂: "罒絹",
	羃: "罒幕",
	羄: "罒綽",
	羅: "罒維",
	羆: "罒熊",
	羍: "大羊",
	羣: "君羊",
	羹: "羔美",
	羿: "羽廾",
	翁: "公羽",
	翆: "羽卆",
	翇: "羽犮",
	翌: "羽立",
	習: "羽白",
	翕: "合羽",
	翚: "羽军",
	翜: "羽夾",
	翟: "羽隹",
	翠: "羽卒",
	翡: "非羽",
	翣: "羽妾",
	翥: "者羽",
	翦: "前羽",
	翨: "羽是",
	翬: "羽軍",
	翯: "羽高",
	翳: "殹羽",
	翼: "羽異",
	耄: "老毛",
	耆: "老日",
	耈: "老句",
	耋: "老至",
	耍: "而女",
	耎: "而大",
	耑: "山而",
	耷: "大耳",
	耸: "从耳",
	聂: "耳双",
	聋: "龙耳",
	聓: "巩耳",
	聚: "取乑",
	聟: "知耳",
	聱: "敖耳",
	聲: "殸耳",
	聳: "從耳",
	聶: "耳聑",
	聻: "漸耳",
	聾: "龍耳",
	肎: "冖月",
	肏: "入肉",
	肓: "亡月",
	肙: "口月",
	肯: "止月",
	胃: "田月",
	胄: "由月",
	背: "北月",
	胔: "此肉",
	胥: "疋月",
	胬: "奴肉",
	胷: "匈月",
	脀: "丞月",
	脅: "劦月",
	脋: "刕月",
	脔: "亦肉",
	腎: "臤月",
	膂: "旅月",
	膏: "高月",
	膐: "旅肉",
	臀: "殿月",
	臂: "辟月",
	臋: "殿肉",
	臖: "興月",
	臡: "難肉",
	臩: "臦夰",
	臬: "自木",
	臭: "自犬",
	臰: "自死",
	臽: "勹臼",
	舀: "爫臼",
	舁: "臼廾",
	舅: "臼男",
	舊: "萑臼",
	舌: "千口",
	舍: "人舌",
	良: "丶艮",
	艺: "艹乙",
	艻: "艹力",
	艼: "艹丁",
	艽: "艹九",
	艾: "艹乂",
	艿: "艹乃",
	芀: "艹刀",
	芁: "艹几",
	芃: "艹凡",
	芄: "艹丸",
	芅: "艹弋",
	芆: "艹叉",
	芇: "艹巾",
	芉: "艹干",
	芊: "艹千",
	芋: "艹于",
	芌: "艹亏",
	芍: "艹勺",
	芎: "艹弓",
	芏: "艹土",
	芐: "艹下",
	芑: "艹己",
	芒: "艹亡",
	芓: "艹子",
	芔: "屮艸",
	芕: "艹夕",
	芖: "艹大",
	芗: "艹乡",
	芘: "艹比",
	芙: "艹夫",
	芚: "艹屯",
	芛: "艹尹",
	芜: "艹无",
	芝: "艹之",
	芞: "艹气",
	芟: "艹殳",
	芠: "艹文",
	芡: "艹欠",
	芢: "艹仁",
	芣: "艹不",
	芤: "艹孔",
	芥: "艹介",
	芦: "艹户",
	芧: "艹予",
	芨: "艹及",
	芩: "艹今",
	芪: "艹氏",
	芫: "艹元",
	芬: "艹分",
	芭: "艹巴",
	芮: "艹内",
	芯: "艹心",
	芰: "艹支",
	花: "艹化",
	芳: "艹方",
	芴: "艹勿",
	芵: "艹夬",
	芶: "艹勾",
	芷: "艹止",
	芸: "艹云",
	芹: "艹斤",
	芺: "艹夭",
	芼: "艹毛",
	芽: "艹牙",
	芾: "艹巿",
	芿: "艹仍",
	苀: "艹亢",
	苁: "艹从",
	苂: "艹火",
	苃: "艹友",
	苄: "艹卞",
	苅: "艹刈",
	苆: "艹切",
	苇: "艹韦",
	苈: "艹历",
	苉: "艹匹",
	苊: "艹厄",
	苋: "艹见",
	苌: "艹长",
	苍: "艹仓",
	苏: "艹办",
	苑: "艹夗",
	苒: "艹冉",
	苓: "艹令",
	苔: "艹台",
	苕: "艹召",
	苖: "艹由",
	苗: "艹田",
	苘: "艹冋",
	苙: "艹立",
	苚: "艹用",
	苛: "艹可",
	苜: "艹目",
	苝: "艹北",
	苞: "艹包",
	苟: "艹句",
	苠: "艹民",
	苡: "艹以",
	苣: "艹巨",
	苤: "艹丕",
	若: "艹右",
	苦: "艹古",
	苧: "艹宁",
	苨: "艹尼",
	苩: "艹白",
	苪: "艹丙",
	苫: "艹占",
	苬: "艹囚",
	苭: "艹幼",
	苮: "艹仙",
	苯: "艹本",
	苰: "艹弘",
	英: "艹央",
	苲: "艹乍",
	苳: "艹冬",
	苴: "艹且",
	苵: "艹失",
	苶: "艹尒",
	苷: "艹甘",
	苸: "艹乎",
	苹: "艹平",
	苺: "艹母",
	苻: "艹付",
	苼: "艹生",
	苽: "艹瓜",
	苾: "艹必",
	苿: "艹未",
	茀: "艹弗",
	茁: "艹出",
	茂: "艹戊",
	范: "艹氾",
	茄: "艹加",
	茅: "艹矛",
	茆: "艹卯",
	茇: "艹犮",
	茈: "艹此",
	茉: "艹末",
	茊: "艹丘",
	茋: "艹氐",
	茌: "艹仕",
	茏: "艹龙",
	茐: "艹匆",
	茑: "艹鸟",
	茒: "艹邜",
	茓: "艹穴",
	茖: "艹各",
	茗: "艹名",
	茘: "艹刕",
	茙: "艹戎",
	茚: "艹印",
	茛: "艹艮",
	茜: "艹西",
	茞: "艹臣",
	茟: "艹聿",
	茠: "艹休",
	茡: "艹字",
	茢: "艹列",
	茣: "艹吴",
	茤: "艹多",
	茥: "艹圭",
	茦: "艹朿",
	茧: "艹虫",
	茨: "艹次",
	茩: "艹后",
	茪: "艹光",
	茫: "艹汒",
	茬: "艹在",
	茭: "艹交",
	茮: "艹尗",
	茯: "艹伏",
	茱: "艹朱",
	茳: "艹江",
	茴: "艹回",
	茵: "艹因",
	茷: "艹伐",
	茸: "艹耳",
	茹: "艹如",
	茺: "艹充",
	茻: "艸艸",
	茼: "艹同",
	茽: "艹仲",
	茾: "艹开",
	茿: "艹巩",
	荀: "艹旬",
	荁: "艹亘",
	荂: "艹夸",
	荃: "艹全",
	荄: "艹亥",
	荅: "艹合",
	荇: "艹行",
	荈: "艹舛",
	草: "艹早",
	荊: "艹刑",
	荋: "艹而",
	荌: "艹安",
	荍: "艹收",
	荎: "艹至",
	荏: "艹任",
	荐: "艹存",
	荑: "艹夷",
	荒: "艹巟",
	荓: "艹并",
	荔: "艹劦",
	荕: "艹肋",
	荖: "艹老",
	荗: "艹戍",
	荘: "艹壮",
	荙: "艹达",
	荚: "艹夹",
	荛: "艹尧",
	荜: "艹毕",
	荝: "艹则",
	荞: "艹乔",
	荟: "艹会",
	荠: "艹齐",
	荡: "艹汤",
	荢: "艹宇",
	荤: "艹军",
	荨: "艹寻",
	荩: "艹尽",
	荪: "艹孙",
	荫: "艹阴",
	荬: "艹买",
	荭: "艹红",
	荮: "艹纣",
	药: "艹约",
	荰: "艹杜",
	荱: "艹尾",
	荲: "艹里",
	荳: "艹豆",
	荴: "艹扶",
	荵: "艹忍",
	荶: "艹吟",
	荷: "艹何",
	荸: "艹孛",
	荹: "艹步",
	荺: "艹均",
	荻: "艹狄",
	荼: "艹余",
	荽: "艹妥",
	荾: "艹夋",
	荿: "艹成",
	莀: "艹辰",
	莁: "艹巫",
	莂: "艹别",
	莃: "艹希",
	莄: "艹更",
	莅: "艹位",
	莆: "艹甫",
	莇: "艹助",
	莈: "艹没",
	莉: "艹利",
	莊: "艹壯",
	莋: "艹作",
	莌: "艹兑",
	莍: "艹求",
	莎: "艹沙",
	莏: "艹抄",
	莐: "艹沈",
	莑: "艹夆",
	莒: "艹吕",
	莓: "艹每",
	莔: "艹囧",
	莕: "艹杏",
	莖: "艹巠",
	莗: "艹車",
	莘: "艹辛",
	莙: "艹君",
	莚: "艹延",
	莛: "艹廷",
	莜: "艹攸",
	莝: "艹坐",
	莞: "艹完",
	莟: "艹含",
	莠: "艹秀",
	莡: "艹足",
	莢: "艹夾",
	莣: "艹忘",
	莤: "艹酉",
	莥: "艹狃",
	莦: "艹肖",
	莧: "艹見",
	莨: "艹良",
	莩: "艹孚",
	莪: "艹我",
	莫: "艹旲",
	莬: "艹免",
	莭: "艹即",
	莮: "艹男",
	莯: "艹沐",
	莰: "艹坎",
	莱: "艹来",
	莲: "艹连",
	莳: "艹时",
	莴: "艹呙",
	莶: "艹佥",
	获: "艹犾",
	莸: "艹犹",
	莻: "艿叱",
	莼: "艹纯",
	莾: "艹奔",
	莿: "艹刺",
	菀: "艹宛",
	菁: "艹青",
	菂: "艹的",
	菃: "艹洰",
	菄: "艹東",
	菅: "艹官",
	菆: "艹取",
	菇: "艹姑",
	菈: "艹拉",
	菉: "艹录",
	菊: "艹匊",
	菋: "艹味",
	菌: "艹囷",
	菍: "艹念",
	菎: "艹昆",
	菏: "艹河",
	菑: "艹甾",
	菒: "艹杲",
	菓: "艹果",
	菔: "艹服",
	菕: "艹侖",
	菖: "艹昌",
	菗: "艹抽",
	菘: "艹松",
	菙: "艹垂",
	菚: "艹戔",
	菛: "艹門",
	菜: "艹采",
	菝: "艹拔",
	菟: "艹兔",
	菠: "艹波",
	菡: "艹函",
	菢: "艹抱",
	菣: "艹臤",
	菤: "艹卷",
	菥: "艹析",
	菦: "艹近",
	菧: "艹底",
	菨: "艹妾",
	菩: "艹咅",
	菪: "艹宕",
	菬: "艹沼",
	菭: "艹治",
	菮: "艹庚",
	菰: "艹孤",
	菱: "艹夌",
	菲: "艹非",
	菳: "艹金",
	菴: "艹奄",
	菵: "艹罔",
	菶: "艹奉",
	菷: "艹帚",
	菸: "艹於",
	菹: "艹沮",
	菺: "艹肩",
	菻: "艹林",
	菼: "艹炎",
	菽: "艹叔",
	菾: "艹忝",
	菿: "艹到",
	萀: "艹虎",
	萁: "艹其",
	萂: "艹和",
	萃: "艹卒",
	萄: "艹匋",
	萅: "艹旾",
	萆: "艹卑",
	萇: "艹長",
	萉: "艹肥",
	萊: "艹來",
	萋: "艹妻",
	萌: "艹明",
	萍: "艹泙",
	萎: "艹委",
	萏: "艹臽",
	萐: "艹疌",
	萑: "艹隹",
	萒: "艹兖",
	萓: "艹宜",
	萔: "艹招",
	萕: "艹斉",
	萖: "艹兒",
	萗: "艹宗",
	萘: "艹奈",
	萙: "艹枕",
	萚: "艹择",
	萜: "艹帖",
	萝: "艹罗",
	萟: "艹秇",
	萠: "艹朋",
	萡: "艹泊",
	萢: "艹泡",
	萣: "艹定",
	萧: "艹肃",
	萩: "艹秋",
	萪: "艹科",
	萫: "艹香",
	萬: "艹禺",
	萭: "艹禹",
	萮: "艹俞",
	萯: "艹負",
	萰: "艹柬",
	萱: "艹宣",
	萲: "艹爰",
	萳: "艹南",
	萴: "艹則",
	萵: "艹咼",
	萶: "艹春",
	萷: "艹削",
	萸: "艹臾",
	萹: "艹扁",
	萺: "艹冒",
	萻: "艹音",
	萼: "艹咢",
	落: "艹洛",
	萾: "艹盈",
	萿: "艹活",
	葀: "艹括",
	葁: "艹姜",
	葂: "艹勉",
	葃: "艹昨",
	葄: "艹胙",
	葅: "艹俎",
	葆: "艹保",
	葇: "艹柔",
	葈: "艹枲",
	葉: "艹枼",
	葊: "艹弇",
	葋: "艹朐",
	葌: "艹姦",
	葍: "艹畐",
	葎: "艹律",
	葏: "艹津",
	葐: "艹盆",
	葑: "艹封",
	葒: "艹紅",
	葓: "艹洪",
	葔: "艹侯",
	葕: "艹衍",
	葖: "艹突",
	著: "艹者",
	葙: "艹相",
	葚: "艹甚",
	葛: "艹曷",
	葜: "艹契",
	葝: "艹勁",
	葞: "艹弭",
	葟: "艹皇",
	葠: "艹侵",
	葡: "艹匍",
	葢: "艹盇",
	董: "艹重",
	葤: "艹紂",
	葥: "艹前",
	葦: "艹韋",
	葧: "艹勃",
	葨: "艹畏",
	葩: "艹皅",
	葫: "艹胡",
	葭: "艹叚",
	葮: "艹段",
	葯: "艹約",
	葰: "艹俊",
	葱: "艹怱",
	葲: "艹泉",
	葳: "艹威",
	葴: "艹咸",
	葵: "艹癸",
	葶: "艹亭",
	葷: "艹軍",
	葸: "艹思",
	葹: "艹施",
	葺: "艹咠",
	葻: "艹風",
	葽: "艹要",
	葾: "艹怨",
	葿: "艹眉",
	蒀: "艹昷",
	蒁: "艹述",
	蒂: "艹帝",
	蒃: "艹彖",
	蒄: "艹冠",
	蒅: "艹染",
	蒈: "艹皆",
	蒉: "艹贵",
	蒊: "花叱",
	蒋: "艹将",
	蒌: "艹娄",
	蒍: "艹為",
	蒎: "艹派",
	蒐: "艹鬼",
	蒑: "艹殷",
	蒒: "艹師",
	蒓: "艹純",
	蒔: "艹時",
	蒖: "艹真",
	蒗: "艹浪",
	蒘: "艹挐",
	蒙: "艹冡",
	蒚: "艹鬲",
	蒛: "艹缺",
	蒜: "艹祘",
	蒝: "艹原",
	蒞: "艹涖",
	蒟: "艹竘",
	蒠: "艹息",
	蒡: "艹旁",
	蒢: "艹除",
	蒣: "艹徐",
	蒤: "艹涂",
	蒥: "艹留",
	蒦: "艹隻",
	蒨: "艹倩",
	蒩: "艹租",
	蒪: "艹尃",
	蒫: "艹差",
	蒬: "艹冤",
	蒭: "艹芻",
	蒮: "艹隺",
	蒰: "艹般",
	蒱: "艹捕",
	蒲: "艹浦",
	蒳: "艹納",
	蒴: "艹朔",
	蒵: "艹奚",
	蒶: "艹紛",
	蒷: "艹員",
	蒸: "艹烝",
	蒹: "艹兼",
	蒺: "艹疾",
	蒻: "艹弱",
	蒼: "艹倉",
	蒽: "艹恩",
	蒾: "艹迷",
	蒿: "艹高",
	蓀: "艹孫",
	蓁: "艹秦",
	蓂: "艹冥",
	蓃: "艹叟",
	蓄: "艹畜",
	蓅: "艹流",
	蓆: "艹席",
	蓇: "艹骨",
	蓈: "艹郎",
	蓉: "艹容",
	蓊: "艹翁",
	蓋: "艹盍",
	蓌: "艹夎",
	蓍: "艹耆",
	蓎: "艹唐",
	蓐: "艹辱",
	蓑: "艹衰",
	蓒: "艹軒",
	蓓: "艹倍",
	蓔: "艹羔",
	蓕: "艹桂",
	蓗: "艹徒",
	蓘: "艹衮",
	蓙: "艹座",
	蓚: "艹修",
	蓛: "艹敇",
	蓜: "艹配",
	蓝: "艹监",
	蓞: "艹舀",
	蓠: "艹离",
	蓡: "艹浸",
	蓢: "艹朗",
	蓣: "艹预",
	蓤: "艹凌",
	蓦: "莫马",
	蓧: "艹條",
	蓨: "艹脩",
	蓩: "艹務",
	蓪: "艹通",
	蓫: "艹逐",
	蓬: "艹逢",
	蓭: "艹庵",
	蓮: "艹連",
	蓯: "艹從",
	蓰: "艹徙",
	蓱: "艹洴",
	蓲: "艹區",
	蓳: "艹堇",
	蓴: "艹專",
	蓵: "艹捷",
	蓶: "艹唯",
	蓷: "艹推",
	蓸: "艹曹",
	蓹: "艹御",
	蓺: "艹埶",
	蓻: "艹執",
	蓼: "艹翏",
	蓽: "艹畢",
	蓾: "艹鹵",
	蓿: "艹宿",
	蔀: "艹部",
	蔁: "艹章",
	蔂: "艹累",
	蔃: "艹强",
	蔄: "艹問",
	蔆: "艹淩",
	蔇: "艹既",
	蔈: "艹票",
	蔉: "艹袞",
	蔊: "艹焊",
	蔋: "艹淑",
	蔌: "艹欶",
	蔍: "艹鹿",
	蔎: "艹設",
	蔏: "艹商",
	蔐: "艹啇",
	蔒: "艹焄",
	蔓: "艹曼",
	蔔: "艹匐",
	蔕: "艹帶",
	蔖: "艹虘",
	蔗: "艹庶",
	蔘: "艹參",
	蔙: "艹旋",
	蔚: "艹尉",
	蔛: "艹斛",
	蔜: "艹敖",
	蔝: "艹眯",
	蔞: "艹婁",
	蔟: "艹族",
	蔠: "艹終",
	蔡: "艹祭",
	蔢: "艹婆",
	蔣: "艹將",
	蔤: "艹密",
	蔥: "艹悤",
	蔦: "艹鳥",
	蔧: "艹彗",
	蔨: "艹圈",
	蔩: "艹寅",
	蔪: "艹斬",
	蔫: "艹焉",
	蔬: "艹疏",
	蔭: "艹陰",
	蔮: "艹國",
	蔯: "艹陳",
	蔰: "艹扈",
	蔱: "艹殺",
	蔳: "艹清",
	蔴: "艹麻",
	蔶: "艹責",
	蔷: "艹啬",
	蔸: "艹兜",
	蔹: "艹敛",
	蔻: "艹寇",
	蔼: "艹谒",
	蔽: "艹敝",
	蔾: "艹棃",
	蔿: "艹爲",
	蕀: "艹棘",
	蕁: "艹尋",
	蕂: "艹勝",
	蕃: "艹番",
	蕄: "艹悶",
	蕅: "艹湡",
	蕇: "艹單",
	蕈: "艹覃",
	蕉: "艹焦",
	蕊: "艹惢",
	蕋: "艹歮",
	蕌: "艹晶",
	蕍: "艹渝",
	蕎: "艹喬",
	蕏: "艹猪",
	蕑: "艹閒",
	蕒: "艹買",
	蕓: "艹雲",
	蕔: "艹報",
	蕕: "艹猶",
	蕖: "艹渠",
	蕗: "艹路",
	蕘: "艹堯",
	蕙: "艹惠",
	蕛: "艹稊",
	蕜: "艹悲",
	蕝: "艹絶",
	蕞: "艹最",
	蕟: "艹發",
	蕠: "艹絮",
	蕡: "艹賁",
	蕢: "艹貴",
	蕣: "艹舜",
	蕤: "艹甤",
	蕥: "艹雅",
	蕦: "艹須",
	蕧: "艹復",
	蕨: "艹厥",
	蕩: "艹湯",
	蕪: "艹無",
	蕫: "艹童",
	蕬: "艹絲",
	蕭: "艹肅",
	蕮: "艹舄",
	蕯: "艹隆",
	蕰: "艹温",
	蕱: "艹稍",
	蕳: "艹間",
	蕴: "艹缊",
	蕵: "艹飧",
	蕶: "艹零",
	蕷: "艹預",
	蕸: "艹遐",
	蕹: "艹雍",
	蕺: "艹戢",
	蕼: "艹肆",
	蕽: "艹農",
	蕾: "艹雷",
	蕿: "艹煖",
	薀: "艹溫",
	薁: "艹奥",
	薂: "艹敫",
	薃: "艹滈",
	薄: "艹溥",
	薅: "艹媷",
	薆: "艹愛",
	薇: "艹微",
	薈: "艹會",
	薉: "艹歲",
	薊: "艹魝",
	薋: "艹資",
	薌: "艹鄉",
	薍: "艹亂",
	薏: "艹意",
	薐: "艹稜",
	薑: "艹畺",
	薒: "艹粲",
	薔: "艹嗇",
	薕: "艹廉",
	薖: "艹過",
	薗: "艹園",
	薘: "艹達",
	薙: "艹雉",
	薜: "艹辟",
	薝: "艹詹",
	薞: "艹飱",
	薟: "艹僉",
	薠: "艹煩",
	薡: "艹鼎",
	薢: "艹解",
	薣: "艹鼓",
	薥: "艹蜀",
	薦: "艹廌",
	薩: "艹隡",
	薪: "艹新",
	薬: "艹楽",
	薭: "艹稗",
	薮: "艹数",
	薯: "艹署",
	薰: "艹熏",
	薱: "艹對",
	薲: "艹賓",
	薳: "艹遠",
	薴: "艹寧",
	薵: "艹壽",
	薶: "艹貍",
	薷: "艹需",
	薸: "艹漂",
	薹: "艹臺",
	薺: "艹齊",
	薻: "艹漅",
	薼: "艹塵",
	薽: "艹甄",
	薾: "艹爾",
	薿: "艹疑",
	藀: "艹熒",
	藁: "艹槀",
	藂: "艹聚",
	藃: "艹歊",
	藄: "艹綦",
	藅: "艹罰",
	藆: "艹搴",
	藇: "艹與",
	藈: "艹睽",
	藉: "艹耤",
	藊: "艹稨",
	藋: "艹翟",
	藌: "艹蜜",
	藍: "艹監",
	藎: "艹盡",
	藏: "艹臧",
	藐: "艹貌",
	藑: "艹夐",
	藓: "艹鲜",
	藔: "艹寮",
	藕: "艹耦",
	藖: "艹賢",
	藗: "艹遬",
	藘: "艹慮",
	藙: "艹毅",
	藚: "艹賣",
	藛: "艹寫",
	藜: "艹黎",
	藝: "蓺云",
	藞: "艹磊",
	藟: "艹畾",
	藠: "艹皛",
	藡: "艹適",
	藢: "艹徵",
	藣: "艹罷",
	藤: "艹滕",
	藥: "艹樂",
	藦: "艹摩",
	藧: "艹緩",
	藨: "艹麃",
	藩: "艹潘",
	藪: "艹數",
	藫: "艹潭",
	藬: "艹隤",
	藭: "艹窮",
	藮: "艹樵",
	藯: "艹慰",
	藰: "艹劉",
	藱: "艹瘣",
	藲: "艹樞",
	藳: "艹稾",
	藴: "艹緼",
	藶: "艹歷",
	藷: "艹諸",
	藸: "艹豬",
	藹: "艹謁",
	藺: "艹閵",
	藻: "艹澡",
	藼: "艹憲",
	藽: "艹親",
	藾: "艹賴",
	藿: "艹霍",
	蘀: "艹擇",
	蘁: "艹噩",
	蘂: "艹橤",
	蘅: "艹衡",
	蘆: "艹盧",
	蘇: "艹穌",
	蘈: "艹頽",
	蘊: "艹縕",
	蘋: "艹頻",
	蘌: "艹禦",
	蘍: "艹勳",
	蘏: "艹頴",
	蘐: "艹諼",
	蘑: "艹磨",
	蘒: "艹穐",
	蘔: "艹穎",
	蘕: "艹縫",
	蘖: "薛木",
	蘗: "艹檗",
	蘘: "艹襄",
	蘙: "艹翳",
	蘚: "艹鮮",
	蘜: "艹鞠",
	蘝: "艹歛",
	蘞: "艹斂",
	蘟: "艹隱",
	蘠: "艹牆",
	蘡: "艹嬰",
	蘢: "艹龍",
	蘣: "艹黈",
	蘥: "艹龠",
	蘦: "艹霝",
	蘧: "艹遽",
	蘨: "艹繇",
	蘩: "艹繁",
	蘪: "艹麋",
	蘫: "艹濫",
	蘬: "艹歸",
	蘭: "艹闌",
	蘮: "艹罽",
	蘯: "艹盪",
	蘰: "艹縵",
	蘱: "艹類",
	蘲: "艹壘",
	蘳: "艹黊",
	蘴: "艹豐",
	蘵: "艹職",
	蘶: "艹魏",
	蘸: "艹醮",
	蘹: "艹懷",
	蘺: "艹離",
	蘻: "艹繫",
	蘼: "艹靡",
	蘽: "艹櫐",
	蘾: "艹壞",
	蘿: "艹羅",
	虀: "艹韲",
	虁: "艹夔",
	虂: "艹露",
	虃: "艹瀸",
	虅: "艹騰",
	虆: "艹纍",
	虈: "艹嚻",
	虉: "艹鷊",
	虊: "艹欒",
	虋: "艹釁",
	虌: "艹鼈",
	虽: "口虫",
	虿: "万虫",
	蚃: "乡虫",
	蚉: "文虫",
	蚕: "天虫",
	蚠: "分虫",
	蚤: "叉虫",
	蚻: "札虫",
	蛊: "虫皿",
	蛋: "疋虫",
	蛩: "巩虫",
	蛬: "共虫",
	蛮: "亦虫",
	蛰: "执虫",
	蜇: "折虫",
	蜑: "延虫",
	蜚: "非虫",
	蜜: "宓虫",
	蜝: "其虫",
	蜤: "析虫",
	蜰: "肥虫",
	蜸: "臤虫",
	蝁: "亞虫",
	蝥: "敄虫",
	蝵: "秋虫",
	螀: "将虫",
	螌: "般虫",
	螚: "能虫",
	螫: "赦虫",
	螯: "敖虫",
	螱: "尉虫",
	螴: "陳虫",
	螷: "庳虫",
	螸: "欲虫",
	螿: "將虫",
	蟄: "執虫",
	蟇: "莫虫",
	蟚: "彭虫",
	蟞: "敝虫",
	蟴: "斯虫",
	蟹: "解虫",
	蟼: "敬虫",
	蠁: "鄉虫",
	蠆: "萬虫",
	蠈: "賊虫",
	蠒: "爾虫",
	蠜: "樊虫",
	蠞: "節虫",
	蠥: "辥虫",
	蠪: "龍虫",
	蠱: "蟲皿",
	血: "丿皿",
	衁: "亡血",
	衆: "血乑",
	衾: "今衣",
	袃: "切衣",
	袈: "加衣",
	袋: "代衣",
	袭: "龙衣",
	袰: "母衣",
	裂: "列衣",
	装: "壮衣",
	裔: "衣冏",
	裘: "求衣",
	裚: "折衣",
	裝: "壯衣",
	裟: "沙衣",
	裠: "君衣",
	裴: "非衣",
	裻: "叔衣",
	製: "制衣",
	褜: "胞衣",
	褧: "耿衣",
	褩: "般衣",
	褺: "執衣",
	褽: "尉衣",
	襞: "辟衣",
	襲: "龍衣",
	要: "覀女",
	覂: "覀乏",
	覃: "覀早",
	覄: "覀伏",
	覆: "覀復",
	覈: "覀敫",
	見: "目儿",
	覍: "小見",
	覓: "爫見",
	覔: "不見",
	覱: "斬見",
	觅: "爫见",
	觜: "此角",
	觠: "龹角",
	觱: "咸角",
	觺: "疑角",
	訔: "山言",
	訾: "此言",
	詈: "罒言",
	詟: "龙言",
	誉: "兴言",
	誊: "龹言",
	誓: "折言",
	諅: "其言",
	諐: "侃言",
	謦: "殸言",
	謩: "莫言",
	謷: "敖言",
	謺: "執言",
	謽: "强言",
	譥: "敫言",
	警: "敬言",
	譬: "辟言",
	譶: "言誩",
	譻: "賏言",
	譽: "與言",
	讆: "衛言",
	讋: "龍言",
	讏: "衞言",
	讐: "雔言",
	豈: "山豆",
	豊: "曲豆",
	豎: "臤豆",
	豢: "龹豕",
	貝: "目八",
	貟: "厶貝",
	貢: "工貝",
	貣: "弋貝",
	貧: "分貝",
	貨: "化貝",
	貪: "今貝",
	貫: "毌貝",
	責: "龶貝",
	貰: "世貝",
	貲: "此貝",
	買: "罒貝",
	貸: "代貝",
	費: "弗貝",
	賀: "加貝",
	賁: "卉貝",
	賃: "任貝",
	資: "次貝",
	賈: "覀貝",
	賌: "亥貝",
	賚: "來貝",
	賢: "臤貝",
	賣: "士買",
	賨: "宗貝",
	質: "斦貝",
	賲: "保貝",
	贀: "殹貝",
	贄: "執貝",
	贅: "敖貝",
	贇: "斌貝",
	贊: "兟貝",
	贔: "貝賏",
	贙: "虤貝",
	贠: "厶贝",
	贡: "工贝",
	责: "龶贝",
	货: "化贝",
	贪: "今贝",
	贫: "分贝",
	贯: "毌贝",
	贲: "卉贝",
	贳: "世贝",
	贷: "代贝",
	费: "弗贝",
	贺: "加贝",
	贽: "执贝",
	贾: "覀贝",
	赀: "此贝",
	赁: "任贝",
	资: "次贝",
	赉: "来贝",
	赘: "敖贝",
	赞: "兟贝",
	赟: "斌贝",
	走: "土龰",
	足: "口龰",
	趸: "万足",
	跫: "巩足",
	跾: "攸足",
	踅: "折足",
	蹔: "斬足",
	蹙: "戚足",
	蹩: "敝足",
	蹵: "就足",
	躄: "辟足",
	躉: "萬足",
	躗: "衛足",
	躛: "衞足",
	躠: "薛足",
	軍: "冖車",
	軎: "車口",
	軰: "北車",
	輂: "共車",
	輩: "非車",
	轝: "與車",
	轰: "车双",
	辈: "非车",
	辛: "立十",
	辜: "古辛",
	辠: "自辛",
	辱: "辰寸",
	農: "曲辰",
	辳: "林辰",
	辵: "彡龰",
	邑: "口巴",
	邕: "巛邑",
	鄨: "敝邑",
	酋: "丷酉",
	酓: "今酉",
	醔: "秋酉",
	醤: "将酉",
	醫: "殹酉",
	醬: "將酉",
	釆: "丿米",
	采: "爫木",
	量: "旦里",
	釡: "八金",
	鈭: "此金",
	銎: "巩金",
	銐: "列金",
	銞: "旬金",
	銮: "亦金",
	銴: "折金",
	銺: "壯金",
	鋆: "均金",
	鋈: "沃金",
	鋚: "攸金",
	鋫: "利金",
	鋬: "扳金",
	鋻: "臤金",
	錃: "波金",
	錖: "叔金",
	錱: "金玨",
	錾: "斩金",
	鍌: "洗金",
	鍙: "洪金",
	鍪: "敄金",
	鍫: "秋金",
	鎏: "流金",
	鎜: "般金",
	鎥: "條金",
	鏊: "敖金",
	鏧: "殸金",
	鏨: "斬金",
	鐅: "敝金",
	鐆: "隊金",
	鐜: "敦金",
	鐢: "棥金",
	鐾: "辟金",
	鑋: "輕金",
	鑍: "賏金",
	鑫: "金鍂",
	隻: "隹又",
	隼: "隹十",
	隽: "隹乃",
	雀: "小隹",
	集: "隹木",
	雙: "雔又",
	雥: "隹雔",
	雧: "雥木",
	雩: "雨亏",
	雪: "雨彐",
	雫: "雨下",
	雬: "雨木",
	雭: "雨及",
	雮: "雨毛",
	雯: "雨文",
	雰: "雨分",
	雱: "雨方",
	雲: "雨云",
	雳: "雨历",
	雴: "雨立",
	雵: "雨央",
	零: "雨令",
	雷: "雨田",
	雸: "雨甘",
	雹: "雨包",
	雺: "雨矛",
	雼: "雨石",
	雽: "雨乎",
	雾: "雨务",
	雿: "雨兆",
	需: "雨而",
	霁: "雨齐",
	霂: "雨沐",
	霃: "雨沈",
	霄: "雨肖",
	霅: "雨言",
	霆: "雨廷",
	震: "雨辰",
	霈: "雨沛",
	霉: "雨每",
	霋: "雨妻",
	霌: "雨周",
	霍: "雨隹",
	霎: "雨妾",
	霏: "雨非",
	霐: "雨泓",
	霑: "雨沾",
	霓: "雨兒",
	霔: "雨注",
	霖: "雨林",
	霗: "雨泠",
	霘: "雨洞",
	霙: "雨英",
	霚: "雨敄",
	霜: "雨相",
	霞: "雨叚",
	霟: "雨洪",
	霡: "雨脉",
	霢: "雨脈",
	霣: "雨員",
	霤: "雨留",
	霥: "雨冡",
	霦: "雨彬",
	霧: "雨務",
	霨: "雨尉",
	霩: "雨郭",
	霪: "雨淫",
	霫: "雨習",
	霬: "雨異",
	霭: "雨谒",
	霮: "雨湛",
	霯: "雨登",
	霰: "雨散",
	霱: "雨矞",
	露: "雨路",
	霳: "雨隆",
	霵: "雨戢",
	霶: "雨滂",
	霷: "雨暘",
	霹: "雨辟",
	霺: "雨微",
	霻: "雨豊",
	霽: "雨齊",
	霾: "雨貍",
	靀: "雨蒙",
	靁: "雨畾",
	靂: "雨歷",
	靃: "雨雔",
	靄: "雨謁",
	靇: "雨龍",
	靈: "霝巫",
	靊: "雨豐",
	靋: "雨瀝",
	靌: "雨寳",
	靎: "雨鵭",
	靏: "雨鶴",
	靑: "龶円",
	青: "龶月",
	靟: "非毛",
	靠: "告非",
	面: "丆囬",
	靣: "丆回",
	鞌: "安革",
	鞏: "巩革",
	鞤: "封革",
	鞪: "敄革",
	鞶: "般革",
	鞷: "鬲革",
	韏: "龹韋",
	韭: "非一",
	韮: "艹韭",
	音: "立日",
	韾: "殸音",
	響: "鄉音",
	頾: "須此",
	顰: "頻卑",
	颦: "频卑",
	颪: "下風",
	食: "人良",
	飬: "龹良",
	飺: "此食",
	餈: "次食",
	餋: "龹食",
	餥: "非食",
	餮: "殄食",
	饏: "敢食",
	饔: "雍食",
	饕: "號食",
	饗: "鄉食",
	香: "禾日",
	馨: "殸香",
	馵: "馬廾",
	馽: "馬中",
	駌: "夗馬",
	駑: "奴馬",
	駕: "加馬",
	駡: "吅馬",
	駦: "龹馬",
	騖: "敄馬",
	騭: "陟馬",
	騺: "執馬",
	驀: "莫馬",
	驁: "敖馬",
	驇: "埶馬",
	驚: "敬馬",
	驡: "龍馬",
	驫: "馬騳",
	驽: "奴马",
	驾: "加马",
	骂: "吅马",
	骘: "陟马",
	骛: "敄马",
	骜: "敖马",
	髠: "髟几",
	髡: "髟兀",
	髢: "髟也",
	髣: "髟方",
	髤: "髟木",
	髥: "髟冄",
	髦: "髟毛",
	髧: "髟冘",
	髨: "髟元",
	髩: "髟丏",
	髪: "髟友",
	髫: "髟召",
	髬: "髟丕",
	髭: "髟此",
	髮: "髟犮",
	髯: "髟冉",
	髰: "髟世",
	髱: "髟包",
	髲: "髟皮",
	髳: "髟矛",
	髴: "髟弗",
	髵: "髟而",
	髶: "髟耳",
	髷: "髟曲",
	髸: "髟共",
	髹: "髟休",
	髺: "髟舌",
	髻: "髟吉",
	髼: "髟夆",
	髽: "髟坐",
	髾: "髟肖",
	髿: "髟沙",
	鬀: "髟弟",
	鬁: "髟利",
	鬂: "髟兵",
	鬃: "髟宗",
	鬄: "髟易",
	鬅: "髟朋",
	鬆: "髟松",
	鬇: "髟争",
	鬈: "髟卷",
	鬊: "髟春",
	鬋: "髟前",
	鬍: "髟胡",
	鬎: "髟剌",
	鬏: "髟秋",
	鬐: "髟耆",
	鬑: "髟兼",
	鬒: "髟真",
	鬓: "髟宾",
	鬔: "髟逢",
	鬕: "髟莫",
	鬖: "髟參",
	鬘: "髟曼",
	鬙: "髟曾",
	鬚: "髟須",
	鬜: "髟閒",
	鬝: "髟間",
	鬞: "髟農",
	鬟: "髟睘",
	鬠: "髟會",
	鬡: "髟寧",
	鬢: "髟賓",
	鬣: "髟巤",
	鬤: "髟襄",
	鬵: "兓鬲",
	鬶: "规鬲",
	鬹: "規鬲",
	鬻: "粥鬲",
	魙: "漸鬼",
	魯: "魚日",
	鮆: "此魚",
	鮝: "龹魚",
	鮤: "列魚",
	鯊: "沙魚",
	鰲: "敖魚",
	鰵: "敏魚",
	鱀: "既魚",
	鱉: "敝魚",
	鲁: "鱼日",
	鲝: "羊鱼",
	鲞: "龹鱼",
	鲨: "沙鱼",
	鳌: "敖鱼",
	鳖: "敝鱼",
	鳘: "敏鱼",
	鳧: "鳥几",
	鳶: "弋鳥",
	鴌: "天鳥",
	鴍: "文鳥",
	鴐: "加鳥",
	鴑: "奴鳥",
	鴛: "夗鳥",
	鴜: "此鳥",
	鴦: "央鳥",
	鴷: "列鳥",
	鴽: "如鳥",
	鵀: "任鳥",
	鵉: "亦鳥",
	鵞: "我鳥",
	鵟: "狂鳥",
	鵥: "判鳥",
	鶑: "炏鳥",
	鶖: "秋鳥",
	鶦: "胡鳥",
	鶩: "敄鳥",
	鶭: "紡鳥",
	鶿: "兹鳥",
	鷔: "敖鳥",
	鷕: "唯鳥",
	鷖: "殹鳥",
	鷘: "敕鳥",
	鷙: "執鳥",
	鷟: "族鳥",
	鷥: "絲鳥",
	鷩: "敝鳥",
	鷲: "就鳥",
	鷺: "路鳥",
	鷿: "辟鳥",
	鸄: "敫鳥",
	鸉: "楊鳥",
	鸎: "賏鳥",
	鸑: "獄鳥",
	鸒: "與鳥",
	鸗: "龍鳥",
	鸢: "弋鸟",
	鸯: "央鸟",
	鸳: "夗鸟",
	鸶: "丝鸟",
	鸷: "执鸟",
	鸾: "亦鸟",
	鹙: "秋鸟",
	鹜: "敄鸟",
	鹥: "殹鸟",
	鹫: "就鸟",
	鹭: "路鸟",
	麄: "分鹿",
	麉: "幵鹿",
	麊: "米鹿",
	麓: "林鹿",
	麥: "來夂",
	麦: "龶夂",
	黒: "里灬",
	黛: "代黑",
	黳: "殹黑",
	黾: "口电",
	黿: "元黽",
	鼀: "圥黽",
	鼁: "去黽",
	鼂: "旦黽",
	鼃: "圭黽",
	鼄: "朱黽",
	鼅: "知黽",
	鼇: "敖黽",
	鼈: "敝黽",
	鼊: "辟黽",
	鼋: "元黾",
	鼌: "旦黾",
	鼏: "冖鼎",
	鼐: "乃鼎",
	鼒: "才鼎",
	鼕: "鼓冬",
	鼖: "卉鼓",
	鼗: "兆鼓",
	鼙: "鼓卑",
	鼚: "鼓長",
	鼛: "鼓咎",
	鼜: "鼓蚤",
	鼝: "淵鼓",
	鼞: "鼓堂",
	鼟: "鼓登",
	鼻: "自畀",
	齍: "齊皿",
	齤: "龹齒",
	齹: "差齒",
	齾: "獻齒",
	龏: "龍廾",
	龑: "龍天",
	龒: "龍示",
	龔: "龍共",
	龕: "合龍",
	龗: "霝龍",
	龘: "龍龖",
	龚: "龙共",
	龛: "合龙",
	龞: "敝龜",
	龩: "艹靳",
	龷: "卄一",
	龹: "丷夫",
	龺: "十早",
	龿: "艹佳",
	鿀: "艹醫",
	鿊: "艹沚",
	鿒: "艹页",
	鿓: "艹頁",
	鿖: "合一",
	鿪: "遂灬"
};
//#endregion
//#region src/utils.ts
function f(e, t) {
	let n = {};
	for (let r of e) r in t && !(r in n) && (n[r] = t[r]);
	return n;
}
function p(e, t) {
	let n = t === "v" ? {} : u, r = t === "h" ? {} : d, i = f(e, n), a = f(e, r);
	return {
		hSplitMap: i,
		vSplitMap: a,
		useH: Object.keys(i).length > 0,
		useV: Object.keys(a).length > 0
	};
}
function m() {
	return typeof window > "u";
}
function h() {
	return typeof window < "u";
}
function g(e, t) {
	return t ? {
		...e,
		...t
	} : e;
}
function _(e, t, n) {
	let r = new Blob([e], { type: n }), i = URL.createObjectURL(r), a = document.createElement("a");
	a.href = i, a.download = t, a.click(), URL.revokeObjectURL(i);
}
//#endregion
//#region src/render/html.ts
function v(e, t) {
	let n = g(r, t), { hSplitMap: i, vSplitMap: a, useH: o, useV: s } = p(e, n.mode);
	return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>汉字拆分</title>
  <style>${y(n, o, s)}</style>
</head>
<body>
  <div id="box">${b(e, i, a, n)}</div>
</body>
</html>`;
}
function y(e, t, n) {
	let r = e.showBoxBorder ? `border: 1px solid ${e.boxBorderColor};` : "border: none;", i = `\
    body {
      font-family: ${e.fontFamily};
      font-size: 32px;
      padding: 40px;
      line-height: 1.8;
    }

    #box {
      letter-spacing: 0;
    }

    .split-char {
      display: inline-flex;
      align-items: center;
      justify-content: center;
      width: ${e.boxWidth}px;
      height: ${e.boxHeight}px;
      vertical-align: middle;
      margin: 0 ${e.boxMarginH}px;
      box-sizing: border-box;
      ${r}
      overflow: hidden;
    }`;
	return t && (i += `\
    .split-char.h-split {
      flex-direction: row;
    }
    .split-char.h-split .part {
      width: 50%;
      height: 100%;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: ${e.partScale}em;
      line-height: 1;
      transform-origin: center center;
      font-weight: ${e.fontWeight};
      -webkit-text-stroke: ${e.textStroke} ${e.textStrokeColor};
    }
    .part-left {
      transform: scaleX(${e.hLeftScaleX}) translateX(${e.hLeftOffsetX}em);
    }
    .part-right {
      transform: scaleX(${e.hRightScaleX}) translateX(${e.hRightOffsetX}em);
    }`), n && (i += `\
    .split-char.v-split {
      flex-direction: column;
    }
    .split-char.v-split .part {
      width: 100%;
      height: 50%;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: ${e.partScale}em;
      line-height: 1;
      transform-origin: center center;
      font-weight: ${e.fontWeight};
      -webkit-text-stroke: ${e.textStroke} ${e.textStrokeColor};
    }
    .part-top {
      transform: scaleY(${e.vTopScaleY}) translateY(${e.vTopOffsetY}em);
    }
    .part-bottom {
      transform: scaleY(${e.vBottomScaleY}) translateY(${e.vBottomOffsetY}em);
    }`), i;
}
function b(e, t, n, r) {
	return e.split("").map((e) => {
		if (t[e]) {
			let [n, r] = t[e].split("");
			return `<span class="split-char h-split"><span class="part part-left">${n}</span><span class="part part-right">${r}</span></span>`;
		}
		if (n[e]) {
			let [t, r] = n[e].split("");
			return `<span class="split-char v-split"><span class="part part-top">${t}</span><span class="part part-bottom">${r}</span></span>`;
		}
		return `<span class="split-char">${e}</span>`;
	}).join("");
}
//#endregion
//#region src/render/svg.ts
function x(e, t) {
	let n = g(i, t), { hSplitMap: r, vSplitMap: a } = p(e, n.mode), o = e.split(""), s = [];
	o.forEach((e, t) => {
		let i = t % n.cols, o = Math.floor(t / n.cols), c = i * (n.boxWidth + n.boxGapH), l = o * (n.boxHeight + n.boxGapV);
		s.push(S(e, c, l, r, a, n));
	});
	let c = Math.ceil(o.length / n.cols);
	return `<svg xmlns="http://www.w3.org/2000/svg" width="${n.cols * (n.boxWidth + n.boxGapH) - n.boxGapH}" height="${c * (n.boxHeight + n.boxGapV) - n.boxGapV}">${s.join("")}</svg>`;
}
function S(e, t, n, r, i, a) {
	let o = a.boxWidth, s = a.boxHeight, c = t + o / 2, l = n + s / 2, u = a.showBoxBorder ? `<rect x="${t}" y="${n}" width="${o}" height="${s}" fill="none" stroke="${a.boxBorderColor}" stroke-width="1"/>` : "", d = a.boxHeight * a.partScale, f = `font-family="${a.fontFamily}" font-size="${d}px" text-anchor="middle" dominant-baseline="central" fill="#000"`;
	if (!r[e] && !i[e]) return `${u}<text x="${c}" y="${l}" ${f}>${e}</text>`;
	if (r[e]) {
		let [n, i] = r[e].split(""), s = t + o * .25 + a.hLeftOffsetX, c = t + o * .75 + a.hRightOffsetX;
		return `${u}<text transform="${`translate(${s}, ${l}) scale(${a.hLeftScaleX}, 1)`}" ${f}>${n}</text><text transform="${`translate(${c}, ${l}) scale(${a.hRightScaleX}, 1)`}" ${f}>${i}</text>`;
	}
	if (i[e]) {
		let [t, r] = i[e].split(""), o = n + s * .25 + a.vTopOffsetY, l = n + s * .75 + a.vBottomOffsetY;
		return `${u}<text transform="${`translate(${c}, ${o}) scale(1, ${a.vTopScaleY})`}" ${f}>${t}</text><text transform="${`translate(${c}, ${l}) scale(1, ${a.vBottomScaleY})`}" ${f}>${r}</text>`;
	}
	return "";
}
//#endregion
//#region src/render/text.ts
function C(e, t) {
	let n = g(o, t), { hSplitMap: r, vSplitMap: i } = p(e, n.mode), { sep: a } = n;
	if (a === "auto") {
		let t = e.split("").map((e) => {
			if (i[e]) {
				let [t, n] = i[e].split("");
				return {
					type: "v",
					content: t + "\n" + n
				};
			}
			return r[e] ? {
				type: "h",
				content: r[e]
			} : {
				type: "n",
				content: e
			};
		}), n = "";
		return t.forEach((e, t) => {
			t > 0 && (n += e.type === "v" ? "\n\n" : " "), n += e.content;
		}), n;
	}
	let s = e.split("").map((e) => r[e] ? r[e] : i[e] ? i[e] : e);
	return typeof a == "string" ? s.join(a) : s.join("");
}
//#endregion
//#region src/render/typst.ts
function w(e, t) {
	let n = g(a, t), { hSplitMap: r, vSplitMap: i } = p(e, n.mode), o = T(r), s = T(i);
	return `#set page(width: auto, height: auto, margin: 1cm)

#let params = (
  box-size: ${n.boxSize}pt,
  cols: ${n.cols},
  box-gap: ${n.boxGap}pt,
  font-size: ${n.fontSize}pt,
  h-scale: ${(n.hScale * 100).toFixed(0)}%,
  v-scale: ${(n.vScale * 100).toFixed(0)}%,
  h-tightness: ${n.hTightness}pt,
  v-tightness: ${n.vTightness}pt,
  stroke: ${n.stroke},
)

#let h-split-map = ${o}
#let v-split-map = ${s}

#let char-cell(char) = {
  box(
    width: params.box-size,
    height: params.box-size,
    stroke: params.stroke,
    clip: true,
    {
      set align(center + horizon)
      set text(size: params.font-size, font: "SimSun")
      if char in h-split-map {
        let (left, right) = h-split-map.at(char)
        stack(dir: ltr, spacing: params.h-tightness, scale(x: params.h-scale, reflow: true, left), scale(x: params.h-scale, reflow: true, right))
      } else if char in v-split-map {
        let (top, bottom) = v-split-map.at(char)
        stack(dir: ttb, spacing: params.v-tightness, scale(y: params.v-scale, reflow: true, top), scale(y: params.v-scale, reflow: true, bottom))
      } else {
        scale(88%, char)
      }
    }
  )
}

#grid(
  columns: (params.box-size,) * params.cols,
  column-gutter: params.box-gap,
  row-gutter: params.box-gap,
  ..("${e}".clusters().map(char => char-cell(char)))
)`;
}
function T(e) {
	return `(${Object.entries(e).map(([e, t]) => {
		let [n, r] = t.split("");
		return `"${e}": ("${n}", "${r}")`;
	}).join(", ")})`;
}
//#endregion
//#region src/node/binary.stub.ts
var E = /* @__PURE__ */ n({ generateBinary: () => D });
async function D(e, t) {
	throw Error("Binary generation is not supported in browser environment");
}
var O = t((() => {})), k = /* @__PURE__ */ n({ saveFile: () => A });
async function A(e, t) {
	throw Error("File saving is not supported in browser environment");
}
var j = t((() => {}));
//#endregion
//#region src/core.ts
function M(e) {
	switch (e) {
		case "html": return "text/html";
		case "svg": return "image/svg+xml";
		case "text": return "text/plain";
		case "typ": return "text/plain";
		case "png": return "image/png";
		case "jpg": return "image/jpeg";
		case "pdf": return "application/pdf";
		default: return "application/octet-stream";
	}
}
function N(e) {
	return m() && e && typeof e == "object" && e.constructor && e.constructor.isBuffer?.(e);
}
var P = class {
	constructor(e) {
		this.outputType = "html", this.content = "", this.text = e;
	}
	to(e) {
		return this.outputType = l[e] || e, this;
	}
	with(e) {
		return this.customParams = e, this;
	}
	async generate() {
		if (this.content) return this.content;
		switch (this.outputType) {
			case "html":
				this.content = v(this.text, this.customParams);
				break;
			case "svg":
				this.content = x(this.text, this.customParams);
				break;
			case "text":
				this.content = C(this.text, this.customParams);
				break;
			case "typ":
				this.content = w(this.text, this.customParams);
				break;
			case "png":
			case "jpg":
			case "pdf":
				if (!m()) throw Error(`${this.outputType} is not supported in browser`);
				this.content = await this.generateBinary();
				break;
			default: throw Error(`Unknown output type: ${this.outputType}`);
		}
		return this.content;
	}
	async generateBinary() {
		if (!m()) throw Error("Binary generation only works in Node.js");
		let { generateBinary: e } = await Promise.resolve().then(() => (O(), E));
		return e(this.outputType === "pdf" ? w(this.text, this.customParams) : x(this.text, this.customParams), this.outputType);
	}
	log() {
		return this.generate().then((e) => {
			if (N(e)) {
				let t = e;
				console.log(`[${this.outputType.toUpperCase()}] Generated binary data (${t.length} bytes)`);
			} else console.log(e);
		}), this;
	}
	async saveAs(e) {
		let t = await this.generate();
		if (m()) {
			let { saveFile: n, saveBinaryFile: r } = await Promise.resolve().then(() => (j(), k));
			N(t) ? await r(e, t) : await n(e, t);
		} else h() && _(t, e, M(this.outputType));
	}
	getMimeType() {
		switch (this.outputType) {
			case "html": return "text/html";
			case "svg": return "image/svg+xml";
			case "text": return "text/plain";
			case "typ": return "text/plain";
			case "png": return "image/png";
			case "jpg": return "image/jpeg";
			case "pdf": return "application/pdf";
			default: return "application/octet-stream";
		}
	}
	async toStr() {
		let e = await this.generate();
		if (N(e)) throw Error(`Cannot convert binary data (${this.outputType}) to string. Use saveAs() to save to file instead.`);
		return e;
	}
};
function F(e) {
	return new P(e);
}
//#endregion
export { P as Cradic, s as DEFAULT_PARAMS, c as OUTPUT_TYPES, F as from };
